...
#else
    onRequest(pRI->pCI->requestNumber, NULL, 0, pRI);
#endif
}
...
#else
    onRequest(pRI->pCI->requestNumber, pStrings, datalen, pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &args, sizeof(args), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &dial, sizeOfDial, pRI);
#endif
...
static int responseEcallStatus(Parcel &p, void *response, size_t responselen) {
    if (response == NULL || responselen != sizeof(RIL_Ecall_Unsol_Indications)) {
        if (response == NULL) {
          RFX_LOG_D(LOG_TAG, "invalid response: NULL");
        } else {
            RFX_LOG_D(LOG_TAG, "responseEcallStatus: invalid response length %d expecting len: %d",
 ...
 *   String msd_data
 */
static void dispatchFastEcall (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    ...
    status = p.readInt32(&t);
    eCallReqMsg.domain = (RIL_Dial_Domain)t;

    startRequest;
    appendPrintBuf("%secall_cat=%d,ecall_variant=%d, address=%s, domain=%d", printBuf,
        eCallReqMsg.ecall_cat, eCallReqMsg.ecall_variant, (char*)eCallReqMsg.address, eCallReqMsg.domain);
    closeRequest;
    printRequest(pRI->token, pRI->pCI->requestNumber);
...
#if defined(ANDROID_MULTI_SIM)
    onRequest(pRI->pCI->requestNumber, &eCallReqMsg, size, pRI, pRI->socket_id);
#else
    onRequest(pRI->pCI->requestNumber, &eCallReqMsg, size, pRI);
#endif
 ...
 *   String msd_data
 */
static void dispatchSetMsd (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
...
#if defined(ANDROID_MULTI_SIM)
    onRequest(pRI->pCI->requestNumber, &eCallSetMsd, size, pRI, pRI->socket_id);
#else
    onRequest(pRI->pCI->requestNumber, &eCallSetMsd, size, pRI);
#endif
 ...
 *   char* address
 */
static void dispatchEcallRecord (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
...
#else
    onRequest(pRI->pCI->requestNumber, &emergencyDial, sizeOfEmergencyDial, pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &simIO, size, pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &apdu, sizeof(RIL_SIM_APDU), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &cff, sizeof(cff), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, const_cast<void *>(data), len, pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &rcsm, sizeof(rcsm), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &rcsa, sizeof(rcsa), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &rcsw, sizeof(rcsw), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &pf, sizeof(pf), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &nvri, sizeof(nvri), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &nvwi, sizeof(nvwi), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &uicc_sub, sizeof(uicc_sub), pRI);
#endif
...
#else
    onRequest(pRI->pCI->requestNumber, &p_Record, sizeof(p_Record), pRI);
#endif
...
int responseDataCallListV11(Parcel &p, void *response, size_t responselen) {
    ...
    for (i = 0; i < num; i++) {
        ...
        appendPrintBuf("%s[status=%d,retry=%d,cid=%d,%s,%s,%s,%s,%s,%s,%s,%d],", tempPrintBuf,
        p_cur[i].status,
        p_cur[i].suggestedRetryTime,
        p_cur[i].cid,
        (p_cur[i].active==0)?"down":"up",
...
void responseRilSignalStrengthV10(Parcel &p, RIL_SignalStrength_v10 *p_cur) {
    ...
    if (p_cur != NULL) p.writeInt32(p_cur->TD_SCDMA_SignalStrength.rscp);
}
...
int responseRilSignalStrength(Parcel &p,
    ...
    if (p_cur == NULL) {
        RFX_LOG_E(LOG_TAG, "responseRilSignalStrength response is null");
        return 0;
    }
...
int responseCellInfoListV6(Parcel &p, void *response, size_t responselen) {
    ...
    if (p_cur == NULL) {
        RFX_LOG_E(LOG_TAG, "invalid RIL_CellInfo: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
    }
    startResponse;
    int i;
...
int responseCellInfoListV12(Parcel &p, void *response, size_t responselen) {
    ...
    if (p_cur == NULL) {
        RFX_LOG_E(LOG_TAG, "invalid RIL_CellInfo_v12: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
    }
...
int responseGetSystemSelectionChannels(Parcel &p, void *response, size_t responselen) {
    RFX_LOG_D(LOG_TAG, "[%s] responselen %zu", __FUNCTION__, responselen);
    if (response == NULL) {
        RFX_LOG_E(LOG_TAG, "invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
    }
    ...
    if (responselen != sizeof(RIL_SystemSelectionChannels)) {
        RFX_LOG_E(LOG_TAG, "invalid response length was %d expected %d",
                  (int)responselen, (int)sizeof(RIL_SystemSelectionChannels));
        return RIL_ERRNO_INVALID_RESPONSE;
    }

    RIL_SystemSelectionChannels* p_cur = (RIL_SystemSelectionChannels*)response;
    if (p_cur == NULL) {
        RFX_LOG_E(LOG_TAG, "invalid RIL_SystemSelectionChannels: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
    }
    p.writeInt32(p_cur->specifyChannels);
    p.writeUint32(p_cur->specifiers_length);
    for (int i = 0; i < MAX_RADIO_ACCESS_NETWORKS; i++) {
        p.writeInt32(p_cur->specifiers[i].radio_access_network);
        p.writeUint32(p_cur->specifiers[i].bands_length);
        for (int j = 0; j < MAX_BANDS; j++) {
            switch (p_cur->specifiers[i].radio_access_network) {
            case GERAN:
                p.writeInt32(p_cur->specifiers[i].bands.geran_bands[j]);
                break;

            case UTRAN:
                p.writeInt32(p_cur->specifiers[i].bands.utran_bands[j]);
                break;

            case EUTRAN:
                p.writeInt32(p_cur->specifiers[i].bands.eutran_bands[j]);
                break;

            case NEW_RADIO:
                p.writeInt32(p_cur->specifiers[i].bands.ngranBands[j]);
                break;
        ...
        p.writeUint32(p_cur->specifiers[i].channels_length);
        for (int j = 0; j < MAX_CHANNELS; j++) {
            p.writeUint32(p_cur->specifiers[i].channels[j]);
        }
    }
