...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxStringsData, RfxBarringCallInfosData, RFX_MSG_REQUEST_GET_BARRING_SEPCIFIC_CALL);
...
void RmcSuppServRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
    ...
    switch(request) {
        ...
        case RFX_MSG_REQUEST_GET_BARRING_SEPCIFIC_CALL:
            requestGetBarringCalls(msg);
            break;

        case RFX_MSG_REQUEST_SET_BARRING_SEPCIFIC_CALL:
            requestSetBarringCalls(msg);
            break;
...
void RmcSuppServRequestHandler::requestChangeBarringPassword(const sp<RfxMclMessage>& msg) {
    ...
    if (msg->getData()->getDataLength() == 3 * sizeof(char*)) {
        ...
        cmdRet = asprintf(&cmd, "AT+ECUSD=1,1,\"**03*%s*%s*%s*%s#\"",
        ...
        logE(TAG, "requestChangeBarringPassword parameter error.");
        goto error;
    }

    if (cmdRet < 0) {
        logE(TAG, "requestChangeBarringPassword OOM");
        goto error;
    }

    if (cmd == NULL) {
        logE(TAG, "requestChangeBarringPassword, cmd is NULL");
        goto error;
    }
...
void RmcSuppServRequestHandler::requestSendCNAP(const sp<RfxMclMessage>& msg) {
    ...
    if (isInImsCall && wfcState != 1) {
        logD(TAG, "requestSendCnap, return error because there is a VoLTE call ongoing");
        goto error;
    }
...
void RmcSuppServRequestHandler::requestSetClip(const sp<RfxMclMessage>& msg) {
    ...
    if (msg->getData()->getDataLength() != 0) {
         ...
         */
        if (asprintf(&cmd, "AT+CLIP=%d", n[0]) < 0) {
            logE(TAG, "requestSetClip fail at asprintf");
            goto error;
        }

        if (cmd == NULL) {
            logE(TAG, "requestSetClip, cmd is NULL");
            goto error;
        }
...
void RmcSuppServRequestHandler::requestSetSuppSvcNotification(const sp<RfxMclMessage>& msg) {
    ...
    if (asprintf(&cmd, "AT+CSSN=%d,%d", n[0], n[0]) < 0) {
        logE(TAG, "requestSetSuppSvcNotification OOM");
        goto error;
    }

    if (cmd == NULL) {
        logE(TAG, "requestSetSuppSvcNotification, cmd is NULL");
        goto error;
    }
...
void RmcSuppServRequestHandler::requestGetModemVersion() {
    ...
    if (asprintf(&cmd, "AT+EXCAPCFGGET=\"version\"") < 0) {
        logE(TAG, "requestGetModemVersion OOM.");
        goto error;
    }

    if (cmd == NULL) {
        logE(TAG, "requestGetModemVersion, cmd is NULL");
        goto error;
    }
...
void RmcSuppServRequestHandler::requestSetXcapConfig(const sp<RfxMclMessage>& msg) {
   ...
   logD(TAG, "requestSetXcapConfig: data1 = %s", data[1]);

    const char *delim = ",";
    char* name = strtok_r(data[1], delim, &savePtr);
    char* value = strtok_r(NULL, delim, &savePtr);

    if (name == NULL /*|| value == NULL*/) {
        logE(TAG, "SetXcapConfig name/value is NULL");
        goto error;
    }
    ...
    if (atoi(modem_version) != 0) {
        ...
        if (value != NULL && strlen(value) > 0) {
            ...
            if (!isResetSession) {
                if (asprintf(&cmd, "AT+EXCAPCFGSET=\"%s\",\"%s\"", config_name, value) < 0) {
                    logE(TAG, "Out of memory in !isResetSession part");
                    goto error;
                }

                if (cmd == NULL) {
                    logE(TAG, "!isResetSession, cmd is NULL");
                    goto error;
                }
            ...
            if (strcmp(config_name, "reset") == 0) {
                ...
                if (asprintf(&cmd, "AT+EXCAPCFGSET=\"%s\"", config_name) < 0) {
                    logE(TAG, "Out of memory in reset part");
                    goto error;
                }

                if (cmd == NULL) {
                    logE(TAG, "reset session, cmd is NULL");
                    goto error;
                }
...
void RmcSuppServRequestHandler::syncCLIRToStatusManager() {
    ...
    int len = snprintf(clir_n, sizeof(clir_n), "0");
    if (len < 0) {
        logE(TAG, "syncCLIRToStatusManager, snprintf error");
        return;
    }
...
void RmcSuppServRequestHandler::requestGetBarringCalls(const sp<RfxMclMessage>& msg) {
    RIL_Errno ret = RIL_E_GENERIC_FAILURE;

    logE(TAG, "requestGetBarringCalls: NOT support");

    sp<RfxMclMessage> response = RfxMclMessage::obtainResponse(msg->getId(), ret,
            RfxBarringCallInfosData(NULL, 0), msg);

    responseToTelCore(response);
}

void RmcSuppServRequestHandler::requestSetBarringCalls(const sp<RfxMclMessage>& msg) {
    logE(TAG, "requestSetBarringCalls: NOT support");
    RIL_Errno ret = RIL_E_GENERIC_FAILURE;
