...
#define RFX_LOG_TAG "RtmPhb"
#define PROPERTY_RIL_PHB_READY "vendor.gsm.sim.ril.phbready"

using ::android::String8;

/*****************************************************************************
 * Class RtmPhbController
 *****************************************************************************/

RFX_IMPLEMENT_CLASS("RtmPhbController", RtmPhbController, RfxController);
RFX_REGISTER_DATA_TO_URC_ID(RfxIntsData, RFX_MSG_URC_PHB_READY_NOTIFICATION);

RtmPhbController::RtmPhbController() {
    m_timer = NULL;
}
...
void RtmPhbController::onInit() {
    ...
    getNonSlotScopeStatusManager()->registerStatusChanged(RFX_STATUS_KEY_MODEM_POWER_OFF,
            RfxStatusChangeCallback(this, &RtmPhbController::onMdOffChanged));

    // register callbacks to get sim plugout event
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_SIM_ESIMS_CAUSE,
            RfxStatusChangeCallback(this, &RtmPhbController::onSimPlugOut));
}

void RtmPhbController::onSimPlugOut(RfxStatusKeyEnum key,
...
void RtmPhbController::onMdOffChanged(RfxStatusKeyEnum key,
    RfxVariant oldValue, RfxVariant newValue) {
    RFX_UNUSED(key);
    RFX_UNUSED(oldValue);
...
bool RtmPhbController::onCheckIfRejectMessage(const sp<RfxMessage>& message,
...
bool RtmPhbController::onPreviewMessage(const sp<RfxMessage>& msg) {
    int msgId = msg->getId();
...
bool RtmPhbController::onCheckIfResumeMessage(const sp<RfxMessage>& msg) {
    int msgId = msg->getId();
    if (msgId == RFX_MSG_REQUEST_QUERY_PHB_STORAGE_INFO && m_timer != NULL) {
        return false;
    }
...
bool RtmPhbController::onHandleUrc(const sp<RfxMessage>& message) {
    int msgId = message->getId();
    ...
    if (msgId == RFX_MSG_URC_PHB_READY_NOTIFICATION) {
        ...
        if (pInt[0] == 1) {
            ...
            m_timer = NULL;
            m_timer = RfxTimer::start(RfxCallback0(this, &RtmPhbController::onTimer),
                    s2ns(PHB_READY_TIME));
        }
...
bool RtmPhbController::onHandleRequest(const sp<RfxMessage>& message) {
    int msgId = message->getId();
...
bool RtmPhbController::onHandleResponse(const sp<RfxMessage>& message) {
    int id = message->getId();
    logD(RFX_LOG_TAG, "onHandleResponse, slotId: %d, MsgId: %s", m_slot_id, RFX_ID_TO_STR(id));

    if (id != RFX_MSG_REQUEST_PHB_CURRENT_STORAGE_RESET) {
        responseToRilj(message);
    }
    return true;
}

void RtmPhbController::onTimer() {
