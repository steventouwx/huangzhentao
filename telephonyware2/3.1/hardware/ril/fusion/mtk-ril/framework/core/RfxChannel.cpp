...
RfxChannel::RfxChannel(int channelId, char *muxPath) :
    ...
    } else if (RfxRilUtils::isMipcSupport() == true || (channelId%RIL_CHANNEL_OFFSET) == RIL_CMD_IMS) {
        //vilte port not confirmed so fd don't open here
        mFd = - 1;
    } else {
        int count = 1;
            ...
            do {
                ...
                if (mFd < 0) {
                    ...
                    RFX_LOG_E(RFX_LOG_TAG, "could not connect to %s: %s, count:%d",
                            mMuxPath, strerror(errno), count);
                    if ((channelId%RIL_CHANNEL_OFFSET) == RIL_CMD_IMS) {
                        if (count == 10) {
                            mFd = -1;
                            RFX_LOG_E(RFX_LOG_TAG, "giveup connectting to %s", mMuxPath);
                            break;
                        }
                    }
                    count++;
    ...
    if (RfxRilUtils::isMipcSupport() == true && (channelId%RIL_CHANNEL_OFFSET) != RIL_CMD_IMS) {
        mSender = new RfxSender(mFd, mChannelId, mContext);
        if (mSender) {
            RFX_LOG_E(RFX_LOG_TAG, "mSender(%d) is null", channelId);
        }
    } else {
        mReader = new RfxReader(mFd, mChannelId, mContext);
        if (mReader) {
            RFX_LOG_E(RFX_LOG_TAG, "mReader(%d) is null", channelId);
        }
        mSender = new RfxSender(mFd, mChannelId, mContext);
        if (mSender) {
            RFX_LOG_E(RFX_LOG_TAG, "mSender(%d) is null", channelId);
...
void RfxChannel::run() {
    if (mReader != NULL) {
        android::status_t ret = mReader->run(String8::format("RFXReader_%d", mChannelId).string());
        if (ret != android::OK) {
            RFX_LOG_E(RFX_LOG_TAG, "RfxRender(%d) run fail, err(%d)", mChannelId, ret);
        }
    }
    if (mSender != NULL) {
        android::status_t ret = mSender->run(String8::format("RFXSender_%d", mChannelId).string());
        if (ret != android::OK) {
            RFX_LOG_E(RFX_LOG_TAG, "RfxSender(%d) run fail, err(%d)", mChannelId, ret);
        }
    } else {
        RFX_LOG_E(RFX_LOG_TAG, "RfxSender(%d) is null", mChannelId);
    }
}
