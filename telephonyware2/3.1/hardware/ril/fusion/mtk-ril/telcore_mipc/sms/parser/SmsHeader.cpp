...
SmsHeader* SmsHeader::fromByteArray(BYTE* data, int headerLength) {
    ...
    while (curr < headerLength) {
        ...
        if (curr >= headerLength) {
            RFX_LOG_E(RFX_LOG_TAG, "fromByteArray header format wrong %d %d", curr, headerLength);
            return smsHeader;
        }
        int length = data[curr++];
        RFX_LOG_D(RFX_LOG_TAG, "fromByteArray header element id: %d, len: %d", id, length);
        ConcatRef concatRef;
        PortAddrs portAddrs;
        int temp1 = 0;
        int temp2 = 0;
        ...
        switch (id) {
            case ELT_ID_CONCATENATED_8_BIT_REFERENCE: {
                if (curr + 2 >= headerLength) {
                    RFX_LOG_E(RFX_LOG_TAG, "fromByteArray header format wrong id:%d, len:%d, "
                            "cur:%d, headerlen: %d", id, length, curr, headerLength);
                    return smsHeader;
                }
            ...
            default: {
                if (length < 0 || curr + length - 1 >= headerLength) {
                    RFX_LOG_E(RFX_LOG_TAG, "fromByteArray header format wrong id:%d, len:%d, "
                            "cur:%d, headerlen: %d", id, length, curr, headerLength);
                    return smsHeader;
                }
