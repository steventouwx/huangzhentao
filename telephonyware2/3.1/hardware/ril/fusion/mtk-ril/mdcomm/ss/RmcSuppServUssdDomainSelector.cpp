...
void RmcSuppServUssdDomainSelector::requestSendUSSD(const sp<RfxMclMessage>& msg,
    ...
    if (asprintf(&cmd, "AT+ECUSD=2,1,\"%s\",72", pTmpStr) < 0) { /* <dcs> = 0x48 */
        logE(TAG, "requestSendUSSD error in asprintf");
    ...
    if (cmd == NULL) {
        logE(TAG, "requestSendUSSD cmd is NULL");
...
void RmcSuppServUssdDomainSelector::requestSendUssdDomainSelect(const sp<RfxMclMessage>& msg) {
    ...
    if (isInImsCall()) {
        // An IMS call -> send USSI -> USSI will be blocked
        if (SSConfig::isNotSupportUSSIOverCall(currentMccmnc.string())) {
        ...
        // ALPS07640779, send USSI -> receive a cal -> continue USSI service -> USSD send successfully
        // and call is still alive
        if (!mNeedUserReply && SSConfig::isNotSupportStartUSSIDuringCall(currentMccmnc.string())) {
            logD(TAG, "requestSendUssdDomainSelect: doesn't allow to start USSI during call");
            sendFailureReport(msg, RIL_E_OPERATION_NOT_ALLOWED);
            return;
        }
    }
    ...
    if (SSConfig::isUssdDomainCsOnly(currentMccmnc.string())) {
        requestSendUSSD(msg, SEND_RESPONSE_BACK);
        return;
    }
...
sp<RfxMclMessage> RmcSuppServUssdDomainSelector::convertUssiToUssdUrc(
    ...
    if (strcmp(m, "1") == 0) {
        ...
        if (strcmp(n, "0") == 0) {
            asprintf(&newUssdUrcStrings[0], "%d", 0);  // USSD_MODE_NOTIFY (0)
                                                       // <m> = 3, <n> = 0, execute success
        } else {
            String8 currentMccmnc = getMclStatusManager()->
                    getString8Value(RFX_STATUS_KEY_UICC_GSM_NUMERIC, String8("0"));
            if (SSConfig::notRetryUssdOverCs(currentMccmnc)) {
                asprintf(&newUssdUrcStrings[0], "%d", 5);  // IMS return error, No CSFB - USSD_MODE_NW_TIMEOUT
            }
        }
    }
