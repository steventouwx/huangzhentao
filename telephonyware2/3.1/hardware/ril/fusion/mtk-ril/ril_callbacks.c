...
static void updateConnectionState(RIL_SOCKET_ID socketId, int isConnected, unsigned int clientType);
...
unsigned int toRealSlot(unsigned int slotId);
...
#endif
{
    ...
    if(request == RIL_REQUEST_GET_CURRENT_CALLS) {
        request = handleRequest(request, data, datalen, t, toRealSlot(socId));
    }
...
static void onSapRequest(int request, void *data, size_t datalen, RIL_Token t) {
    ...
    /// MTK_RIL_ADAPTER @{
    request = handleSapRequest(request, data, datalen, t, toRealSlot(socketId));
    if (request == -1) {
        return;
    }
    /// @}
...
void setRadioState(RIL_RadioState newState, RIL_SOCKET_ID rid) {
    ...
    err = pthread_mutex_lock(&s_state_mutex[(unsigned int)rid]);
    if (err != 0) {
        mtkAssert((char*)"setRadioState lock fail");
    }
    ...
    err = pthread_mutex_unlock(&s_state_mutex[(unsigned int)rid]);
    if (err != 0) {
        mtkAssert((char*)"setRadioState unlock fail");
    }
    ...
    /// MTK_RIL_ADAPTER @{
#if defined(ANDROID_MULTI_SIM)
    handleSetRadioState(oldState, *pState, rid);
#else
    handleSetRadioState(oldState, *pState, RIL_SOCKET_1);
#endif
    /// @}
#endif
}
...
RIL_RadioState getRadioState(RIL_SOCKET_ID rid)
{
    unsigned int realSlot = rid % MAX_SIM_COUNT;
    RIL_RadioState radioState = s_radio_state[realSlot];
...
static void updateConnectionState(RIL_SOCKET_ID socketId, int isConnected, unsigned int clientType) {
    rfx_update_connection_state(socketId, isConnected, clientType);
}
...
#ifdef TBOX
    /// MTK_RIL_ADAPTER @{
    initMtkRilAdapter(s_rilenv);
    /// @}
#endif

    ret = pthread_attr_init(&attr);
    if (ret != 0) {
        mtkAssert((char*)"RIL_Init pthread init fail");
    }
    ret = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    if (ret != 0) {
        mtkAssert((char*)"RIL_Init set detach state fail");
    }
...
#ifndef MTK_HYP_DUAL_TELE_SUPPORT
    /// MTK_RIL_ADAPTER @{
    handleRequestComplete(t, e, response, responselen);
    /// @}
#endif
#endif
...
#ifdef TBOX
    /// MTK_RIL_ADAPTER @{
    unsolResponse = handleUnsolicitedResponse(unsolResponse, data, datalen, toRealSlot(socket_id));
    /// @}
#endif
...
#ifdef TBOX
    /// MTK_RIL_ADAPTER @{
    unsolResponse = handleUnsolicitedResponse(unsolResponse, data, datalen, toRealSlot(RIL_SOCKET_1));
    /// @}
#endif
...
#ifndef MTK_HYP_DUAL_TELE_SUPPORT
    /// MTK_RIL_ADAPTER @{
    handleSapRequestComplete(t, e, response, responselen);
    /// @}
#endif
#endif
...
#ifdef TBOX
    /// MTK_RIL_ADAPTER @{
    handleSapUnsolicitedResponse(unsolResponse, data, datalen, toRealSlot(socket_id));
    /// @}
#endif
...
#ifdef TBOX
    /// MTK_RIL_ADAPTER @{
    handleSapUnsolicitedResponse(unsolResponse, data, datalen, toRealSlot(RIL_SOCKET_1));
    /// @}
#endif
