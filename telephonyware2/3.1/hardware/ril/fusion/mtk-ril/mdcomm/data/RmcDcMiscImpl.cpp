...
RmcDcMiscImpl::RmcDcMiscImpl(RfxBaseHandler* handler, int slotId) :
        mHandler(handler), mSlotId(slotId), mCurrentFdState(1) {
}
...
void RmcDcMiscImpl::syncIwlanOperationModeToMd() {
    ...
    if (mHandler->getFeatureVersion(feature, 1) == 2) {
        // for Inter Wireless LAN Priority List Enable
        // AT+EIWLPLEN = <on/off>, 2: on with new format, 1:on with legacy format, 0:off
        //
        // [Legacy]
        // AT+EIWLPL=1
        // +EIWLPL="<cmd>", <PDN type>, <priority for setup>, <priority for cellular>, <priority for wifi>, "<for log pring>"
        //
        // [New]
        // AT+EIWLPL=2
        // +EIWLPL="<cmd>", <PDN type>, <number of RAT list>[, <rat1>[, !K]]
        responseFromModem = mHandler->atSendCommand(String8::format("AT+EIWLPLEN =%d", 2));
        return ;
    } else {
        // for Inter Wireless LAN Priority List Enable
...
void RmcDcMiscImpl::setFdMode(const sp<RfxMclMessage>& msg) {
    ...
    if (atoi(propertyValue) == 0) {
        RFX_LOG_E(RFX_LOG_TAG, "FastDormancy initialize not finish");
        responseToTcl = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS,
                RfxVoidData(), msg, true);
        responseToTelCore(responseToTcl);
        return;
...
void RmcDcMiscImpl::initializeFastDormancy() {
    ...
    if (response != NULL && !response->isAtResponseFail()) {
        ...
        atSendCommand(String8::format("AT+EFD=%d", mCurrentFdState));
    }
}
