...
#include "mtk_log.h"
#undef LOG_TAG
#define LOG_TAG "SmsHeader"
/*****************************************************************************
 * Class SmsHeader
 *****************************************************************************/
//RFX_IMPLEMENT_CLASS("SmsHeader", SmsHeader, RfxObject);
...
SmsHeader* SmsHeader::fromByteArray(BYTE* data, int headerLength) {
    ...
    while (curr < headerLength) {
        ...
        SLOGD("fromByteArray header element length: %d, id: %d", id, length);
        ConcatRef concatRef;
        PortAddrs portAddrs;
        int temp1 = 0;
        int temp2 = 0;
        ...
        case ELT_ID_CONCATENATED_8_BIT_REFERENCE:
            ...
            SLOGD("fromByteArray concatRef.refNumber: %d,"
        ...
        case ELT_ID_APPLICATION_PORT_ADDRESSING_8_BIT:
            ...
            SLOGD("fromByteArray portAddrs.destPort: %d,"
        ...
        case ELT_ID_NATIONAL_LANGUAGE_SINGLE_SHIFT:
            ...
            SLOGD("fromByteArray mLanguageShiftTable: %d", smsHeader->mLanguageShiftTable);
            break;
        case ELT_ID_NATIONAL_LANGUAGE_LOCKING_SHIFT:
            smsHeader->mLanguageTable = data[curr++];
            SLOGD("fromByteArray mLanguageShiftTable: %d", smsHeader->mLanguageTable);
            break;
        case ELT_ID_SPECIAL_SMS_MESSAGE_INDICATION:
            ...
            SLOGD("fromByteArray specialSmsMsg.msgIndType: %d,"
        ...
        default:
            ...
            SLOGD("fromByteArray miscElt.id: %d", miscElt.id);
            break;
        }
    }
    return smsHeader;
...
BYTE * SmsHeader::toByteArray(SmsHeader* smsHeader) {
    int curr = 0;
    if ((smsHeader->mSpecialSmsMsgList.empty()) &&
        (smsHeader->mMiscEltList.empty()) &&
        (smsHeader->mLanguageShiftTable == 0) &&
        (smsHeader->mLanguageTable == 0)) {
        return NULL;
    }
    BYTE * output = new BYTE[MAX_USER_DATA_BYTES];
    ConcatRef concatRef = smsHeader->mConcatRef;
    if(concatRef.isEightBits) {
        output[curr++] = ELT_ID_CONCATENATED_8_BIT_REFERENCE;
        output[curr++] = 3;
        output[curr++] = concatRef.refNumber;
    } else {
        output[curr++] = ELT_ID_CONCATENATED_16_BIT_REFERENCE;
        output[curr++] = 4;
        output[curr++] = concatRef.refNumber >> 8;
        output[curr++] = concatRef.refNumber & 0x00FF;
    }
    output[curr++] = concatRef.msgCount;
    output[curr++] = concatRef.seqNumber;

    PortAddrs portAddrs = smsHeader->mPortAddrs;
    return output;
    //TODO : something wrong in the SMS Header variable mConcatRef and mportAddrs, this should be a pointer
}
