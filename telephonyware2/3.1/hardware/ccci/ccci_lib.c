...
#include "platform/ccci_lib_platform.h"
#include "ccci_lib.h"

/*=================  debug option setting ============================*/
/* #define CCB_POLL_LOG_EN */
//----------------debug log define-----------------//
#define CCCI_LIB_TAG   "[ccci_lib]"
...
static const char *mdimg_node[IMG_NODE_SIZE] = {
    ...
    "/dev/disk/by-partlabel/md3img"
};

/*return 0 if image is found, and return negative value if not*/
int find_image_from_pt_internal(int which_md_fd, const char *img_name, int *offset, int *img_size)
{
    ...
    *offset = 0;
    ...
    do {
        ...
        p_hdr->info.name[IMG_NAME_SIZE - 1] = '\0';
        ...
        if (p_hdr->info.magic!=IMG_MAGIC) {
            *offset += sec_padding_size;
                        ...
                        if (ret < 0) {
                                LOGE("lseek img fail! offset = %d, ret = %d, errno = %d\n",
                                *offset, ret, errno);
                                goto find_exit;
                        }
            load_size = read(fd, (char *)p_hdr, sizeof(prt_img_hdr_t));
...
int find_image_from_pt(const char *img_name, int *offset, int *img_size)
{
    int fd = -1, ret = -1, i = 0;
...
{
        int ret = 0, fd = -1, r_len = 0, rr_len = 0, offset = 0, img_size = 0;
        ...
        ret = snprintf(partition_path, PARTITION_PATH_LEN, "%s%s", mdimg_node[which_md], buf);
        if (ret <= 0 || ret >= PARTITION_PATH_LEN) {
                LOGE("snprintf md1img node fail!\n");
                close(fd);
                return -2;
        }
        ret = 0;
...
static int parse_info(const char raw_data[], int raw_size, char name[], char val[], int size)
{
    int i, j=0;
    char tmp_name[32];
    char tmp_val[32] = {0};
...
static int ccci_ccb_ctrl_get(struct ccb_ctrl_info *ctrl_info, unsigned char **address)
{
    ...
    ret = snprintf(dev_port, 32, "%s", "/dev/ccci_ccb_ctrl");
    if (ret < 0 || ret >= 32) {
        LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return -EFAULT;
    }
...
{
    int ctrl_fd = -1;
    unsigned char *ctrl_base_address = NULL, *ptr = NULL;
    unsigned int i;
    unsigned int ret = 0;
    struct buffer_header *header_ptr = NULL;
    ...
    ctrl_info.ctrl_length = 0;
    ctrl_info.ctrl_offset = 0;
...
{
    unsigned int i;
    unsigned char *ptr = NULL, *data_buf_ptr = NULL;
    unsigned int data = 0xFF;
    struct ccci_ccb_control_buff *buff = NULL;
...
/*cancel ccci_ccb_poll*/
int ccci_ccb_cancel_poll(unsigned int val)
{
    int ret = 0;
    ...
    if (!user)
        return -EFAULT;
    if (val) {
        LOGD("%s:set %u poll_exit\n", __func__, val);
        poll_exit = true;
        ret = ioctl(user->fd, CCCI_IOC_SMEM_POLL_EXIT);
        if (ret < 0) {
            LOGE("cancel_poll fail, ret=%d\n", ret);
            return ret;
        }
    } else {
        LOGD("%s:set %u reset ccb_poll\n", __func__, val);
        poll_exit = false;
    }
    return ret;
}
...
int ccci_ccb_write_done(unsigned int buffer_id, unsigned char *address, unsigned int length)
{
    struct ccci_ccb_control_buff *buff = NULL;
    struct page_header *page = NULL;
    struct ccci_ccb_config *config = NULL;
    int ret;
    unsigned int data, i;
    ...
    if (!config) {
        LOGE("ccci_ccb_query_config of user%d buffer%d fail!\n", user->user_id, buffer_id);
        return -EFAULT;
    }
    data = config->core_id;
