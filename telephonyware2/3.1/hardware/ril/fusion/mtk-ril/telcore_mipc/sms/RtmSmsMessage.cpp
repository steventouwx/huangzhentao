...
#define RFX_LOG_TAG "RtmSmsMessage"
...
void RtmSmsMessage::createSmsUserDataHeader(unsigned char * header,int len,bool is3Gpp2) {
    mUserDataHeader = new RtmSmsUserDataHeader(header, len, is3Gpp2);
}

/*****************************************************************************
 * RtmGsmSmsMessage
 *****************************************************************************/
RtmGsmSmsMessage::RtmGsmSmsMessage(unsigned char *hexString, size_t dataLen) :
    mPdu(NULL),
...
RtmGsmSmsMessage::RtmGsmSmsMessage(
    const RtmGsmSmsMessage &message,
    int encodingType,
    int numFields,
...
void RtmGsmSmsMessage::parseSmsDeliver(int firstByte) {
    bool hasUserDataHeader = (firstByte & 0x40) == 0x40;

    // Address
    int addressLength = mPdu[mCur] & 0xff;
    int lengthBytes = 2 + (addressLength + 1) / 2;
    setSmsAddress(new RtmGsmSmsAddress(mPdu, mCur, lengthBytes));
    mCur += lengthBytes;
...
int RtmGsmSmsMessage::parseDataCodingScheme(int dataCodingScheme) {
 ...
 * RtmCdmaSmsMessage
 *****************************************************************************/
RtmCdmaSmsMessage::RtmCdmaSmsMessage(RIL_CDMA_SMS_Message *pMessage) {
    if ((pMessage->bIsServicePresent != 1) &&
            (pMessage->uTeleserviceID == TELESERVICE_WMT ||
            pMessage->uTeleserviceID == TELESERVICE_WEMT)) {
        InboundSmsInfo info;
        RmmCdmaSmsConverter::getInboundSmsInfo(pMessage, &info);
        ...
        setSmsAddress(new RtmCdmaSmsAddress(pMessage->sAddress));
    }
    mMsg = *pMessage;
}

RtmCdmaSmsMessage::RtmCdmaSmsMessage(
        const RtmCdmaSmsMessage &message,
        int encodingType,
        int numFields,
        const Vector<unsigned char> &userDataPayload) {
    if (encodingType != message.getEncodingType()) {
    ...
    RmmCdmaSmsConverter::replaceUserDataPayload(&mMsg, userDataPayload, msgEncoding, 0, numFields);
}
