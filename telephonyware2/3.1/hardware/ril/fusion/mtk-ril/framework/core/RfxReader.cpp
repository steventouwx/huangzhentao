...
char* RfxReader::readline(char *buffer) {
    ...
    while (p_eol == NULL) {
        ...
        do {
            ...
            if (RfxRilUtils::isUserLoad() != 1) {
                if (count < 0) {
                    printLog(ERROR, String8::format("AT read end: %zd (err: %d - %s)\n", count,
        ...
        if (count > 0) {
            ...
            if (count >= (MAX_AT_RESPONSE - 1)) {
                printLog(ERROR, String8::format("AT read buffer full end:%zd\n", count));
                char fullString[100] = {0};
                if (strncpy(fullString, p_read, 99) != NULL) {
                    fullString[99] = '\0';
                    printLog(ERROR, String8::format("AT read buffer full end:%s\n", fullString));
                }
            }
...
void RfxReader::handleUnsolicited(RfxAtLine* line1, RfxAtLine* line2) {
    ...
    // send each raw urc to oem module
    char enabled[RFX_PROPERTY_VALUE_MAX] = {0};
    rfx_property_get("persist.vendor.radio.oem_urc_enable", enabled, "0");
    if (atoi(enabled) == 1) {
        sp<RfxMclMessage> oemMsg = RfxMclMessage::obtainEvent(RFX_MSG_EVENT_OEM_RAW_URC,
                RfxStringData(msg->getRawUrc()->getLine(), strlen(msg->getRawUrc()->getLine())),
                RIL_CMD_PROXY_3, m_channel_id/RIL_CHANNEL_OFFSET);
        RfxMclDispatcherThread::enqueueMclMessage(oemMsg);
    }

    // set to MclDispatcherThread
...
void RfxReader::readerLoopForFragData() {
    ...
    while (m_fd < 0) {
        if (m_fd == IMS_FD_USELESS) {
            printLog(INFO, String8::format("submarine ims reader2, quit as useless"));
            free(header);
            return;
        }
        usleep(50 * 1000);
    }
    printLog(INFO, String8::format("Vilte fd initialized done"));
