...
unsigned char* RfxFragmentEncoder::encodeHeader(RfxFragmentData data) {
    ...
    for (size_t i=0; i<HEADER_SIZE; i++) {
        /* When something wrong on asprintf, tempLog will be undefined(not NULL) and
           the return value of asprintf will be -1. */
        retVal = asprintf(&tmpLog, "%02X", header[i]);
        if ((retVal >= 0) && (tmpLog != NULL)) {
