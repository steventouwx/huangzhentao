...
#include "platform/fsd_platform.h"
#include <pthread.h>
#include "../ccci_common.h"

#define PROFILE_NVRAM_API
/* debug for syscall time cost, write log to main_log */
//#define DEBUG_SYSATO_TIMEST
...
static char FsRootDir_MD1[FS_MAX_DIR_NUM][36] = {FS_ROOT_DIR_MD1, FS_ROOT_DIR1_MD1,
                                                 FS_ROOT_DIR2_MD1, FS_ROOT_DIR_DSP, FS_ROOT_COMMON,
                                                 FS_ROOT_MDLPM, FS_ROOT_DIR3_MD1, FS_ROOT_DIR4_MD1,
                                                 FS_ROOT_DIR5_MD1, FS_ROOT_DIR6_MD1};
static char *MdRootDir[FS_MAX_DIR_NUM] = {MD_ROOT_DIR, MD_ROOT_DIR1, MD_ROOT_DIR2, MD_ROOT_DIR_DSP,
                                          ...
                                          MD_ROOT_DIR7, MD_ROOT_DIR8};

static unsigned DrvIdx[FS_MAX_DIR_NUM] = {'Z', 'X', 'Y', 'W', 'V', 'U', 'T', 'S', 'R', 'Q'};

#define STS_NO_READ    0
#define STS_START_READ 1
#define STS_READING    2

static int stream_support = 0;
static int s_need_stop = 0;
static int s_read_status = STS_NO_READ;
static int DeviceFd = -1;
static pthread_mutex_t s_mtx_lock = PTHREAD_MUTEX_INITIALIZER;
...
static int s_print_error_log = 0;
static int first_recv_req_flg = 0;
static long long cccireadtmcost = 0;
static unsigned int cccireadtimes = 0;
static fsd_sysctminfo g_sysctmInfo[MAX_FSD_SYSCTYPE];
...
static pthread_t sg_main_thread;
static int sg_main_argc;
static int sg_main_md_id;
static int s_is_v1;


void SyscallTimeInfoInit(void)
{
    cccireadtmcost = 0;
    cccireadtimes = 0;
    memset(g_sysctmInfo, 0, (sizeof(fsd_sysctminfo) * MAX_FSD_SYSCTYPE));
}


void setfirstrcvreqflg(int flg)
{
    first_recv_req_flg = flg;
}


static long long FS_Timestamp(struct timespec time_s, struct timespec time_e)
{
    long long diff = 0;

    if (time_e.tv_nsec < time_s.tv_nsec) {
        diff = (long long)(1000000000 + time_e.tv_nsec - time_s.tv_nsec) ;
        diff += (long long)(time_e.tv_sec - time_s.tv_sec - 1) * 1000000000;
    }    else {
        diff = (long long)(time_e.tv_nsec - time_s.tv_nsec);
        diff += (long long)(time_e.tv_sec - time_s.tv_sec) * 1000000000;
    }
...
static const char *GetsyscallName(unsigned int sysctype)
{
    switch(sysctype)
    {
    case CCCI_FSD_READ:
    ...
    case CCCI_FSD_FSYNC:
        return "fsync";
    case CCCI_FSD_UNLINK:
        return "unlink";
    case CCCI_FSD_OPENDIR:
        return "opendir";
    case CCCI_FSD_READDIR:
        return "readdir";
    case CCCI_FSD_CLOSEDIR:
        return "closedir";
    case CCCI_FSD_STAT:
        return "stat";
    case CCCI_FSD_MKDIR:
    ...
    case CCCI_FSD_RMDIR:
        return "rmdir";
    case CCCI_FSD_ACCESS:
        return "access";
    case CCCI_FSD_UMASK:
        return "umask";
    case CCCI_FSD_SIGNAL:
        return "signal";
    case CCCI_FSD_IOCTL:
        return "ioctl";
    case CCCI_FSD_RENAME:
        return "rename";
    case CCCI_FSD_CREAT:
        return "creat";
    case CCCI_FSD_MMAP:
        return "nmap";
    case CCCI_FSD_MUNMAP:
        return "munmap";
    case CCCI_FSD_LOCALTIME:
        return "localtime";
    default:
        return "undefined";
    }
...
static void SaveSyscallTime(int syscalltype, struct timespec time_start,
                            const char *__attribute__((unused))func)
{
    ...
    timecost = FS_Timestamp(time_start, time_end);

#ifdef DEBUG_SYSATO_TIMEST
    LOGI("%s : [%s] [sys_interval] : %lld ns\n",func, GetsyscallName(syscalltype), timecost);
#endif

    if(syscalltype < 0 || syscalltype >= MAX_FSD_SYSCTYPE) {
        LOGE("SaveSyscallTime: [error]syctype  = %d\n", syscalltype);
        return;
    }

    g_sysctmInfo[syscalltype].times++;
    g_sysctmInfo[syscalltype].timecost += timecost;

    return;
}


void WriteSyscalltimeInfoTolog(unsigned int logflg, struct timespec time_start)
{
    unsigned int i = 0;
    unsigned int sysctsum = 0;
    long long tmcstsum = 0;
    const char *sycname;
    struct timespec time_end;

    clock_gettime(CLOCK_MONOTONIC, &time_end);

    for(i = 0; i < MAX_FSD_SYSCTYPE; i++)
    {
        if(g_sysctmInfo[i].times == 0) {
            continue;
        }
        sycname = GetsyscallName(i);
        if(sycname == NULL) {
            LOGE("GetSyscallName: [error]sycname is NULL\n");
            continue;
        }
        sysctsum += g_sysctmInfo[i].times;
        tmcstsum += g_sysctmInfo[i].timecost;
        LOGI("syscalltime: [%s]  [times]: %d [timecost] %lld ns\n",
             sycname, g_sysctmInfo[i].times, g_sysctmInfo[i].timecost);
        //write_ccci_dump("syscalltime: [%s]  [times]: %d [timecost] %lld ns\n",
        //                sycname, g_sysctmInfo[i].times, g_sysctmInfo[i].timecost);
        g_sysctmInfo[i].times = 0;
        g_sysctmInfo[i].timecost = 0;

    }
    LOGI("total fs sys call(except read for wait packet): [times]: %d [timecost]: %lld ns\n",
         sysctsum, tmcstsum);
    LOGI("total read sys call for wait packet: [times]: %d [timecost]: %lld ns\n",
         cccireadtimes, cccireadtmcost);
    //write_ccci_dump("total fs sys call(except read for wait packet) : [times]: %d [timecost]: %lld ns\n",
    //                sysctsum, tmcstsum);
    //write_ccci_dump("total read sys call for wait packet: [times]: %d [timecost]: %lld ns\n",
    //                cccireadtimes, cccireadtmcost);
    cccireadtimes = 0;
    cccireadtmcost = 0;

    if(logflg == 0) {
        LOGI("md bootup end time  :(%ld %ld)\r\n",time_end.tv_sec, time_end.tv_nsec);
        //write_ccci_dump("md bootup end time  :(%ld %ld)\r\n",time_end.tv_sec, time_end.tv_nsec);
    }
    else {
        if(time_start.tv_sec == 0 && time_start.tv_nsec == 0) {
            CCCI_LOGI("MD power off time :(%ld %ld)\n", time_end.tv_sec, time_end.tv_nsec);
            return;
        }
        else {
            CCCI_LOGI("MD power off time [start]: (%ld %ld) [end] :(%ld %ld), [timecost]: %lld ns\n ",
                      time_start.tv_sec, time_start.tv_nsec, time_end.tv_sec,
                      time_end.tv_nsec, FS_Timestamp(time_start, time_end));
            return;
        }
    }
...
static int Read_Tm(int fildes, void *buf, size_t count, const char *func)
{
    ssize_t count_ret = 0;
    struct timespec time_start;

    clock_gettime(CLOCK_MONOTONIC, &time_start);
    count_ret = read(fildes, buf, count);
    SaveSyscallTime(CCCI_FSD_READ, time_start, func);
    return count_ret;
}

static int Read_CCCI_Tm(int fildes, void *buf, size_t count)
{
    ssize_t count_ret = 0;
    ...
    count_ret = read(fildes, buf, count);
    clock_gettime(CLOCK_MONOTONIC, &time_end);
    cccireadtmcost += FS_Timestamp(time_start, time_end);
    cccireadtimes++;
    return count_ret;
}


static int Write_Tm(int fildes, const void *buf, size_t count,
                    const char *__attribute__((unused)) func)
{
    ssize_t count_ret = 0;
    struct timespec time_start;
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    count_ret = write(fildes, buf, count);
    SaveSyscallTime(CCCI_FSD_WRITE, time_start, func);
    return count_ret;
}


static int Open_Tm(const char *path, int oflags, const char *__attribute__((unused)) func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    SaveSyscallTime(CCCI_FSD_OPEN, time_start, func);
    return ret;
}


static int Open_Tm3(const char *path, int oflags, mode_t mode,
                    const char *__attribute__((unused)) func)
{
    int ret = 0;
    struct timespec time_start;
...
static int Close_Tm(int fildes, const char *func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    ret = close(fildes);
    SaveSyscallTime(CCCI_FSD_CLOSE, time_start, func);
    return ret;
}


static int Lseek_Tm(int fildes, off_t offset, int whence, const char *func)
{
    off_t offset_addr = 0;
    struct timespec time_start;

    clock_gettime(CLOCK_MONOTONIC, &time_start);
    offset_addr = lseek(fildes, offset, whence);
    SaveSyscallTime(CCCI_FSD_LSEEK, time_start, func);
    return offset_addr;
}


static int Fsync_Tm(int fildes, const char *func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    ret = fsync(fildes);
    SaveSyscallTime(CCCI_FSD_FSYNC, time_start, func);
    return ret;
}


static int Fstat_Tm(int fildes, struct stat *buf,const char *func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    SaveSyscallTime(CCCI_FSD_FSTAT, time_start, func);
    return ret;
}


static int Unlink_Tm(const char *pathname, const char *func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    ret = unlink(pathname);
    SaveSyscallTime(CCCI_FSD_UNLINK, time_start, func);
    return ret;
}


static DIR* Opendir_Tm(const char *pathname, const char *func)
{
    DIR* dir = NULL;
    struct timespec time_start;
    ...
    SaveSyscallTime(CCCI_FSD_OPENDIR, time_start, func);

    return dir;
}


static struct dirent *Readdir_Tm(DIR *dir_handle, const char *func)
{
    ...
    SaveSyscallTime(CCCI_FSD_READDIR, time_start, func);
    return dir;
}


static int Closedir_Tm(DIR *dir_handle, const char *func)
{
    int ret = 0;
    struct timespec time_start;

    clock_gettime(CLOCK_MONOTONIC, &time_start);
    ret = closedir(dir_handle);
    SaveSyscallTime(CCCI_FSD_CLOSEDIR, time_start, func);
    return ret;
}
...
static int Mkdir_Tm(const char *filename, mode_t mode, const char *func)
{
    int ret = 0;
    struct timespec time_start;

    clock_gettime(CLOCK_MONOTONIC, &time_start);
    //ret = mkdir(filename,mode);
    ret = makeDirs(filename,mode);
    SaveSyscallTime(CCCI_FSD_MKDIR, time_start, func);
    return ret;
}


static int Rmdir_Tm(const char *pathname, const char *func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    ret = rmdir(pathname);
    SaveSyscallTime(CCCI_FSD_RMDIR, time_start, func);
    return ret;
}


static int Statfs_Tm(const char *pathname, struct statfs *buf, const char *func)
{
    int ret = 0;
    struct timespec time_start;

    clock_gettime(CLOCK_MONOTONIC, &time_start);
    ret = statfs(pathname,buf);
    SaveSyscallTime(CCCI_FSD_STATFS, time_start, func);
    return ret;
}


static int Access_Tm(const char *pathname, int mode, const char *func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    SaveSyscallTime(CCCI_FSD_ACCESS, time_start, func);
    return ret;
}


static int Stat_Tm(const char *file, struct stat *StatBuf, const char *func)
{
    int ret = 0;
    struct timespec time_start;
    ...
    SaveSyscallTime(CCCI_FSD_STAT, time_start, func);
    return ret;
}


static mode_t Umask_Tm(mode_t mask, const char *func)
{
    mode_t mode = 0;
    struct timespec time_start;
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    mode = umask(mask);
    SaveSyscallTime(CCCI_FSD_UMASK, time_start, func);
    return mode;
}
...
static int FS_IsFile(const char *file)
{
    ...
    if(Stat_Tm(file, &StatBuf, __FUNCTION__) == -1)
    {
        local_errno = errno;
...
static int FS_GetFATAttr(const char *file)
{
    ...
    fd = Open_Tm(file, O_RDONLY, __FUNCTION__);
    if (fd < 0)
        goto err;
    clock_gettime(CLOCK_MONOTONIC, &time_s);
    rst = ioctl(fd, FAT_IOCTL_GET_ATTRIBUTES, &attr);
    SaveSyscallTime(CCCI_FSD_IOCTL, time_s, __FUNCTION__);

    if (ret)
        goto err;
    Close_Tm(fd, __FUNCTION__);
    return attr;
...
static void FS_EntryLinuxToDos(FS_DOSDirEntry *DosFileInfo, const char *file)
{
    ...
    SaveSyscallTime(CCCI_FSD_LOCALTIME, time_s, __FUNCTION__);
    ...
    if(AccessTime != NULL)
    {
        ...
        DosFileInfo->CreateDateTime = (((AccessTime->tm_sec  & 0x1F)) |
    ...
    if(ModifyTime != NULL)
    {
        ...
        //Month: 4 bit in DOS DateTime stucture
        //Year: 7 bit in DOS DateTime stucture and year in DOS is from 1980 but year in Linux is from 1900
        DosFileInfo->DateTime = (((ModifyTime->tm_sec  & 0x1F)) |
...
RETRY_WRITE:
            ...
            while ((int)(data_sent + sizeof(CCCI_BUFF_T) + sizeof(unsigned int)) < DataLength) {
                ...
                // Add CCCI header and operation ID size to packet size,
                // be aware of that OP_ID is not cosindered as payload,
                // so not counted in MAX_FS_PKT_BYTE
                ...
                if (ret != pkt_size) {
                    ...
                    LOGD("Write %d bytes to slot %d, CCCI_H(0x%X)(0x%X)(0x%X)(0x%X)\n",
                         ret, BufIndex, dst_ccci_h->data[0], dst_ccci_h->data[1],
                         dst_ccci_h->channel, dst_ccci_h->reserved);
                }
...
static void CCCI_Dump_FS_Path(const char *FsFileName)
{
    ...
    if (f_point_head == NULL)
    {
        LOGE("invalid filename\n");
        return;
    }
        ...
        if (f_point_tail != NULL) {
            if ((f_point_tail - f_point_head) != 0) {
                *f_point = '\0';
                ...
                FsFileName_new[f_point_tail - f_point_head] = '\0';
                ret = Access_Tm(FsFileName_new, F_OK, __FUNCTION__);
...
static int FS_CCCI_Open(const wchar_t *FileName, unsigned int Flag)
{
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if(strncmp(ConvFileName, MdRootDir[i], strlen(MdRootDir[i])) == 0)
        {
            ...
            if(strncmp(ConvFileName, MD_ROOT_DIR_DSP, strlen(MD_ROOT_DIR_DSP)) == 0)
            {
                ...
                SaveSyscallTime(CCCI_FSD_IOCTL, time_s, __FUNCTION__);
                FsFileNameCount = snprintf(FsFileName, sizeof(FsFileName), "%s/dsp_%s.bin",
                                           FsRootDir[i], md_post_fix);
                if(FsFileNameCount >= sizeof(FsFileName)) {
                    LOGE("%s-%d : snprintf fail , FsFileNameCount = %d\n",
                         __func__, __LINE__, FsFileNameCount);
                    ret = FS_GENERAL_FAILURE;
                    goto _Exit;
                }

                if(Access_Tm(FsFileName, F_OK, __FUNCTION__) != 0) {
                    LOGE("DSP image %s not exist, try CIP path %s\n", FsFileName, FS_ROOT_DIR_DSP_CIP);
                    FsFileNameCount = snprintf(FsFileName, sizeof(FsFileName), "%s/dsp_%s.bin",
                                               FS_ROOT_DIR_DSP_CIP, md_post_fix);
                    if(FsFileNameCount >= sizeof(FsFileName)) {
                        LOGE("%s-%d : snprintf fail , FsFileNameCount = %d\n",
                             __func__, __LINE__, FsFileNameCount);
                        ret = FS_GENERAL_FAILURE;
                ...
                FsFileNameCount = snprintf(FsFileName, sizeof(FsFileName), "%s%s",
                                           FsRootDir[i], (ConvFileName + strlen(MdRootDir[i])));
                if(FsFileNameCount >= sizeof(FsFileName)) {
                    LOGE("%s-%d : snprintf fail , FsFileNameCount = %d\n",
                         __func__, __LINE__, FsFileNameCount);
                    ret = FS_GENERAL_FAILURE;
...
#endif
    Fd = Open_Tm3(FsFileName, LinuxFlag, 0660, __FUNCTION__);
#ifdef PROFILE_NVRAM_API
    if (slower_100ms.profiling)
    {
        ...
        if (elapsed_in_ms > 100)
        {
            ...
            LOGE("Write: [overtime]elapsed_in_ms: %ld %ld %ld %ld, %d\n", time_start.tv_sec,
    ...
    else
    {
        ...
        if(FsFileNameCount <= -1)
        {
            LOGE("%s : FsFileNameCount <= -1\n", __func__);
            ret = FS_GENERAL_FAILURE;
            goto _Exit;
        }
...
_Exit:
    ...
    if (debug_wakeup)
        print_file_name("O", ConvFileName, strlen(ConvFileName));
    else if(len >= 8) {
        ...
        LOGD("O: [%02X%02X%02X%02X%02X%02X%02X%02X](0x%08X): (%d %d)\n",
             (ConvFileName[len - 8] - 32), (ConvFileName[len - 7] - 32),
             (ConvFileName[len - 6] - 32), (ConvFileName[len - 5] - 32),
             (ConvFileName[len - 4] - 32), (ConvFileName[len - 3] - 32),
             (ConvFileName[len - 2] - 32), (ConvFileName[len - 1] - 32),
             Flag, Fd, ret);
    }
    else {
        LOGD("O: len < 8(%d)\n", len);
...
static int FS_CCCI_Seek(int HandleIndex, int Offset, int Whence)
{
    ...
    if (HandleIndex >= FS_FILE_MAX || HandleIndex < 0) {
        ...
        goto Exit;
              ...
              g_FsInfo.hFileHandle[HandleIndex].fInUse != true)
    {
        LOGE("Seek: [error]Invalid handle for %d, %ld, %d\n", HandleIndex,
    ...
    NewPos = Lseek_Tm(Fd, (off_t) Offset, LinuxWhence, __FUNCTION__);
    if(NewPos == (off_t) -1)
    {
        local_errno = errno;
...
Exit:
    ...
    if (debug_wakeup && HandleIndex < FS_FILE_MAX && HandleIndex >= 0)
        print_file_name("S", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
...
static int FS_CCCI_Read(int HandleIndex, void *pBuffer, int NumOfByte, unsigned int *ReadByte)
{
    ...
    if (HandleIndex >= FS_FILE_MAX || HandleIndex < 0) {
        ...
        goto Exit;
              ...
              g_FsInfo.hFileHandle[HandleIndex].fInUse != true)
    {
        LOGE("Read: [error]Invalid handle for %d, %ld, %d\n", HandleIndex,
             g_FsInfo.hFileHandle[HandleIndex].hFile, g_FsInfo.hFileHandle[HandleIndex].fInUse);
        ret = FS_INVALID_FILE_HANDLE;
        goto Exit;
    }
    ...
    //HandleIndex, Fd);
    *ReadByte = Read_Tm(Fd, pBuffer, (size_t)NumOfByte, __FUNCTION__);
...
Exit:
    ...
    if (debug_wakeup && HandleIndex < FS_FILE_MAX && HandleIndex >= 0)
        print_file_name("R", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
                        strlen(g_FsInfo.hFileHandle[HandleIndex].pFsFileName));
    else
...
static int FS_CCCI_Write(int HandleIndex, void *pBuffer, int NumOfByte, int *WriteByte)
{
    ...
    if (HandleIndex >= FS_FILE_MAX || HandleIndex < 0) {
        ...
        goto Exit;
              ...
              g_FsInfo.hFileHandle[HandleIndex].fInUse != true)
    {
        LOGE("Write: [error]Invalid handle for %d, %ld, %d\n", HandleIndex,
             g_FsInfo.hFileHandle[HandleIndex].hFile, g_FsInfo.hFileHandle[HandleIndex].fInUse);
        ret = FS_INVALID_FILE_HANDLE;
        goto Exit;
    }
    ...
    clock_gettime(CLOCK_REALTIME, &time_start);
#endif
    *WriteByte = Write_Tm(Fd, pBuffer, (size_t)NumOfByte, __FUNCTION__);
#ifdef PROFILE_NVRAM_API
    if (slower_100ms.profiling)
    {
        clock_gettime(CLOCK_REALTIME, &time_end);
...
Exit:
    ...
    if (debug_wakeup && HandleIndex < FS_FILE_MAX && HandleIndex >= 0)
        print_file_name("W", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
                        strlen(g_FsInfo.hFileHandle[HandleIndex].pFsFileName));
    else
...
static int FS_CCCI_Close(int HandleIndex)
{
    ...
    if (HandleIndex < 0 || HandleIndex >= FS_FILE_MAX) {
        LOGE("Close: [error] Handle index %d out-of-border.\n", HandleIndex);
        ret = FS_INVALID_FILE_HANDLE;
        goto Exit;
              ...
              g_FsInfo.hFileHandle[HandleIndex].fInUse != true)
    {
        LOGE("Close: [error]Invalid handle for : %d, %ld, %d\n",
             HandleIndex,
             g_FsInfo.hFileHandle[HandleIndex].hFile, g_FsInfo.hFileHandle[HandleIndex].fInUse);
        ret = FS_INVALID_FILE_HANDLE;
        goto Exit;
    ...
    //LOGE("FS_CCCI_Close:%s: (%d, %d)\n", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
    //    HandleIndex, g_FsInfo.hFileHandle[HandleIndex].hFile);
    if(g_FsInfo.hFileHandle[HandleIndex].fSearch)
    {
        if(Closedir_Tm((DIR*)g_FsInfo.hFileHandle[HandleIndex].hFile, __FUNCTION__) != 0)
        {
            local_errno = errno;
        ...
        if (g_FsInfo.hFileHandle[HandleIndex].Flag & (FS_CREATE|FS_CREATE_ALWAYS|FS_COMMITTED)) {
            ...
            ret=Fsync_Tm(g_FsInfo.hFileHandle[HandleIndex].hFile, __FUNCTION__);
            LOGD("C:fsync- %d, %d, FLAG:0x%x\n", HandleIndex, ret,
                 g_FsInfo.hFileHandle[HandleIndex].Flag);
            if(ret < 0)
            {
        ...
        clock_gettime(CLOCK_REALTIME, &time_start);
#endif
        if(Close_Tm(g_FsInfo.hFileHandle[HandleIndex].hFile, __FUNCTION__) == -1) {
            local_errno = errno;
            LOGE("Close: [error]fail close %s(%d): %d\n",
                 g_FsInfo.hFileHandle[HandleIndex].pFsFileName, HandleIndex, local_errno);
            ret = FS_ErrorConv(local_errno);
        ...
        if (slower_100ms.profiling)
        {
            ...
            if (elapsed_in_ms > 100)
            {
                slower_100ms.count++;
                slower_100ms.totaltime += elapsed_in_ms;
                LOGE("Close: [overtime]elapsed_in_ms: %ld %ld %ld %ld, %d\n", time_start.tv_sec,
                     time_start.tv_nsec, time_end.tv_sec, time_end.tv_nsec, elapsed_in_ms);
            }
        }
#endif
    ...
    if (debug_wakeup && g_FsInfo.hFileHandle[HandleIndex].pFsFileName) {
        print_file_name("C", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
                        strlen(g_FsInfo.hFileHandle[HandleIndex].pFsFileName));
        debug_wakeup = 0;
    }

    free(g_FsInfo.hFileHandle[HandleIndex].pFsFileName);
    g_FsInfo.hFileHandle[HandleIndex].pFsFileName = NULL;
...
int FS_CCCI_CloseAll(void)
{
    int i;
    int ret = FS_GENERAL_FAILURE;
    int local_errno;

    LOGE("CloseAll: FS_FILE_MAX: %d; FileNum: %d\n", FS_FILE_MAX, g_FsInfo.FileNum);

    for(i = 0; i < FS_FILE_MAX; i++)
    {
        if(g_FsInfo.hFileHandle[i].fInUse)
        {
            LOGE("CloseAll: hFileHandle[%d]; fInUse: %d; fSearch: %d; hFile: %ld; pFsFileName: %s\n",
                 i, g_FsInfo.hFileHandle[i].fInUse,
                 g_FsInfo.hFileHandle[i].fSearch,
                 g_FsInfo.hFileHandle[i].hFile,
                 g_FsInfo.hFileHandle[i].pFsFileName);
            ...
            {
                if(Close_Tm(g_FsInfo.hFileHandle[i].hFile, __FUNCTION__) == -1)
                {
                    local_errno = errno;
...
static int FS_CCCI_CreateDir(const wchar_t *FileName)
{
    ...
    if(ConvFileName[0] == 'U')
    {
        if(Stat_Tm(FS_ROOT_MDLPM, &buf, __FUNCTION__) < 0) {
            LOGD("%s:stat %s dir error, ret=%d errno=%d.\n",
                 __func__, FS_ROOT_MDLPM, ret, errno);
            if(Mkdir_Tm(FS_ROOT_MDLPM, MD_DIR_ATTR, __FUNCTION__) < 0)
                LOGE("%s:mkdir %s failed, ret=%d,errno=%d.\n",
                     __func__, FS_ROOT_MDLPM, ret, errno);
        }
    }
    if(Mkdir_Tm(FsFileName, MD_DIR_ATTR, __FUNCTION__) == -1)
    {
        local_errno = errno;
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("CD", ConvFileName, len);
    else if(len >= 8) {
...
static int FS_CCCI_RemoveDir(const wchar_t *FileName)
{
    ...
    if(Rmdir_Tm(FsFileName, __FUNCTION__) == -1) {
        local_errno = errno;
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("MD", ConvFileName, len);
    else if(len >= 8) {
        if (is_protect_path(ConvFileName))
            LOGW("MD: %s, ret %d\n", ConvFileName, ret);
...
static int FS_CCCI_GetFileSize(int HandleIndex, unsigned int *pFileSize)
{
    ...
    if (HandleIndex >= FS_FILE_MAX || HandleIndex < 0) {
        ...
        goto Exit;
              ...
              g_FsInfo.hFileHandle[HandleIndex].fInUse != true)
    {
        LOGE("GetFileSize: [error]Invalid handle %d, %ld, %d\n", HandleIndex,
             g_FsInfo.hFileHandle[HandleIndex].hFile, g_FsInfo.hFileHandle[HandleIndex].fInUse);
        ret = FS_INVALID_FILE_HANDLE;
        goto Exit;
    }
    ...
    if(Fstat_Tm(Fd, &StatBuf, __FUNCTION__) == -1) {
        local_errno = errno;
...
Exit:
    ...
    if (debug_wakeup && HandleIndex < FS_FILE_MAX && HandleIndex >= 0)
        print_file_name("F", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
                        strlen(g_FsInfo.hFileHandle[HandleIndex].pFsFileName));
    else
...
static int FS_CCCI_Rename(const wchar_t *FileName, const wchar_t *NewFileName)
{
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_s);
    rst = rename(FsFileName, FsNewFileName);
    SaveSyscallTime(CCCI_FSD_RENAME, time_s, __FUNCTION__);
...
Exit:
    ...
    if (debug_wakeup) {
        print_file_name("RN_0", ConvFileName, len1);
        print_file_name("RN_1", NewConvFileName, len2);
    }
    else if((len1 >= 8) && (len2 >= 8)) {
        LOGD("RN: [%02X%02X%02X%02X%02X%02X%02X%02X] -> [%02X%02X%02X%02X%02X%02X%02X%02X]: %d\n",
             (ConvFileName[len1 - 8] - 32), (ConvFileName[len1 - 7] - 32),
             (ConvFileName[len1 - 6] - 32), (ConvFileName[len1 - 5] - 32),
             (ConvFileName[len1 - 4] - 32), (ConvFileName[len1 - 3] - 32),
             (ConvFileName[len1 - 2] - 32), (ConvFileName[len1 - 1] - 32),
             (NewConvFileName[len2 - 8] - 32), (NewConvFileName[len2 - 7] - 32),
             (NewConvFileName[len2 - 6] - 32), (NewConvFileName[len2 - 5] - 32),
             (NewConvFileName[len2 - 4] - 32), (NewConvFileName[len2 - 3] - 32),
             (NewConvFileName[len2 - 2] - 32), (NewConvFileName[len2 - 1] - 32),
             ret);
    }
    else {
        LOGD("RN: len1:%d, len2:%d\n", len1, len2);
    }
    return ret;
}
...
static int FS_CCCI_Delete(const wchar_t *FileName)
{
    ...
    if(Unlink_Tm(FsFileName, __FUNCTION__) == -1)
    {
        local_errno = errno;
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("D", ConvFileName, len);
    else if(len >= 8) {
        if (is_protect_path(ConvFileName))
            LOGW("D: %s, ret %d\n", ConvFileName, ret);
...
static int FS_CCCI_GetClusterSize(unsigned int DriverIdx)
{
    ...
    if( Statfs_Tm(FsRootDir[i], &StatfsBuf, __FUNCTION__) == -1)
    {
        local_errno = errno;
...
static int FS_RecursiveSearch(const char *FsDirName, unsigned int Flag, unsigned int *pClusterCount)
{
    ...
    pDir = Opendir_Tm(FsDirName, __FUNCTION__);
    if(pDir == NULL)
    {
        local_errno = errno;
    ...
    while((pDirent = Readdir_Tm(pDir, __FUNCTION__))!= NULL)
    {
        ...
        if(Stat_Tm(FsFileName, &StatBuf, __FUNCTION__) == -1)
        {
            local_errno = errno;
...
static int FS_CCCI_GetFolderSize(const wchar_t *FullPath, unsigned int Flag)
{
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if(strncmp(ConvFullPath, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            ...
            if (FsFileNameCount < 0 || FsFileNameCount >= sizeof(FsFileName)) {
                LOGE("%s-%d:snprintf fail,FsFileNameCount = %d\n", __func__, __LINE__,
                     FsFileNameCount);
                goto Exit;
            }
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("FS", ConvFullPath, len);
    else if(len >= 8) {
        LOGD("FS: [%02X%02X%02X%02X%02X%02X%02X%02X](0x%08X): %d\n",
             (ConvFullPath[len - 8] - 32), (ConvFullPath[len - 7] - 32),
             (ConvFullPath[len - 6] - 32), (ConvFullPath[len - 5] - 32),
             (ConvFullPath[len - 4] - 32), (ConvFullPath[len - 3] - 32),
             (ConvFullPath[len - 2] - 32), (ConvFullPath[len - 1] - 32),
             Flag, ret);
    }
    else {
        LOGD("FS: len<8(%d)\n", len);
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("CN", ConvFullPath, len);
    else if(len >= 8) {
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("A", ConvFileName, len);
    else if(len >= 8) {
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("GetFileDetail", ConvFileName, len);
    else if(len >= 8) {
...
Exit:
    ...
    if (debug_wakeup)
        print_file_name("X", ConvFileName, len);
    else if(len >= 8) {
        if (is_protect_path(ConvFileName))
            LOGW("X: %s, ret %d\n", ConvFileName, ret);
...
static int FS_CCCI_Move(const wchar_t *SrcFullPath, const wchar_t *DstFullPath, unsigned int Flag)
{
    ...
    if(Flag & FS_MOVE_COPY)
    {
        ...
        fd = Open_Tm(FsFileName, O_RDONLY, __FUNCTION__);
        if(fd  < 0)
        {
            local_errno = errno;
        ...
        size = Lseek_Tm(fd, 0, SEEK_END, __FUNCTION__) - Lseek_Tm(fd, 0, SEEK_SET, __FUNCTION__);
        if(size < 0)
        {
        ...
        Lseek_Tm(fd, 0, SEEK_SET, __FUNCTION__);

        ret  = Read_Tm(fd, tempbuf, size, __FUNCTION__);
        if(ret < size )
        {
            if(ret < 0)
            {
        ...
        SaveSyscallTime(CCCI_FSD_CREAT, time_s, __FUNCTION__);
        if(newfd  < 0)
        {
            local_errno = errno;
            LOGE("Move: [error]fail create %s: %d\n", FsNewFileName, local_errno);
        ...
        ret = Write_Tm(newfd, tempbuf, size, __FUNCTION__);
        if(ret < size)
        {
            if(ret < 0)
            {
...
Exit:
    ...
    if (debug_wakeup) {
        print_file_name("M_0", ConvFullPath, len1);
        print_file_name("M_1", DstConvFullPath, len2);
    }
    else if((len1 >= 8) && (len2 >= 8)) {
        if (is_protect_path(ConvFullPath))
            LOGW("M: %s, ret %d\n", ConvFullPath, ret);
        LOGD("M: [%02X%02X%02X%02X%02X%02X%02X%02X] -> [%02X%02X%02X%02X%02X%02X%02X%02X](0x%08X): %d\n",
             (ConvFullPath[len1 - 8] - 32), (ConvFullPath[len1 - 7] - 32),
             (ConvFullPath[len1 - 6] - 32), (ConvFullPath[len1 - 5] - 32),
             (ConvFullPath[len1 - 4] - 32), (ConvFullPath[len1 - 3] - 32),
             (ConvFullPath[len1 - 2] - 32), (ConvFullPath[len1 - 1] - 32),
             (DstConvFullPath[len2 - 8] - 32), (DstConvFullPath[len2 - 7] - 32),
             (DstConvFullPath[len2 - 6] - 32), (DstConvFullPath[len2 - 5] - 32),
             (DstConvFullPath[len2 - 4] - 32), (DstConvFullPath[len2 - 3] - 32),
             (DstConvFullPath[len2 - 2] - 32), (DstConvFullPath[len2 - 1] - 32),
             Flag, ret);
    }
    else {
...
static int FS_CCCI_FindFirst(const wchar_t *PatternName, char Attr, char AttrMask,
                             FS_DOSDirEntry *pFileInfo, wchar_t *FileName, unsigned int *pMaxLength)
{
    ...
    *pMaxLength = 0;
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if(strncmp(ConvPatternName, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            if((strlen(ConvPatternName + strlen(MdRootDir[i])) +
                strlen(FsRootDir[i]) + 1) > PATH_MAX) {
                LOGE("FindFirst: [error]file path too long: %s\n", ConvPatternName);
                ret = FS_PATH_OVER_LEN_ERROR;
                goto Exit;
            }
            //Convert the file name
            FsFileNameCount = snprintf(FsPatternName, sizeof(FsPatternName), "%s%s",
                                       FsRootDir[i], (ConvPatternName + strlen(MdRootDir[i])));
            break;
        }
    }
    ...
    if(bFound == true)
    {
        ...
        g_FsInfo.FileNum++;
        g_FsInfo.hFileHandle[HandleIndex].hFile = (long int)pDir;
        g_FsInfo.hFileHandle[HandleIndex].fInUse = true;
        ...
        *pMaxLength = 0;
        ...
        if(pDir != NULL) {
            if(Closedir_Tm(pDir, __FUNCTION__) != 0) {
...
Exit:
    if(pDirent == NULL) {
        ...
        if (debug_wakeup)
            print_file_name("FF", FsDirName, len1);
        else if(len1 >= 8) {
            LOGD("FF: [%02X%02X%02X%02X%02X%02X%02X%02X](%d %d %d): [null] %d\n",
                 (FsDirName[len1 - 8] - 32), (FsDirName[len1 - 7] - 32), (FsDirName[len1 - 6] - 32),
                 (FsDirName[len1 - 5] - 32), (FsDirName[len1 - 4] - 32), (FsDirName[len1 - 3] - 32),
                 (FsDirName[len1 - 2] - 32), (FsDirName[len1 - 1] - 32),
        ...
        if (debug_wakeup) {
            print_file_name("FF_0", FsDirName, len1);
            print_file_name("FF_1", pDirent->d_name, len2);
        }
        else if( (len2 >= 8) && (len1 >= 8)) {
            LOGD("FF: [%02X%02X%02X%02X%02X%02X%02X%02X](%d %d %d): [%02X%02X%02X%02X%02X%02X%02X%02X] %d\n",
                 (FsDirName[len1 - 8] - 32), (FsDirName[len1 - 7] - 32), (FsDirName[len1 - 6] - 32),
                 (FsDirName[len1 - 5] - 32), (FsDirName[len1 - 4] - 32), (FsDirName[len1 - 3] - 32),
                 (FsDirName[len1 - 2] - 32), (FsDirName[len1 - 1] - 32),
                 Attr, AttrMask, *pMaxLength,
                 (pDirent->d_name[len2 - 8] - 32), (pDirent->d_name[len2 - 7] - 32),
                 (pDirent->d_name[len2 - 6] - 32), (pDirent->d_name[len2 - 5] - 32),
                 (pDirent->d_name[len2 - 4] - 32), (pDirent->d_name[len2 - 3] - 32),
                 (pDirent->d_name[len2 - 2] - 32), (pDirent->d_name[len2 - 1] - 32),
                 ret);
        }
        else {
    ...
    *pMaxLength = 0;
    ...
    if (HandleIndex >= FS_FILE_MAX || HandleIndex < 0) {
        ...
        goto Exit;
              ...
              g_FsInfo.hFileHandle[HandleIndex].fInUse != true)
    {
        LOGE("FindNext: [error]Invalid handle for %d, %ld, %d\n", HandleIndex,
             g_FsInfo.hFileHandle[HandleIndex].hFile, g_FsInfo.hFileHandle[HandleIndex].fInUse);
        ret = FS_INVALID_FILE_HANDLE;
        goto Exit;
    }
...
Exit:
    if(pDirent == NULL)
    {
        ...
        if (debug_wakeup)
            print_file_name("FN", pDirent->d_name, len);
        else if(len >= 8) {
            LOGD("FN: %d %d: [%02X%02X%02X%02X%02X%02X%02X%02X] %d\n", HandleIndex, *pMaxLength,
                 (pDirent->d_name[len - 8] - 32), (pDirent->d_name[len - 7] - 32),
                 (pDirent->d_name[len - 6] - 32), (pDirent->d_name[len - 5] - 32),
                 (pDirent->d_name[len - 4] - 32), (pDirent->d_name[len - 3] - 32),
                 (pDirent->d_name[len - 2] - 32), (pDirent->d_name[len - 1] - 32),
                 ret);
        }
        else {
            LOGD("FN: stren < 8(%d)\n", len);
...
static int FS_CCCI_FindClose(int HandleIndex)
{
    ...
    if (HandleIndex >= FS_FILE_MAX || HandleIndex < 0) {
        ...
        goto Exit;
              ...
              g_FsInfo.hFileHandle[HandleIndex].fInUse != true)
    {
        LOGE("FindClose: [error]Invalid handle for %d, %ld, %d\n", HandleIndex,
             g_FsInfo.hFileHandle[HandleIndex].hFile, g_FsInfo.hFileHandle[HandleIndex].fInUse);
        ret = FS_INVALID_FILE_HANDLE;
        goto Exit;
    }

    LOGD("FindClose:%s: (%d, %ld)\n", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
         HandleIndex, g_FsInfo.hFileHandle[HandleIndex].hFile);
    ...
    if(g_FsInfo.hFileHandle[HandleIndex].pFsFileName != NULL) {
        ...
        if (debug_wakeup) {
            print_file_name("FC", g_FsInfo.hFileHandle[HandleIndex].pFsFileName,
                            strlen(g_FsInfo.hFileHandle[HandleIndex].pFsFileName));
            debug_wakeup = 0;
        }
...
static int FS_CCCI_GetDiskInfo(FS_DiskInfo *pDiskInfo)
{
    ...
    if(Statfs_Tm(FsRootDir[0], &StatfsBuf, __FUNCTION__) == -1) {
...
static int FS_CCCI_Restore(const wchar_t *FileName)
{
    ...
    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        if(strncmp(ConvFileName, MdRootDir[i], strlen(MdRootDir[i])) == 0) {
            ...
            //Convert the file name
            snprintf(FsFileName, sizeof(FsFileName), "%s%s",
                     FsRootDir[i], (ConvFileName + strlen(MdRootDir[i])));
            break;
        }
    }
...
static int FS_CCCI_CMPT_Write(wchar_t *name, nvram_fs_para_cmptw_t *in_para, void *pBuffer)
{
    ...
    if(in_para->opid_map & FS_CMPT_WRITE) {
        ret[3] =  FS_CCCI_Write(HandleIndex, pBuffer, in_para->length,
                                (int *)&in_para->act_write_size);
        if(ret[3] < 0) {
...
/* CMPT read enhance
 * Supports flexable operation mode selection based on opid_map
 */
static int FS_CCCI_CMPT_Read_EX(wchar_t* name, nvram_fs_para_cmpt_ex_t *in_para)
{
    int ret;
    unsigned int ret_bitmap = 0;
    ...
    if(in_para->opid_map & FS_CMPT_OPEN) {
        ...
        in_para->file_handle =  FS_CCCI_Open(name, in_para->flag);

        if(in_para->file_handle < 0) {
            if (s_print_error_log)
                LOGE("%s, open fail, err=%d\n", __func__, errno);

            in_para->ret[0] = ret_bitmap;
            in_para->ret[1] = in_para->file_handle;
            in_para->file_handle_ptr = INVALID_HANDLE_VALUE;
            return -1;
        } else {
            ret_bitmap |= FS_CMPT_OPEN;
        }
    ...
    if (in_para->file_handle < 0) {
        LOGE("%s, unexpected file_handle=%d\n", __func__, in_para->file_handle);
        return -1;
    }

    if(in_para->opid_map & FS_CMPT_GETFILESIZE) {
        ret =  FS_CCCI_GetFileSize(in_para->file_handle, &(in_para->file_size));
        if(ret < 0) {
            LOGE("%s, GetSize fail, err=%d\n", __func__, errno);
            in_para->ret[0] = ret_bitmap;
            in_para->ret[1] = ret;
            FS_CCCI_Close(in_para->file_handle);
            in_para->file_handle_ptr = INVALID_HANDLE_VALUE;
            return -2;
        } else {
            ret_bitmap |= FS_CMPT_GETFILESIZE;
    ...
    if(in_para->opid_map & FS_CMPT_SEEK) {
        ret = FS_CCCI_Seek(in_para->file_handle, in_para->offset, in_para->whence);
        if(ret < 0) {
            LOGE("%s, Seek fail, err=%d\n", __func__, errno);
            in_para->ret[0] = ret_bitmap;
            in_para->ret[1] = ret;
            FS_CCCI_Close(in_para->file_handle);
            in_para->file_handle_ptr = INVALID_HANDLE_VALUE;
    ...
    if(in_para->opid_map & FS_CMPT_READ) {
        ret = FS_CCCI_Read(in_para->file_handle, in_para->data_ptr, in_para->length, &in_para->act_read);
        if(ret < 0) {
            LOGE("%s, read fail, err=%d\n", __func__, errno);
            in_para->ret[0] = ret_bitmap;
            in_para->ret[1] = ret;
            FS_CCCI_Close(in_para->file_handle);
            in_para->file_handle_ptr = INVALID_HANDLE_VALUE;
    ...
    if(in_para->opid_map & FS_CMPT_CLOSE) {
        ret = FS_CCCI_Close(in_para->file_handle);
        if(ret < 0) {
            LOGE("%s, close fail, err=%d\n", __func__, errno);
            in_para->ret[0] = ret_bitmap;
            in_para->ret[1] = ret;
            in_para->file_handle_ptr = INVALID_HANDLE_VALUE;
            return -5;
    ...
    in_para->ret[0] = ret_bitmap;
    in_para->ret[1] = 0;
    in_para->file_handle_ptr = in_para->file_handle;
...
void FS_Init(int md_id)
{
    ...
    Umask_Tm(0007, __FUNCTION__);
    ret = Stat_Tm("/mnt/vendor/nvdata", &buf, __FUNCTION__);

    if (ret < 0) {
        LOGD("%s:stat /mnt/vendor/nvdata failed, ret=%d, errno=%d.\n", __func__, ret, errno);
        ret = Mkdir_Tm("/mnt/vendor/nvdata",MD_DIR_ATTR, __FUNCTION__);
        if (ret < 0)
            LOGE("%s:mkdir /mnt/vendor/nvdata failed,ret=%d, errno=%d.\n", __func__, ret, errno);
    }

    ret = Stat_Tm(FS_ROOT_COMMON, &buf, __FUNCTION__);
    if (ret < 0) {
        LOGD("%s:stat %s dir error, ret=%d errno=%d.\n",
             __func__, FS_ROOT_COMMON, ret, errno);
        ret = Mkdir_Tm(FS_ROOT_COMMON, MD_DIR_ATTR, __FUNCTION__);
        if (ret < 0)
            LOGE("%s:mkdir %s failed,ret=%d,errno=%d.\n",
                 __func__, FS_ROOT_COMMON, ret, errno);
    }
    ...
    switch(md_id) {
    case 0:
        ret = Stat_Tm(FS_ROOT_DIR_MD1, &buf, __FUNCTION__);
        if (ret < 0) {
            ret = Mkdir_Tm(FS_ROOT_DIR_MD1, MD_DIR_ATTR, __FUNCTION__);
            if (ret < 0)
        ...
        if (Stat_Tm(FS_ROOT_DIR1_MD1, &buf, __FUNCTION__) < 0) {
            ret = Mkdir_Tm(FS_ROOT_DIR1_MD1, MD_DIR_ATTR, __FUNCTION__);
            if (ret < 0)
                LOGE("%s:mkdir %s for md1 failed,ret=%d, errno=%d.\n",
        ...
        if (Stat_Tm(FS_ROOT_DIR2_MD1, &buf, __FUNCTION__) < 0) {
            ret = Mkdir_Tm(FS_ROOT_DIR2_MD1, MD_DIR_ATTR, __FUNCTION__);
            if (ret < 0)
                LOGE("%s:mkdir %s for md1 failed,ret=%d, errno=%d.\n",
...
static int ccci_open_md_status_fd(int md_id)
{
    ...
    fd = Open_Tm(dev_path, O_RDWR, __FUNCTION__);
    if (fd < 0) {
...
static void ccci_wait_md_status_to_HS1()
{
    ...
    LOGD("[%s] start. md_status_fd = %d\n", __func__, md_status_fd);
    while (1) {
        count = read(md_status_fd, &status_buf, sizeof(struct md_status_event));
        if (count > 0) {
            LOGD("[%s] curr md status = %d\n", __func__, status_buf.event_type);
            ...
            LOGE("[%s] warning, read() = %d\n", __func__, count);

    }
    LOGD("[%s] end.\n", __func__);
}

static void get_print_error_log_flag(void)
{
    char buf[PROPERTY_VALUE_MAX];
    int ret;
    char *prop_name = "persist.vendor.md.fsd.log";

    ret = property_get(prop_name, buf, "0");
    if (ret < 0)
        LOGE("[%s] get prop error, ret: %d, errno: %d\n",
             __func__, ret, errno);
    else if (ret == 0)
        LOGE("[%s] has no this property: '%s'\n",
             __func__, prop_name);
    else
        s_print_error_log = atoi(buf);

    LOGW("[%s] '%s': %d\n",
         __func__, prop_name, s_print_error_log);
}

static void set_read_status(int read_status)
{
    pthread_mutex_lock(&s_mtx_lock);
    s_read_status = read_status;
    pthread_mutex_unlock(&s_mtx_lock);
}

static int get_read_status(void)
{
    int status;
...
int first_open_fs_device(void)
{
    int retry_time = 0;
    int ret = 0;

    LOGI("[%s] dev_name: %s;  DeviceFd: %d\n",
         __func__, "/dev/ccci_fs", DeviceFd);

    while(1) {
        DeviceFd = Open_Tm("/dev/ccci_fs", O_RDWR, __FUNCTION__);
        if (DeviceFd < 0) {
            usleep(10 * 1000);
            retry_time++;
        } else {
            LOGE("ccci_fsd is opend,cost %d ms.\n", retry_time * 10);
            break;
        }
    }

    ret = close(DeviceFd);
    LOGI("[%s] dev_name: %s;  DeviceFd: %d closed, ret=%d\n",
         __func__, "/dev/ccci_fs", DeviceFd, ret);
    DeviceFd = -1;
...
static int open_fs_device(void)
{
    ...
    while(1) {
        DeviceFd = open("/dev/ccci_fs", O_RDWR);
        if (DeviceFd < 0) {
            port_open_retry--;
            LOGE("%s:open %s: retry=%d,errno=%d", __func__, "/dev/ccci_fs\n",
                 port_open_retry, errno);
            if(port_open_retry > 0) {
                continue;

            }
            else {
                LOGE("fail to open %s: %d", "/dev/ccci_fs\n", errno);
                perror("");
                pthread_mutex_unlock(&s_mtx_lock);
                return -1;
            }
            ...
            LOGD("%s is opend(%d).", "/dev/ccci_fs\n", port_open_retry);
            break;
        }
    }

    if(!stream_support && g_FsInfo.pFsBuf == NULL) {
        clock_gettime(CLOCK_MONOTONIC, &time_s);
        g_FsInfo.pFsBuf = mmap(NULL, sizeof(fs_stream_buffer_t) * 5,
                               PROT_READ | PROT_WRITE, MAP_SHARED, DeviceFd, 0);
        SaveSyscallTime(CCCI_FSD_MMAP, time_s, __FUNCTION__);
    }
    pthread_mutex_unlock(&s_mtx_lock);

    LOGE("[%s] dev_name: %s;  DeviceFd: %d\n",
         __func__, "/dev/ccci_fs", DeviceFd);

    return 0;
}

static int close_fs_device(void)
{
    int ret = 0;
...
void stop_read_from_kernel(void)
{
    int status;
    int signal_ret;
    int ret = 0;
    ...
    if (!stream_support && DeviceFd >= 0) {
        clock_gettime(CLOCK_MONOTONIC, &time_s);
        ret = ioctl(DeviceFd, CCCI_FS_IOC_SEND_MSG, NULL);
        SaveSyscallTime(CCCI_FSD_IOCTL, time_s, __FUNCTION__);

        if(0 != ret)
            LOGE("[%s] error: ioctl(CCCI_FS_IOC_SEND_MSG)\n", __func__);
        else
            usleep(20 * 1000);
    }

    LOGE("[%s] status: %d; DeviceFd: %d\n",
         __func__, get_read_status(), DeviceFd);

    do {
        status = get_read_status();
        if (status == STS_NO_READ) {
            if(!stream_support && g_FsInfo.pFsBuf) {
                clock_gettime(CLOCK_MONOTONIC, &time_s);
                munmap(g_FsInfo.pFsBuf, sizeof(fs_stream_buffer_t) * 5);
                SaveSyscallTime(CCCI_FSD_MUNMAP, time_s, __FUNCTION__);
                g_FsInfo.pFsBuf = NULL;
            }
            break;
        }

        if (status == STS_START_READ) {
            signal_ret = pthread_kill(sg_main_thread, SIGUSR1);
            if (signal_ret == ESRCH)
                LOGE("[%s] error: ccci_fsd thread is not found.\n",
                     __func__);
        }
...
int start_read_from_kernel(void)
{
    int ret;
    ...
    pthread_mutex_lock(&s_mtx_lock);
    if (s_need_stop) {
        LOGE("[%s] start s_need_stop: %d; status: %d\n",
             __func__, s_need_stop, s_read_status);
        s_need_stop = 0;
        pthread_cond_signal(&s_wait_cond);
        LOGE("[%s] end s_need_stop: %d; status: %d\n",
             __func__, s_need_stop, s_read_status);
    }
    pthread_mutex_unlock(&s_mtx_lock);
...
static void check_stop_read_status(void)
{
    int need_stop;

    pthread_mutex_lock(&s_mtx_lock);

    need_stop = s_need_stop;
    if (need_stop)
        LOGE("[%s] start; s_need_stop: %d; status: %d\n", __func__, s_need_stop, s_read_status);

    while (s_need_stop)
        pthread_cond_wait(&s_wait_cond, &s_mtx_lock);
...
int fsd_main(int argc, int curr_md_id, int is_v1)
{
    int mdstatus;
    int ReqBufIndex = 0;
    FS_BUF *pFsBuf = NULL;
    ...
    int i = 0;
    CCCI_BUFF_T *ccci_h = NULL;
    char *pkt_buff = NULL;//[MAX_FS_BUF_BYTE] = {0};
    ...
    nvram_fs_para_cmpt_ex_t in_cmpt_ex_para;
    ...
    struct timespec time_s, time_1, time_2, time_3, time_e, time_ss, time_main_begin;
    long diff;
    int elapsed_in_ms;
    int ret = 0;

    //make folder for ccci
    mkdir("/data/vendor/ccci_cfg", 0770);
    mkdir("/data/vendor/mdlpm", 0770);
    mkdir("/data/vendor/md3", 0770);
    mkdir("/data/vendor_de/md", 0770);

    clock_gettime(CLOCK_MONOTONIC, &time_s);
    LOGI("Enter fsd_main time  :(%ld %ld)\r\n",time_s.tv_sec, time_s.tv_nsec);

    SyscallTimeInfoInit();
    LOGE("md_fsd Ver:v2.3, CCCI Ver:%d\n", ccci_get_version());

    g_FsInfo.pFsBuf = NULL;

    get_print_error_log_flag();
    ...
    if(is_v1 || argc != 2) {
        ...
        if(curr_md_id == 0)
            ...
            LOGE("Invalid md sys id(%d)!\n", curr_md_id);
            return -1;
        }
    }

    for (i = 0; i < FS_MAX_DIR_NUM; i++) {
        has_write = snprintf(FsRootDir[i], 36, "%s", FsRootDir_MD1[i]);
        if (has_write >= 36)
            LOGE("[Error]line:%d get [%d]%s fail\n", __LINE__, i, FsRootDir_MD1[i]);
    }
    ...
    if (PackInfo == NULL) {
        LOGE("Main: [error]malloc FS_PACKET_INFO fail:%d\n", errno);
        return -1;
    }
    memset(PackInfo, 0, sizeof(FS_PACKET_INFO) * FS_MAX_ARG_NUM);

    pkt_buff = malloc(MAX_FS_BUF_BYTE);
    if (pkt_buff == NULL) {
        LOGE("Main: [error]malloc MAX_FS_BUF_BYTE fail:%d\n", errno);
        free(PackInfo);
        return -1;
    }
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_ss);
    ...
    SaveSyscallTime(CCCI_FSD_SIGNAL, time_ss, __FUNCTION__);
    clock_gettime(CLOCK_MONOTONIC, &time_main_begin);
    LOGI("fsd_main before signal  :(%ld %ld)\r\n",time_main_begin.tv_sec, time_main_begin.tv_nsec);
    //write_ccci_dump("fsd_main before signal  :(%ld %ld)\r\n",
    //                time_main_begin.tv_sec, time_main_begin.tv_nsec);

    while(exit_signal == SIGUSR1 || exit_signal == 0)
    {
stop_fsd:
        if (exit_signal == SIGUSR1)
            exit_signal = 0;

        check_stop_read_status();
        /* clear buff state and buff offset */
        for (i = 0; i < FS_BUFFER_SLOT_NUM; i++) {
            g_FsInfo.fs_buff_state[i] = FS_BUFF_IDLE;
            g_FsInfo.fs_buff_offset[i] = 0;
        }
...
retry:
        if(!stream_support) {
            pthread_mutex_lock(&s_mtx_lock);
            if (s_need_stop) {
                s_read_status = STS_NO_READ;
                pthread_mutex_unlock(&s_mtx_lock);
                goto stop_fsd;
            }
            s_read_status = STS_START_READ;
            pthread_mutex_unlock(&s_mtx_lock);
            clock_gettime(CLOCK_MONOTONIC, &time_ss);
            ReqBufIndex = ioctl(DeviceFd, CCCI_FS_IOCTL_GET_INDEX, 0);
            SaveSyscallTime(CCCI_FSD_IOCTL, time_ss, __FUNCTION__);
            FS_WAKE_LOCK();

            pthread_mutex_lock(&s_mtx_lock);
            if(ReqBufIndex == -10000)
                s_need_stop = 1;
            if (s_need_stop) {
                s_read_status = STS_NO_READ;
                pthread_mutex_unlock(&s_mtx_lock);
                LOGE("Main: goto stop_fsd\n");
                goto stop_fsd;
            }
            s_read_status = STS_READING;
            pthread_mutex_unlock(&s_mtx_lock);
            ...
            while (1) {
                ...
                RetVal = Read_CCCI_Tm(DeviceFd, pkt_buff,
                                      (MAX_FS_PKT_BYTE + sizeof(CCCI_BUFF_T) + sizeof(unsigned int)));
                clock_gettime(CLOCK_MONOTONIC, &time_s);
                if(first_recv_req_flg ==0)
                {
                    first_recv_req_flg = 1;
                    LOGI("First Read Time: (%ld %ld); begin->firstreadreq: [interval] : %lld ns\n",
                         time_s.tv_sec, time_s.tv_nsec, FS_Timestamp( time_main_begin, time_s));
                    //write_ccci_dump("First Read Time: (%ld %ld); begin->firstread: [interval] : %lld ns\n",
                    //                time_s.tv_sec, time_s.tv_nsec, FS_Timestamp( time_main_begin, time_s));
                }
                ...
                if (RetVal <= 0) {
                    if (errno != EINTR)
                        LOGE("Failed to read from FS device (%d) !! errno = %d\n", RetVal, errno);
                    goto retry;
                }
                else {
                    LOGD("Read %d bytes from FS device\n", RetVal);
                }
                FS_WAKE_LOCK();
                ...
                if (((ccci_h->channel&0xFFFF)  != 0x0e) || (ReqBufIndex < 0) ||
                    ...
                    LOGE("Main: [error]packet data check fail: ch =0x%x, fs_buf_idx=%d, data[1] = 0x%x, 0x%zu\n",
        ...
        clock_gettime(CLOCK_MONOTONIC, &time_3);
        ...
        case FS_CCCI_OP_CMPT_READ:
        {
            ...
            nvram_fs_para_cmpt_32_t *in_cmpt_para_32 = NULL;
            ...
            in_cmpt_para_32 = (nvram_fs_para_cmpt_32_t *)PackInfo[1].pData;
            in_cmpt_para.opid_map = in_cmpt_para_32->opid_map;
            // in_cmpt_para.ret[0] and in_cmpt_para.ret[1] updated at FS_CCCI_CMPT_Read
            in_cmpt_para.flag = in_cmpt_para_32->flag;
            in_cmpt_para.file_size = in_cmpt_para_32->file_size;
            in_cmpt_para.offset = in_cmpt_para_32->offset;
            in_cmpt_para.whence = in_cmpt_para_32->whence;
            ...
            in_cmpt_para.length = in_cmpt_para_32->length;
            in_cmpt_para.act_read = in_cmpt_para_32->act_read;
        ...
        {
            wchar_t *FileName = (wchar_t *)PackInfo[0].pData;
            nvram_fs_para_cmptw_32_t *in_cmptw_para_32 = NULL;
            ...
            in_cmptw_para_32 = (nvram_fs_para_cmptw_32_t *)PackInfo[1].pData;
            in_cmptw_para.opid_map = in_cmptw_para_32->opid_map;
            // in_cmptw_para.ret[0] and in_cmptw_para.ret[1] updated at FS_CCCI_CMPT_Write
            in_cmptw_para.flag = in_cmptw_para_32->flag;
            in_cmptw_para.offset = in_cmptw_para_32->offset;
            in_cmptw_para.whence = in_cmptw_para_32->whence;
            //in_cmptw_para.data_ptr is not used
            in_cmptw_para.length = in_cmptw_para_32->length;
            in_cmptw_para.file_handle = in_cmptw_para_32->file_handle;
            //in_cmptw_para.file_handle_ptr updated at FS_CCCI_CMPT_Write
            //in_cmptw_para.act_write_size updated at FS_CCCI_CMPT_Write
        ...
        case FS_CCCI_OP_CMPTREADEX:
        {
            wchar_t* FileName = (wchar_t*)PackInfo[0].pData;
            nvram_fs_para_cmpt_ex_32_t *in_cmpt_ex_para_32 = NULL;

            in_cmpt_ex_para_32 = (nvram_fs_para_cmpt_ex_32_t *)PackInfo[1].pData;
            in_cmpt_ex_para.opid_map = in_cmpt_ex_para_32->opid_map;
            // in_cmpt_ex_para.ret[0] ~ ret[1] updated in FS_CCCI_CMPT_Read_EX
            in_cmpt_ex_para.flag = in_cmpt_ex_para_32->flag;
            in_cmpt_ex_para.file_size = in_cmpt_ex_para_32->file_size;
            in_cmpt_ex_para.offset = in_cmpt_ex_para_32->offset;
            in_cmpt_ex_para.whence = in_cmpt_ex_para_32->whence;
            in_cmpt_ex_para.data_ptr = pFsBuf->Buffer + CMPT_READ_DATA_PTR_OFFSET * sizeof(int);
            in_cmpt_ex_para.length = in_cmpt_ex_para_32->length;
            in_cmpt_ex_para.act_read = in_cmpt_ex_para_32->act_read;
            in_cmpt_ex_para.file_handle = in_cmpt_ex_para_32->file_handle;
            //in_cmpt_ex_para_32->file_handle_ptr updated in FS_CCCI_CMPT_Read_EX

            FS_ConvWcsToCs(FileName, ConvFileName);
            RetVal = FS_CCCI_CMPT_Read_EX(FileName, &in_cmpt_ex_para);

            PackInfo[PacketNum].Length = sizeof(in_cmpt_ex_para.ret);
            PackInfo[PacketNum++].pData = (void*)in_cmpt_ex_para.ret;
            PackInfo[PacketNum].Length = sizeof(unsigned int);
            PackInfo[PacketNum++].pData = (void*)&in_cmpt_ex_para.file_size;
            PackInfo[PacketNum].Length = sizeof(unsigned int);
            PackInfo[PacketNum++].pData = (void*)&in_cmpt_ex_para.act_read;
            PackInfo[PacketNum].Length = in_cmpt_ex_para.act_read;
            PackInfo[PacketNum++].pData = in_cmpt_ex_para.data_ptr;
            PackInfo[PacketNum].Length = sizeof(unsigned int);
            PackInfo[PacketNum++].pData = (void*)&in_cmpt_ex_para.file_handle_ptr;

            LOGD("cmpt_read_ex ret (%d)\n", RetVal);
            break;
        }
        ...
        case FS_CCCI_OP_FINDFIRST:
        {
            ...
            RetVal = FS_CCCI_FindFirst(PatternName, Attr, AttrMask, &DosDirEntry,
                                       FileNameTemp, &MaxLength);
            PackInfo[PacketNum].Length = sizeof(int);
            PackInfo[PacketNum++].pData = (void *)&RetVal;
        ...
        if (time_e.tv_nsec < time_s.tv_nsec) {
            diff = (long)(1000000000 + time_e.tv_nsec - time_s.tv_nsec) / 1000000;
            diff += (long)(time_e.tv_sec - time_s.tv_sec - 1) * 1000;
        }
        else {
            diff = (long)(time_e.tv_nsec - time_s.tv_nsec) / 1000000;
            diff += (long)(time_e.tv_sec - time_s.tv_sec) * 1000;
        }
        elapsed_in_ms = (int)diff;
        if (time_s.tv_sec != 0 && elapsed_in_ms > 1000)
        {
            LOGE("error-> id=%x; s:(%ld %ld); 1:(%ld %ld); 2:(%ld %ld); 3:(%ld %ld); e:(%ld %ld); %d\n",
                 pFsBuf->OperateID, time_s.tv_sec, time_s.tv_nsec, time_1.tv_sec, time_1.tv_nsec,
                 time_2.tv_sec,time_2.tv_nsec, time_3.tv_sec, time_3.tv_nsec,
                 time_e.tv_sec, time_e.tv_nsec, elapsed_in_ms);
        }
...
_Next:
        if ((ReqBufIndex < 0) || (ReqBufIndex >= FS_REQ_BUFFER_MUN) || (pFsBuf == NULL)) {
            ...
            clock_gettime(CLOCK_MONOTONIC, &time_ss);
            ret = ioctl(DeviceFd, CCCI_IOC_FORCE_MD_ASSERT, &RetVal);
            SaveSyscallTime(CCCI_FSD_IOCTL, time_ss, __FUNCTION__);
            if(ret != 0)
                LOGD("update modem type to kernel fail: err=%d\n", errno);
        }
...
static void ccci_main_thread_handle()
{
    int ret;
    struct md_status_event status_buf;

    LOGD("[%s] start. argc = %d; md_id = %d\n",
         __func__, sg_main_argc, sg_main_md_id);

    ret = fsd_main(sg_main_argc, sg_main_md_id, s_is_v1);

    LOGE("[%s] end. ret = %d\n", __func__, ret);
}

void ccci_create_main_thread(int argc, int md_id, int is_v1)
{
    int ret;
    sg_main_argc = argc;
    sg_main_md_id = md_id;
    ...
    ret = pthread_create(&sg_main_thread, NULL, (void *)ccci_main_thread_handle, NULL);
    if(ret != 0) {
        LOGE("[%s] error: pthread_create() = %d\n", __func__, ret);
        exit (1);
...
static void ccci_status_thread_handle()
{
    int count;
    int md1statusfd = -1;
    char md1statuspath[] = "/dev/ccci_md1_sta";
    struct md_status_event status_buf;
    struct timespec time_start = {0, 0};

    md1statusfd = open(md1statuspath, O_RDWR);
    LOGD("[%s]  md_status_fd = %d\n", __func__, md1statusfd);
    ...
    while (1) {
        count = read(md1statusfd, &status_buf, sizeof(struct md_status_event));
        if (count == (int)sizeof(struct md_status_event)) {
            LOGD("[%s] curr md status = %d\n", __func__, status_buf.event_type);
            if (status_buf.md_id != 0) {
                LOGE("[%s] status_buf.md_id = %d\n", __func__, status_buf.md_id);
                continue;
            }
            if (status_buf.event_type == MD_STA_EV_RILD_POWEROFF_START) {
                clock_gettime(CLOCK_MONOTONIC, &time_start);
                SyscallTimeInfoInit();
            }
            else if (status_buf.event_type == MD_STA_EV_ENTER_FLIGHT_E_REQUEST)
                WriteSyscalltimeInfoTolog(1, time_start);
            else if (status_buf.event_type == MD_STA_EV_READY)
                WriteSyscalltimeInfoTolog(0, time_start);
        }
        else
            LOGE("[%s] warning, read() = %d\n", __func__, count);
    }
...
void ccci_create_md_status_listen_thread()
{
    int ret;
