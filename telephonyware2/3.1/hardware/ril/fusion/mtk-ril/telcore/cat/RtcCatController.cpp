...
void RtcCatController::onInit() {
    ...
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_STATE,
        RfxStatusChangeCallback(this, &RtcCatController::onSimOnOffStateChanged));
    // register callbacks to get required information
...
void RtcCatController::onSimOnOffStateChanged(RfxStatusKeyEnum key,
    RfxVariant old_value, RfxVariant value) {
    int oldState = -1, newState = -1;
...
void RtcCatController::onDeinit() {
    ...
    getStatusManager()->unRegisterStatusChanged(RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_STATE,
            RfxStatusChangeCallback(this, &RtcCatController::onSimOnOffStateChanged));
...
char* RtcCatController::decodeStkRefreshFileChange(char *str, int **cmd, int *cmd_length)
{
    ...
    if (file_num == ULONG_MAX && errno == ERANGE) {
        logE(RFX_LOG_TAG, "decodeStkRefreshFileChange: strtoul fail !!!");
        return NULL;
    }
    efId_str = (char*)calloc(1, (file_num * (sizeof(char) * 4)) + 1);
    if (efId_str == NULL) {
        logE(RFX_LOG_TAG, "decodeStkRefreshFileChange: calloc efId_str FAIL!!!");
        return efId_str;
    }
...
void RtcCatController::handleSimRefresh(char* urc) {
    ...
    logD(RFX_LOG_TAG, "aid = %s, %d.", simRefreshRspV7.aid, aid_len);
    ...
    if (NULL != efId_str && 0 < strlen(efId_str)) {
        for (i = 0; i < files_num; i++) {
            ...
            if (refresh_urc != NULL) {
                ...
                logD(RFX_LOG_TAG, "fail to malloc refresh_urc");
            }
    }
...
void RtcCatController::handleSimRefreshAfterQueryCpin() {
    ...
    if (pEfIdString != NULL && strlen(pEfIdString) > 0) {
