...
bool RfxVariant::isEqualWith(const RfxVariant &other) {
    if (m_type == other.m_type) {
        ...
        case DATA_TYPE_STRING8_VECTOR:
            // check num of string8
            ret = (*(const Vector<String8>*)(m_value.string8VectorBuffer)).size() ==
                other.asString8Vector().size();
            if (ret) {
                // check if any string8 is different
                int size = other.asString8Vector().size();
                for (int i = 0; i < size; i++) {
                    if ((*(const Vector<String8>*)(m_value.string8VectorBuffer))[i] !=
                            other.asString8Vector()[i]) {
                        return false;
                    }
                }
                ret = true;
        ...
        case DATA_TYPE_IMS_CALL_INFO_VECTOR:
            ...
            if (ret) {
                int size = other.asImsCallInfo().size();
                for (int i = 0; i < size; i++) {
                    if ((*(const Vector<RfxImsCallInfo*>*)(m_value.imsCallVectorBuffer))[i] !=
                            other.asImsCallInfo()[i]) {
                        return false;
                    }
                }
                ret = true;
