...
void RilOemClient::requestComplete(RIL_Token token, RIL_Errno e, const void *response,
    ...
    if (RFX_MSG_REQUEST_QUERY_MODEM_THERMAL == info->request) {
        ...
        if(mThermalFd >= 0){
            ...
            if (len > 255) {
                RFX_LOG_D(RFX_LOG_TAG, "send thermal response data is too len:%zu", len);
            } else {
        ...
        if (mRsuFd >= 0) {
            if (RfxRilUtils::isUserLoad() != 1) {
                    RFX_LOG_I(RFX_LOG_TAG,
                            "[RSU-SIMLOCK] mRsuFd is valid strResult is %s", strResult);
            }
            int len = 0;
            int ret;
            int fd = mRsuFd;
...
void RilOemClient::executeRsuOpt(char* arg) {
    ...
    if (line->atTokHasmore()) {
        ...
        if (RfxRilUtils::isUserLoad() != 1) {
            ...
            RFX_LOG_D(RFX_LOG_TAG, "executeRsuOpt data=***");
        }
    }
    if (mRsuFd >= 0) {
