...
#include <../include/telephony/ril_nv_items.h>
#include <../include/telephony/ril_msim.h>
...
#define CDMA_ALPHA_INFO_BUFFER_LENGTH_H 64
#define CDMA_NUMBER_INFO_BUFFER_LENGTH_H 81
...
typedef enum {
   ...
   /* reasons for data call drop - network/modem disconnect */
    ...
    PDP_FAIL_VSNCP_TERMINATED = -8, /* for NW send VSNCP terminated
                                       message fail cause,*/
...
typedef struct {
    ...
    RIL_BarringInfo barringInfos[2];
...
typedef struct {
  char alpha_len;
  char alpha_buf[CDMA_ALPHA_INFO_BUFFER_LENGTH_H];
...
typedef struct {
  char len;
  char buf[CDMA_NUMBER_INFO_BUFFER_LENGTH_H];
  char number_type;
  char number_plan;
  char pi;
  char si;
...
#define SS_INFO_MAX_H 4
#define NUM_SERVICE_CLASSES_H 7
...
typedef struct {
  ...
  RIL_CallForwardInfo cfInfo[NUM_SERVICE_CLASSES_H]; /* This is the response data
...
typedef struct {
  ...
  union {
    int ssInfo[SS_INFO_MAX_H]; /* This is the response data for most of the SS GET/SET
...
typedef enum {
    ...
    RIL_APN_TYPE_VSIM         = 0x1000,       // APN type for RCS
    RIL_APN_TYPE_BIP          = 0x2000,       // APN type for BIP
    RIL_APN_TYPE_ENTERPRISE   = 0x4000,       // APN type for ENTERPRISE
    RIL_APN_TYPE_RCS          = 0x8000,       // APN type for VSIM

    RIL_APN_TYPE_COUNT        = 15,
...
//Warning: some mrkspril request/unsol id maybe is differrnt between 93 and 90,need check one by one
#include "mtk_ril_sp.h"
//Warning: request/unsol id defined by mtk_ril_ivt is only for telematic, 93 not support now
#include "mtk_ril_ivt.h"
#ifdef __cplusplus
}
#endif
