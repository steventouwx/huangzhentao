...
// build fail for IPV6_MIN_MTU and SIOCGIFHWADDR
...
#define BUFFER_SIZE_MAC_ADDR 30
...
int createRawSocket(const char* iface) {
    ...
    if (setsockopt(rawSocket, SOL_SOCKET, SO_SNDTIMEO, &timeout, len) < 0) {
        ...
        if (close(rawSocket) == -1) {
            mtkLogE(LOG_TAG, "createRawSocket setsockopt close error", strerror(errno));
        }
        return ERROR;
    }
    if (setsockopt(rawSocket, SOL_SOCKET, SO_BINDTODEVICE, iface, strlen(iface) + 1) < 0) {
        ...
        if (close(rawSocket) == -1) {
            mtkLogE(LOG_TAG, "createRawSocket setsockopt1 close error", strerror(errno));
        }
        return ERROR;
    }
    ...
    if (setupRaSocket(rawSocket, index) < 0) {
        ...
        if (close(rawSocket) == -1) {
            mtkLogE(LOG_TAG, "createRawSocket setupRaSocket close error", strerror(errno));
        }
        return ERROR;
    }
...
void getHwAddr(const char* iface, byte* mac) {
    ...
    char mac_addr[BUFFER_SIZE_MAC_ADDR] = {0};
    int len = 0;
    ...
    if ((ioctl(sockId, SIOCGIFHWADDR, &ifr_mac)) < 0) {
        if (close(sockId) == -1) {
            mtkLogE(LOG_TAG, "getHwAddr close error:%s", strerror(errno));
        } else {
    ...
    len = snprintf(mac_addr, sizeof(mac_addr), "%02x:%02x:%02x:%02x:%02x:%02x",
            (unsigned char) mac[0],
            (unsigned char) mac[1],
            (unsigned char) mac[2],
            (unsigned char) mac[3],
    ...
    if (len < 0 || len >= BUFFER_SIZE_MAC_ADDR) {
        mtkLogE(LOG_TAG, "getHwAddr get mac_addr error");
    } else {
        mtkLogD(LOG_TAG, "getHwAddr:%s %s", iface, mac_addr);
    }

    if (close(sockId) == -1) {
        mtkLogE(LOG_TAG, "getHwAddr close1 error:%s", strerror(errno));
    }
}
...
int triggerIpv6Rs(char* iface) {
    ...
    sendRs(sockFd, iface);
    if (close(sockFd) == -1) {
        mtkLogE(LOG_TAG, "triggerIpv6Rs close socket error %s", strerror(errno));
    }
    return SUCCESS;
}
