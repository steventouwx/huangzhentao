 ...
 * Include
 *****************************************************************************/
#include "RtmCommSimRadioConfigController.h"
...
#define RC_LOG_TAG "RtmRadioConfig"

/*****************************************************************************
 * Class RfxController
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("RtmCommSimRadioConfigController", RtmCommSimRadioConfigController,
        RfxController);
...
RtmCommSimRadioConfigController::RtmCommSimRadioConfigController() {
    pp_slotStatus = NULL;
    sending_count = 0;
}
...
void::RtmCommSimRadioConfigController::onDeinit() {
}

void RtmCommSimRadioConfigController::onInit() {
    ...
    logD(RC_LOG_TAG, "RtmCommSimRadioConfigController::onInit");
    pp_slotStatus = (RIL_SimSlotStatus **)malloc(RfxRilUtils::rfxGetSimCount() * sizeof(RIL_SimSlotStatus *));
    RFX_ASSERT(pp_slotStatus != NULL);
}

bool RtmCommSimRadioConfigController::onPreviewMessage(const sp<RfxMessage>& message) {
    if (message->getType() == URC) {
        return true;
    }

    if (message->getId() == RFX_MSG_REQUEST_SET_LOGICAL_TO_PHYSICAL_SLOT_MAPPING) {
        if ((message->getType() == REQUEST)  && isSwitchSimTypeProcessing()) {
            return false;
        }
        return true;
    }
...
bool RtmCommSimRadioConfigController::onCheckIfResumeMessage(const sp<RfxMessage>& message) {
    if (message->getType() == URC) {
        return true;
    }

    if (message->getId() == RFX_MSG_REQUEST_SET_LOGICAL_TO_PHYSICAL_SLOT_MAPPING) {
        if (!isSwitchSimTypeProcessing()) {
            return true;
        }
        return false;
    }
...
bool RtmCommSimRadioConfigController::onHandleRequest(const sp<RfxMessage>& message) {
...
bool RtmCommSimRadioConfigController::onHandleUrc(const sp<RfxMessage>& message) {
    // int msgId = message->getId();
    responseToRilj(message);
    return true;
}

bool RtmCommSimRadioConfigController::onHandleResponse(const sp<RfxMessage>& response) {
...
bool RtmCommSimRadioConfigController::onCheckIfRejectMessage(const sp<RfxMessage>& message,
...
bool RtmCommSimRadioConfigController::isSwitchSimTypeProcessing() {
    for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
