...
int RmcCatCommonRequestHandler::sMdStatusFd = -1;
...
RmcCatCommonRequestHandler::RmcCatCommonRequestHandler(int slot_id,
    ...
    //SUBMARINE, 0:disable, 1:enable
    logD(RFX_LOG_TAG, "query SUBMARINE support, %s=%d",feature, support);
    if (support == 1) {
...
void RmcCatCommonRequestHandler::requestStkSendEnvelopeCommandWithStatus (const sp<RfxMclMessage>& msg) {
    ...
    for ( ; atLine != NULL; atLine = atLine->getNext()) {
        line = atLine->getLine();
        if (NULL == line) {
...
void RmcCatCommonRequestHandler::initMdStatusQueryLoop() {
...
void *RmcCatCommonRequestHandler::mdStatusQueryLoop(void* params __unused) {
return NULL;
}

// get reset lock at bootup
void RmcCatCommonRequestHandler::getResetLock() {
}
...
void RmcCatCommonRequestHandler::releaseResetLock() {
}
#else
void *RmcCatCommonRequestHandler::mdStatusQueryLoop(void* params __unused) {
    int count = -1;
