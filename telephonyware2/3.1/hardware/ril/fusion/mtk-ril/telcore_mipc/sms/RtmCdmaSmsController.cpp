...
/*****************************************************************************
 * Class RtmCdmaSmsController
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("RtmCdmaSmsController", RtmCdmaSmsController, RfxController);
RtmCdmaSmsController::RtmCdmaSmsController() :m_timer(NULL), m_needStatusReport(false) {
}


RtmCdmaSmsController::~RtmCdmaSmsController() {
}


void RtmCdmaSmsController::onInit() {
...
bool RtmCdmaSmsController::onHandleRequest(const sp<RfxMessage>& message) {
    handleRequest(message);
    return true;
}

bool RtmCdmaSmsController::onHandleUrc(const sp<RfxMessage>& message) {
    if (message->getId() == RFX_MSG_URC_CDMA_NEW_SMS) {
        int messageId = -1;
        bool statusReport = RmmCdmaSmsConverter::isStatusReport(
        ...
        if (statusReport &&
                ((RtmImsSmsController *)getParent())->removeReferenceIdCached(
                        messageId)) {
            sp<RfxMessage> urc;
        ...
        logD(TAG, "isCtVolteSupport = %d isCtMixVolteSupport = %d",
               RfxRilUtils::isCtVolteSupport(), RfxRilUtils::isCtMixVolteSupport());
        logD(TAG, "c2k_volte_state = %d isCdmaPhoneMode = %d", c2k_volte_state, isCdmaPhoneMode());
        if ((RfxRilUtils::isCtVolteSupport() || RfxRilUtils::isCtMixVolteSupport()) &&
                (c2k_volte_state > 0) && (isCdmaPhoneMode() == false)) {
...
void RtmCdmaSmsController::onTimer() {
    ...
    sp<RfxMessage> newMsg = RfxMessage::obtainRequest(m_slot_id,
            ...
            RfxCdmaSmsAckData((void *)&ackCdma, sizeof(ackCdma)));
    requestToMcl(newMsg);
...
void RtmCdmaSmsController::handleMoSmsRequests(const sp<RfxMessage>& message) {
    if (message->getId() == RFX_MSG_REQUEST_IMS_SEND_SMS ||
        ...
        req = RfxMessage::obtainRequest(newId,
                RfxCdmaSmsMsgData((void *)pIms->message.cdmaMessage, sizeof(RIL_CDMA_SMS_Message)),
                message, false);
        if (newId == RFX_MSG_REQUEST_IMS_SEND_CDMA_SMS_EX) {
            m_needStatusReport = RmmCdmaSmsConverter::isNeedStatusReport(
                    pIms->message.cdmaMessage);
        }
        requestToMcl(req);
    } else {
        RFX_ASSERT(message->getId() == RFX_MSG_REQUEST_CDMA_SEND_SMS ||
                message->getId() == RFX_MSG_REQUEST_CDMA_SEND_SMS_EXPECT_MORE);
...
void RtmCdmaSmsController::handleSmscAdressResponses(const sp<RfxMessage>& message) {
...
void RtmCdmaSmsController::handleRequest(const sp<RfxMessage>& message) {
...
void RtmCdmaSmsController::handleMoSmsResponses(const sp<RfxMessage>& msg) {
        ...
        case RFX_MSG_REQUEST_IMS_SEND_CDMA_SMS_EX:
            {
                ...
                if (newId  == RFX_MSG_REQUEST_IMS_SEND_SMS_EX) {
                    ...
                    if (m_needStatusReport && (msg->getError() == RIL_E_SUCCESS) &&
                            moRsp->messageRef > 0) {
                        ((RtmImsSmsController *)getParent())->addReferenceId(
                                moRsp->messageRef);
...
bool RtmCdmaSmsController::onCheckCdmaSupported(const sp<RfxMessage>& msg) {
...
bool RtmCdmaSmsController::onCheckSimStatus(const sp<RfxMessage>& msg) {
...
bool RtmCdmaSmsController::onHandleResponse(const sp<RfxMessage>& msg) {
...
bool RtmCdmaSmsController::onPreviewMessage(const sp<RfxMessage>& message) {
    ...
    switch (message->getId()) {
        ...
        case RFX_MSG_URC_CDMA_RUIM_SMS_STORAGE_FULL: {
            // If SMS framework is not ready, we should not allow
            bool smsFwkReady = getStatusManager()->getBoolValue(
                    RFX_STATUS_KEY_SMS_FW_READY, false);
            if (!smsFwkReady) {
                logD(TAG, "SMS framework isn't ready yet. queue %s",
        ...
        case RFX_MSG_URC_CDMA_NEW_SMS: {
            ...
            m_timer = RfxTimer::start(RfxCallback0(this, &RtmCdmaSmsController::onTimer),
                    s2ns(MT_SMS_ACK_TIME));
            // If SMS framework is not ready, we should not allow CDMA_NEW_SMS
            bool smsFwkReady = getStatusManager()->getBoolValue(
                    RFX_STATUS_KEY_SMS_FW_READY, false);
            if (!smsFwkReady) {
                logD(TAG, "SMS framework isn't ready yet. queue %s",
...
bool RtmCdmaSmsController::onCheckIfResumeMessage(const sp<RfxMessage>& message) {
    switch (message->getId()) {
        case RFX_MSG_REQUEST_IMS_SEND_SMS:
        case RFX_MSG_REQUEST_IMS_SEND_SMS_EX:
        case RFX_MSG_REQUEST_CDMA_SEND_SMS:
        ...
        case RFX_MSG_URC_CDMA_NEW_SMS: {
            bool smsFwkReady = getStatusManager()->getBoolValue(
                    RFX_STATUS_KEY_SMS_FW_READY, false);
            if (smsFwkReady) {
                logD(TAG, "SMS framework is ready. Let's resume %s!",
                        idToString(message->getId()));
                return true;
            }
            break;
...
bool RtmCdmaSmsController::onCheckIfRejectMessage(
...
bool RtmCdmaSmsController::previewMessage(const sp<RfxMessage>& message) {
    return onPreviewMessage(message);
}

bool RtmCdmaSmsController::checkIfResumeMessage(const sp<RfxMessage>& message) {
    return onCheckIfResumeMessage(message);
}
#include "RfxStringData.h"
bool RtmCdmaSmsController::onHandleNewSms(const sp<RfxMessage> & msg) {
    RIL_CDMA_SMS_Message *message = (RIL_CDMA_SMS_Message*)msg->getData()->getData();
    RtmCdmaSmsMessage *cdmaMessage = new RtmCdmaSmsMessage(message);
    ...
    if (convertedMessage == NULL || convertedMessage->isError()) {
        ...
        if (parentCtrl != NULL) {
            ...
            parentCtrl->sendGsmSms((RtmGsmSmsMessage *)convertedMessage);
            ret = true;
        }
    }
    if (root != NULL) {
...
void RtmCdmaSmsController::handleCdmaSmsAck(const sp<RfxMessage>& message) {
...
void RtmCdmaSmsController::sendCdmaSms(RtmCdmaSmsMessage * msg) {
    sp<RfxMessage> urc = RfxMessage::obtainUrc(getSlotId(), RFX_MSG_URC_CDMA_NEW_SMS,
        RfxCdmaSmsMsgData((void *)&msg->getMessage(), sizeof(RIL_CDMA_SMS_Message)));
    responseToRilj(urc);
    getStatusManager()->setBoolValue(
            RFX_STATUS_KEY_SMS_FORMAT_CHANGE_ONGOING, true);
}

void RtmCdmaSmsController::sendCdmaSmsAck(const sp<RfxMessage>& message) {
    if (m_timer != NULL) {
        logD(TAG, "Stop MT SMS ack timer");
        RfxTimer::stop(m_timer);
        m_timer = NULL;
    ...
    sp<RfxMessage> newMsg = RfxMessage::obtainRequest(
            RFX_MSG_REQUEST_CDMA_SMS_ACKNOWLEDGE_INTERNAL,
            RfxCdmaSmsAckData((void *)&ackCdma, sizeof(ackCdma)), message, false);
    requestToMcl(newMsg);
}

bool RtmCdmaSmsController::isSupportSmsFormatConvert() {
    RtmImsSmsController *parentCtrl = (RtmImsSmsController *)getParent();
    if (parentCtrl != NULL) {
...
bool RtmCdmaSmsController::isCdmaPhoneMode() {
