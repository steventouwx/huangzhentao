 ...
 * 编写者:
 * 编写日期: 2026-03-05
 * 简要描述: 定义sv_audio_pcm相关接口、数据结构及功能能力。
 * 修改记录:
 *******************************************/
...
-------------------------------------------------------------------------------------------------*/
#ifndef __SV_AUDIO_PCM_H__
#define __SV_AUDIO_PCM_H__
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
void sv_audio_playback_state_ind_cb_set(sv_audio_playback_state_cb_f cb, void *params);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
sv_audio_handle_t sv_audio_playback_open(SV_AUDIO_FE_PCM_DEV_E fe_pcm, uint32_t be_dai_mask);

/*-----------------------------------------------------------------------------------------------*/
/**
  @brief  This function prepares for audio file playback.

  @param[in] hdl  The handle returned by sv_audio_playback_open().
  ...
  @note  Before calling this function, call sv_audio_playback_open() first to obtain a handle.
         If an audio file is expected to be played back, call this function first to prepare for
         the playback and then sv_audio_playback_play() to start playback.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_file_prepare(sv_audio_handle_t hdl,
...
/**
  ...
  @param[in] hdl  The API sv_audio_playback_open return results
  @param[in] pcm_config  Pcm config, including sample rate, channel nums,
  ...
  @note  Before calling this function, call sv_audio_playback_open() first to obtain a handle.
         ...
         for the audio stream playback and then sv_audio_playback_push_stream() to start playback.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_stream_prepare(sv_audio_handle_t hdl,
                                     sv_audio_pcm_config_t *p_config,
                                     sv_audio_playback_state_cb_f state_cb,
                                     void *params);
...
/**
  ...
  @note  Before calling this function, call sv_audio_playback_file_prepare() first to prepare the audio file
         to be played back, otherwise the audio data cannot be played back successfully.This function also supports
         playback of audio stream data. In this case, call sv_audio_playback_stream_prepare() first to prepare
         the audio stream to be played back, then this function to start playback, and finally
         sv_audio_playback_push_stream() to play back the audio stream in buffer.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_play(sv_audio_handle_t hdl);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_buffer_size_get(sv_audio_handle_t hdl, uint32_t *p_size);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_push_stream(sv_audio_handle_t hdl, void *buffer, uint32_t size);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_stop(sv_audio_handle_t hdl);
...
/**
  ...
  @Note  After an audio playback ends, you must call this function to close the audio context,
         otherwise subsequent call of sv_audio_playback_open() will fail.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_close(sv_audio_handle_t hdl);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_pause(sv_audio_handle_t hdl);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_resume(sv_audio_handle_t hdl);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
void sv_audio_playback_get_state(sv_audio_handle_t hdl, SV_AUDIO_PLAYBACK_STATE_E *p_state);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_playback_set_block_flag(sv_audio_handle_t hdl, uint8_t flags);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
void sv_audio_capture_state_ind_cb_set(sv_audio_capture_state_cb_f cb, void *params);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
sv_audio_handle_t sv_audio_capture_open(SV_AUDIO_FE_PCM_DEV_E fe_pcm, uint32_t be_dai_mask);

/*-----------------------------------------------------------------------------------------------*/
/**
  @brief This function prepares for audio file capturing.

  @param[in] hdl  The handle returned by sv_audio_capture_open().
  ...
  @note  Before calling this function, call sv_audio_capture_open() first to obtain a handle.
         If an audio file is expected to be captured, call this function first to prepare for the audio file
         capturing and then sv_audio_capture_record() to start capturing.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_file_prepare(sv_audio_handle_t hdl,
                                  const char *file_name,
...
/**
  ...
  @note  Before calling this function, call sv_audio_capture_open() first to obtain a handle.
         ...
         the audio stream capturing and then sv_audio_capture_push_stream() to start capturing.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_stream_prepare(sv_audio_handle_t hdl,
                                    sv_audio_pcm_config_t *p_config,
                                    sv_audio_capture_state_cb_f state_cb,
                                    void *params);
...
/**
  ...
  @note Before calling this function, call sv_audio_capture_file_prepare() first to prepare the audio
        file to be captured, otherwise the audio data cannot be captured successfully.This function also
        supports capturing of audio stream data. In this case, call sv_audio_capture_stream_prepare()
        first to prepare the audio stream to be captured, then this function to start capturing,
        and finally sv_audio_capture_pull_stream() to capture the audio stream in buffer.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_record(sv_audio_handle_t hdl);
...
/**
  ...
  @note  This function should be called after sv_audio_capture_stream_prepare
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_buffer_size_get(sv_audio_handle_t hdl, uint32_t *p_size);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_pull_stream(sv_audio_handle_t hdl, void *buffer, uint32_t size);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_stop(sv_audio_handle_t hdl);
...
/**
  ...
  @note  After audio capturing ends, you must call this function to close the audio context,
         otherwise subsequent call of sv_audio_capture_open() will fail.
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_close(sv_audio_handle_t hdl);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_pause(sv_audio_handle_t hdl);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_capture_resume(sv_audio_handle_t hdl);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
void sv_audio_capture_get_state(sv_audio_handle_t hdl, SV_AUDIO_CAPTURE_STATE_E *p_state);
...
/**
  ...
  */
/*-----------------------------------------------------------------------------------------------*/
int sv_audio_decode_amrFile_to_wavFile(const char *amr_file_name, const char *wav_file_name);
