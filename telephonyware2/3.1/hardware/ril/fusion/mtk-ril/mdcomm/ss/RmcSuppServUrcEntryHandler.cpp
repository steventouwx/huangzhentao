...
void RmcSuppServUrcEntryHandler::handleOnUssd(const sp<RfxMclMessage>& msg) {
    ...
    if (strcmp(finalData[0], "2") == 0
            && (finalData[1] != NULL || isUssdConvertToNotify())) {
        newMode = strdup("0");
        finalData[0] = newMode;
...
void RmcSuppServUrcEntryHandler::handleOnLteSuppSvcNotification(const sp<RfxMclMessage>& msg) {
    ...
    */
    ...
    switch (service) {
        ...
        case 111:  /* Called party is idle */
            svcNotify.code = CODE_IMS_MO_CALL_CALLED_PARTY_IDLE;
            break;
        ...
        case 260:
            ...
            cmosString = line->atTokNextstr(&ret);
            logD(TAG, "cmosString: %s", cmosString);

            if (ret < 0 || strlen(cmosString) > 255) {
                logE(TAG, "+ECMCCSS: Invalid parameters about cmos string");
                return;
            }
            tmpString = (char*) calloc(1, 256 * sizeof(char));
            if (tmpString == NULL) {
            ...
            svcNotify.code = CODE_IMS_MO_CALL_CMOS;
            strncpy(tmpString, cmosString, strlen(cmosString));
            svcNotify.number = tmpString;
            break;

        default:
            logE(TAG, "onLteSuppSvcNotification: unknown service = %d", service);
            return ;
    }
...
static bytes_t hexString2Byte(const char* hexString) {
    ...
    if (byteArray == NULL) {
        ...
        for(count = 0; count < byteLength; count++) {
            if (sscanf(pos, "%2hhx", &byteArray[count]) != EOF) {
                pos += 2;
            }
        }
    }
...
void RmcSuppServUrcEntryHandler::handleOnUssi(const sp<RfxMclMessage>& msg) {
    ...
    if (asprintf(&p_data[7], "%d", m_slot_id) < 0) {
        logD(TAG, "onUssi, allocate p_data[7] failed.");
        goto error;
    }

    if (p_data[7] == NULL) {
        logE(TAG, "onUssi, p_data[7] is NULL");
        goto error;
    }
...
void RmcSuppServUrcEntryHandler::handleOnXui(const sp<RfxMclMessage>& msg) {
    ...
    if (asprintf(&p_data[3], "%d", m_slot_id) < 0) {
        logD(TAG, "onXui, allocate p_data[3] failed.");
        goto error;
    }

    if (p_data[3] == NULL) {
        logE(TAG, "onXui, p_data[3] is NULL");
        goto error;
    }
