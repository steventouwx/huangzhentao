...
void RilClient::handleStateInitializing() {
    ...
    if (listenFd < 0) {
        ...
        char path[255 + 1] = {0};
        if (snprintf(path, 255, "%s", socketName) < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "sprintf fail, closed");
...
void RilClient::requestComplete(RIL_Token token, RIL_Errno e, const void *response,
        size_t responselen) {
    if (commandFd < 0) {
        RFX_LOG_D(RFX_LOG_TAG, "command Fd not ready here");
        return;
    ...
    if (response != NULL) {
        ...
        ret = pRI->pCI->responseFunction(p, (void *)response, responselen);
