...
ATRESPONSE_t pas_cclk_handler(char* cmdline, ATOP_t opType, char* response) {
    ...
    switch (opType) {
        case AT_READ_OP: {
            ...
            if(time(&timep)) {
                localtime_r(&timep, &localtime_p);
            } else {
                LOGATCI(LOG_ERR, "get local time fail");
                return AT_ERROR;
            }

            LOGATCI(LOG_INFO, "local timetone, tm_zone: %s, tm_gmtoff: %d", localtime_p.tm_zone, (int)(localtime_p.tm_gmtoff));
            ...
            if (gCclkTz == 0xFFFF) {
                ...
                qTz = (int)(localtime_p.tm_gmtoff) / 3600 * 4;
            } else {
                qTz = gCclkTz;
            }
            ...
            if (gCsdfAuxmode == 1) {
                if (qTz >= 0) {
                    if (sprintf(response, "+CCLK: \"%02d/%02d/%02d,%02d:%02d:%02d+%02d\"\n",
                            (1900+gm_p->tm_year)%100, (1+gm_p->tm_mon), gm_p->tm_mday,
                            gm_p->tm_hour, gm_p->tm_min, gm_p->tm_sec, qTz) < 0) {
                ...
                if (qTz >= 0) {
                    if (sprintf(response, "+CCLK: \"%04d/%02d/%02d,%02d:%02d:%02d+%02d\"\n",
                            1900+gm_p->tm_year, (1+gm_p->tm_mon), gm_p->tm_mday,
                            gm_p->tm_hour, gm_p->tm_min, gm_p->tm_sec, qTz) < 0) {
                        LOGATCI(LOG_ERR, "[pas_cclk_handler] sprintf fail");
                        return AT_ERROR;
                    }
...
ATRESPONSE_t pas_mergemsg_handler(char* cmdline, ATOP_t opType, char* response) {
    int err = 0, cmdID = 0;
    char msg_merge_mode[MTK_PROPERTY_VALUE_MAX] = {0};
    int mergemode = 0;
    LOGATCI(LOG_DEBUG, "pas_mergemsg_handler enter with opType: %d", opType);
    ...
    switch (opType) {
        case AT_SET_OP:
            ...
            if (1 == cmdID)
                mtk_property_set(ATCI_MERGE_MSG_CMD_PROP, "1");
            else if (0 == cmdID)
                mtk_property_set(ATCI_MERGE_MSG_CMD_PROP, "0");
            return AT_OK;
            break;
        case AT_TEST_OP:
            writeDataToSerial("+MERGEMSG:(0,1)",strlen("+MERGEMSG:(0,1)"));
            return AT_OK;
            break;
        case AT_READ_OP:
            mtk_property_get(ATCI_MERGE_MSG_CMD_PROP, msg_merge_mode, "0");
            mergemode = atoi(msg_merge_mode);
            if (1 == mergemode) {
                if (snprintf(response, strlen("+MERGEMSG:1\r\n") + 1, "%s", "+MERGEMSG:1\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_mergemsg_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else if (0 == mergemode) {
                if (snprintf(response, strlen("+MERGEMSG:0\r\n") + 1, "%s", "+MERGEMSG:0\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_mergemsg_handler] snprintf fail");
                    return AT_ERROR;
                }
            } else {
                if (snprintf(response, strlen("+MERGEMSG:invalid value.\r\n") + 1, "%s",
                        "+MERGEMSG:invalid value.\r\n") < 0) {
                    LOGATCI(LOG_ERR, "[pas_mergemsg_handler] snprintf fail");
                    return AT_ERROR;
                }
            }
