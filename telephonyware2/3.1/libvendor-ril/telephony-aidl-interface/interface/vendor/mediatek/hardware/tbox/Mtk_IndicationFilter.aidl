...
@VintfStability
@Backing(type="int")
enum Mtk_IndicationFilter {
    NONE = 0,
    ALL = ~0,
    /**
     * When this bit is set, modem must send the signal strength update
     */
    SIGNAL_STRENGTH = 1 << 0,
    ...
    /**
     * When this bit is set, modem must send the link capacity update
     */
    LINK_CAPACITY_ESTIMATE = 1 << 3,
    /**
     * When this bit is set, the modem must send the physical channel configuration update
     */
