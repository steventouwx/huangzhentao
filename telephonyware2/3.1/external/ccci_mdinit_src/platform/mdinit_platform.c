...
extern void write_ccci_dump(const char *cmd, ...);
...
void store_md_statue(int md_id, char *buf, int len)
{
    ...
    ret = snprintf(name, sizeof(name), "vendor.mtk.md%d.status",md_id + 1); /*stdio.h*/
    if (ret < 0 || ret >= sizeof(name)) {
        CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return;
    }
    CCCI_LOGD("%s set md status:%s=%s\n", __func__, name, buf);
    buf[len - 1] = '\0';
    ret = snprintf(prop_val, sizeof(prop_val),"%s", buf);
    if (ret < 0 || ret >= sizeof(name)) {
        CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return;
    }
    ...
    if (md_id == MD_SYS3) {
        ret = snprintf(name, sizeof(name), "vendor.net.cdma.mdmstat");
        if (ret < 0 || ret >= sizeof(name)) {
            CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
            return;
        }
...
#define CXP_SBP "/mnt/vendor/nvdata/APCFG/APRDCL/CXP_SBP"
    ...
    if (fd < 0) {
        CCCI_LOGE("fail to open %s: %d\n", CXP_SBP, errno);
        sbp_code = -1;
    } else {
        RetVal = read(fd, sbp_id, sizeof(sbp_id) - 1);
        if (RetVal <= 0) {
            CCCI_LOGE("Failed to read from %s (%d) !! errno = %d\n", CXP_SBP, RetVal, errno);
            sbp_code = -1;
        } else {
            CCCI_LOGD("Read %d bytes from %s,sbp_id:%s\n", RetVal, CXP_SBP, sbp_id);
...
static bool check_nvram_ready(void)
{
    ...
    //write_ccci_dump("Begin to check_nvram_ready");
    while (true)
    {
        read_nvram_ready_retry++;
        //property_get("vendor.service.nvram_restore", nvram_init_val, NULL);
        ...
        {
            //CCCI_LOGI("%s(), property_get(\"vendor.service.nvram_restore\") = %s, read_nvram_ready_retry = %d\n",
    ...
    //write_ccci_dump("check_nvram_ready done, retry %d times", read_nvram_ready_retry);
    return ret;
}
...
int get_md_sbp_code(int md_id, int version)
{
    ...
    if (usp_sbp_value >= 0) { /* Carrier 2.0 */
        ...
        ret = snprintf(sbp_buf, sizeof(sbp_buf), "%d", sbpc);
        if (ret < 0 || ret >= sizeof(sbp_buf))
            CCCI_LOGE("%s-%d: sbp snprintf fail, ret = %d\n", __func__, __LINE__, ret);
        property_set("vendor.mtk.md.sbp", sbp_buf);
        return sbpc;
    }
...
int md_image_dep_check(int md_id)
{
    ...
    property_get("persist.vendor.md_c2k_cap_dep_check", value, NULL);
    value[PROPERTY_VALUE_MAX - 1] = '\0';
    retpropget = atoi(value);
