...
/*****************************************************************************
 * Class RtmCapabilityGetController
 *****************************************************************************/
#define RFX_LOG_TAG "RtmCapa"

RFX_IMPLEMENT_CLASS("RtmCapabilityGetController", RtmCapabilityGetController, RfxController);

RFX_REGISTER_DATA_TO_URC_ID(RfxRadioCapabilityData, RFX_MSG_URC_RADIO_CAPABILITY);

RtmCapabilityGetController::RtmCapabilityGetController() : m_cslot(0), m_first_urc(true) {
    logI(RFX_LOG_TAG, "constructor entered");
}

RtmCapabilityGetController::~RtmCapabilityGetController() {
}

void RtmCapabilityGetController::onInit() {
    ...
    // TODO:  get main protocol and init the property and status key
    //rfx_property_set("persist.vendor.radio.simswitch", String8::format("%d", main_sim).string());
}

bool RtmCapabilityGetController::onHandleUrc(const sp<RfxMessage> &message) {
...
// Only allow to be called by RtmModeSwitchController for once after SIM switch
void RtmCapabilityGetController::updateRadioCapability(int cslot) {
    ...
    if (session_id != -1 || getNonSlotScopeStatusManager()->getIntValue(
        ...
        if (RtmCapabilitySwitchUtil::isDssNoResetSupport() == false) {
        ...
        getNonSlotScopeStatusManager()->registerStatusChanged(
                ...
                RfxStatusChangeCallback(this, &RtmCapabilityGetController::onModeSwitchFinished));
                */
...
/*
void RtmCapabilityGetController::onModeSwitchFinished(RfxStatusKeyEnum key, RfxVariant old_value,
                                                            RfxVariant value) {
    RFX_UNUSED(key);
    logD(RFX_LOG_TAG, "onModeSwitchFinished (%d, %d)", old_value.asInt(), value.asInt());
    RtmCapabilitySwitchController* switchController = (RtmCapabilitySwitchController *)
            findController(RFX_OBJ_CLASS_INFO(RtmCapabilitySwitchController));
...
void RtmCapabilityGetController::updateRadioCapability(int cslot, int session_id) {
    int radio_capability;

    m_cslot = cslot;
    m_first_urc = false;

    if (RtmCapabilitySwitchUtil::isDisableC2kCapability()) {
        for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
...
void RtmCapabilityGetController::sendRadioCapabilityURC(int slot_id, int session_id, int radio_capability) {
...
void RtmCapabilityGetController::updateRadioCapabilityForWMChange(int world_mode) {
    ...
    for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
        ...
        getStatusManager(i)->setIntValue(
                RFX_STATUS_KEY_SLOT_CAPABILITY, radio_capability, false, false);
        if (RtmCapabilitySwitchUtil::isDisableC2kCapability() == false) {
            // always send CDMA capability from Android Q
            radio_capability |= (RAF_CDMA_GROUP | RAF_EVDO_GROUP);
        }
