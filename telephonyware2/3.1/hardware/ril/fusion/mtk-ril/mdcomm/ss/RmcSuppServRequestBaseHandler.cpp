...
void RmcSuppServRequestBaseHandler::requestClirOperation(const sp<RfxMclMessage>& msg) {
    ...
    int simState = getMclStatusManager()->getIntValue(RFX_STATUS_KEY_SIM_STATE);
    logD(TAG, "requestClirOperation, simState = %d", simState);


    if (msg->getData()->getDataLength() != 0) {
        if (asprintf(&cmd, "AT+CLIR=%d", n[0]) < 0) {
            logE(TAG, "requestClirOperation fail at asprintf");
            goto error;
        }

        if (cmd == NULL) {
            logE(TAG, "requestClirOperation, cmd is NULL");
            goto error;
        }
        ...
        if (simState == RFX_SIM_STATE_ABSENT || simState == RFX_SIM_STATE_NOT_READY) {
            logE(TAG, "requestClirOperation Fail, SIM is absent or not ready yet");
            goto error;
        }
        /**
         * Get CLIR: +CLIR?
    ...
    switch (p_response->atGetCmeError()) {
        ...
        case CME_412_PRECONDITION_FAILED:
            ...
            handleErrorMessageFromXcap(p_response, CME_412_PRECONDITION_FAILED);
            goto error;
            break;
        case CME_415_UNSUPPORTED_MEDIA_TYPE:
            ret = RIL_E_415_UNSUPPORTED_MEDIA_TYPE;
            handleErrorMessageFromXcap(p_response, CME_415_UNSUPPORTED_MEDIA_TYPE);
            goto error;
            break;
        case CME_500_INTERNAL_SERVER_ERROR:
            ret = RIL_E_500_INTERNAL_SERVER_ERROR;
            handleErrorMessageFromXcap(p_response, CME_500_INTERNAL_SERVER_ERROR);
            goto error;
            break;
        case CME_503_SERVICE_UNAVAILABLE:
            ret = RIL_E_503_SERVICE_UNAVAILABLE;
            handleErrorMessageFromXcap(p_response, CME_503_SERVICE_UNAVAILABLE);
            goto error;
            break;
...
void RmcSuppServRequestBaseHandler::handleSetClirResponse(int clir_n) {
    ...
    length = snprintf(str, sizeof(str), "%d", clir_n);
    if (length < 0) {
        logE(TAG, "handleSetClirResponse error!");
    }
...
void RmcSuppServRequestBaseHandler::handleGetClirResponse(int (&responses)[2],
    ...
    char clir_n[MTK_PROPERTY_VALUE_MAX] = {0};
    char clir_m[MTK_PROPERTY_VALUE_MAX] = {0};

    // Setting default value first
    // <n> : NETWORK_DEFAULT (0)
    // <m> : PRESENTATION_ALLOWED_TEMPORARY (4)
    if (snprintf(clir_n, sizeof(clir_n), "0") >= 0) {
        getMSimProperty(m_slot_id, (char *) PROPERTY_TBCLIR_N, clir_n);
    }

    if (snprintf(clir_m, sizeof(clir_m), "4") >= 0) {
...
void RmcSuppServRequestBaseHandler::resetClirProperty() {
    ...
    length = snprintf(str_n, sizeof(str_n), "%d", 0);
    if (length < 0) {
        logE(TAG, "resetClirProperty error!");
    }

    length = snprintf(str_m, sizeof(str_m), "%d", 4);
    if (length < 0) {
        logE(TAG, "resetClirProperty error!");
    }
    ...
    // sync value to RfxStatusManager
    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_TBCLIR_N, atoi(str_n));
}
...
void RmcSuppServRequestBaseHandler::requestCallForwardOperation(const sp<RfxMclMessage>& msg, CallForwardOperationE op) {
    ...
    sp<RfxAtResponse> p_response = new RfxAtResponse();
    int err;
     ...
     */
     ...
     if ((CCFC_E_QUERY == op) && (p_args->reason == CF_ALL || p_args->reason == CF_ALLCOND)) {
     ...
     */
    if (CCFC_E_QUERY == op) {
        logD(TAG, "Call Forwarding: change DEACTIVATE to INTERROGATE");
        p_args->status = SS_INTERROGATE;
    }

    if ((p_args->number != NULL) && (p_args->status == SS_ACTIVATE)) {
        logD(TAG, "Call Forwarding: change ACTIVATE to REGISTER");
    ...
    if ((p_args->number != NULL)
        ...
        char *extractedNumber = extractNetworkPortionAlt(p_args->number);
        p_args->number = replaceSetCFNumberIfNeeded(extractedNumber, currentMccmnc.string());
        free((char*) extractedNumber);
        ...
        char *tmpNumber = encryptString(p_args->number);
        ...
        for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
            ...
            if (p_cur->atTokHasmore()) {
                ...
                p_CfInfoResponse->number
                    = replaceGetCFNumberIfNeeded(p_CfInfoResponse->number, currentMccmnc.string());
                if (err < 0) {
            ...
            if (p_cur->atTokHasmore()) {
                ...
                if (p_CfInfoResponse->timeSeconds < 0) {
                    logE(TAG, "+CCFC: timeSeconds:%d", p_CfInfoResponse->timeSeconds);
                    p_CfInfoResponse->timeSeconds = 0;
                }
            }
             ...
             */
            if (p_CfInfoResponse->serviceClass == CLASS_DATA_SYNC && isVideoServiceClass(serviceClass)) {
                logD(TAG, "requestCallForward, change CLASS_DATA_SYNC to CLASS_MTK_VIDEO");
                p_CfInfoResponse->serviceClass = serviceClass;
            }

            /* Add this check for sensitive log */
            char *tmpNumber = encryptString(p_CfInfoResponse->number);
...
void RmcSuppServRequestBaseHandler::requestCallWaitingOperation(const sp<RfxMclMessage>& msg,
    ...
    logD(TAG, "tbCWStatus = %s", tbCWStatus);
    ...
    if (msg->getData()->getDataLength() == sizeof(int)) {
        ...
        if (asprintf(&cmd, "AT+ECUSD=1,1,\"*#43#\"") < 0) {
            logE(TAG, "requestCallWaitingOperation fail at asprintf by querying");
            goto error;
        }

        if (cmd == NULL) {
            logE(TAG, "requestCallWaitingOperation, cmd is NULL");
            goto error;
        }
        ...
        if (p_int[1] != 0) {
            /* with InfoClass */
            cmdRet = asprintf(&cmd, "AT+ECUSD=1,1,\"%s43*%s#\"",
                             ssStatusToOpCodeString(SsStatusE(p_int[0])),
                             InfoClassToMmiBSCodeString(AtInfoClassE(p_int[1])));
        } else {
            /* User did not input InfoClass */
            cmdRet = asprintf(&cmd, "AT+ECUSD=1,1,\"%s43#\"",
                            ssStatusToOpCodeString(SsStatusE(p_int[0])));
        }

        if (cmdRet < 0) {
            logE(TAG, "requestCallWaitingOperation fail as cmdRet error.");
            goto error;
        }
    ...
    if (p_response->getIntermediates() != NULL && !bHandleTBCW) {
        ...
        for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
            ...
            */
            ...
            if (sendBsCode != 0) {
                // ALPS07660149, change to CLASS_MTK_VIDEO only for video request
                // From call settings: sendBsCode = 512(CLASS_MTK_VIDEO)
                // From MMI command: sendBsCode = 528(CLASS_MTK_VIDEO + CLASS_DATA_SYNC), SI:24
                if (sendBsCode == bsCode || (bsCode == CLASS_DATA_SYNC && isVideoServiceClass(sendBsCode))) {
                   /*
                    ...
                    */
                    ...
                    // ALPS07660149, change to CLASS_MTK_VIDEO only for video request
                    if (bsCode == CLASS_DATA_SYNC && isVideoServiceClass(sendBsCode)) {
                ...
                if (responses[0] == 1) {
                    // ALPS07660149, change to CLASS_MTK_VIDEO only for video request
                    responses[1] |=
                        (bsCode == CLASS_DATA_SYNC && isVideoServiceClass(sendBsCode)) ? sendBsCode : bsCode;
                }
...
void RmcSuppServRequestBaseHandler::requestColpOperation(const sp<RfxMclMessage>& msg) {
    ...
    if (msg->getData()->getDataLength() != 0) {
         ...
         *  <n> (parameter sets the adjustment for outgoing calls)
         */
        if (asprintf(&cmd, "AT+COLP=%d", n[0]) < 0) {
            logE(TAG, "requestColpOperation asprintf fail in setting");
            goto error;
        }

        if (cmd == NULL) {
            logE(TAG, "requestColpOperation, cmd is NULL");
            goto error;
        }
...
void RmcSuppServRequestBaseHandler::requestColrOperation(const sp<RfxMclMessage>& msg) {
    ...
    if (msg->getData()->getDataLength() != 0) {
         ...
         * Set COLR:
         */
        if (asprintf(&cmd, "AT+COLR=%d", n[0]) < 0) {
            logE(TAG, "requestColrOperation OOM");
            goto error;
        }

        if (cmd == NULL) {
            logE(TAG, "requestColrOperation, cmd is NULL");
            goto error;
        }
...
void RmcSuppServRequestBaseHandler::requestCallBarring(const sp<RfxMclMessage>& msg, CallBarringOperationE op) {
    ...
    do {
        ...
        if ( msg->getData()->getDataLength() == 4*sizeof(char*) ) {
            ...
            cmdRet = asprintf(&cmd, "AT+ECUSD=1,1,\"*#%s#\"",
                            callBarFacToServiceCodeStrings(strings[0]));
            //}

            if (cmd == NULL) {
                logE(TAG, "requestCallBarring, cmd is NULL");
                goto error;
            }

            char *tmpCmd = encryptString(cmd);
            logD(TAG, "Query call barring cmd = %s", tmpCmd);
            if (tmpCmd != NULL) {
                free(tmpCmd);
            }

            if (cmdRet < 0) {
                logE(TAG, "Set Facility Lock: OOM -1");
            ...
            if ((NULL != strings[3]) && (0 != strcmp (strings[3],"0"))) {
                ...
                if ( 0 == strcmp (strings[1],"0")) {
                    cmdRet = asprintf(&cmd, "AT+ECUSD=1,1,\"#%s*%s*%s#\"",
                                    callBarFacToServiceCodeStrings(strings[0]),
                                    strings[2],
                ...
                if (strlen(strings[2]) == 0) {  // User doesn't input password
                    ...
                    if ( 0 == strcmp (strings[1],"0")) {
                        cmdRet = asprintf(&cmd, "AT+ECUSD=1,1,\"#%s*%s#\"",
                                        callBarFacToServiceCodeStrings(strings[0]),
                                        strings[2]);
                    } else {
                        cmdRet = asprintf(&cmd, "AT+ECUSD=1,1,\"*%s*%s#\"",
            ...
            if (cmd == NULL) {
                logE(TAG, "Set Facility Lock: cmd is NULL");
                goto error;
            }

            char *tmpCmd = encryptString(cmd);
            logD(TAG, "Set call barring cmd = %s", tmpCmd);
            if (tmpCmd != NULL) {
                free(tmpCmd);
            }
    ...
    if (p_response->getIntermediates() != NULL) {
        ...
        for (p_cur = p_response->getIntermediates(); p_cur != NULL; p_cur = p_cur->getNext()) {
            ...
            if ((sendBsCode == serviceClass)
                    || ((serviceClass == CLASS_DATA_SYNC) && isVideoServiceClass(sendBsCode))) {
                break;
            }
            ...
            if ((sendBsCode == 0) && (response != 0)) {       //For voice/video service
                ...
                if ((serviceClass == CLASS_DATA_SYNC) && isVideoServiceClass(sendBsCode)) {
...
error:
    ...
    logD(TAG, "requestCallBarring response: %d, msg id: %d, ret: %d", response, msgid, ret);
