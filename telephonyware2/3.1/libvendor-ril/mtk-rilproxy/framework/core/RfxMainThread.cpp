 ...
 *****************************************************************************/
#define RFX_MAIN_THREAD_WATCHDOG_TIME (300*1000)

static timer_t sWatchDogTimer;
static struct timespec sStartReal;
static struct timespec sStartMono;
static int sTimeOutMsgId = -1;
static int sTimeOutSlotId = -1;
static int sTimeOutMsgType = -1;
...
static void _watch_dog(sigval_t sig) {
    ...
    RFX_LOG_I("RFX_WATCHDOG", "timeout message id: %d, %s", sTimeOutMsgId, requestToString(sTimeOutMsgId));
    ...
    notifyTeleServiceMainThreadTimeout(sTimeOutSlotId, sTimeOutMsgId,sTimeOutMsgType,
        sStartMono.tv_sec, sStartMono.tv_nsec, endMono.tv_sec, endMono.tv_nsec, "extra info");
...
protected:
    ...
    virtual void onHandleMessage(const Message& message) {
        ...
        // do further dispatch
        sTimeOutMsgId = m_msg->getId();
        sTimeOutSlotId = m_msg->getSlotId();
        sTimeOutMsgType = m_msg->getType();
        RfxRootController *root = RFX_OBJ_GET_INSTANCE(RfxRootController);
...
sp<RfxMainThread> RfxMainThread::s_self = NULL;
...
void RfxMainThread::enqueueMessage(const sp<RfxMessage>& message) {
    RFX_LOG_D(RFX_LOG_TAG, "enqueueMessage(), start");
    RFX_ASSERT(s_self != NULL && s_self->m_looper != NULL);
    RFX_LOG_D(RFX_LOG_TAG, "enqueueMessage(), after");
