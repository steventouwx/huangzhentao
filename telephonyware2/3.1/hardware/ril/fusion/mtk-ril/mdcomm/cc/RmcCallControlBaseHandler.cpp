...
void RmcCallControlBaseHandler::setVDSAuto(bool isAuto, bool isEcc) {
    ...
    */
    ...
    if (!isEcc) {
        ...
        if (isAuto) {
            enableVDSReport();
            if (isVoiceGsmRat() && prefRat == 3) {      // gsm_pref
                mode = 0;
                prefMode = 1;
            } else if (isVoiceGsmRat() || isCdmaRat() || prefRat == 0) {  // ims+gsm+cdma
                mode = 0;
                prefMode = 0;
            } else {  // no service with pref
                if (prefRat == 1) {  // ims+gsm
                    mode = 2;
                    prefMode = 1;//ALPS05919649, Modem requires to set prefer WG for GSM only number
            ...
            if (isVoiceGsmRat() || isCdmaRat() || prefRat == 0) {  // gsm+cdma
                mode = 5;
                prefMode = 0;
            } else {  // no service with pref
                if (prefRat == 1) {  // gsm
...
bool RmcCallControlBaseHandler::isVoiceGsmRat() {
...
String8 RmcCallControlBaseHandler::handleNumberWithPause(char* number) {
    ...
    if (tmp != NULL) {
        ...
        if (RfxRilUtils::isUserLoad() != 1) {
            String8 encodedFormatNumber = String8::format("%s",
                    RfxRilUtils::pii(RFX_LOG_TAG, formatNumber));
            String8 encodedOrigNumber = String8::format("%s",
                    RfxRilUtils::pii(RFX_LOG_TAG, origNumber));
            logD(RFX_LOG_TAG, "%s, number:%s, formatNum:%s, origNum:%s", __FUNCTION__,
                    RfxRilUtils::pii(RFX_LOG_TAG, number), encodedFormatNumber.string(),
                    encodedOrigNumber.string());
        }
