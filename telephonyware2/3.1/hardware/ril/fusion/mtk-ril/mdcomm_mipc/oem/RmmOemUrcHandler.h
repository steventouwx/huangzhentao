...
#ifndef __RMM_OEM_URC_HANDLER_H__
#define __RMM_OEM_URC_HANDLER_H__
...
class RmmOemUrcHandler : public RfxBaseHandler {
    RFX_DECLARE_HANDLER_CLASS(RmmOemUrcHandler);

    public:
        RmmOemUrcHandler(int slot_id, int channel_id);
        virtual ~RmmOemUrcHandler();

    protected:
        virtual void onHandleUrc(const sp<RfxMclMessage>& msg);

    private:
        void handleEWarning(RfxMipcData* data);
        void handleTXPower(RfxMipcData* data);
        void handleTXPowerStatus(RfxMipcData* data);
        void handleNvErr(RfxMipcData* data);
