...
#define OP_RIL_MIPC_PATH "libmtkmipc-rilop.so"
...
void RfxOpUtils::initOpLibrary() {
    ...
    if (RfxRilUtils::isMipcSupport() == true) {
        RFX_LOG_D(RFX_LOG_TAG, "initOpLibrary, init mipc path");
        sDlOpHandler = dlopen(OP_RIL_MIPC_PATH, RTLD_NOW);
    } else {
        RFX_LOG_D(RFX_LOG_TAG, "initOpLibrary, init non mipc path");
        sDlOpHandler = dlopen(OP_RIL_PATH, RTLD_NOW);
    }
    if (sDlOpHandler == NULL) {
        RFX_LOG_D(RFX_LOG_TAG, "[%s] dlopen failed error: %s",
...
const rfx_ctrler_factory_init *RfxOpUtils::getPreNonSlotOpCtlerList() {
    ...
    if (func == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%s] destroy not defined or exported error:%s",
