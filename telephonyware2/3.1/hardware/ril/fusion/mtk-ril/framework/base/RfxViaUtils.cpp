...
#define VIA_RIL_MIPC_PATH "libviamipc-ril.so"
...
void RfxViaUtils::initViaHandler() {
    ...
    if (mode == 1) {
        m_dlHandler = dlopen(VIA_RIL_MIPC_PATH, RTLD_NOW);
        if (m_dlHandler == NULL) {
            RFX_LOG_I(RFX_LOG_TAG, "[%s] dlopen failed in %s: %s",
                    __FUNCTION__, VIA_RIL_MIPC_PATH, dlerror());
            return;
        }
    } else {
    ...
    if (createViaHandler == NULL) {
        if (mode == 1) {
            RFX_LOG_E(RFX_LOG_TAG, "[%s] create not defined or exported in %s: %s",
                    __FUNCTION__, VIA_RIL_MIPC_PATH, dlsym_error);
        } else {
            RFX_LOG_E(RFX_LOG_TAG, "[%s] create not defined or exported in %s: %s",
