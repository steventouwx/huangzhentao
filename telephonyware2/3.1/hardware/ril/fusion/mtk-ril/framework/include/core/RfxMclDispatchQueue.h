...
#ifndef __RFX_MCL_DISPATCH_QUEUE_H__
#define __RFX_MCL_DISPATCH_QUEUE_H__
   ...
   /**
    ...
    public:
       ...
       /**
         * Queue constructor.
         */
        Mcl_dispatch_queue(void);

       /**
         * get an element with a particular message id and token.
         *
         * @param Request token.
         */
        T* get(int token);
...
template <typename T>
Mcl_dispatch_queue<T>::Mcl_dispatch_queue(void) {
...
template <typename T>
T* Mcl_dispatch_queue<T>::dequeue(void) {
    T* temp = NULL;
...
template <typename T>
void Mcl_dispatch_queue<T>::enqueue(T* request) {
...
template <typename T>
T* Mcl_dispatch_queue<T>::checkAndDequeue(int token) {
    int ret = 0;
    T* temp = NULL;
    ...
    for(T **ppCur = &(this->front); *ppCur != NULL; ppCur = &((*ppCur)->p_next)) {
        if (token == (*ppCur)->msg->getMclToken()) {
            ret = 1;
            temp = *ppCur;
            *ppCur = (*ppCur)->p_next;
            break;
...
template <typename T>
T* Mcl_dispatch_queue<T>::get(int token) {
    T* temp = NULL;
...
template <typename T>
T* Mcl_dispatch_queue<T>::getFront(void) {
    T* temp = NULL;
...
template <typename T>
T* Mcl_dispatch_queue<T>::getClonedObj(int token) {
    int ret = 0;
    T* temp = NULL;
...
template <typename T>
int Mcl_dispatch_queue<T>::empty(void) {
...
#endif /* __RFX_MCL_DISPATCH_QUEUE_H__ */
