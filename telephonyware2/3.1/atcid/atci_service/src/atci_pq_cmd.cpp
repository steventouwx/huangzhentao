...
using ::aidl::vendor::mediatek::hardware::pq_aidl::IoctlRequest;
using ::aidl::vendor::mediatek::hardware::pq_aidl::DispPQIoctlParams;
using ::aidl::vendor::mediatek::hardware::pq_aidl::parcelable_execIoctl;
using ::aidl::vendor::mediatek::hardware::pq_aidl::parcelable_getTuningField;
using ::aidl::vendor::mediatek::hardware::pq_aidl::parcelable_getTDSHPFlag;
...
execIoctl_return execIoctl(DispPQIoctlParams argv, int LCMIndex = 0);
int setTuningField(int pqModule, int field, int value, int LCMIndex = 0);
int setTDSHPFlag(int flag, int LCMIndex = 0);
getTuning_return getTuningField(int pqModule, int field, int LCMIndex = 0);
getTdshp_return getTDSHPFlag(int LCMIndex = 0);
...
int pq_cmd_handler(char* cmdline, ATOP_t at_op, char* response)
{
    ...
    static const std::string kInstance =
            std::string() + IPictureQuality_AIDL::descriptor + "/default";
    ...
    switch(at_op){
        ...
        case AT_SET_OP:
            ...
            if (actionID == HW_READ)
            {
                ...
                parcelable_execIoctl _aidl_return;
                ScopedAStatus ret = service->execIoctlMPq(panelId,
                        PANEL_ID_FLAG, arg, &_aidl_return);
                if (_aidl_return.retval == Result::OK)
                {
                    params.val = _aidl_return.retParam.val;
                }
                else
                {
...
#else
                execIoctl_return _aidl_return;

                arg.request = IOCTL_READ_REG;
                arg.reg = params.reg;
                arg.val = params.val;
                arg.mask = params.mask;
                _aidl_return = execIoctl(arg, panelId);
                if (_aidl_return.retval == Result::OK)
                {
                    params.val = _aidl_return.retParam.val;
                }
                ...
                {
                    ALOGE("[PQ_CMD] execIoctl [%d] failed", arg.request);
                }
            ...
            else if (actionID == HW_WRITE)
            {
                ...
                if((unsigned int)RegBase == 0xEEEEEEEE)
                {
                    if(RegValue == 0)
                    {
                        ...
                        if (_aidl_return.retval != Result::OK)
                        {
                            __UNUSED(_aidl_return.retParam);
                            ALOGE("[PQ_CMD] service->execIoctl [%d] failed", arg.request);
                        }
                        if (!ret.isOk()) {
                            ALOGE("[PQ_CMD] IPictureQuality::execIoctl failed!");
                        ...
                        arg.request = IOCTL_MUTEX_CONTROL;
                        arg.val = 1;
                        _aidl_return = execIoctl(arg, panelId);
                        if (_aidl_return.retval == Result::OK)
                        {
            ...
            else if (actionID == PQ_SW_READ)
            {
                ...
                if ((unsigned int)RegBase < 0xFFFF0000) {
                        ...
                        ScopedAStatus ret = service->getTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, module, field, &_aidl_return);
                        if (_aidl_return.retval == Result::OK)
                        {
...
#else
                        int32_t module, field;
                        getTuning_return _aidl_return;
                        int32_t getvalue = 0;

                        module = (RegBase >> 16) & 0xffff;
                        field = (RegBase & 0xffff);
                        _aidl_return = getTuningField(module, field, panelId);
                        if (_aidl_return.retval == Result::OK)
                        {
                            getvalue = _aidl_return.value;
                        }
                    ...
                    if (params.reg >= SWREG_PQDS_DS_EN && params.reg < SWREG_PQDS_END) {
                        ...
                        parcelable_getTuningField _aidl_return;
                        ScopedAStatus ret = service->getTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, MOD_DYNAMIC_SHARPNESS,
                                RegBase - SWREG_PQDS_DS_EN, &_aidl_return);
                        #endif
                        getTuning_return _aidl_return;
                        _aidl_return = getTuningField(MOD_DYNAMIC_SHARPNESS, RegBase - SWREG_PQDS_DS_EN, panelId);
                        if (_aidl_return.retval == Result::OK)
                        {
                            getvalue = _aidl_return.value;
                        }
                        ...
                        parcelable_getTuningField _aidl_return;
                        ScopedAStatus ret = service->getTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, MOD_TDSHP_REG,
                                RegBase - SWREG_TDSHP_REG_GAIN_MID, &_aidl_return);
                        #endif
                        getTuning_return _aidl_return;
                        _aidl_return = getTuningField(MOD_TDSHP_REG, RegBase - SWREG_TDSHP_REG_GAIN_MID, panelId);
                        if (_aidl_return.retval == Result::OK)
                        {
                            getvalue = _aidl_return.value;
                        }
                        ...
                        #if defined(MTK_PQ_SERVICE)
                        parcelable_getTDSHPFlag _aidl_return;
                        ScopedAStatus ret = service->getTDSHPFlagMPq(panelId,
                                PANEL_ID_FLAG, &_aidl_return);
                        #endif
                        getTdshp_return _aidl_return;
                        _aidl_return = getTDSHPFlag(panelId);

                        if (_aidl_return.retval == Result::OK)
                        {
                            getvalue = _aidl_return.TDSHPFlag;
                        }
                        else
                        {
                        ...
                        parcelable_getTuningField _aidl_return;
                        ScopedAStatus ret = service->getTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, MOD_DYNAMIC_CONTRAST,
                                RegBase - SWREG_BLACK_EFFECT_ENABLE, &_aidl_return);
                        #endif
                        getTuning_return _aidl_return;
                        _aidl_return = getTuningField(MOD_DYNAMIC_CONTRAST,
                                RegBase - SWREG_BLACK_EFFECT_ENABLE, panelId);
            ...
            else if (actionID == PQ_SW_WRITE)
            {
                ...
                if ((unsigned int)RegBase < 0xFFFF0000) {
                        ...
                        #if defined(MTK_PQ_SERVICE)
                        Result result;
                        ScopedAStatus ret = service->setTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, module, field, value, &result);
                        #endif
                        int retValue = setTuningField(module, field, value, panelId);
                    ...
                    if (params.reg >= SWREG_PQDS_DS_EN && params.reg < SWREG_PQDS_END) {
                        ...
                        ScopedAStatus ret = service->setTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, MOD_DYNAMIC_SHARPNESS,
                                RegBase - SWREG_PQDS_DS_EN, value, &result);
                        #endif
                        int retValue = setTuningField(MOD_DYNAMIC_SHARPNESS,
                                    RegBase - SWREG_PQDS_DS_EN, value, panelId);
                        if (retValue < 0) {
                            ALOGE("[PQ_CMD] IPictureQuality::setTuningField failed!");
                        }
                    }
                        ...
                        ScopedAStatus ret = service->setTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, MOD_TDSHP_REG,
                                RegBase - SWREG_TDSHP_REG_GAIN_MID, value, &result);
                        #endif
                        int retValue = setTuningField(MOD_TDSHP_REG,
                                    RegBase - SWREG_TDSHP_REG_GAIN_MID, value, panelId);
                        ...
                        #if defined(MTK_PQ_SERVICE)
                        Result result;
                        ScopedAStatus ret = service->setTDSHPFlagMPq(panelId,
                                PANEL_ID_FLAG, params.val, &result);
                        #endif
                        int retValue =  setTDSHPFlag(params.val, panelId);
                        ...
                        ScopedAStatus ret = service->setTuningFieldMPq(panelId,
                                PANEL_ID_FLAG, MOD_DYNAMIC_CONTRAST,
                                RegBase - SWREG_BLACK_EFFECT_ENABLE, value, &result);
                        #endif
                        int retValue = setTuningField(MOD_DYNAMIC_CONTRAST,
                                    RegBase - SWREG_BLACK_EFFECT_ENABLE, value, panelId);
                    ...
                    else
                    {
                        ...
                        arg.request = IOCTL_WRITE_SW_REG;
                        _aidl_return = execIoctl(arg, panelId);
            ...
            else if (actionID == PANEL_SEL)
            {
#if 1
                at_tok_nexthexint(&cmdline, &panelId);
                if (panelId >= PANEL_MAX) {
                    ALOGE("[PQ_CMD] invalid panelId %d", panelId);
                    panelId = 0;
                }
#else
                ALOGE("[PQ_CMD] Not support tuning");
            ...
            else if (actionID == IDLEMGR_CONTROL)
            {
                ...
                if (s <= 0)
                {
                    ALOGE("could not write /proc/mtkfb");
                    break;
                }
            ...
            break;
        default:
            ALOGE("%s at_op %d", __FUNCTION__, at_op);
            break;
    }
