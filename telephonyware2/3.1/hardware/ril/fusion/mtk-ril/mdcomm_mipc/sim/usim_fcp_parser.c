...
unsigned char fcp_tlv_search_tag(unsigned char *in_ptr, unsigned short len, usim_fcp_tag_enum tag, unsigned char **out_ptr)
{
    ...
    for (*out_ptr = in_ptr; len > 0; *out_ptr += tag_len)
    {
        ...
        if (len <= tag_len) {
            // mtkLogD(LOG_TAG,"fcp_tlv_search_tag, len: %d, tag_len: %d", len, tag_len);
            len = 0;
        } else {
            len -= tag_len;
        }
    ...
    if(len != 0)
    {
        mtkLogE(LOG_TAG, "fcp_tlv_search_tag, error len = %d", len);
    }
    *out_ptr = NULL;
    return 0;
}
            ...
            unsigned char **out_ptr)
{
    ...
    if(len != 0)
    {
        mtkLogE(LOG_TAG, "prop_info_tlv_search_tag, error len = %d", len);
    }
    *out_ptr = NULL;
    return 0;
}
...
unsigned char fcp_file_descriptor_query(unsigned char *in_ptr, unsigned short len, usim_file_descriptor_struct *filled_struct_ptr)
{
    ...
    {
        // mtkAssert("fcp_file_descriptor_query");
        // mtkLogE(LOG_TAG, "fcp_file_descriptor_query, no valid file descriptor! value_len: %d", value_len);
        return FALSE;
    }
    if (out_ptr != NULL) {
...
unsigned char fcp_file_identifier_query(unsigned char *in_ptr, unsigned short len, usim_file_identifier_struct *filled_struct_ptr)
{
    ...
    if (NULL != out_ptr)
    {
        if(value_len != 2)
        {
            // mtkAssert("fcp_file_identifier_query");
            mtkLogD(LOG_TAG, "fcp_file_identifier_query, error value_len = %d", value_len);
            return FALSE;
        }
...
unsigned char fcp_df_name_query(unsigned char *in_ptr, unsigned short len, usim_df_name_struct *filled_struct_ptr)
{
    ...
    if(value_len > 16)
    {
        // mtkAssert("fcp_df_name_query");
        mtkLogD(LOG_TAG, "fcp_df_name_query, error value_len = %d", value_len);
        return FALSE;
    }
            ...
            usim_proprietary_information_struct *filled_struct_ptr)
{
    ...
    if(value_len > 17) {
        //mtkLogW(LOG_TAG,"WARNING_FCP_PROP_INFO_SIZE:%d",value_len);
    }
    ...
    if (value_len)
    {
        ...
        if (prop_len)
        {
            if(prop_len != 1)
            {
                // mtkAssert("fcp_proprietary_info_query, PROP_UICC_CHAR_T");
                mtkLogD(LOG_TAG, "fcp_proprietary_info_query, PROP_UICC_CHAR_T error prop_len = %d", prop_len);
                return FALSE;
            }
        ...
        if (prop_len)
        {
            if(prop_len != 3)
            {
                // mtkAssert("fcp_proprietary_info_query, PROP_APP_PWR_T");
                mtkLogD(LOG_TAG, "fcp_proprietary_info_query, PROP_APP_PWR_T error prop_len = %d", prop_len);
                return FALSE;
            }
        ...
        if (prop_len)
        {
            if(prop_len != 1)
            {
                // mtkAssert("fcp_proprietary_info_query, PROP_MIN_APP_CLK_T");
                mtkLogD(LOG_TAG, "fcp_proprietary_info_query, PROP_MIN_APP_CLK_T error prop_len = %d", prop_len);
                return FALSE;
            }
        ...
        if (prop_len)
        {
            ...
            if(prop_len > 4)
            {
                // mtkAssert("fcp_proprietary_info_query, PROP_AVAIL_MEM_T");
                mtkLogD(LOG_TAG, "fcp_proprietary_info_query, PROP_AVAIL_MEM_T error prop_len = %d", prop_len);
                return FALSE;
            }
            ...
            usim_life_cycle_status_integer_struct *filled_struct_ptr)
{
    ...
    {
        // mtkAssert("fcp_life_cycle_query");
        mtkLogD(LOG_TAG, "fcp_life_cycle_query, error value_len = %d", value_len);
        return FALSE;
    }
        ...
        while (value_len > 0)
        {
            ...
            switch (*tag)
            {
                ...
                case PIN_USAGE_QUALIFIER_T:
                    // mtkLogD(LOG_TAG, "PIN_DO: usage qualifier found!");
                    break;
                case PIN_KEY_REF_T:
                    if((count / 8) >= length)
                    {
                        // mtkAssert("fcp_pin_status_do_query, PIN_KEY_REF_T");
                        mtkLogD(LOG_TAG,
                                "fcp_pin_status_do_query, PIN_KEY_REF_T error count:%d, length:%d",
                                count, length);
                        break;
                    }
                    ...
                    break;
                default:
                    // mtkAssert("fcp_pin_status_do_query, default");
                    mtkLogD(LOG_TAG, "fcp_pin_status_do_query, default error tag:%d", *tag);
                    break;
            }
            tag += (length + 2);
            value_len -= (length + 2);
...
unsigned char fcp_total_file_size_query(unsigned char *in_ptr, unsigned short len, usim_total_file_size_struct *filled_struct_ptr)
{
    ...
    if (NULL != out_ptr)
    {
        ...
        if(value_len > 4)
        {
            // mtkAssert("fcp_total_file_size_query");
            mtkLogD(LOG_TAG, "fcp_total_file_size_query, error value_len = %d", value_len);
            return FALSE;
        }
...
unsigned char fcp_file_size_query(unsigned char *in_ptr, unsigned short len, usim_file_size_struct *filled_struct_ptr)
{
    ...
    if (NULL != out_ptr)
    {
        if(value_len != 2)
        {
            // mtkAssert("fcp_file_size_query");
            mtkLogD(LOG_TAG, "fcp_file_size_query, error value_len:%d", value_len);
            return FALSE;
        }
...
unsigned char fcp_sfi_query(unsigned char *in_ptr, unsigned short len, usim_short_file_identifier_struct *filled_struct_ptr)
{
    ...
    if (NULL != out_ptr)
    {
        ...
        else
        {
            if(value_len != 1)
            {
                // mtkAssert("fcp_sfi_query");
                mtkLogD(LOG_TAG, "fcp_sfi_query, error value_len = %d", value_len);
                return FALSE;
            }
...
unsigned char usim_fcp_query_tag(unsigned char *in_ptr, unsigned short len, usim_fcp_tag_enum tag, void *filled_struct_ptr)
{
    ...
    {
        // mtkAssert("usim_fcp_query_tag, 1");
        mtkLogD(LOG_TAG, "usim_fcp_query_tag, error len or in_ptr");
        return FALSE;
    }
    ...
    switch (tag)
    {
        ...
        case FCP_SFI_T:
            ...
            break;
        default:
            // mtkAssert("usim_fcp_query_tag, default");
            mtkLogD(LOG_TAG, "usim_fcp_query_tag, default error tag:%d", tag);
            return FALSE;
            break;
    }
}
...
static int toByte(char c)
{
    ...
    // mtkLogD(LOG_TAG, "toByte Error: %c",c);
    return 0;
}
...
int hexStringToByteArray(unsigned char* hexString, unsigned char ** byte)
{
    ...
    if (buffer == NULL) {
        mtkAssert("hexStringToByteArray");
    }
    int i = 0;
...
unsigned char* byteArrayToHexString(unsigned char* array,int length)
{
    ...
    if (buf == NULL) {
        mtkAssert("byteArrayToHexString");
    }
    int bufIndex = 0;
    int i = 0;
    for (i = 0 ; i < length; i++)
