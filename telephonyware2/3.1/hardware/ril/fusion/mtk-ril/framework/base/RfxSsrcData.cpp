...
RfxSsrcData::RfxSsrcData(void *data, int length) : RfxBaseData(data, length)  {
    ...
    int number = length / sizeof(RIL_SignalStrength_Reporting_Criteria);
    RIL_SignalStrength_Reporting_Criteria *pData =
        (RIL_SignalStrength_Reporting_Criteria *) calloc(number, sizeof(RIL_SignalStrength_Reporting_Criteria));

    if (pData != NULL) {
        for (int i = 0; i < number; i++) {
            memcpy(pData + i, pTmp + i, sizeof(RIL_SignalStrength_Reporting_Criteria));
        }
        m_data = pData;
        m_length = number * sizeof(RIL_SignalStrength_Reporting_Criteria);
    }
}
