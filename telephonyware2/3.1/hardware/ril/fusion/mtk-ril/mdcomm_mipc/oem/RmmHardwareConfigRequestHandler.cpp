...
#include "RmmHardwareConfigRequestHandler.h"
#include "RfxRilUtils.h"
#include "RfxVoidData.h"
#include <string.h>

#define RFX_LOG_TAG "RmmHdcHdr"
...
RFX_IMPLEMENT_HANDLER_CLASS(RmmHardwareConfigRequestHandler, RIL_CMD_PROXY_1);

RmmHardwareConfigRequestHandler::RmmHardwareConfigRequestHandler(int slot_id, int channel_id) :
...
RmmHardwareConfigRequestHandler::~RmmHardwareConfigRequestHandler() {
}

void RmmHardwareConfigRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
    int id = msg->getId();
...
void RmmHardwareConfigRequestHandler::onHandleEvent(const sp<RfxMclMessage>& msg) {
    int id = msg->getId();
...
void RmmHardwareConfigRequestHandler::handleHardwareConfigRequest(const sp<RfxMclMessage>& msg) {
...
void RmmHardwareConfigRequestHandler::sendHardwareConfigUrc() {
    RIL_HardwareConfig hardwareConfigs[2];
...
void RmmHardwareConfigRequestHandler::getHardwareConfig(RIL_HardwareConfig *hardwareConfigs) {
    ...
    strncpy(hardwareConfigs[0].uuid, logicalModemId, MAX_UUID_LENGTH - 1);
    ...
    strncpy(hardwareConfigs[1].uuid, logicalModemId, MAX_UUID_LENGTH - 1);
    hardwareConfigs[1].state = RIL_HARDWARE_CONFIG_STATE_ENABLED;
    strncpy(hardwareConfigs[1].cfg.sim.modemUuid, logicalModemId, MAX_UUID_LENGTH - 1);
}
