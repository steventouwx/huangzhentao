...
typedef bool (*EVENTCALLBACK)(int slotId, int channelId, int eventId, void *data, size_t len,
        int clientId, int token, nsecs_t delayTime, MTK_RIL_REQUEST_PRIORITY priority);
typedef void (*PROPCALLBACK)(const char *key, const char *value);
...
typedef enum {
    ...
    RFX_PS_RAT_FAMILY_IWLAN = 4,
} RfxPsRatFamily;

class RfxRilUtils {

public:
    static unsigned int rfxGetSimCount();
    ...
    static void setPropCallbackForGT(PROPCALLBACK cb);
    static void updatePropToGT(const char *key, const char *value);
    static void setEventCallbackForGT(EVENTCALLBACK cb);
    static bool updateEventToGT(int slotId, int channelId, int eventId, void *data, size_t len,
        int clientId, int token, nsecs_t delayTime, MTK_RIL_REQUEST_PRIORITY priority);
    static void sendEventForGT(int slotId, int channelId, int eventId, void *data, size_t len,
        int clientId, int token, nsecs_t delayTime, MTK_RIL_REQUEST_PRIORITY priority);
    /// M: add for op09 volte setting @{
    static bool isOp09();
    static bool isCtVolteSupport();
    static bool isCtMixVolteSupport();
    static bool isCtHvolteSupport();
    /// @}
    ...
    static bool isMipcSupport();
    static const char * pii(const char * tag, const char * hiddenData);
    static RfxPsRatFamily getPsRatFamily(int radioTechnology);
    static int executeCmdForRild(char * cmd, bool print);
