...
static MUTEX *txid_mutex_ptr;

void mipc_create_txid_mutex()
{
    txid_mutex_ptr = CREATE_MUTEX();
}
...
mipc_msg_t *mipc_msg_init(uint16_t msg_id, mipc_msg_sim_ps_id_enum sim_ps_id)
{
    ...
    if ((msg_id & MIPC_MSG_TYPE_MASK) == MIPC_MSG_TYPE_IND || (msg_id & MIPC_MSG_TYPE_MASK) == MIPC_MSG_TYPE_NTF) {
        ...
        LOCK_MUTEX(txid_mutex_ptr);
        msg_ptr->hdr.msg_txid = HTOMS(_mipc_msg_txid());
        UNLOCK_MUTEX(txid_mutex_ptr);
    }
...
uint8_t *mipc_msg_serialize(mipc_msg_t *msg_ptr, uint16_t *msg_len_ptr)
{
    uint8_t *head_ptr;
    uint8_t *tail_ptr;
    uint16_t msg_len = sizeof(mipc_msg_hdr_t) + msg_ptr->tlvs_len;
    // msg_len is too long
    if (msg_len < msg_ptr->tlvs_len) {
        ErrMsg("mipc_msg_serialize msg len error(%u)(%zu)(%u)",
                msg_ptr->tlvs_len, sizeof(mipc_msg_hdr_t), msg_len);
        return NULL;
    }
