...
RmcSimSlotLockDeviceLockInfo* RmcSimBaseHandler::getSimSlotLockDeviceLockInfo() {
    ...
    RFX_ASSERT(retry != NULL);
    memset(retry, 0, sizeof(RmcSimSlotLockDeviceLockInfo));
    retry->lock_state = -1;
    retry->algo = -1;
    retry->maximum = -1;
    retry->remain = -1;
...
void RmcSimBaseHandler::makeSimRspFromUsimFcp(unsigned char ** simResponse, int fileId) {
    int format_wrong = 0;
...
void RmcSimBaseHandler::sendIccidChanged(const char *iccid) {
