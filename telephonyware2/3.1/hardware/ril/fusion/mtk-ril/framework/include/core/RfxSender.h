...
class RfxSender : public Thread {
    ...
    public:
        ...
        int findNextEOL(char *cur);
        mipc_msg_sim_ps_id_enum slotIdChangeToMipcSlotId(int slotId);
        int mipcSlotIdChangeToSlotId(unsigned int slotId);
        void handleFinalResponse(RfxAtLine* line);
        void handleRequestAck();
        void processLine(const char *line);
        RfxAtLine* setRequestResponseLine(char* line, int type, int isNumericSet);
        int getFd() const {
            return m_fd;
        }
        void setFd(int fd) {
    ...
    private:
        ...
        void printHiddenLog(const char *command);
