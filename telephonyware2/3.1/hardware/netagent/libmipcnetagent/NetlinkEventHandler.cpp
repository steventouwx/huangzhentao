...
void NetlinkEventHandler::onEvent(NetlinkEvent *evt) {
    ...
    if (strcmp(subsys, "net") == 0) {
        ...
        const char *iName = m_pNetAgent->getInterfaceName();
        ...
        if (interfaceName != NULL) {
            if (strncmp(iName, interfaceName, strlen(iName)) == 0) {
                interfaceName += strlen(iName);
        ...
        if (action == NetlinkEvent::Action::kAddressUpdated ||
            ...
            char *tmpFlags = const_cast<char *>(evt->findParam("FLAGS"));
            char *tmpScope = const_cast<char *>(evt->findParam("SCOPE"));
            char *tmpIfIndex = const_cast<char *>(evt->findParam("IFINDEX"));
            char *tmpPrefered = const_cast<char *>(evt->findParam("PREFERRED"));
            char *tmpValid = const_cast<char *>(evt->findParam("VALID"));
            NA_LOG_D("[%s] %s: addr=%s, flags=%s, scope=%s, ifIndex=%s, prefered=%s, valid=%s",
                    __FUNCTION__, actionToString(act),
                    m_pNetAgent->isUserBuild() ? "xxx" : tmpAddress, tmpFlags, tmpScope,
                    tmpIfIndex, tmpPrefered, tmpValid);
            char *address = NULL;
            ...
            int flags = stringToBinaryBase(tmpFlags, 10, &err);
            if (err < 0) {
                flags = 0;
                NA_LOG_E("[%s] error occurs when converting flags to binary"
                        ", err = %d", __FUNCTION__, err);
            }

            long realTime = android::elapsedRealtime();
            NA_LOG_D("[%s] realTime = %ld", __FUNCTION__, realTime);
            long deprecationTime = stringToBinaryBaseLong(tmpPrefered, 10, &err);
            if (err < 0) {
                deprecationTime = -1;
                NA_LOG_E("[%s] error occurs when converting deprecationTime to binary"
                        ", err = %d", __FUNCTION__, err);
            } else {
                if (deprecationTime == UINT_MAX) {
                    deprecationTime = LONG_MAX;
                } else if (deprecationTime >= 0) {
                    deprecationTime *= 1000;
                    deprecationTime += realTime;
                } else {
                    deprecationTime = -1;
                }
            }
            long expirationTime = stringToBinaryBaseLong(tmpValid, 10, &err);
            if (err < 0) {
                expirationTime = -1;
                NA_LOG_E("[%s] error occurs when converting expirationTime to binary"
                        ", err = %d", __FUNCTION__, err);
            } else {
                if (expirationTime == UINT_MAX) {
                    expirationTime = LONG_MAX;
                } else if (expirationTime >= 0) {
                    expirationTime *= 1000;
                    expirationTime += realTime;
                } else {
                    expirationTime = -1;
                }
            }

            // Fix issue that set IPV6 address to kernel before interface up completely.
            // For this scenario, kernel reported flags=192(IFA_F_TENTATIVE | IFA_F_PERMANENT)
            // and ConnectivityService takes it as not global IPV6 address,
            // so it can't be used for clatd feature.
            // Here covernt it to 0 except for multi-homing feature which need this flags.
...
void NetlinkEventHandler::enqueueReqInfo(NA_CMD cmd, char *address, int ipv6PrefixLength,
        int interfaceId, ACTION action, int flags, long deprecationTime, long expirationTime) {
    if (m_pNetAgent == NULL) {
        NA_LOG_E("[%s] m_pNetAgent is NULL", __FUNCTION__);
        return;
    }
    ...
    pNetlinkEventObj->flags = flags;
    pNetlinkEventObj->deprecationTime = deprecationTime;
    pNetlinkEventObj->expirationTime = expirationTime;
...
int NetlinkEventHandler::getFlags(void *obj, int *flags) {
    NetlinkEventReqInfo *pReqInfo = reinterpret_cast<NetlinkEventReqInfo *>(obj);
    ...
    *flags = pReqInfo->flags;
    return NETLINK_RET_SUCCESS;
}

int NetlinkEventHandler::getDeprecationTime(void *obj, long *deprecationTime) {
    NetlinkEventReqInfo *pReqInfo = reinterpret_cast<NetlinkEventReqInfo *>(obj);
    ...
    *deprecationTime = pReqInfo->deprecationTime;
    return NETLINK_RET_SUCCESS;
}

int NetlinkEventHandler::getExpirationTime(void *obj, long *expirationTime) {
    NetlinkEventReqInfo *pReqInfo = reinterpret_cast<NetlinkEventReqInfo *>(obj);
    ...
    *expirationTime = pReqInfo->expirationTime;
    return NETLINK_RET_SUCCESS;
}
...
int NetlinkEventHandler::stringToBinaryBase(char *str, int base, int *err) {
    ...
    *err = 0;
    ...
    if (ul == ULONG_MAX) {
        NA_LOG_E("[%s] error on strtoul", __FUNCTION__);
        *err = -2;
        return 0;
    }
    out = (int)ul;
...
long NetlinkEventHandler::stringToBinaryBaseLong(char *str, int base, int *err) {
    long out;
    unsigned long ul;
    *err = 0;
