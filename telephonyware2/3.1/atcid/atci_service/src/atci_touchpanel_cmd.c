...
int touchpanel_fwver_handler(char * cmdline, ATOP_t at_op, char *response){
    ...
    value[ret] = '\0';
    if (sprintf(com_date, "%s", value) < 0) {
        ALOGE("com date failed");
        return -1;
    }
    ...
    switch(at_op){
            ...
            case AT_READ_OP:
                if (sprintf(log_info, "\r\n%s\r\n", value) < 0) {
                    ALOGE("AT_ACTION_OP or AT_READ_OP failed");
                    return -1;
                }
                break;
            case AT_TEST_OP:
        ...
        case AT_SET_OP:
            if (!strcmp(cmdline,com_date)) {
                if (sprintf(log_info,"\r\n\r\n OK\r\n\r\n") < 0) {
                    ALOGE("AT_SET_OP OK failed");
                    return -1;
                }
            } else {
                if (sprintf(log_info,"\r\n\r\n ERROR\r\n\r\n") < 0) {
                    ALOGE("AT_SET_OP ERROR failed");
                    return -1;
                }
            }
            break;
    ...
    if (snprintf(response, strlen(log_info) + 5, "\r\n%s \n\r\n", log_info) < 0) {
        ALOGE("resp error");
        return -1;
    }
    close(fd);
