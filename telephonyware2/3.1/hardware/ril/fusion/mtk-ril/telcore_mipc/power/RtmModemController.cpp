...
/*****************************************************************************
 * Class RtmModemController
 *****************************************************************************/

RFX_IMPLEMENT_CLASS("RtmModemController", RtmModemController, RfxController);

void RtmModemController::onInit() {
...
void RtmModemController::onDeinit() {
    RfxController::onDeinit();
}

RtmModemController::RtmModemController() {
}
...
bool RtmModemController::onPreviewMessage(const sp<RfxMessage>& message) {
...
bool RtmModemController::onCheckIfResumeMessage(const sp<RfxMessage>& message) {
    if (REQUEST == message->getType()) {
        if (canHandleRequest(message)) {
            return true;
        } else {
...
bool RtmModemController::canHandleRequest(const sp<RfxMessage>& message __unused) {
...
bool RtmModemController::onHandleRequest(const sp<RfxMessage>& message) {
    int msgId = message->getId();
    logD(RFX_LOG_TAG, "RtmModemController: handle request id %d", msgId);
    ...
    case RFX_MSG_REQUEST_SHUTDOWN:
        if (msgId == RFX_MSG_REQUEST_SHUTDOWN &&
            ...
            if (modemOffState == MODEM_OFF_IN_IDLE) {
                ...
                logD(RFX_LOG_TAG, "RtmModemController: set RFX_STATUS_KEY_MODEM_OFF_STATE to %d",
                        MODEM_OFF_BY_POWER_OFF);
            }
...
void RtmModemController::requestModemPower(bool power, const sp<RfxMessage>& message) {
...
bool RtmModemController::onHandleResponse(const sp<RfxMessage>& message) {
    int msgId = message->getId();
    logD(RFX_LOG_TAG, "RtmModemController: handle response id %d", msgId);
...
bool RtmModemController::onCheckIfRejectMessage(const sp<RfxMessage>& message,
...
void RtmModemController::responseModemPower(const sp<RfxMessage>& message) {
