...
#define FS_ROOT_DIR5_MD1            "/vendor/etc/md"
#define FS_ROOT_DIR6_MD1            "/data/vendor_de/md"
...
#define MD_ROOT_DIR7                     "R:"
#define MD_ROOT_DIR8                     "Q:"
...
} __attribute__((__packed__)) FS_DOSDateTime;
...
typedef struct _nvram_fs_para_cmpt_32
{
    unsigned int opid_map;      // Indicate which operation shal be taken: open, get file size, seek, read, close
    unsigned int ret[2];        // return value from AP FSD: first is bitmap, second is error code
    unsigned int flag;          // FS_OPEN [in] param
    ...
    unsigned int offset;        // FS_SEEK [in] param
    unsigned int whence;        // FS_SEEK [in] param
    unsigned int resv_data_ptr; // FS_READ [out] param
    unsigned int length;        // FS_READ [in] param
    unsigned int act_read;      // FS_READ [out] param (not same to MD side defination)
} nvram_fs_para_cmpt_32_t;

typedef struct _nvram_fs_para_cmpt_ex
{
    unsigned int opid_map;    // Indicate which operation shal be taken: open, get file size, seek, read, close
    int ret[2];               // return value from AP FSD: first is bitmap, second is error code
    unsigned int flag;        // FS_OPEN [in] param
    unsigned int file_size;   // FS_GETFILESIZE [out] param (not same to MD side defination)
    unsigned int offset;      // FS_SEEK [in] param
    unsigned int whence;      // FS_SEEK [in] param
    ...
    unsigned int length;      // FS_READ [in] param
    unsigned int act_read;    // FS_READ [out] param (not same to MD side defination)
    int file_handle;          // fs_cmpt_read [in] param. MD to AP
    int file_handle_ptr;      // fs_cmpt_read [out] param. AP to MD
} nvram_fs_para_cmpt_ex_t;

typedef struct _nvram_fs_para_cmpt_ex_32
{
    unsigned int opid_map;      // Indicate which operation shal be taken: open, get file size, seek, read, close
    int ret[2];                 // return value from AP FSD: first is bitmap, second is error code
    unsigned int flag;          // FS_OPEN [in] param
    ...
    int file_handle;            // fs_cmpt_read [in] param. MD to AP
    int file_handle_ptr;        // fs_cmpt_read [out] param. AP to MD
} nvram_fs_para_cmpt_ex_32_t;


// "PacketNum" and "PackInfo" defined in the fsd_main()
// PacketNum                             1 DWORD
...
#define CMPT_READ_DATA_PTR_OFFSET    (1+1+2+1+1+1+1+1)

typedef struct _nvram_fs_para_cmptw_32
{
    ...
    unsigned int resv_data_ptr;  // FS_WRITE[in] parameter : data buffer point
