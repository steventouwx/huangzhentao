...
void RtcRedialController::onEmergencyModeChanged(RfxStatusKeyEnum __unused key,
        RfxVariant __unused oldValue, RfxVariant value) {
    if (!isSendEmcsBeforeTurnOnRadio()) {
        return;
    }
...
bool RtcRedialController::onHandleAtciRequest(const sp<RfxMessage>& message) {
    ...
    if (msgId == RFX_MSG_REQUEST_OEM_HOOK_ATCI_INTERNAL) {
        const char  *data = (char *)message->getData()->getData();
        int targetSlotId = 0;
        ...
        logD(LOG_REDIAL_TAG, "Inject AT command %s (length:%d)", data, (int)strlen(data));
        if (strncmp(data, "ATD", 3) == 0) {
            ret = handleAtciDialRequest(data);
        }
    }
...
bool RtcRedialController::handleAtciDialRequest(const char* data) {
    char *callNumber = NULL;
    const int offset = 4;
    int len = strlen(data);

    // Only handle ATDE.
    if (strncmp(data, "ATDE", offset) != 0) {
        return false;
    }
...
void RtcRedialController::setEccController(RtcEccController *eccController) {
    mEccController = eccController;
}

RtcEccController *RtcRedialController::getEccController() {
    return mEccController;
}
...
sp<RfxMessage> RtcRedialController::getRedialRequest() {
    ...
    if (mIsUserHangUping) {
        ...
        // need to resume RAT and exit emergency session.
        handleEmergencyCallbackMode(EMERGENCY_CALLBACK_MODE_EXIT);
        resumeDeferMsg();
        return NULL;
    }
