...
#ifndef __RMM_OEM_CONTROLLER_H__
#define __RMM_OEM_CONTROLLER_H__
...
class RtmOemController : public RfxController {
    RFX_DECLARE_CLASS(RtmOemController); // Required: declare this class

    public:
        RtmOemController();
        virtual ~RtmOemController();
    ...
    private:
        ...
        void onHandleSetModemConfig(const sp<RfxMessage>& message);
        void onModeSwitchFinished(RfxStatusKeyEnum key, RfxVariant old_value,
                RfxVariant value);
        bool sendOemHookRawToNwModule(const sp<RfxMessage>& message);
};
