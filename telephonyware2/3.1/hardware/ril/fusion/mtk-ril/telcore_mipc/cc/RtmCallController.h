...
class RtmCallController : public RfxController {
    // Required: declare this class
    RFX_DECLARE_CLASS(RtmCallController);

public:
    RtmCallController();
    virtual ~RtmCallController();
    void onMergeStart();
...
protected:
    ...
    void handleEcallFastMakeEcall(const sp<RfxMessage>& message);
...
private:
    ...
    void onAutoAnswerSettingChanged(RfxStatusKeyEnum key, RfxVariant old_value, RfxVariant value);
    void handleGetSimultaneousCallingSupport(const sp<RfxMessage>& message);
    void onDsdaModeChanged(RfxStatusKeyEnum key, RfxVariant oldValue, RfxVariant newValue);
    ...
    void approveIncoming(int callId, int seqNo, bool approve, int cause,
    ...
    // key = seqNumber, value = TxID
    std::map<int, int> mApproveIncomingCallTxIds;
    ...
    RtmEccController *mEccController;
    ...
    bool isDsdaSupport();
    bool isDrdsdaSupport();

    string normalizeNumberFromCLIR(string number);
    string replaceAll(string &str, const string &old_value, const string &new_value);
    bool isDsdaSupport(int mode);
};
