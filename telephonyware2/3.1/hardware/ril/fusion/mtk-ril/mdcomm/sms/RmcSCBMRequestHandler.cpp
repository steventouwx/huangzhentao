...
#include "RmcSCBMRequestHandler.h"
...
// Register handler
RFX_IMPLEMENT_HANDLER_CLASS(RmcSCBMRequestHandler, RIL_CMD_PROXY_1);
 ...
 * Class RfxController
 *****************************************************************************/
RmcSCBMRequestHandler::RmcSCBMRequestHandler(int slot_id, int channel_id) :
        RfxBaseHandler(slot_id, channel_id) {
    setTag(String8("RmcSCBMRequest"));
    registerToHandleRequest(reqList, sizeof(reqList)/sizeof(int));
    logD(mTag, "RmcSCBMRequestHandler: constructor");
}

RmcSCBMRequestHandler::~RmcSCBMRequestHandler() {
}

void RmcSCBMRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
    int id = msg->getId();
...
void RmcSCBMRequestHandler::requestExitSCBM(const sp<RfxMclMessage>& msg) {
