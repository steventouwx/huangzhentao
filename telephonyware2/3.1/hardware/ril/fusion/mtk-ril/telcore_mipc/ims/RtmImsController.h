...
class RtmImsController : public RfxController {
    // Required: declare this class
    RFX_DECLARE_CLASS(RtmImsController);

public:
    RtmImsController();
    virtual ~RtmImsController();
...
private:
    ...
    void setNonOp09VolteState(int slot_id, const char new_iccid[]);
    void checkNonOp09VolteState(int slot_id, const char new_iccid[]);
    void sendImsSettingstoMcl(int value);
    void setDefaultVolteState(int slot_id, const char new_iccid[], int card_type);
    void setVolteStateProperty(int slot_id, bool isEnable);
    bool isOp09SimCard(int slot_id, const char icc_id[], int card_type);
    ...
    std::string getUserNameFromSipTelUriString(std::string uriString);
    std::vector<std::string> splitString(std::string str, std::string c);
...
private:
    ...
    static char sLastBootIccId[4][RFX_PROPERTY_VALUE_MAX];
    ...
    int mLatestImsCfgToken;
    bool mIsOtaWait = false; // ALPS08650506 ota case
};
