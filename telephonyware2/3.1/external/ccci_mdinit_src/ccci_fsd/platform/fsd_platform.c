...
char debug_level = ANDROID_LOG_INFO;
char debug_wakeup = 0;
...
int get_modem_status(void)
{
    ...
    int retpropget = 0, retlen = 0, ret = CCCI_MD_STA_UNDEFINED;

    retlen = snprintf(mdstatus, sizeof(mdstatus), "vendor.mtk.md%d.status", md_id + 1);
    if (retlen <= 0 || retlen >= sizeof(mdstatus)) {
        LOGI("snprintf mdstatus err len: %d\n", retlen);
        return ret;
    }
