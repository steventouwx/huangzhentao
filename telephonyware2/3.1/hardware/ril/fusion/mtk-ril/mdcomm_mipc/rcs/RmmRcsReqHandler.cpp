...
#define RFX_LOG_TAG "RmmRcsReq"

using namespace vendor::rmm::rcs;

/*****************************************************************************
 * Class RmmRcsReqHandler
 *****************************************************************************/
RFX_IMPLEMENT_HANDLER_CLASS(RmmRcsReqHandler, RIL_CMD_PROXY_IMS);
...
bool RmmRcsReqHandler::m_debug = true;
bool RmmRcsReqHandler::m_wait_cnf = true;

RmmRcsReqHandler::RmmRcsReqHandler(int slot_id, int channel_id)
    : RfxBaseHandler(slot_id, channel_id) {

    logI(RFX_LOG_TAG, "RmmRcsReqHandler create (slot_id = %d)", slot_id);

    if (!RmmRcsReqHandler::isRcsUaSupport()) {
        return;
    }
...
RmmRcsReqHandler::~RmmRcsReqHandler() {
    if (m_state_channel)
...
void RmmRcsReqHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
...
void RmmRcsReqHandler::onHandleEvent(const sp<RfxMclMessage> & msg) {
...
void RmmRcsReqHandler::handleDataSending(const sp<RfxMclMessage>& msg) {
...
void RmmRcsReqHandler::handleDataReceiving(const sp<RfxMclMessage>& msg) {
    uint8_t *data = (uint8_t *)msg->getData()->getData();
    int length = msg->getData()->getDataLength();
...
bool RmmRcsReqHandler::isRcsUaSupport() {
...
void RmmRcsReqHandler::startRcsService(const sp<RfxMclMessage>& msg) {
...
void RmmRcsReqHandler::stopRcsService(const sp<RfxMclMessage>& msg) {
    RIL_Errno error = RIL_E_SUCCESS;
...
void RmmRcsReqHandler::requestRegistrationInfo(const sp<RfxMclMessage>& msg) {
...
void RmmRcsReqHandler::enableDeregInd(const sp<RfxMclMessage>& msg) {
    RIL_Errno error = RIL_E_GENERIC_FAILURE;
...
void RmmRcsReqHandler::resumeDeregistration(const sp<RfxMclMessage>& msg) {
    RIL_Errno error = RIL_E_GENERIC_FAILURE;
...
void RmmRcsReqHandler::triggerRestoration(const sp<RfxMclMessage>& msg) {
    RIL_Errno error = RIL_E_GENERIC_FAILURE;
...
void RmmRcsReqHandler::sendSipMessage(const sp<RfxMclMessage>& msg) {
    RIL_Errno error = RIL_E_GENERIC_FAILURE;
...
void RmmRcsReqHandler::switchRcsRoiStatus(const sp<RfxMclMessage>& msg) {
    ...
    switch (status) {
        ...
        case 1:
        {
            sp<RfxMipcData>  reqData =
                RfxMipcData::obtainMipcData(MIPC_CALL_SET_RCS_STATE_AND_FEATURE_REQ, m_slot_id);
            reqData->addMipcTlvUint32(MIPC_CALL_SET_RCS_STATE_AND_FEATURE_REQ_T_STATE,
                                     (uint32_t)status);
...
void RmmRcsReqHandler::updateRcsCapabilities(const sp<RfxMclMessage>& msg) {
    ...
    if (pData) {
        ...
        reqData->addMipcTlvUint32(MIPC_CALL_SET_RCS_STATE_AND_FEATURE_REQ_T_STATE,
                                  (uint32_t)atoi(mode));
        reqData->addMipcTlvUint32(MIPC_CALL_SET_RCS_STATE_AND_FEATURE_REQ_T_FEATURE,
                                  (uint32_t)atoi(featureTags));
        sp<RfxMipcData> cnfData = callToMipcMsgSync(reqData);
        if (cnfData->getResult() != MIPC_RESULT_SUCCESS) {
            logE(RFX_LOG_TAG, "[%s] error!", __FUNCTION__);
            ret = RIL_E_GENERIC_FAILURE;
            ...
            // only update mode and features in activate/deactivate operation
...
void RmmRcsReqHandler::updateRcsSessionInfo(const sp<RfxMclMessage>& msg) {
    int *pData = (int *)msg->getData()->getData();
    ...
    switch (count) {
        ...
        {
            sp<RfxMipcData>  reqData =
                RfxMipcData::obtainMipcData(MIPC_CALL_UPDATE_RCS_SESSION_INFO_REQ, m_slot_id);
            reqData->addMipcTlvUint8(MIPC_CALL_UPDATE_RCS_SESSION_INFO_REQ_T_STATUS, (uint8_t)count);
            sp<RfxMipcData> cnfData = callToMipcMsgSync(reqData);
            if (cnfData->getResult() != MIPC_RESULT_SUCCESS) {
                logE(RFX_LOG_TAG, "[%s] error!", __FUNCTION__);
                ret = RIL_E_GENERIC_FAILURE;
...
void RmmRcsReqHandler::getUserAgent(const sp<RfxMclMessage>& msg) {
    logD(RFX_LOG_TAG, "[%s]", __FUNCTION__);
    sp<RfxMipcData>  reqData = RfxMipcData::obtainMipcData(MIPC_IMS_GET_CONFIG_REQ, m_slot_id);
    reqData->addMipcTlvUint32(MIPC_IMS_GET_CONFIG_REQ_T_CLASS, MIPC_IMS_CONFIG_CLASS_IMS_CONFIG);
    reqData->addMipcTlvStr(MIPC_IMS_GET_CONFIG_REQ_T_TYPE, (char*)"user_agent");
    sp<RfxMipcData> cnfData = callToMipcMsgSync(reqData);
    ...
    if (cnfData->getResult() != MIPC_RESULT_SUCCESS) {
        ...
        uint16_t len = 0;
        char* value = (char*) cnfData->getMipcVal(MIPC_IMS_GET_CONFIG_CNF_T_DATA , &len);
        logV(RFX_LOG_TAG, "[%s] value:%s", __FUNCTION__, value);
        response = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS,
                                                 RfxStringData(value, len), msg, false);
    }
    responseToTelCore(response);
}

void RmmRcsReqHandler::handleOutgoingSipResponse(const RcsStackChannel::ChannelData *chnlData) {
...
void RmmRcsReqHandler::handleRegistrationInfo(const RcsStackChannel::ChannelData *chnlData) {
...
void RmmRcsReqHandler::handleIncomingSipMessage(const RcsStackChannel::ChannelData *chnlData) {
...
void RmmRcsReqHandler::handleImsDeregInd(const RcsStackChannel::ChannelData *chnlData) {
...
void RmmRcsReqHandler::checkAndInitImsFd() {
    if (m_slot_id) {
...
char *RmmRcsReqHandler::getDebugSipData(const uint8_t *rawData, int dataLen, int maxSize) {
