...
#define MAX_RESPONSE_BYTES 0xffff
...
void RilAtciClient::processCommands(void *buffer, size_t buflen, int clientId) {
    ...
    if (buffer == NULL || buflen <= 0 || buflen > MAX_COMMAND_BYTES) {
        RFX_LOG_E(RFX_LOG_TAG, "Ignore invalid string");
        clientState = CLIENT_DEACTIVE;
        return;
    }
...
#else
        rfx_enqueue_request_message_client(request, atBuffer, buflen, pRI);
#endif
        free(atBuffer);
    } else {
...
void RilAtciClient::requestComplete(RIL_Token token, RIL_Errno e, const void *response,
        size_t responselen) {
    RFX_UNUSED(e);
    ...
    if (info != NULL && RFX_MSG_REQUEST_OEM_HOOK_ATCI_INTERNAL == info->request) {
        ...
        if (commandFd >= 0) {
            ...
            if (len <= MAX_RESPONSE_BYTES) {
                ...
                RFX_LOG_E(RFX_LOG_TAG, "invalid len");
            }
        } else {
