...
extern "C" void mipcUrcIndCallback(mipc_msg_t *msg_ptr, void* priv_ptr);
extern "C" void mipcCmdCallback(mipc_msg_t *msg_ptr, void* priv_ptr);
extern "C" void mipcResponseCallback(mipc_msg_t *msg_ptr, void* priv_ptr);


typedef struct MipcAyncCallbackData {
    int msg_type;
    int slot_id;
    int token;
    int mipcToken;
} MipcAyncCallbackData;
...
class RfxBaseHandler {
    ...
    public:
        ...
        void registerToHandleMipcURC(const int *urc_id_list, size_t length);

        void registerToMipcInd(const int *urc_id_list, size_t length);

        void registerToHandleMipcCmd(const int *cmd_id_list, size_t length);

        void unregisterToHandleMipcURC(const int *urc_id_list, size_t length);

        void unregisterToMipcInd(const int *urc_id_list, size_t length);
        ...
        int callToMipcMsgAsync(const sp<RfxMclMessage>& msg, sp<RfxMipcData> mipcData);

        sp<RfxMipcData> callToMipcMsgSync(sp<RfxMipcData> mipcMsg, uint32_t timeout_ms = 300000);  // 5 * 60 * 1000

        int callToMipcMsgCmdResponse(sp<RfxMipcData> mipcMsg);

        int callToMipcMsgNtf(sp<RfxMipcData> mipcMsg);
        ...
        static int checkMipcAsyncMessageQueueTimeOut(int token);
    ...
    private:
        sp<RfxSender> getSender();
        friend void::mipcUrcIndCallback(mipc_msg_t *msg_ptr, void* priv_ptr);
        friend void::mipcCmdCallback(mipc_msg_t *msg_ptr, void* priv_ptr);
        friend void::mipcResponseCallback(mipc_msg_t *msg_ptr, void* priv_ptr);
        static int getMipcAyncTimeOut(int msg_id);
