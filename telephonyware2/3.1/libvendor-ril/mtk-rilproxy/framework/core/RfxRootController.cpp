...
#define RFX_LOG_TAG "RfxRootController"
...
bool RfxRootController::processMessage(const sp<RfxMessage>& message) {
    ...
    if (message->getType() == REQUEST) {
        ...
        if(message->getPId() == RIL_REQUEST_RADIO_POWER) {
            handler = NULL;
        }
    ...
    if (handler == NULL) {
        //logD(RFX_LOG_TAG, "processMessage() handler = NULL");
        if (REQUEST == message->getType()) {
            logI(RFX_LOG_TAG, "processMessage() msg = [%s], handler = NULL ==> request",
                    message->toString().string());
            requestToRild(message);
            ret = true;
        } else {
            logI(RFX_LOG_TAG, "processMessage() msg = [%s], handler = NULL ==> response/urc",
                    message->toString().string());
            ret = responseToRilj(message);
        }
    } else {
        logI(RFX_LOG_TAG, "processMessage() msg = [%s], handler = %p, %s",
                message->toString().string(), handler, handler->getClassInfo()->getClassName());
        ret = handler->processMessage(message);
    }

    //logD(RFX_LOG_TAG, "processMessage() end");
    return ret;
}
...
bool RfxRootController::processAtciRequest(const sp<RfxMessage>& message) {
    ...
    logD(RFX_LOG_TAG, "processAtciRequest() begin");
    logD(RFX_LOG_TAG, "msg = [%s]", message->toString().string());
    ...
    if (message->getType() == RESPONSE) {
        ...
        if (handler != NULL) {
            logD(RFX_LOG_TAG, "processAtciRequest() find response handler = %p, %s",
    ...
    if (handled == false) {
        for (int i = HIGHEST; i >= NORMAL; i--) {
            ...
            if (handler != NULL) {
                logD(RFX_LOG_TAG, "processAtciRequest() handler = %p, %s priority = %d",
                ...
                logD(RFX_LOG_TAG, "processAtciRequest() handler = NULL priority = %d", i);
            }
        }
    }

    logD(RFX_LOG_TAG, "processAtciRequest() end handled = %s", handled ? "true" : "false");
    return handled;
}
...
void RfxRootController::suspendMessage(RfxController *controller, const sp<RfxMessage>& message) {
    ...
    logD(RFX_LOG_TAG, "suspendMessage() controller = %p, %s",
