...
public:
    ...
    void setEccController(RtcEccController *eccController);
    RtcEccController *getEccController();

    void setEmergencyModeExt(bool emergencyMode);
    void setEccCallId(int callId); // for 380 case.
    void onEmergencyModeChanged(RfxStatusKeyEnum key,
            RfxVariant oldValue, RfxVariant value);

private:
    void handleEmergencyDial(const sp<RfxMessage>& message);
    bool handleAtciDialRequest(const char* data);
