...
static void set_current_md_status(int status)
{
    ...
    len = snprintf(name, sizeof(name), "vendor.mtk.md%d.status", md_id + 1);
    ...
    if(md_id == MD_SYS3)
    {
        len = snprintf(name, sizeof(name), "vendor.net.cdma.mdmstat");
        if (len < 0 || len >= sizeof(name)) {
            CCCI_LOGD("%s-%d:snprintf fail,len = %d\n", __func__, __LINE__, len);
            return;
        }
...
static void check_to_restart_md(void)
{
    ...
    if(md_id == MD_SYS1) {
        val = snprintf(this_md_status_key, 32, "%s", "vendor.mtk.md1.status");
        if (val < 0 || val >= 32) {
            CCCI_LOGD("%s-%d:snprintf fail,val = %d\n",
                      __func__, __LINE__, val);
            return;
        }
        val = snprintf(the_other_md_status_key, 32, "%s", "vendor.mtk.md3.status");
        if (val < 0 || val >= 32) {
            CCCI_LOGD("%s-%d:snprintf fail,val = %d\n",
                      __func__, __LINE__, val);
            return;
        ...
        val = snprintf(this_md_status_key, 32, "%s", "vendor.mtk.md3.status");
        if (val < 0 || val >= 32) {
            CCCI_LOGD("%s-%d:snprintf fail,val = %d\n",
                      __func__, __LINE__, val);
            return;
        }
        val = snprintf(the_other_md_status_key, 32, "%s", "vendor.mtk.md1.status");
        if (val < 0 || val >= 32) {
            CCCI_LOGD("%s-%d:snprintf fail,val = %d\n",
                      __func__, __LINE__, val);
            return;
...
static int store_nvram_ef_data(int fid, int recsize, void *pdata)
{
    ...
    if (recsize <= 0) {
        CCCI_LOGE("store_nvram_ef_data recsize is illegal:%d\n", recsize);
        return -EINVAL;
    }
...
static void wait_service_ready_to_reset_state_handler(int message)
{
    ...
    case CCCI_MD_MSG_READY_TO_RESET:
        ...
        if(0 == ret) {
            val = snprintf(md_id_str, 16, "%d", curr_md_type);
            if (val < 0 || val >= 16) {
                CCCI_LOGD("%s:snprintf fail\n", __func__);
                return;
            }
            ...
            int ret = property_set("vendor.ril.active.md", md_id_str);
...
static void flight_state_handler(int message)
{
    ...
    case CCCI_MD_MSG_LEAVE_FLIGHT_MODE:
        ...
        CCCI_LOGD("MD%d leave flight mode\n", md_id+1);
//#endif
        set_current_md_status(CCCI_MD_STA_WAIT_SRV_RDY_TO_RST);
...
static int trigger_modem_to_run(char *file_name)
{
    ...
    if (fd < 0) {
        CCCI_LOGD("fail to open %s: %d\n", file_name, errno);
        perror("");
        return -1;
    }

    if(md_id == 0) {
        ret = snprintf(data, 4, "%s", MD1_INIT_CMD);
        if (ret < 0 || ret >= 4) {
        ...
        ret = snprintf(data, 4, "%s", MD2_INIT_CMD);
        if (ret < 0 || ret >= 4) {
            CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
            goto fail_exit;
        }
        ...
        ret = snprintf(data, 4, "%s", MD3_INIT_CMD);
        if (ret < 0 || ret >= 4) {
            CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
            goto fail_exit;
        }
        ...
        CCCI_LOGD("Fail to trigger md run, invalid md id%d\n", md_id);
        perror("");
        goto out;
    }
    if (s <= 0) {
        CCCI_LOGD("fail to write %s to %s: %d\n", data, file_name, errno);
        perror("");
    }
...
static void update_service_name(void)
{
    ...
    ret = snprintf(muxd_name, 32, MUXD_FOR_MD1_NAME);
    if (ret < 0 || ret >= 32) {
        CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return;
    }
    ret = snprintf(muxd_service_status, PROPERTY_KEY_MAX,
                   "%s%s", pre_service_status, muxd_name);
    if (ret < 0 || ret >= PROPERTY_KEY_MAX) {
        CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return;
    }
    ret = snprintf(rild_name, 32, RILD_FOR_MD1_NAME);
    if (ret < 0 || ret >= 32) {
        CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return;
    }
    ret = snprintf(rild_service_status, PROPERTY_KEY_MAX,
                   "%s%s", pre_service_status, rild_name);
    if (ret < 0 || ret >= PROPERTY_KEY_MAX) {
        CCCI_LOGD("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return;
    }
...
#define CIP_MD2_SBP_FILE "CIP_MD2_SBP"
    ...
    if (md_id == MD_SYS1) {
        val = snprintf(md_cip_sbp_file, sizeof(md_cip_sbp_file),
                       "%s%s", md_img_cip_folder, CIP_SBP_FILE);
        if (val < 0 || val >= sizeof(md_cip_sbp_file)) {
            CCCI_LOGD("%s-%d:snprintf fail,val = %d\n", __func__, __LINE__, val);
            return 0;
        }
    }
        ...
        val = snprintf(md_cip_sbp_file, sizeof(md_cip_sbp_file),
                       "%s%s", md_img_cip_folder, CIP_MD2_SBP_FILE);
        if (val < 0 || val >= sizeof(md_cip_sbp_file)) {
            CCCI_LOGD("%s-%d:snprintf fail,val = %d\n", __func__, __LINE__, val);
            return 0;
        }
...
static void cal_md_starttime(CCCI_BUFF_T buff)
{
    ...
    else if(buff.data[1] == CCCI_MD_MSG_BOOT_READY)
    {
        time_HS2 = time(NULL);
        if (time_HS2 < 0) {
            CCCI_LOGE("get time_HS2 error\n");
            return;
        }
        if (snprintf(time_val, sizeof(time_val),"%lds %s",
                     time_HS2-time_HS1, ctime(&time_HS2)) >= PROPERTY_VALUE_MAX)
        {
        ...
        ret = snprintf(name, sizeof(name), "vendor.mtk.md%d.starttime", md_id+1);
...
int main_v1(int input_md_id, int sub_ver)
{
    ...
    {
    case 0:
        ret = snprintf(dev_port, 32, "%s", ccci_get_node_name(USR_CCCI_CTRL, MD_SYS1));
        if (ret < 0 || ret >= 32) {
            CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
            return -1;
        }
        ...
        if(ccci_get_version() == ECCCI || ccci_get_version() == EDSDA) {
            ...
            ret = snprintf(mdlogger_name, 32, "%s", MD_LOG_FOR_MD1_NAME);
            if (ret < 0 || ret >= 32) {
                CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
                return -1;
            }
        ...
        break;
    case 1:
        ret = snprintf(dev_port, 32, "%s", ccci_get_node_name(USR_CCCI_CTRL, MD_SYS2));
        if (ret < 0 || ret >= 32) {
            CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
            return -1;
        }
        ...
        if(ccci_get_version() == ECCCI || ccci_get_version() == EDSDA) {
            ...
            ret = snprintf(mdlogger_name, 32, "%s", MD_LOG_FOR_MD2_NAME);
            if (ret < 0 || ret >= 32) {
                CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
                return -1;
            }
        ...
        break;
    case 2:
        ret = snprintf(dev_port, 32, "%s", ccci_get_node_name(USR_CCCI_CTRL, MD_SYS3));
        if (ret < 0 || ret >= 32) {
            CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
            return -1;
        }
        ...
        if (ccci_get_version() == ECCCI || ccci_get_version() == EDSDA) {
            ...
            ret = snprintf(mdlogger_name, 32, "%s", MD_LOG_FOR_MD3_NAME);
            if (ret < 0 || ret >= 32) {
                CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
                return -1;
            }
    ...
    ret = snprintf(muxd_service_status, PROPERTY_KEY_MAX, "%s%s", pre_service_status, muxd_name);
    if (ret < 0 || ret >= PROPERTY_KEY_MAX) {
        CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return -1;
    }
    ret = snprintf(rild_service_status, PROPERTY_KEY_MAX, "%s%s", pre_service_status, rild_name);
    if (ret < 0 || ret >= PROPERTY_KEY_MAX) {
        CCCI_LOGE("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
        return -1;
    }
    ...
    set_md_boot_env_data(md_id, dev_monitor_fd);
    tmp = 3; /* MT6580 onlly support 3 */
    if(ioctl(fd, CCCI_IOC_RELOAD_MD_TYPE, &tmp) != 0)
        CCCI_LOGD("update modem type to kernel fail: err=%d\n", errno);
