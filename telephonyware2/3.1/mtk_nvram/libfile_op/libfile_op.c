...
//   int iBackup_Partition=get_partition_numb("nvram");
	...
	if (length < 0) {
		NVBAK_LOG("%s:snprintf failed \n", __func__);
	} else if (length >= sizeof(cMtdDevName)) {
		NVBAK_LOG("%s:snprintf truncated \n", __func__);
	}
...
bool FileOp_CreatMdFileList(const char* SrcDirName)
{
	...
	if (sprintf(filelistPathName, "%s/FILELIST", SrcDirName) < 0) {
		NVBAK_LOG("%s:sprintf failed \n", __func__);
		return false;
	}
...
//   ret=FileOp_BackupData_All();
	...
	iResult = read(fd, tempBitmap1, NvRamBlockNum * sizeof(char));
        if ((iResult != NvRamBlockNum * sizeof(char)) || (iResult < 0)) {
...
bool FileOp_BackupToBinRegion_All_Ex(int value) {
	...
	for (i = 0; i < g_RecordCaller_Count; i++) {
		if (value == aRecordCallerList[i].iRecordCallerIndex) {
                        memcpy(cCallerName, aRecordCallerList[i].cRecordCallerName,
                               strlen(aRecordCallerList[i].cRecordCallerName) + 1);
			bMarkCaller = true;
			break;
		}
	}
...
//   int iBackup_Partition=get_partition_numb("nvram");
   ...
   FileOp_CreatMdFileList("/mnt/vendor/nvdata/md/NVRAM/NVD_IMEI");
	...
	//Add NVRAM history log
	length = snprintf(cLog, sizeof(cLog),"%s", "Enter BackupToBinRegion_all");
	if (length < 0) {
		NVBAK_LOG("%s, snprintf failed\n", __func__);
	} else if (length >= sizeof(cLog)) {
		NVBAK_LOG("%s, snprintf truncated\n", __func__);
	}
	if (bMarkCaller) {
		//strcat(cLog, "_");
...
bool FileOp_ClearBinRegion(void)
{
	...
	length = snprintf(path, sizeof(path), "%s", g_NVRAM_BACKUP_DEVICE);
	if (length < 0) {
		NVBAK_LOG("%s:snprintf failed \n", __func__);
...
bool FileOp_BackupDataToFiles(int * iFileMask, bool bWorkForBinRegion) {
	...
	for (eBackupType = APBOOT; eBackupType < ALL; eBackupType++) {
		...
		while (1) {
			...
			if (sprintf(acSrcPathName, "%s/%s", lpSrcDirName, entry->d_name) < 0) {
				NVBAK_LOG("%s:sprintf failed \n", __func__);
				return false;
			}
...
bool FileOp_BackupToBinRegionForDM() {
	...
	while (1) {
		...
		if (sprintf(acSrcPathName, "%s/%s", strDMFileFolderPath, entry->d_name) < 0) {
			NVBAK_LOG("%s:sprintf failed \n", __func__);
			return false;
		}
		NVBAK_LOG("FileOp_BackupData (%s)\n", acSrcPathName);
	...
	if (stat(strUSBFilePath, &st) >= 0) {
		...
		iResult = read(fd, tempBuf, iFileSize);
                if ((iResult != iFileSize) || (iResult < 0)) {
			NVBAK_LOG("Error FileOp_BackupSpecialFile:read1:%d\n", iResult);
			close(iFileDesc_map);
			close(iFileDesc_file);
			close(fd);
