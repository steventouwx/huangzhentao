 ...
 * Class RfxController
 *****************************************************************************/
class RtmWpController : public RfxController {
    // Required: declare this class
    RFX_DECLARE_CLASS(RtmWpController);

public:
    RtmWpController();
    virtual ~RtmWpController();
...
private:
    ...
    void resumeWorldModeChangeWithRadioOff(bool isStandAlone);
    void requestRadioOn(int slotId);
    void requestRadioOff(int slotId);
    ...
    void createNewMessageToRmm(int messageId, int slotId, int value);
    void moduleRequestRadioPowerRadioOffDone(RIL_Errno err, int slotId);
    void resetWorldPhoneSettings(void);
    bool isStandAloneChange(int cause);
private:
    ...
    // record calling slot id when world mode change
    int mCallSlotId;
    int mRadioOffValue;
    int mModifyRadioNum;
};
