...
#define RFX_LOG_TAG "RtmImsDialog"

const string RtmImsDialogHandler::TAG_NEXT_LINE("<ascii_10>");
const string RtmImsDialogHandler::TAG_RETURN("<ascii_13>");
const string RtmImsDialogHandler::TAG_DOUBLE_QUOTE("<ascii_34>");

RtmImsDialogHandler::RtmImsDialogHandler(int slot){
    mSlot = slot;
    RFX_LOG_D(RFX_LOG_TAG, "RtmImsDialogHandler()");
}

RtmImsDialogHandler::~RtmImsDialogHandler() {
}

//Handle IMS Dialog event package raw data.
void RtmImsDialogHandler::handleImsDialogMessage(const sp<RfxMessage>& message) {
    ...
    if (size > 0) {
        ...
        RtmImsConferenceController *ctrl =
                (RtmImsConferenceController *)root->findController(mSlot,
                        RFX_OBJ_CLASS_INFO(RtmImsConferenceController));
...
string RtmImsDialogHandler::concatData(int isFirst, string origData, string appendData) {
    if (isFirst) {
        return appendData;
    }
...
string RtmImsDialogHandler::recoverDataFromAsciiTag(string data) {
...
string RtmImsDialogHandler::replaceAll(string &str, const string &old_value, const string &new_value) {
    while(true) {
