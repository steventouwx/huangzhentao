...
class RmcDcMiscHandler : public RfxBaseHandler {
    ...
    private:
        ...
        void handleGetSlicingConfigRequest(const sp<RfxMclMessage>& msg);
        void handleUpdateSlicingConfigRequest(const sp<RfxMclMessage>& msg);
        void parseRulesFromAt(RIL_UrspRule *rule, RfxAtLine *pLine);
        void getUrspRules(RIL_SlicingConfig &sc, String8 plmn);
        bool parseDefaultAndRejectedNssai(RIL_SliceInfo *nssais, int *index, RfxAtLine *pLine);
        bool parseConfiguredAndAllowedNssai(RIL_SliceInfo *nssais,int *index, RfxAtLine *pLine);
        void getSliceInfo(RIL_SlicingConfig &sc, String8 plmn);
        void dumpSliceConfig(RIL_SlicingConfig sc);
        void getSliceConfig(const sp<RfxMclMessage>& msg);
        void handleUePolicyChanged(const sp<RfxMclMessage>& msg);
