...
int RfxRilUtils::mIsMipcSupport = -1;
...
int RfxRilUtils::mIsCtHvolteSupport = -1;
/// @}

RilRunMode RfxRilUtils::m_rilRunMode = RilRunMode::RIL_RUN_MODE_NORMORL;
STATUSCALLBACK RfxRilUtils::s_statusCallback = NULL;
EVENTCALLBACK  RfxRilUtils::s_eventCallback  = NULL;
PROPCALLBACK   RfxRilUtils::s_propCallback   = NULL;
...
unsigned int RfxRilUtils::rfxGetSimCount() {
    return getSimCount();
}
...
int RfxRilUtils::isEngLoad() {
    if (RfxRilUtils::m_isEngLoad == -1) {
        ...
        property_value[RFX_PROPERTY_VALUE_MAX - 1] = '\0';
...
int RfxRilUtils::isUserLoad() {
    if (RfxRilUtils::m_isUserLoad == -1) {
        ...
        property_value_emulation[RFX_PROPERTY_VALUE_MAX - 1] = '\0';
...
#else
    if (snprintf(dev_node, 32, "%s", ccci_get_node_name(USR_RILD_IOCTL, MD_SYS1)) < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "snprintf error");
        return -1;
    }
...
int RfxRilUtils::triggerPhantomPacket(String8 s) {
    ...
    RFX_LOG_D(RFX_LOG_TAG, "Phantom pkt content(%zu): %s", s.size() / 2,
              mode4_msg);

#if defined(MTK_RIL_THIN_MD_SUPPORT)
    const char *port = "/dev/wwanD0HPEG";
#else
    const char *port = "/dev/ttyC6";
#endif
    fd = open(port, O_RDWR);
    if (fd < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "Fail to open %s: %d", port, errno);
        return -1;
    } else {
        RFX_LOG_D(RFX_LOG_TAG, "%s is opened.", port);
    }
    ...
    RFX_LOG_D(RFX_LOG_TAG, "Write %d bytes to %s", ret, port);
    close(fd);
    return 0;
}
...
void RfxRilUtils::setPropCallbackForGT(PROPCALLBACK cb) {
    s_propCallback = cb;
}

void RfxRilUtils::updatePropToGT(const char *key, const char *value) {
    if (s_propCallback != NULL && key != NULL && value != NULL) {
        RFX_LOG_D(RFX_LOG_TAG, "updatePropToGT [%s, %s]", key, value);
        s_propCallback(key, value);
    } else {
        RFX_LOG_E(RFX_LOG_TAG, "updatePropToGT is NULL");
    }
}

void RfxRilUtils::setEventCallbackForGT(EVENTCALLBACK cb) {
    s_eventCallback = cb;
}

bool RfxRilUtils::updateEventToGT(int slotId, int channelId, int eventId, void *data, size_t len,
        int clientId, int token, nsecs_t delayTime, MTK_RIL_REQUEST_PRIORITY priority) {
    if (s_eventCallback != NULL) {
        RFX_LOG_D(RFX_LOG_TAG, "updateEventToGT id = %d", eventId);
        return s_eventCallback(slotId, channelId, eventId, data, len,
                clientId, token, delayTime, priority);
    } else {
        RFX_LOG_E(RFX_LOG_TAG, "updateEventToGT is NULL");
        return false;
    }
}

void RfxRilUtils::sendEventForGT(int slotId, int channelId, int eventId, void *data, size_t len,
        int clientId, int token, nsecs_t delayTime, MTK_RIL_REQUEST_PRIORITY priority) {
    sp<RfxMclMessage> msg = RfxMclMessage::obtainEvent(eventId, RfxBaseData(data, len),
            channelId, slotId, clientId,
            token, delayTime, priority);
    msg->setMainProtocolSlotId(RfxMclStatusManager::getNonSlotMclStatusManager()->getIntValue(
...
bool RfxRilUtils::isCtVolteSupport() {
    if (mIsCtVolteSupport == -1) {
        ...
        ctStr[RFX_PROPERTY_VALUE_MAX - 1] = '\0';
        if (strcmp(ctStr, "1") == 0) {
            mIsCtVolteSupport = 1;
        } else {
            mIsCtVolteSupport = 0;
...
bool RfxRilUtils::isCtHvolteSupport() {
    if (mIsCtHvolteSupport == -1) {
        char ctStr[RFX_PROPERTY_VALUE_MAX] = { 0 };
        rfx_property_get("persist.vendor.mtk_ct_volte_support", ctStr, "");
        ctStr[RFX_PROPERTY_VALUE_MAX - 1] = '\0';
        if (strcmp(ctStr, "3") == 0) {
            mIsCtHvolteSupport = 1;
        } else {
            mIsCtHvolteSupport = 0;
        }
    }
    return (mIsCtHvolteSupport == 1);
}
/// @}
...
int RfxRilUtils::getMajorSim() {
    ...
    int simId = 0;
    int mainSlot = RfxMclStatusManager::getNonSlotMclStatusManager()->getIntValue(
            RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT, -1);
    rfx_property_get("persist.vendor.radio.simswitch", tmp, "1");
    simId = atoi(tmp);

    // correct main slot system property
    if ((mainSlot >= 0) && (mainSlot < getSimCount()) && (simId != mainSlot + 1)) {
        RFX_LOG_D(RFX_LOG_TAG, "correct main slot from %d to %d", simId, mainSlot + 1);
        rfx_property_set("persist.vendor.radio.simswitch",
                String8::format("%d", mainSlot + 1).string());
        simId = mainSlot + 1;
    }
...
#define LIBRARY_AED "libaedv.so"
typedef int (*AEE_MODEM_WARNING_FUNC)(const char *module, const char* path, unsigned int flags, const char *modem_warning, const char *modem_version);

int RfxRilUtils::handleAee(const char *modem_warning, const char *modem_version) {
    void *dlHandle;
    int ret = 1;
    int (*aee_modem_warning_func)(const char *module, const char* path, unsigned int flags, const char *modem_warning, const char *modem_version);
    dlHandle = dlopen(LIBRARY_AED, RTLD_NOW);
    if (dlHandle != NULL) {
        aee_modem_warning_func = (AEE_MODEM_WARNING_FUNC)dlsym(dlHandle, "aee_modem_warning");
        if (aee_modem_warning_func != NULL) {
            dlerror();
            // #define DB_OPT_DUMMY_DUMP (1<<20)
            ret = aee_modem_warning_func("Modem Warning", NULL, 1<<20, modem_warning, modem_version);
            dlclose(dlHandle);
            return ret;
        } else {
            RFX_LOG_E(RFX_LOG_TAG, "libaedv.so aee_modem_warning not exsist");
            dlclose(dlHandle);
            return 1;
        }
    } else {
        RFX_LOG_E(RFX_LOG_TAG, "libaedv.so dlopen failed: %s", dlerror());
        return 1;
    }
}
...
int RfxRilUtils::getAkaSimSlot() {
    char property_value[RFX_PROPERTY_VALUE_MAX] = {0};
    rfx_property_get("vendor.gsm.prefered.aka.sim.slot", property_value, "-1");
    return atoi(property_value);
}
// External SIM [End]
...
bool RfxRilUtils::isWfcEnable(int slotId) {
    char wfcEnable[RFX_PROPERTY_VALUE_MAX] = { 0 };
...
bool RfxRilUtils::isVilteEnable(int slotId) {
    char vilteEnable[RFX_PROPERTY_VALUE_MAX] = { 0 };

    bool vilteSupport = isVilteSupport();
    rfx_property_get("persist.vendor.mtk.vilte.enable", vilteEnable, "0");
...
void RfxRilUtils::getLogicalModemId(char *modemId, int size, int slotId) {
    ...
    if (RatConfig_isNrSupported()) {
        char temp[RFX_PROPERTY_VALUE_MAX] = {0};
        int type;
        rfx_property_get("vendor.ril.nr.type", temp, "1");
        type = atoi(temp);

        if (sprintf(modemId, "modem_sys%d_ps%d_nr%d", 1,
                (getProtocolStackId(slotId) - 1), type) < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "sprintf failed");
        }
    } else {
        if (sprintf(modemId, "modem_sys%d_ps%d", 1, (getProtocolStackId(slotId) - 1)) < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "sprintf failed");
        }
    }
}
...
bool RfxRilUtils::isGwsdSupport() {
    ...
    rfx_property_get("ro.vendor.mtk_gwsd_capability", gwsd, "0");
    return (atoi(gwsd) != 0);
}

RfxPsRatFamily RfxRilUtils::getPsRatFamily(int radioTechnology) {
    if (radioTechnology == RADIO_TECH_GPRS
        ...
        return RFX_PS_RAT_FAMILY_CDMA;
    } else if (radioTechnology == RADIO_TECH_IWLAN) {
        return RFX_PS_RAT_FAMILY_IWLAN;
    } else {
        return RFX_PS_RAT_FAMILY_UNKNOWN;
    }
}

// Please don't call this function more than once in a log function.
// It may print same value due to they point to the same data.
const char * RfxRilUtils::pii(const char * tag, const char * hiddenData) {
    ...
    char * tagProp = NULL;
    char tagLevel[RFX_PROPERTY_VALUE_MAX] = { 0 };
    err = asprintf(&tagProp, "persist.log.tag.%s", tag);
    if (err >= 0 && tagProp != NULL) {
        rfx_property_get(tagProp, tagLevel, "I");
        tagLevel[RFX_PROPERTY_VALUE_MAX - 1] = '\0';
        free(tagProp);
    ...
    if (strlen(hiddenData) <= 2) {
        ...
        truncatedData[6] = '*';
        truncatedData[7] = '*';
        int len = strlen(hiddenData) > 254 ? 254 : strlen(hiddenData);
        for (i = 8; i < len; i++) {
            truncatedData[i] = hiddenData[i];
        }
        truncatedData[i]='\0';
...
bool RfxRilUtils::isMipcSupport() {
    if (RfxRilUtils::m_rilRunMode == RilRunMode::RIL_RUN_MODE_MOCK) {
        RfxRilUtils::mIsMipcSupport = -1;
    }
    if (RfxRilUtils::mIsMipcSupport == -1) {
        char rilMode[RFX_PROPERTY_VALUE_MAX] = { 0 };
        rfx_property_get("ro.vendor.mtk_mipc_support", rilMode, "0");
        RfxRilUtils::mIsMipcSupport = atoi(rilMode);
    }
    return (RfxRilUtils::mIsMipcSupport == 1);
}

int RfxRilUtils::executeCmdForRild(char * cmd, bool print)
{
    RFX_LOG_D(RFX_LOG_TAG, "%s, cmd:%s\n", __FUNCTION__, cmd);
    FILE *fp = NULL;
    char buf[512] = {0};
    int status = 0;
    fp = popen(cmd, "r");
    if (!fp) {
        RFX_LOG_D(RFX_LOG_TAG, "popen  %s Fail!\n", cmd);
        return -1;
    }
