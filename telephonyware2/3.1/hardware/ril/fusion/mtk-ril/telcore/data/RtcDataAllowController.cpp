...
void RtcDataAllowController::onInit() {
    ...
    int (*doWpfaInit)(void) = NULL;
    dlWpfaHandle = dlopen(LINK_LIBWPFA_PATH, RTLD_NOW);
    if (dlWpfaHandle == NULL) {
        logI(RTC_DAC_LOG_TAG, "[%s] dlopen: module = %s, failed (%s)", __FUNCTION__, LINK_LIBWPFA_PATH, dlerror());
    } else {
        logI(RTC_DAC_LOG_TAG, "[%s] dlopen: module = %s, pass", __FUNCTION__, LINK_LIBWPFA_PATH);
        doWpfaInit = (int (*)()) dlsym(dlWpfaHandle, "wpfaInit");
    }
    ...
    if (imsSubmarinesupport == 1) {
        ...
        if (doWpfaInit != NULL) {
            ...
            logI(RTC_DAC_LOG_TAG, "doWpfaInit function is not working.");
        }
    }
...
bool RtcDataAllowController::dequeueNetworkRequest(int r_id, int slotId) {
    ...
    if (r_id != RIL_DATA_PROFILE_DEFAULT && r_id != RIL_DATA_PROFILE_IMS) {
        int ret = checkRequestExistInQueue(r_id, slotId);
        if (ret < 0) {
            logW(RTC_DAC_LOG_TAG, "dequeueNetworkRequest: request not found, curr queue: %d",
                    (int)mOnDemandQueue.size());
...
void RtcDataAllowController::handleDataConnectionAttachRequest(const sp<RfxMessage>& message) {
    ...
    if (pReqData[0] == 1) {
        ...
        if (radioController == NULL) {
            logE(RTC_DAC_LOG_TAG,
                    "handleDataConnectionAttachRequest: fail to find RtcRadioController");
            onAttachOrDetachDone(message);
            return;
        }
...
void RtcDataAllowController::handleDataConnectionDetachRequest(const sp<RfxMessage>& message) {
    ...
    if (pReqData[0] == 1) {
        ...
        if (radioController == NULL) {
            logE(RTC_DAC_LOG_TAG,
                    "handleDataConnectionDetachRequest: fail to find RtcRadioController");
            onAttachOrDetachDone(message);
            return;
        }
...
void RtcDataAllowController::setAllowDataSlot(bool allow, int slot) {
    ...
    if (allow) {
        rfx_property_set(ALLOW_DATA_SLOT_PROPERTY, String8::format("%d", slot).string());
    } else {
        int oldAllowDataSlot = getAllowDataSlot();
        if (oldAllowDataSlot == slot) {
            logI(RTC_DAC_LOG_TAG, "setAllowDataSlot clear allow data slot=%d", slot);
            rfx_property_set(ALLOW_DATA_SLOT_PROPERTY, "-1");
        }
    }
}
...
int RtcDataAllowController::getAllowDataSlot() {
    ...
    rfx_property_get(ALLOW_DATA_SLOT_PROPERTY, allowDataSlot, "-1");
 ...
 */
void RtcDataAllowController::resendAllowData(bool byPrefer) {
    if (RfxRilUtils::getRilRunMode() == RIL_RUN_MODE_MOCK) {
        RFX_LOG_I(RTC_DAC_LOG_TAG, "resendAllowData should be skiped on UT");
        return;
    }
    ...
    int allowSlot = 0;
    if (byPrefer) {
        allowSlot = RtcDataUtils::getPreferDataSlotFromProperty();
    } else {
        allowSlot = getAllowDataSlot();
    }
    logI(RTC_DAC_LOG_TAG,
            "resendAllowData, byPrefer=%d, allowSlot=%d, simCount=%d",
            byPrefer, allowSlot, simCount);

    // Process the request again
    for (i = 0; i < simCount; i++) {
    // Always resend the slot of EDALLOW=1
    //TODO: check if we need to re-send EDALLOW=0
        if (simCount == 1 || i == allowSlot) {
            allowMessage = ALLOW_DATA;
            sp<RfxMessage> msg = RfxMessage::obtainRequest(
                    i, RFX_MSG_REQUEST_ALLOW_DATA, RfxIntsData(&allowMessage, 1));
            msg->setSlotId(i);
...
void RtcDataAllowController::onPreferredChanged(RfxStatusKeyEnum key,
    ...
    if(mOnDemandQueue.size() == 0 && dataSim >= 0){
        logD(RTC_DAC_LOG_TAG, "onPreferredChanged, try to set allow to slot %d", dataSim);
