...
void RmcSuppServUssdBaseHandler::handleUssdDomainInfoReq(const sp<RfxMclMessage>& msg) {
    ...
    /**
     * Some operators do not support STK USSD over IMS.
     * If AP tries STK USSI fisrt, MD would reports USSI error followed
     * by a successed USSD result. It leads to two USSD popups no the screen.
     */
    if (SSConfig::notSupportStkUssi(currentMccmnc.string())) {
        domain = 0;
    } else if (isImsRegOn()) {
        if (wfcState == 1) {
            domain = 1;
...
void RmcSuppServUssdBaseHandler::sendFailureReport(const sp<RfxMclMessage>& msg, RIL_Errno ret) {
    ...
    // error meesage
    /// ALPS05400267, IMS USSD FND failure
    // @{ remove RFX_MSG_REQUEST_SEND_USSD, USSI also should do this @}
...
bool RmcSuppServUssdBaseHandler::isFdnAllowed(const char* ussi) {
     ...
     */
    if (asprintf(&cmd, "AT+EAPPROVE=\"%s\"", ussi) < 0) {
        logE(TAG, "isFdnAllowed fail at asprintf");
        goto error;
    }

    if (cmd == NULL) {
        logE(TAG, "isFdnAllowed cmd is NULL");
        goto error;
    }
