...
#include "RmmOpRadioRequestHandler.h"
#include "libmtkrilutils.h"

#define RFX_LOG_TAG "RmmOpRadioReq"

RFX_IMPLEMENT_OP_HANDLER_CLASS(RmmOpRadioRequestHandler, RIL_CMD_PROXY_9);

RmmOpRadioRequestHandler::RmmOpRadioRequestHandler(int slotId, int channelId):
        RmmRadioRequestHandler(slotId, channelId) {
    const int requests[] = {RFX_MSG_REQUEST_COMMAND_BEFORE_RADIO_POWER,
            };
    registerToHandleRequest(requests, sizeof(requests)/sizeof(int));
}

RmmOpRadioRequestHandler::~RmmOpRadioRequestHandler() {
}

void RmmOpRadioRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
    int id = msg->getId();
    ...
    switch(id) {
        case RFX_MSG_REQUEST_COMMAND_BEFORE_RADIO_POWER:
            ...
            break;
        default:
            RmmRadioRequestHandler::onHandleRequest(msg);
            break;
    }
}

void RmmOpRadioRequestHandler::onhandlePreRadioCommand(const sp<RfxMclMessage>& msg) {
    if (isOp12Support()) {
