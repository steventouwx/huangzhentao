...
typedef struct {
    char ra_payload[MAX_RA_PAYLOAD_LENGTH];
} netagent_io_rapayload_parameter_t;
...
typedef struct {
    ...
    union {
        ...
        netagent_io_rapayload_parameter_t   rapayload;  // cmd = NETAGENT_IO_CMD_RAPAYLOAD
    } parameter;
...
netagent_io_init() {
    ...
    n = snprintf(path_buf, sizeof(path_buf), "/dev/radio/pttynwcmd");
    if (n < 0) {
    ...
    n = snprintf(path_buf, sizeof(path_buf), "/dev/radio/pttynwurc");
    if (n < 0) {
        ErrMsg("unknown error");
        goto init_fail;
    }
...
netagent_io_cmd_arp_result_alloc( netagent_io_arp_parameter_t  * arp) {
    ...
    if (!fail_code) {
        ...
        DbgMsg("get mac success");
    } else {
        //ErrMsg("get mac fail, ip = %s, err = %d", arp->ip, fail_code);
        ErrMsg("get mac fail, err = %d", fail_code);
    }
...
void *
netagent_io_cmd_rapayload_alloc(char *raPayload) {
    netagent_io_cmd_obj_t   *cmd_obj = 0;
    ...
    cmd_obj->cmd = NETAGENT_IO_CMD_RAPAYLOAD;
    memcpy(cmd_obj->parameter.rapayload.ra_payload, raPayload, MAX_RA_PAYLOAD_LENGTH - 1);
...
void *
netagent_io_cmd_ip_add_del_alloc(unsigned int if_id, netagent_io_cmd_e cmd, int result,
        netagent_io_addr_type_e addr_type, unsigned int *addr, int ipv6PrefixLength, int mtuV6) {
    netagent_io_cmd_obj_t   *cmd_obj = 0;
    ...
    if (addr_type == NETAGENT_IO_ADDR_TYPE_IPv6 ) {
        if (cmd_obj->cmd == NETAGENT_IO_CMD_IPADD) {
            ...
            cmd_obj->parameter.ipadd.mtu = mtuV6;
...
void *
netagent_io_cmd_free_multi_spi_alloc(unsigned int transaction_id,
    unsigned int action, unsigned int count, const char* response) {
    netagent_io_cmd_obj_t   *cmd_obj = 0;
    ...
    cmd_obj->trans_intf_id = transaction_id;
    cmd_obj->cmd = NETAGENT_IO_CMD_SPI_RSVN;
    cmd_obj->reason = strdup(response); // use reason as response
    cmd_obj->parameter.spi.action = action;
    cmd_obj->parameter.spi.max = count;

    return cmd_obj;
}

void *
netagent_io_cmd_spi_query_alloc() {
    netagent_io_cmd_obj_t   *cmd_obj = 0;
    ...
    cmd_obj->cmd = NETAGENT_IO_CMD_SPI_QUERY;
...
int
netagent_io_get_spi_addr_v4(void *co, netagent_io_sip_addr_whom_e whom, unsigned int *addr) {
    netagent_io_cmd_obj_t   *cmd_obj = co;
    ...
    switch (cmd_obj->cmd) {
        case NETAGENT_IO_CMD_SPI_RSVN :
            if (whom == NETAGENT_IO_SIP_ADDR_SRC) {
...
int
netagent_io_get_spi_addr_v6(void *co, netagent_io_sip_addr_whom_e whom, unsigned int *addr) {
    netagent_io_cmd_obj_t   *cmd_obj = co;
...
netagent_io_ipv4_str2bin(char *ip_str, unsigned int *ip_bin) {
    ...
    for (i=0 ; i<4 && ptr && *ptr ; ++i) {
        ip_buf[i] =  (unsigned char)stringToInt(ptr, 10);
        ptr = strchr(ptr, '.');
        if (ptr) {
            ptr++;
        }
...
netagent_io_ipv6_str2bin(char *ip_str, unsigned int *ip_bin) {
    ...
    for (i=0 ; i<16 && ptr && *ptr ; ++i) {
        ip_buf[i] =  (unsigned char)stringToInt(ptr, 16);
        ptr = strchr(ptr, ':');
        if (ptr) {
            ptr++;
        }
...
void *
netagent_io_recv(void *o, int isUserBuild) {
    netagent_io_t *io = o;
    netagent_io_cmd_obj_t *cmd_obj = 0;;
    ...
    while (1) {
        ...
        if (line) {
            ...
            if (strncmp(line, "+EIF: ", 6) == 0) {
                ...
                cmd_obj = netagent_io_cmd_alloc(stringToInt(argv[0], 10), NETAGENT_IO_CMD_MAX);
                if (!cmd_obj) {
                ...
                if (strcmp(argv[1], "ifup") == 0) {
                    if (isUserBuild) {
                        DbgMsg("recv urc : +EIF: %s, ifup, %s, xxx", argv[0], argv[2]);
                    }
                    if (num < 3) {
                    ...
                    cmd_obj->parameter.ip.addr_type = stringToInt(argv[2], 16);
                    ...
                    cmd_obj->parameter.mtu.mtu_size = stringToInt(argv[2], 10);
                    //set mtu size
                } else if (strcmp(argv[1], "ifchg") == 0) {
                    if (isUserBuild) {
                        DbgMsg("recv urc : +EIF: %s, ifchg, %s, xxx", argv[0], argv[2]);
                    }
                    if (num < 3) {
                        netagent_io_cmd_free(cmd_obj);
                        cmd_obj = 0;
                    ...
                    cmd_obj->parameter.ho.info.hostate = stringToInt(argv[3], 10);
                    cmd_obj->parameter.ho.info.is_succ = stringToInt(argv[4], 10);

                    /** Rat value
                     *  0: Known
                     *  1: Cellular
                     *  2: IWLAN
                     *  3: DATA_1 (IMS over SIM1's cellular)
                     *  4: DATA_2 (IMS over SIM2's cellular)
                     *  5: DATA_3 (IMS over SIM3's cellular)
                     *  6: DATA_4 (IMS over SIM4's cellular)
                     */
                    cmd_obj->parameter.ho.info.src_ran = stringToInt(argv[5], 10);
                    if (cmd_obj->parameter.ho.info.src_ran >= 3
                            && cmd_obj->parameter.ho.info.src_ran <= 6) {
                        // Value from 3~6 will be view as IWLAN, which is related to VoData feature
                        cmd_obj->parameter.ho.info.src_ran = NETAGENT_IO_HO_RAN_WIFI;
                    }
                    cmd_obj->parameter.ho.info.tgt_ran = stringToInt(argv[6], 10);
                    if (cmd_obj->parameter.ho.info.tgt_ran >= 3
                            && cmd_obj->parameter.ho.info.tgt_ran <= 6) {
                        // Value from 3~6 will be view as IWLAN, which is related to VoData feature
                        cmd_obj->parameter.ho.info.tgt_ran = NETAGENT_IO_HO_RAN_WIFI;
                    }

                    cmd_obj->parameter.ho.info.addr_type = stringToInt(argv[7], 16);
                    ...
                    if (isUserBuild) {
                        DbgMsg("recv urc : +EIF: %s, ipchg, %s, xxx", argv[0], argv[2]);
                    }
                    if (num < 3) {
                        netagent_io_cmd_free(cmd_obj);
                        cmd_obj = 0;
                    ...
                    if (isUserBuild) {
                        DbgMsg("recv urc : +EIF: %s, ipadd, %s, xxx", argv[0], argv[2]);
                    }
                    if (num < 3) {
                        netagent_io_cmd_free(cmd_obj);
                        cmd_obj = 0;
                    ...
                    cmd_obj->parameter.ipadd.addr_type = stringToInt(argv[2], 16);
                    netagent_io_parser_ip_add_del_addr_str(cmd_obj, argv[3]);
                } else if (strcmp(argv[1], "ipdel") == 0) {
                    if (isUserBuild) {
                        DbgMsg("recv urc : +EIF: %s, ipdel, %s, xxx", argv[0], argv[2]);
                    }
                    if (num < 3) {
                        netagent_io_cmd_free(cmd_obj);
                        cmd_obj = 0;
                    ...
                    cmd_obj->parameter.ipdel.addr_type = stringToInt(argv[2], 16);
                    netagent_io_parser_ip_add_del_addr_str(cmd_obj, argv[3]);
                } else {
                    if (isUserBuild) {
                        DbgMsg("recv urc : %s", line);
                ...
                */
                ...
                cmd_obj->parameter.tcp.action = stringToInt(argv[1], 10);
                ...
                cmd_obj->parameter.tcp.port = stringToInt(argv[5], 10);
                ...
                /*
                    ...
                    +EIPSPI: <transaction id>, <2: free>, <source IP address>, <destination IP address>,
                ...
                */
                ...
                cmd_obj->cmd = NETAGENT_IO_CMD_SPI_RSVN;
                cmd_obj->parameter.spi.action = stringToInt(argv[1], 10);
                ...
                if (cmd_obj->parameter.spi.action == 0) {
                    ...
                    cmd_obj->parameter.spi.min = stringToInt(argv[6], 10);
                    cmd_obj->parameter.spi.max = stringToInt(argv[7], 10);
                } else {
                    // using 'mode' to remember the spi to be freed
                    cmd_obj->parameter.spi.mode = stringToInt(argv[5], 10);
                }
                if (cmd_obj->parameter.spi.action == 2) {
                    cmd_obj->parameter.spi.min = stringToInt(argv[6], 10);
                    cmd_obj->parameter.spi.max = stringToInt(argv[7], 10);
                }
            }
...
int netagent_io_send(void *o, void *co, int isUserBuild) {
    netagent_io_t *io = o;
    netagent_io_cmd_obj_t *cmd_obj = co;
    int ret = 0, n = 0, isPrintCmd = 1;
    char cmd_buf[512] = {0};
    ...
    switch (cmd_obj->cmd) {
        ...
        case NETAGENT_IO_CMD_IPUPDATE : {
            ...
            n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIF=%d, \"ipupdate\", %d, \"",
                cmd_obj->trans_intf_id,
            ...
            if (cmd_obj->parameter.ip.addr_type == NETAGENT_IO_ADDR_TYPE_IPv4) {
                ...
                for (i = 0 ; i<4 ; ++i) {
                    if (i == 0) {
                        n = snprintf(cmd_buf+offset, sizeof(cmd_buf)-offset, "%d", *addr_ptr);
                    } else {
                        n = snprintf(cmd_buf+offset, sizeof(cmd_buf)-offset, ".%d", *addr_ptr);
                    }
                    if (n < 0) {
                ...
                for (i = 0 ; i<16 ; ++i) {
                    if (i == 0) {
                        n = snprintf(cmd_buf+offset, sizeof(cmd_buf)-offset, "%02X", *addr_ptr);
                    } else {
                        n = snprintf(cmd_buf+offset, sizeof(cmd_buf)-offset, ":%02X", *addr_ptr);
                    }
                    if (n < 0) {
                ...
                if (ipv6_prefix_length != INVALID_IPV6_PREFIX_LENGTH) {
                    n = snprintf(cmd_buf+offset, sizeof(cmd_buf)-offset, "/%d", ipv6_prefix_length);
                    if (n < 0) {
                        ErrMsg("snprintf error");
                        return NETAGENT_IO_RET_GENERIC_FAILURE;
                    }
            ...
            n = snprintf(cmd_buf+offset, sizeof(cmd_buf)-offset, "\"");
            if (n < 0) {
                ErrMsg("snprintf error");
                return NETAGENT_IO_RET_GENERIC_FAILURE;
            }
        ...
        case NETAGENT_IO_CMD_IPADD : {
            ...
            if (cmd_obj->parameter.ipadd.result == 99) {
                n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIF=%d, \"ipadd\", %d, \"",
                    cmd_obj->trans_intf_id,
                    cmd_obj->parameter.ipadd.addr_type);
            } else {
                n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIF=%d, \"ipadd\", %d, %d, \"",
                    cmd_obj->trans_intf_id,
            ...
            if (cmd_obj->parameter.ipadd.mtu != -1) {
                offset += n;
                n = snprintf(cmd_buf + offset, sizeof(cmd_buf) - offset,
                        ", %d", cmd_obj->parameter.ipadd.mtu);
                if (n < 0) {
                    ErrMsg("snprintf error");
                    return NETAGENT_IO_RET_GENERIC_FAILURE;
                }
        ...
        case NETAGENT_IO_CMD_IPDEL : {
            ...
            if (cmd_obj->parameter.ipdel.result == 99) {
                n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIF=%d, \"ipdel\", %d, \"",
                    cmd_obj->trans_intf_id,
                    cmd_obj->parameter.ipdel.addr_type);
            } else {
                n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIF=%d, \"ipdel\", %d, %d, \"",
                    cmd_obj->trans_intf_id,
        ...
        case NETAGENT_IO_CMD_RA :
            n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIF=%d, \"ra\", \"%s\"",
                    cmd_obj->trans_intf_id,
            ...
            break;
        case NETAGENT_IO_CMD_PDNHO :
            n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIF=%d, \"ho\", 1", cmd_obj->trans_intf_id);
            if (n < 0) {
                ErrMsg("snprintf error");
                return NETAGENT_IO_RET_GENERIC_FAILURE;
            }
        ...
        case NETAGENT_IO_CMD_SYNC_CAPABILITY: {
            ...
            n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIFCONFIG=%d", capability);
            if (n < 0) {
                ErrMsg("snprintf error");
                return NETAGENT_IO_RET_GENERIC_FAILURE;
            }
        ...
        case NETAGENT_IO_CMD_UDP_RSVN:
             ...
             n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIPPORT=%d, %d, %d",
                     cmd_obj->trans_intf_id,
         ...
         case NETAGENT_IO_CMD_SPI_RSVN:
             ...
             // AT+EIPSPI=<transaction id>, <2: multi free>, <count>, <spi-1>,<state-1>,...........
             if (cmd_obj->parameter.spi.action == 2) {
                 n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIPSPI=%d, %d, %d, %s",
                     cmd_obj->trans_intf_id,
                     cmd_obj->parameter.spi.action,
                     cmd_obj->parameter.spi.max,
                     cmd_obj->reason);
                 FREEIF(cmd_obj->reason);
             } else {
                 n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIPSPI=%d, %d, %u",
                     cmd_obj->trans_intf_id,
                     cmd_obj->parameter.spi.action,
         ...
         case NETAGENT_IO_CMD_SPI_QUERY:
             n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+EIPSPI?");
             if (n < 0) {
                 ErrMsg("snprintf error");
                 return NETAGENT_IO_RET_GENERIC_FAILURE;
             }
         ...
         case NETAGENT_IO_CMD_RAPAYLOAD:
             n = snprintf(cmd_buf, sizeof(cmd_buf), "AT+ERAPAYLOAD=%s",
                    cmd_obj->parameter.rapayload.ra_payload);
             if (n < 0) return NETAGENT_IO_RET_GENERIC_FAILURE;
             break;
    }
...
int netagent_io_flush_ipsec_policy(
        const char *src_addr, netagent_io_addr_type_e addr_type, int isUserLoad) {
    int status = 0;
    char prefix[22] = "";
    ...
    if (isUserLoad != 1) {
        ...
        DbgMsg("flush IPSEC policy, addr ***, addr_type %d,prefix %s",  addr_type, prefix);
    }
    ...
    switch (addr_type) {
        ...
        case NETAGENT_IO_ADDR_TYPE_IPv4 :
            ...
            netagent_io_exec_fmt(&status,
                                 ...
                                 src_addr);
            netagent_io_exec_fmt(&status,
                                 SYSTEM_IP_WRAPPER_BIN_DIR " xfrm policy deleteall dir out src %s action block",
                                 src_addr);
            netagent_io_exec_fmt(&status,
                                 SYSTEM_IP_WRAPPER_BIN_DIR " xfrm policy deleteall dir in dst %s action block",
                                 src_addr);
            netagent_io_exec_fmt(&status,
                                 SYSTEM_IP_WRAPPER_BIN_DIR " xfrm policy deleteall dir fwd dst %s action block",
                                 src_addr);
            break;
        ...
        case NETAGENT_IO_ADDR_TYPE_IPv6 :
            ...
            netagent_io_exec_fmt(&status,
                                 ...
                                 prefix);
            netagent_io_exec_fmt(&status,
                                 SYSTEM_IP_WRAPPER_BIN_DIR " xfrm policy deleteall dir out src %s/64 action block",
                                 prefix);
            netagent_io_exec_fmt(&status,
                                 SYSTEM_IP_WRAPPER_BIN_DIR " xfrm policy deleteall dir in dst %s/64 action block",
                                 prefix);
            netagent_io_exec_fmt(&status,
                                 SYSTEM_IP_WRAPPER_BIN_DIR " xfrm policy deleteall dir fwd dst %s/64 action block",
                                 prefix);
            break;
        default:
