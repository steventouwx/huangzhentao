...
std::mutex mtxx;
RmmVtMsgParser* RmmVtMsgParser::s_RmmVtMsgParser;

RmmVtMsgParser::RmmVtMsgParser() {
    ...
    mServiceStatusStruct = NULL;
    mRildStatusStruct = NULL;
    mGetOpIdReqStruct = NULL;
...
void RmmVtMsgParser::startSession(char* input, int type) {
    ...
    } else if (MSG_ID_WRAP_IMSVT_RILD_VTSERVICE_STATUS == mType) {

        RFX_LOG_I(RFX_LOG_TAG, "[VT  PARSER] MSG_ID_WRAP_IMSVT_RILD_VTSERVICE_STATUS");
        mServiceStatusStruct = (VT_STATUS_REQ*)input;

    } else if (MSG_ID_WRAP_IMSVT_RILD_RILD_STATUS == mType) {

        RFX_LOG_I(RFX_LOG_TAG, "[VT  PARSER] MSG_ID_WRAP_IMSVT_RILD_RILD_STATUS");
        mRildStatusStruct = (VT_RIL_STATUS_IND*)input;

    } else if (MSG_ID_WRAP_IMSVT_RILD_GET_OP_ID_REQ == mType) {
        RFX_LOG_I(RFX_LOG_TAG, "[VT  PARSER] MSG_ID_WRAP_IMSVT_RILD_GET_OP_ID_REQ");
        mGetOpIdReqStruct = (VT_GET_OP_ID_REQ*)input;
...
void RmmVtMsgParser::stopSession() {
...
int RmmVtMsgParser::getSlot() {
    ...
    if (MSG_ID_WRAP_IMSVT_IMCB_GET_CAP_IND == mType) {
        ...
        return mServiceStatusStruct->sim_slot_id;

    } else if (MSG_ID_WRAP_IMSVT_RILD_RILD_STATUS == mType) {

        return mRildStatusStruct->sim_slot_id;

    } else if (MSG_ID_WRAP_IMSVT_RILD_GET_OP_ID_REQ == mType) {

        return mGetOpIdReqStruct->sim_slot_id;
...
RmmVtMsgParser* RmmVtMsgParser::getInstance() {
    if (s_RmmVtMsgParser == NULL) {
        mtxx.lock();
        if (s_RmmVtMsgParser == NULL) {
            s_RmmVtMsgParser = new RmmVtMsgParser();
        }
        mtxx.unlock();
    }
    return s_RmmVtMsgParser;
}
