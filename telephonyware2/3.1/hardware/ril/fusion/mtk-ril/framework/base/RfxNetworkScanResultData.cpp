            ...
            for (int i = 0; i < countCell; i++) {
                switch(pData->network_infos[i].cellInfoType) {
                    ...
                    case RIL_CELL_INFO_TYPE_NR:
                        asprintf(&(pData->network_infos[i].CellInfo.nr.cellidentity.operName.long_name),
                                "%s", pResult->network_infos[i].CellInfo.nr.cellidentity.operName.long_name);
                        asprintf(&(pData->network_infos[i].CellInfo.nr.cellidentity.operName.short_name),
                                "%s", pResult->network_infos[i].CellInfo.nr.cellidentity.operName.short_name);
                        break;
