...
class RmcDcCommonReqHandler : public RfxBaseHandler, public NetObject {
    public:
        ...
        static int getCmdIndexFromApnTable(unsigned int slot_id, ApnTableReq *pApnTableReq);
    ...
    protected:
        ...
        virtual void onNwAct(const sp<RfxMclMessage>& msg);
        virtual void onNwDeact(const sp<RfxMclMessage>& msg);
        ...
        virtual void onMePdnPropertyChange(const sp<RfxMclMessage>& msg, const int aid,
                PDN_CHANGE_REASON reason = PDN_CHANGE_REASON_NORMAL);
        ...
        virtual void onKernelAddressUpdated(const sp<RfxMclMessage>& msg);
        virtual void updateMtuV6FromKernel(int interfaceId, int mtuV6);
        virtual void onAttachApnChanged(const char* apnName);
        virtual void onReleaseRfOccupy(const sp<RfxMclMessage>& msg);
        virtual void releaseRfOccupyTimeout(const sp<RfxMclMessage>& msg);
        ...
        int activatePdn(const char *reqApn, const char *profileType, Vector<int> *vAidList,
                ...
                RIL_TrafficDescriptor *td, bool matchAllRuleAllowed);
        ...
        int getCurrentRat();
        bool updateSecondaryPdpDynamicParameters(int aid);
        ...
        void createDataResponse(int transIntfId, int protocol,
                MTK_RIL_Data_Call_Response_v11* response, MTK_RIL_NetlinkIpv6Info *netInfo = NULL);
        ...
        void sendDataCallListResponse(const sp<RfxMclMessage>& msg, int deactivatedAid = INVALID_AID,
                int sscMode = SSC_UNKNOWN, bool deprecated = false);
        ...
        void updateMdDefaultEmergencyApn();
        ...
        void handleQualifiedNetworkTypeChanged_V1(const sp<RfxMclMessage>& msg);
        void handleQualifiedNetworkTypeChanged_V2(const sp<RfxMclMessage>& msg);
        void updateTdInfo(int aid);
        void updateTdInfo();
        void onUrspReevaluate(const sp<RfxMclMessage>& msg);
        void onDataRatUpdated(const sp<RfxMclMessage>& msg);
        int queryAndUpdateMtu(unsigned int aid);
        void enableVolteIotFirewall(bool enable, int transIntfId);
        void notifyDataFailureCause(bool reqDataCall);
        bool isLocalFailCase(int cause);
        bool lockAndClearApn();
        bool unlockApn();
        bool syncApnItem(int apnIndex);
        bool isImsTypeApnExist();
        bool syncDefaultImsApn();
    ...
    protected:
        ...
        int causeType;
        int DMF_support = getFeatureVersion((char*)"DMF_TC02");
    ...
    private:
        ...
        static volatile int s_mtuV6[MAX_CCMNI_INTERFACE_COUNT];
        bool mIsUserBuild;
        bool mMdrsraSupport;
        char *mAttachApn;
        bool mRfOccupy;
