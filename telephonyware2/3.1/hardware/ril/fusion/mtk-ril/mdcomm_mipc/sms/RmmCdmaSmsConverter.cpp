...
#define RFX_LOG_TAG "RmmCdmaSmsCvt"
...
String8 CdmaSmsMessage::toHexBody() {
     ...
     for (RfxObject *i = getFirstChildObj(); i != NULL; i = i->getNextObj()) {
        RmmTlvNode *pNode = RFX_OBJ_DYNAMIC_CAST(i, RmmTlvNode);
        if (pNode != NULL) {
 ...
 * Node
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("RmmTlvNode", RmmTlvNode, RfxObject);

RmmTlvNode::RmmTlvNode(int tag, int length) :
        m_tag(tag), m_length(length), m_error(0) {
}

RmmTlvNode::RmmTlvNode(const char* hexPdu, int len) : m_error(0) {
    hex2Bin(hexPdu, len);
    if (len != m_length * 2 + 4) {
        m_error = -1;
    }
...
char RmmTlvNode::char2Int(char c) {
    if (c >= '0' && c <= '9') {
        return c - '0';
    } else if (c >= 'A' && c <= 'F') {
        return c - 'A' + 10;
...
void RmmTlvNode::hex2Bin(const char *src, int len) {
    RFX_ASSERT(len % 2 == 0);
    RFX_ASSERT(len >= 6);
 ...
 * TeleServiceId
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("TeleServiceId", TeleServiceId, RmmTlvNode);

TeleServiceId::TeleServiceId(int id) : RmmTlvNode(TELESERVICE_ID, TELESERVICE_ID_LEN) {
   m_value.push((id & 0xFFFF) >> 8);
   m_value.push(id & 0xFF);
}


TeleServiceId::TeleServiceId(const char* hexPdu, int len) :  RmmTlvNode(hexPdu, len){
 ...
 * ServiceCategory
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("ServiceCategory", ServiceCategory, RmmTlvNode);

ServiceCategory::ServiceCategory(const char* hexPdu, int len) :  RmmTlvNode(hexPdu, len){
 ...
 * CdmaSmsAddress
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("CdmaSmsAddress", CdmaSmsAddress, RmmTlvNode);

CdmaSmsAddress::CdmaSmsAddress(RIL_CDMA_SMS_Address &addr, bool isMo) :RmmTlvNode(-1, -1) {
...
CdmaSmsAddress::CdmaSmsAddress(const char* hexPdu, int len) :  RmmTlvNode(hexPdu, len) {
 ...
 * CdmaSmsSubAddress
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("CdmaSmsSubAddress", CdmaSmsSubAddress, RmmTlvNode);

CdmaSmsSubAddress::CdmaSmsSubAddress(const char* hexPdu, int len) :  RmmTlvNode(hexPdu, len) {
 ...
 * BearerReplyOpt
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("BearerReplyOpt", BearerReplyOpt, RmmTlvNode);

BearerReplyOpt::BearerReplyOpt() : RmmTlvNode(BEARER_REPLY_OPT, BEARER_REPLY_OPT_LEN) {
   m_value.push(0x00);
}


BearerReplyOpt::BearerReplyOpt(const char* hexPdu, int len) :  RmmTlvNode(hexPdu, len) {
 ...
 * CauseCode
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("CauseCode", CauseCode, RmmTlvNode);

CauseCode::CauseCode(RIL_CDMA_SMS_Ack &ack, int replySeqNo) : RmmTlvNode(CAUSE_CODE, -1) {
    int errClass;
...
CauseCode::CauseCode(const char* hexPdu, int len) :  RmmTlvNode(hexPdu, len) {
    if (m_tag != CAUSE_CODE) {
        m_error = -1;
    }
}
 ...
 * BearerData
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("BearerData", BearerData, RmmTlvNode);

BearerData::BearerData(unsigned char *data, int len, bool isMo) : RmmTlvNode(BEARER_DATA, len),
...
BearerData::BearerData(const char* hexPdu, int len) :  RmmTlvNode(hexPdu, len),
        m_needStatusReport(false) {
...
/*****************************************************************************
 * Class RmmCdmaSmsConverter
 *****************************************************************************/
bool RmmCdmaSmsConverter::toHexPdu(
...
bool RmmCdmaSmsConverter::toHexPdu(RIL_CDMA_SMS_Ack* pAck, int replySeqNo,
...
bool RmmCdmaSmsConverter::toMessage(
...
bool RmmCdmaSmsConverter::isStatusReport(RIL_CDMA_SMS_Message *pMessage, int *pMessageId) {
    BearerData *pBearer;
...
bool RmmCdmaSmsConverter::isNeedStatusReport(RIL_CDMA_SMS_Message * pMessage) {
    BearerData *pBearer;
    RFX_OBJ_CREATE_EX(pBearer, BearerData, NULL,
     (pMessage->aBearerData, pMessage->uBearerDataLen));
    RFX_ASSERT(pBearer != NULL);
...
void RmmCdmaSmsConverter::getInboundSmsInfo(RIL_CDMA_SMS_Message *pMessage, InboundSmsInfo *info) {
    BearerData *pBearer;
    RFX_OBJ_CREATE_EX(pBearer, BearerData, NULL,
     (pMessage->aBearerData, pMessage->uBearerDataLen, false));
    RFX_ASSERT(pBearer != NULL);
...
void RmmCdmaSmsConverter::encodeUserData(
...
bool RmmCdmaSmsConverter::replaceUserDataPayload(
...
bool RmmCdmaSmsConverter::replaceUserData(
        RIL_CDMA_SMS_Message *pMessage,
        unsigned char* userData,
        int userDataLen) {
    unsigned int userDataPos = (unsigned int) pMessage->uBearerDataLen;
    int nextSubParameterPos = -1;
    int leftDataLen = -1;
    int oldUserDataLen = -1;

    if (pMessage->uBearerDataLen <= 0 || pMessage->uBearerDataLen > RIL_CDMA_SMS_BEARER_DATA_MAX) {
        return false;
    }

    unsigned int i = 0;
    while (i < pMessage->uBearerDataLen - 1) {
...
int RmmCdmaSmsConverter::hexCharToInt(char c) {
    if (c >= '0' && c <= '9')
        return (c - '0');
    if (c >= 'A' && c <= 'F')
        return (c - 'A' + 10);
...
void RmmCdmaSmsConverter::hexStringToBytes(const char *in, int inLength, char *out,
        int outLength) {
    int i;
