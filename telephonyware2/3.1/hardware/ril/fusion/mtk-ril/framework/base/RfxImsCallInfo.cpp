...
String8 RfxImsCallInfo::toString() {
    String8 name = String8::format("name=%s", RfxRilUtils::pii(RFX_LOG_TAG, mName.string()));
    return String8::format(
            "id=%d, state=%d, number=%s, conf=%d, confHost=%d, ecc=%d, video=%d, mt=%d",
            mCallId, mCallState, RfxRilUtils::pii(RFX_LOG_TAG, mNumber.string()), mIsConference,
            mIsConferenceHost, mIsEcc, mIsVideo, mIsMT) + name;
}
