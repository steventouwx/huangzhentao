...
RFX_REGISTER_DATA_TO_EVENT_ID(RfxStringData, RFX_MSG_EVENT_DATA_NW_ACT);
RFX_REGISTER_DATA_TO_EVENT_ID(RfxStringData, RFX_MSG_EVENT_DATA_NW_DEACT);
...
RFX_REGISTER_DATA_TO_EVENT_ID(RfxStringData, RFX_MSG_EVENT_UE_POLICY_CHANGED);
...
RFX_REGISTER_DATA_TO_EVENT_ID(RfxStringData, RFX_MSG_EVENT_URSP_REEVALUATE);
...
void RmcDcUrcHandler::onHandleUrc(const sp<RfxMclMessage>& msg) {
    ...
    if (strStartsWith(pRawUrc, "+CGEV: NW REACT")) {
        ...
        sendEvent(RFX_MSG_EVENT_DATA_NW_PDN_DEACT, RfxStringData(pRawUrc),
                RIL_CMD_PROXY_5, m_slot_id);
    } else if (strStartsWith(pRawUrc, "+CGEV: NW ACT")) {
        sendEvent(RFX_MSG_EVENT_DATA_NW_ACT, RfxStringData(pRawUrc),
                RIL_CMD_PROXY_5, m_slot_id);
    } else if (strStartsWith(pRawUrc, "+CGEV: NW DEACT")) {
        sendEvent(RFX_MSG_EVENT_DATA_NW_DEACT, RfxStringData(pRawUrc),
                RIL_CMD_PROXY_5, m_slot_id);
        ...
        if (RmcDcUtility::isVerizonOnSiteLteSimCard(m_slot_id)) {
            sendEvent(RFX_MSG_EVENT_ATTACH_APN_CHANGED, RfxStringData(pRawUrc),
                    RIL_CMD_PROXY_5, m_slot_id);
        }
        sendEvent(RFX_MSG_EVENT_QUERY_PCO_WITH_URC, RfxStringData(pRawUrc),
                RIL_CMD_PROXY_5, m_slot_id);
        ...
        sendEvent(RFX_MSG_EVENT_DATA_NW_LIMIT_NOTIFY, RfxStringData(pRawUrc),
                RIL_CMD_PROXY_5, m_slot_id);
    } else if (strStartsWith(pRawUrc, "+EGURSPREEVALU:")) {
        sendEvent(RFX_MSG_EVENT_URSP_REEVALUATE, RfxStringData(pRawUrc),
                RIL_CMD_PROXY_5, m_slot_id);
    } else if (strStartsWith(pRawUrc, "+EGUEPOLICYU:")) {
        sendEvent(RFX_MSG_EVENT_UE_POLICY_CHANGED, RfxStringData(pRawUrc),
                RIL_CMD_PROXY_5, m_slot_id);
    }
}
