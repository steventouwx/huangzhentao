...
void RmcGsmSmsRequestHandler::handleGetGsmCbSmsCfg(const sp<RfxMclMessage>& msg,
    ...
    if (query == GSM_CB_QUERY_CHANNEL) {
        int ret = asprintf(&settings, "%s", channels);
        if (ret < 0) {
            goto error;
        }
        ...
        int ret = asprintf(&settings, "%s", langs);
        if (ret < 0) {
            goto error;
        }
        ...
        char* channels, int *count)
{
    ...
    if (asprintf(&tmpChStr, "%s", channels) < 0) {
        return NULL;
    }
