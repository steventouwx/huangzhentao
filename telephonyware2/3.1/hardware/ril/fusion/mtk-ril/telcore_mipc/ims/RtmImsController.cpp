...
bool RtmImsController::sInitDone = false;
// last hashed iccid
char RtmImsController::sLastBootIccId[4][RFX_PROPERTY_VALUE_MAX] = {{0}, {0}, {0}, {0}};
int RtmImsController::sLastBootVolteState = 0;
/// @}

bool RtmImsController::mImsDisByMms[4] = {false, false, false, false};
int RtmImsController::imscfgcache[4][6] = {{-1}, {-1}, {-1}, {-1}};

/*****************************************************************************
 * Class RfxController
 *****************************************************************************/

#define RFX_LOG_TAG "RtmIms"

RFX_IMPLEMENT_CLASS("RtmImsController", RtmImsController, RfxController);
...
RtmImsController::RtmImsController() :
    ...
    mCachedXuiUrc(NULL),
    mLatestImsCfgToken(0) {
}

RtmImsController::~RtmImsController() {
}

void RtmImsController::requestImsDisable(int slotId, const sp<RfxAction>& action) {
    RFX_UNUSED(action);
...
void RtmImsController::requestImsResume(int slotId, const sp<RfxAction>& action) {
    RFX_UNUSED(action);
...
void RtmImsController::onInit() {
    ...
    getNonSlotScopeStatusManager()->registerStatusChanged(RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT,
        RfxStatusChangeCallback(this, &RtmImsController::onMainCapabilitySlotChanged));
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_SERVICE_STATE,
        RfxStatusChangeCallback(this, &RtmImsController::onServiceStateChanged));
...
void RtmImsController::onDeinit() {
...
bool RtmImsController::onHandleRequest(const sp<RfxMessage>& message) {
    //UT for MMSoverrule
    sp<RfxAction> action;
...
bool RtmImsController::onHandleUrc(const sp<RfxMessage>& message) {
    int msg_id = message->getId();
...
bool RtmImsController::onHandleResponse(const sp<RfxMessage>& message) {
    int msg_id = message->getId();
    ...
    case RFX_MSG_REQUEST_IMS_REGISTRATION_STATE:
        ...
        break;
    /// M: add for op09 volte setting @{
    // the request is from RtmImsConfigController, so do not response to RILJ
...
void RtmImsController::handleXui(const sp<RfxMessage>& message) {
    ...
    std::vector<std::string> uris = splitString(xui, ",");
    Vector<String8> xuis;
    for(int i = 0; i < (int)uris.size(); i++) {
        std::string uri = getUserNameFromSipTelUriString(uris[i]);
        if (!uri.empty()) {
...
void RtmImsController::handleImsRegStateRequest(const sp<RfxMessage>& message) {
    requestToMcl(message);
}

void RtmImsController::handleImsRegStateResponse(const sp<RfxMessage>& message) {
...
void RtmImsController::onServiceStateChanged(RfxStatusKeyEnum key,
...
bool RtmImsController::responseToRilj(const sp<RfxMessage>& msg) {
    return RfxController::responseToRilj(msg);
}

bool RtmImsController::onCheckIfRejectMessage(
    ...
    if((radioState == (int)RADIO_STATE_UNAVAILABLE ||
        radioState == (int)RADIO_STATE_OFF) &&
            (msgId == RFX_MSG_REQUEST_SET_IMSCFG ||
             ...
             msgId == RFX_MSG_REQUEST_DCH_ENABLE_REQ)) {
        //default dont need to log false case since this value will not cause this msg to be rejected
...
/// M: add for op09 volte setting @{
void RtmImsController::initOp09Ims() {
    ...
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_CDMA_CARD_TYPE,
        RfxStatusChangeCallback(this, &RtmImsController::onCardTypeChanged));
...
void RtmImsController::deinitOp09Ims() {
    if (!RfxRilUtils::isCtVolteSupport() && !RfxRilUtils::isCtMixVolteSupport()) {
        return;
    }
...
void RtmImsController::onMainCapabilitySlotChanged(RfxStatusKeyEnum key,
...
void RtmImsController::onCardTypeChanged(RfxStatusKeyEnum key,
    ...
    if (old_type == IMS_CARD_TYPE_INVALID || old_type == IMS_CARD_TYPE_NONE) {
        if (new_type == IMS_CARD_TYPE_NONE) {
            ...
            if (waitCount > 1 || strlen(newIccIdStr) == 0 || strcmp("N/A", newIccIdStr) == 0) {
                ...
                mNoIccidRetryCount = 0;
                mNoIccidTimerHandle =
                    RfxTimer::start(RfxCallback0(this, &RtmImsController::onNoIccIdTimeout),
...
void RtmImsController::onNoIccIdTimeout() {
    mNoIccidTimerHandle = NULL;
    ...
    if (strlen(newIccIdStr) == 0 || strcmp("N/A", newIccIdStr) == 0) {
        ...
        if (mNoIccidRetryCount < READ_ICCID_MAX_RETRY_COUNT &&
            ...
            mNoIccidTimerHandle = RfxTimer::start(RfxCallback0(this,
                            &RtmImsController::onNoIccIdTimeout), ms2ns(READ_ICCID_RETRY_TIME));
        }
    } else {
...
void RtmImsController::setNonOp09VolteState(int slot_id, const char new_iccid[]) {
...
void RtmImsController::checkNonOp09VolteState(int slot_id, const char new_iccid[]) {
    char oldIccIdStr[RFX_PROPERTY_VALUE_MAX] = { 0 };
    // multi ims need save every sim iccid
    rfx_property_get(PROPERTY_LAST_ICCID_SIM[slot_id], oldIccIdStr, "null");
...
void RtmImsController::sendImsSettingstoMcl(int value) {
    RFX_UNUSED(value);
    ...
    logD(RFX_LOG_TAG,
      ...
      // delay 600ms wait AT+EFUN and exist ims disable in RtmImsConfigController
      if (mIsOtaWait) {
...
void RtmImsController::setDefaultVolteState(int slot_id, const char new_iccid[], int card_type) {
    if (sInitDone == false) {
        for (int slot = 0; slot < RFX_SLOT_COUNT; slot++) {
            rfx_property_get(PROPERTY_LAST_ICCID_SIM[slot], sLastBootIccId[slot], "null");
        }
...
void RtmImsController::setVolteStateProperty(int slot_id, bool isEnable) {
    char volteState[RFX_PROPERTY_VALUE_MAX] = { 0 };
    rfx_property_get(PROPERTY_VOLTE_STATE, volteState, "0");
    int stateValue = atoi(volteState);
...
bool RtmImsController::isOp09SimCard(int slot_id, const char icc_id[], int card_type) {
    RFX_UNUSED(slot_id);
...
void RtmImsController::sendDefaultVolteStateUrc(int slot_id, int value) {
...
void RtmImsController::handleSetImsConfigRequest(const sp<RfxMessage>& message) {
...
void RtmImsController::handleSetImsConfigResponse(const sp<RfxMessage>& message) {
    responseToRilj(message);
    logD(RFX_LOG_TAG, "handleSetImsConfigResponse");
}

void RtmImsController::setVolteSettingStatus(const sp<RfxMessage>& message) {
    if (!RfxRilUtils::isCtVolteSupport() && !RfxRilUtils::isCtMixVolteSupport()) {
        return;
    }
...
const std::string RtmImsController::hash256(const char* str) {
    char buf[2];
...
const char* RtmImsController::givePrintableStr(const char* iccId) {
...
void RtmImsController::updateImsRegStatus() {
    sp<RfxMessage> msg = RfxMessage::obtainRequest(getSlotId(),
            RFX_MSG_REQUEST_SET_IMS_REGISTRATION_REPORT, RfxVoidData());
    requestToMcl(msg);
}

bool RtmImsController::onPreviewMessage(const sp<RfxMessage>& message) {
    int msg_id = message->getId();
    bool isInImsCall = false;
    ...
    switch (msg_id) {
        case RFX_MSG_REQUEST_SET_IMSCFG_EX: {
            ...
            isInImsCall = getStatusManager()->getBoolValue(RFX_STATUS_KEY_IMS_CALL_EXIST, false);
            if (isInImsCall) {
...
bool RtmImsController::onCheckIfRemoveSuspendedMessage(const sp<RfxMessage>& message) {
    int msg_id = message->getId();
    if (msg_id == RFX_MSG_REQUEST_SET_IMSCFG_EX) {
        bool isInImsCall = getStatusManager()->getBoolValue(RFX_STATUS_KEY_IMS_CALL_EXIST, false);
        int token = message->getToken();
        if (isInImsCall) {
            if (token < mLatestImsCfgToken) {
                logD(RFX_LOG_TAG,
                    "onCheckIfRemoveSuspendedMessage - %s, token:%d-%d",
                    RFX_ID_TO_STR(msg_id), token, mLatestImsCfgToken);
                return true;
            } else {
                mLatestImsCfgToken = token;
            }
...
bool RtmImsController::onCheckIfResumeMessage(const sp<RfxMessage>& message) {
    int msg_id = message->getId();
    bool isInImsCall = false;
    switch (msg_id) {
        case RFX_MSG_REQUEST_SET_IMSCFG_EX: {
            ...
            if (isInImsCall) {
                void *data = message->getData()->getData();
                int value = ((int*)data)[0];
                //logD(RFX_LOG_TAG, "onCheckIfResumeMessage: value = %d", value);
                if (value == 0) {
                    return false;
                }
            }
            logD(RFX_LOG_TAG, "onCheckIfResumeMessage - %s, token:%d", RFX_ID_TO_STR(msg_id), message->getToken());
...
void RtmImsController::cacheimscfgparams(const sp<RfxMessage>& message) {
    int curSlotId = getSlotId();
    if (curSlotId < 0 || curSlotId >= 4) {
...
void RtmImsController::mmsoverruleimscfg(const sp<RfxMessage>& message) {
    void *data = message->getData()->getData();
    ((int*)data)[0] = 0;
    ((int*)data)[1] = 0;
    ((int*)data)[2] = 0;
...
void RtmImsController::sendCachedXui() {
    if (mCachedXuiUrc != NULL) {
        responseToRilj(mCachedXuiUrc);
    }
}

void RtmImsController::handleImsRegInfoInd(const sp<RfxMessage>& message) {
...
void RtmImsController::sendCachedImsRegInfo() {
    sp<RfxMessage> unsol;
...
std::string RtmImsController::getUserNameFromSipTelUriString(std::string uriString) {
    RFX_LOG_D(RFX_LOG_TAG,
        "getUserNameFromSipTelUriString uriString: %s",
    ...
    std::vector<std::string> hostParts = splitString(uriString, ";");
    std::string headParts = hostParts[0];
    std::vector<std::string> addressParts = splitString(headParts, ":");
    std::string addressPart = addressParts[0];
    if(addressParts.size() > 1) {
        addressPart = addressParts[1];
    }
    std::vector<std::string> umberParts = splitString(addressPart, "@");
    std::string address = umberParts[0];//uri.getSchemeSpecificPart();
    if (address.empty()) {
        return "";
    }
    std::string userName = MtkPhoneNumberUtils::getUsernameFromUriNumber(address);
    if (userName.empty()) {
        return "";
    }
...
std::vector<std::string> RtmImsController::splitString(std::string str, std::string c) {
