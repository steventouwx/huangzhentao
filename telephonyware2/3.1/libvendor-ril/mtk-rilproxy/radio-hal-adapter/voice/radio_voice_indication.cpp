...
#define LOG_TAG "RadioVoiceIndication"

RadioVoiceIndication :: RadioVoiceIndication(int slot, RfxDispatchThread* dispatchThread) {
...
ndk::ScopedAStatus RadioVoiceIndication::callRing(RadioIndicationType type, bool isGsm,
    ...
    RLOGD("callRing: type = %d", type);
    Parcel p;
    p.writeInt32((int32_t)(type));
    p.writeInt32(RIL_UNSOL_CALL_RING);
    p.setDataPosition(0);
    mDispatchThread->enqueueUrcMessage(RADIO_TECH_GROUP_GSM, mSlot, RIL_UNSOL_CALL_RING, &p);
...
ndk::ScopedAStatus RadioVoiceIndication::callStateChanged(RadioIndicationType type) {
    ...
    Parcel p;
    p.writeInt32((int32_t)(type));
    p.writeInt32(RIL_UNSOL_RESPONSE_CALL_STATE_CHANGED);
    p.setDataPosition(0);
    mDispatchThread->enqueueUrcMessage(RADIO_TECH_GROUP_GSM, mSlot, RIL_UNSOL_RESPONSE_CALL_STATE_CHANGED, &p);
    return ndk::ScopedAStatus::ok();
}
