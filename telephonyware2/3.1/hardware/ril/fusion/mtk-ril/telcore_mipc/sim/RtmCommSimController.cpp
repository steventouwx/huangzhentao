 ...
 * Include
 *****************************************************************************/
#include "RtmCommSimController.h"
...
#define SIM_MAX_IO_COUNT     5

RFX_IMPLEMENT_CLASS("RtmCommSimController", RtmCommSimController, RfxController);

RFX_REGISTER_DATA_TO_REQUEST_ID(RfxVoidData, RfxIntsData, \
        RFX_MSG_REQUEST_GET_CURRENT_UICC_CARD_PROVISIONING_STATUS);

/*****************************************************************************
 * Class RtmCommSimController
 *****************************************************************************/
RtmCommSimController::RtmCommSimController() : mCacheUAEMsg(NULL), mSimIoCount(0) {
    setTag(String8("RtmCommSimCtrl"));
}


RtmCommSimController::~RtmCommSimController() {
}

void RtmCommSimController::onInit() {
    ...
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_RADIO_STATE,
            RfxStatusChangeCallback(this, &RtmCommSimController::onRadioStateChanged));

    // register callbacks to re-report
    getStatusManager()->registerStatusChanged(RFX_STATUS_MTK_RADIO_CONNECTION_STATE,
            RfxStatusChangeCallback(this, &RtmCommSimController::onConnectionStateChanged));
    ...
    getNonSlotScopeStatusManager()->registerStatusChanged(RFX_STATUS_KEY_MODESWITCH_FINISHED,
            RfxStatusChangeCallback(this, &RtmCommSimController::onModeSwitchFinished));

    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_GSM_CACHE_FCP,
            RfxStatusChangeCallback(this, &RtmCommSimController::onSimFileChanged));
    ...
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_UICC_RESET_APPLICATION,
            RfxStatusChangeCallback(this, &RtmCommSimController::onSimResetChanged));
}

void RtmCommSimController::onRadioStateChanged(RfxStatusKeyEnum key,
...
void RtmCommSimController::onConnectionStateChanged(RfxStatusKeyEnum key,
        RfxVariant old_value, RfxVariant value) {
    int oldState = -1, newState = -1;
    ...
    logD(mTag, "mtk onConnectionStateChanged (%d, %d) (slot %d)", oldState, newState, getSlotId());
    ...
    if (!oldState && newState) {
        ...
        if (mCacheSmlMsg != NULL) {
            ...
            logD(mTag, "mtk onConnectionStateChanged %s %d %d %d %d %d (slot %d)",
            ...
            logD(mTag, "mtk onConnectionStateChanged mCacheSmlMsg == NULL");
        }
    }
    if (newState == 1) {
...
void RtmCommSimController::onModeSwitchFinished(RfxStatusKeyEnum key,
        RfxVariant old_value, RfxVariant value) {
...
void RtmCommSimController::onSimFileChanged(RfxStatusKeyEnum key, RfxVariant old_value,
        RfxVariant value) {
...
void RtmCommSimController::onSimResetChanged(RfxStatusKeyEnum key,
        RfxVariant old_value, RfxVariant value) {
    RFX_UNUSED(key);
...
void RtmCommSimController::resetSimParameters() {
...
bool RtmCommSimController::onCheckIfRejectMessage(
    ...
    if (!isModemPowerOff && (radioState == (int)RADIO_STATE_OFF) &&
            (msgId == RFX_MSG_REQUEST_GET_SIM_STATUS ||
             ...
             msgId == RFX_MSG_REQUEST_SIM_CLOSE_CHANNEL_WITH_SESSION ||
             msgId == RFX_MSG_REQUEST_SWITCH_SIM_TYPE ||
             msgId == RFX_MSG_REQUEST_GET_SIM_TYPE_INFO ||
...
bool RtmCommSimController::isAllowSimIo() {
    if (mSimIoCount < SIM_MAX_IO_COUNT) {
        return true;
    }
    return false;
}

bool RtmCommSimController::onHandleSimIoRequest(const sp<RfxMessage>& msg) {
    int fileId = 0;
    char *path = NULL;
    ...
    path = pData->path;
    mSimIoCount++;
    logD(mTag, "onHandleSimIoRequest, mSimIoCount = %d", mSimIoCount);
    switch(fileId) {
        case 0x6F3A: // ADN
        case 0x6F3B: // FDN
        case 0x6F49: // SDN
        ...
        case 0x6F4C: { // EXT3
                if (path != NULL) {
                    logD(mTag, "SIM IO fileId 2GSIM %d %s", fileId, path);
                    ...
                    if ((fileId != 0x6F3A) ||
                        ...
                        RfxMainThread::enqueueMessage(phbMsg);
                        mSimIoCount--;
                        logD(mTag, "onHandleSimIoRequest-phb-1, mSimIoCount = %d", mSimIoCount);
                    } else {
                        requestToMcl(msg);
                    }
                } else {
                    logD(mTag, "SIM IO fileId 2GSIM %d path is null", fileId);
                    requestToMcl(msg);
                }
            }
            break;
        case 0x4F30: { // EFPBR
                logD(mTag, "SIM IO fileId PBR %d", fileId);
                sp<RfxMessage> phbMsg = RfxMessage::obtainRequest(msg->getSlotId(),
                        RFX_MSG_REQUEST_PHB_PBR_SIM_IO, msg, true);
                phbMsg->setPriority(MTK_RIL_REQUEST_PRIORITY_LOW);
                RfxMainThread::enqueueMessage(phbMsg);
                mSimIoCount--;
                logD(mTag, "onHandleSimIoRequest-phb-2, mSimIoCount = %d", mSimIoCount);
            }
            break;
        default: {
                RtmPhbSimIoController* phbSimIoController = (RtmPhbSimIoController *)findController(
                        m_slot_id, RFX_OBJ_CLASS_INFO(RtmPhbSimIoController));
                if (phbSimIoController->onCheckIfPhbRequest(fileId) == true) {
                    ...
                    RfxMainThread::enqueueMessage(phbMsg);
                    mSimIoCount--;
                    logD(mTag, "onHandleSimIoRequest-phb-3, mSimIoCount = %d", mSimIoCount);
                } else {
...
bool RtmCommSimController::onHandleRequest(const sp<RfxMessage>& msg) {
...
bool RtmCommSimController::onHandleResponse(const sp<RfxMessage>& msg) {
    int msg_id = msg->getId();
    switch (msg_id) {
        case RFX_MSG_REQUEST_GET_SIM_STATUS :
        case RFX_MSG_REQUEST_ENTER_SIM_PIN :
        ...
        case RFX_MSG_REQUEST_SIM_IO : {
                ...
                if (mSimIoCount > 0) {
                    mSimIoCount--;
                    logD(mTag, "onHandleSimIoRequest-Rsp, mSimIoCount = %d", mSimIoCount);
                } else {
                    logE(mTag, "onHandleSimIoRequest-Rsp, Error mSimIoCount = %d", mSimIoCount);
                }
                responseToRilj(msg);
            }
            break;
        ...
        case RFX_MSG_REQUEST_SIM_SAP_ERROR_RESP : {
            ...
            break;
        case RFX_MSG_REQUEST_SWITCH_SIM_TYPE: {
                handlSwitchSimTypeResponse(msg);
            }
            break;
        default:
            logD(mTag, "Not Support the req %s", idToString(msg_id));
...
bool RtmCommSimController::onHandleUrc(const sp<RfxMessage>& message) {
    switch (message->getId()) {
        ...
        case RFX_MSG_URC_SIM_TYPE_INFO_IND: {
            handleSimTypeInfoChanged(message);
            break;
        }
        default:
            return false;
...
bool RtmCommSimController::onPreviewMessage(const sp<RfxMessage>& message) {
    int msgId = message->getId();
    switch (msgId) {
        case RFX_MSG_REQUEST_ENABLE_UICC_APPLICATIONS: {
        ...
        case RFX_MSG_REQUEST_SWITCH_SIM_TYPE: {
            if ((message->getType() == REQUEST)  &&
                    (isEnableUiccApplicationsProcessingInAnySlots() || isSwitchSimTypeProcessing())) {
                return false;
            }
            return true;
        }
        case RFX_MSG_REQUEST_SIM_IO:
            if ((message->getType() == REQUEST) && !isAllowSimIo()) {
                return false;
            } else {
                return true;
            }
...
bool RtmCommSimController::onCheckIfResumeMessage(const sp<RfxMessage>& message) {
    int msgId = message->getId();
    switch (msgId) {
        case RFX_MSG_REQUEST_ENABLE_UICC_APPLICATIONS: {
        ...
        case RFX_MSG_REQUEST_SWITCH_SIM_TYPE: {
            if (!isEnableUiccApplicationsProcessingInAnySlots() && !isSwitchSimTypeProcessing()) {
                return true;
            }
            return false;
        }
...
void RtmCommSimController::handleCallBarring(const sp<RfxMessage>& msg) {
    int msg_id = msg->getId();
    ...
    // Re-enqueue the message into main thread, RtmSuppServController will handle it
...
bool RtmCommSimController::isCallBarringUsage(const sp<RfxMessage>& msg) {
    bool r = false;
...
void RtmCommSimController::handleGetIccid(const sp<RfxMessage>& msg) {
    String8 iccid = getStatusManager()->getString8Value(RFX_STATUS_KEY_SIM_ICCID);
...
void RtmCommSimController::handleGetImsi(const sp<RfxMessage>& msg) {
...
void RtmCommSimController::handleSimIo(const sp<RfxMessage>& msg) {
    RIL_SIM_IO_v6 *pData = (RIL_SIM_IO_v6*)(msg->getData()->getData());
...
void RtmCommSimController::handleGetCurrentUiccCardProcisioningStatus(const sp<RfxMessage>& msg) {
...
bool RtmCommSimController::isEnableUiccApplicationsSupported() {
...
bool RtmCommSimController::isEnableUiccApplicationsProcessing() {
...
bool RtmCommSimController::isEnableUiccApplicationsProcessingInAnySlots() {
...
bool RtmCommSimController::isSwitchSimTypeProcessing() {
    for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
        if ((getStatusManager(i)->getIntValue(
...
void RtmCommSimController::handlSwitchSimType(const sp<RfxMessage>& msg) {
    getStatusManager()->setIntValue(
            RFX_STATUS_KEY_SIM_SWITCH_TYPE_STATE, SIM_TYPE_STATE_PROCESSING);
    requestToMcl(msg);
}

void RtmCommSimController::handlSwitchSimTypeResponse(const sp<RfxMessage>& msg) {
    responseToRilj(msg);
}

void RtmCommSimController::handleEnableUiccApplications(const sp<RfxMessage>& msg) {
    ...
    if (!isEnableUiccApplicationsSupported()
        ...
        sp<RfxMessage> response = RfxMessage::obtainResponse(m_slot_id,
                RFX_MSG_REQUEST_ENABLE_UICC_APPLICATIONS, RIL_E_SUCCESS, RfxVoidData(), msg);
        responseToRilj(response);
        return;
    }
    int keyState =
        getStatusManager()->getIntValue(RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_STATE);
    if (keyState == UICC_APPLICATIONS_ENABLE_STATE_ENABLED && (*enable) == 1) {
        sp<RfxMessage> response = RfxMessage::obtainResponse(RIL_E_SUCCESS, msg);
        responseToRilj(response);
        return;
    }
...
void RtmCommSimController::handleEnableUiccApplicationsResponse(const sp<RfxMessage>& msg) {
    ...
    if (!isEnableUiccApplicationsProcessing()) {
        // For timing, URC is faster than response because of multi-thread in RMM layer - case 1
...
void RtmCommSimController::handleSimTypeInfoChanged(const sp<RfxMessage>& msg) {
    RIL_SimTypeInfo *pData = (RIL_SimTypeInfo*)(msg->getData()->getData());
    if (pData->actionSource == TYPE_USER_HANDLE) {
        getStatusManager()->setIntValue(
                RFX_STATUS_KEY_SIM_SWITCH_TYPE_STATE, SIM_TYPE_STATE_IDLE);
    }

    responseToRilj(msg);
}

void RtmCommSimController::handleUiccApplicationsEnablementChanged(const sp<RfxMessage>& msg) {
    String8 state(PROPERTY_UICC_APPLICATIONS_ENABLE_STATE);
    char *value = NULL;
    int* enable = (int*)(msg->getData()->getData());
    ...
    if ((mCacheUAEMsg != NULL)) {
        ...
        // For timing URC, URC is faster than response because of multi-thread in RMM layer.
