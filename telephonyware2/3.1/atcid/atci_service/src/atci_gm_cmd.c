...
static int read_NVRAM() {
...
#ifndef  MTK_GENERIC_HAL
    int file_lid = AP_CFG_CUSTOM_FILE_FUEL_GAUGE_LID;
#else
    void *hcustlib;
    int *pui;
    ...
    if (!hcustlib) {
        ALOGE("dlopen() fail! errno=%s\n", dlerror());
        return -1;
    }

    pui = (int *) dlsym(hcustlib, "iAP_CFG_CUSTOM_FILE_FUEL_GAUGE_LID");
    if (!pui) {
        ALOGE("dlsym() fail! errno=%s\n", dlerror());
        dlclose(hcustlib);
        return -1;
    }
    int file_lid = *pui;
    ...
    fg_nvram_fd = NVM_GetFileDesc(file_lid, &rec_size, &rec_num, IsRead);
    ...
    if (fg_nvram_fd.iFileDesc > 0) {   /* >0 means ok */
        ...
        if (strnlen(stFGReadback.dsp_dev, maxsize) != 0) {
...
static int write_NVRAM() {
    ...
    if (fg_nvram_fd.iFileDesc > 0) {   /* >0 means ok */
        ...
        if (strnlen(stFGWriteback.dsp_dev, maxsize) != 0) {
...
int car_tune_cmd_handler(char* cmdline, ATOP_t at_op, char* response) {
    ...
    if (meta_adc_fd == -1) {
        ...
        err = sprintf(response, "%s", "ERROR");
	if (err < 0)
		ALOGE("print ERROR err\n");
        return -1;
    }
    ...
    if (at_op == AT_READ_OP) {
        ...
        if (ret == -1) {
            ...
            err = sprintf(response, "%d", (int)adc_out_data[0]);
		if (err < 0)
			ALOGE("print outdata err\n");
            close(meta_adc_fd);
            return 0;
        }
