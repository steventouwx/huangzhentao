...
pthread_mutex_t RfxMclMessage::mcl_token_mutex = PTHREAD_MUTEX_INITIALIZER;
int RfxMclMessage::mclTokenCounter = 0;
...
RfxMclMessage::RfxMclMessage() :
    ...
    mTimeStamp (0),
    mAddAtFront(false),
    mMipcAsynTimeStamp(0) {
}

int RfxMclMessage::generateMclToken() {
    int next = 0;
    pthread_mutex_lock(&mcl_token_mutex);
    next = mclTokenCounter++;
    if (next == intMaxValue) {
         resetToken();
    }
    pthread_mutex_unlock(&mcl_token_mutex);
    return next;
}

void RfxMclMessage::resetToken() {
    pthread_mutex_lock(&mcl_token_mutex);
    mclTokenCounter = 0;
    pthread_mutex_unlock(&mcl_token_mutex);
}
...
RfxMclMessage::~RfxMclMessage() {
    ...
    if (m_mipc_data) delete(m_mipc_data);
}
...
client_id = %d, token = %d, rawUrc = %s, err = %d, \
MainProtocol = %d, priority = %d, mclToken = %d", m_type, RFX_ID_TO_STR(m_id),
            m_id, m_channel_id, m_slot_id, m_client_id, m_token,
           (strlen(raw_urc1_str) == 0 ? (m_raw_urc == NULL ? "" : m_raw_urc->getLine()) : raw_urc1_str),
            m_err, mSendToMainProtocol, m_priority, m_mcl_token);
}
...
sp<RfxMclMessage> RfxMclMessage::obtainMipcUrc(RfxMipcData* data) {
    sp<RfxMclMessage> msg = new  RfxMclMessage();
    msg->m_mipc_data = data;
    msg->m_type = MIPC_URC;
    msg->m_slot_id = RfxMipcUtils:: mipcSlotIdChangeToSlotId(msg->m_mipc_data->getMipcSlotId());
    msg->m_client_id = -1;
    return msg;
}

sp<RfxMclMessage> RfxMclMessage::obtainMipcCmd(RfxMipcData* data) {
    sp<RfxMclMessage> msg = new  RfxMclMessage();
    msg->m_mipc_data = data;
    msg->m_type = MIPC_CMD;
    msg->m_slot_id = RfxMipcUtils:: mipcSlotIdChangeToSlotId(msg->m_mipc_data->getMipcSlotId());
...
sp<RfxMclMessage> RfxMclMessage::obtainEvent(int id, const RfxBaseData &data, int channel_Id,
    ...
    sp<RfxMclMessage> msg = new RfxMclMessage();
    msg->m_type = EVENT_MESSAGE;
    msg->m_id = id;
    msg->m_data = RfxDataCloneManager::copyData(id, &data, EVENT_MESSAGE);
    msg->m_channel_id = channel_Id%RIL_CHANNEL_OFFSET;
    msg->m_slot_id = slot_id;
