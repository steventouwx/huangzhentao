...
GsmSmsAddress *PduParser::getAddress(/*SmsMessage *parent*/) {
    ...
    address = new GsmSmsAddress(mPdu, mCur, lengthBytes);
    mCur += lengthBytes;
...
/*
string PduParser::getUserDataGSM8bit(int byteCount) {
    string ret = "";
...
string PduParser::getUserDataGSM7Bit(int septetCount, int languageTable,
...
string PduParser::getUserDataUCS2(int byteCount) {
    string ret = "";
    WCHAR tempStr[128];
    for (int i = 0; i < byteCount/2; i++) {
        tempStr[i] = *(mPdu + mCur) << 8;
        mCur++;
        tempStr[i] |= *(mPdu + mCur);
        mCur++;
    }
    char *destStr = new char[byteCount];
    int length = WideCharToMultiByte(CP_ACP, 0,
            tempStr, byteCount/2, destStr, byteCount, NULL, NULL);
    destStr[length] = '\0';
    mCur += byteCount;
    ret = destStr;
    delete destStr;
