...
RfxSipRegInfoData::RfxSipRegInfoData(void *data, int length)
    ...
    if (NULL != data) {
        ...
        if (pIn->cseq != NULL) {
            pOut->cseq = strdup(pIn->cseq);
        }
        if (pIn->reason_cause != NULL) {
            pOut->reason_cause = strdup(pIn->reason_cause);
        }
        if (pIn->reason_text != NULL) {
            pOut->reason_text = strdup(pIn->reason_text);
        }
        if (pIn->callId != NULL) {
            pOut->callId = strdup(pIn->callId);
        }
        m_data = (void*)pOut;
        m_length = length;
    }
...
RfxSipRegInfoData::~RfxSipRegInfoData() {
    if (m_data) {
        ...
        if (p->reason_cause != NULL) {
            free(p->reason_cause);
        }
        if (p->reason_text != NULL) {
            free(p->reason_text);
        }
        if (p->callId != NULL) {
            free(p->callId);
