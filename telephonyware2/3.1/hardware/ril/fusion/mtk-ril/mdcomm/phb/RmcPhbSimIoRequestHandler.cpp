...
void RmcPhbSimIoRequestHandler::makePhbSimRspFromUsimFcp(unsigned char ** simResponse, int fileId) {
    int format_wrong = 0;
    ...
    logI(PHBIO_LOG_TAG, "fileId:%d, file_size:%d, rec_len:%d", fileId, fSize.file_size, fDescriptor.rec_len);
        ...
        if (p_response != NULL && p_response->getSuccess() == 1) {
            ...
            if (line->atTokHasmore()) {
                ...
                if ((appTypeId == UICC_APP_USIM || appTypeId == UICC_APP_CSIM ||
                    ...
                    makePhbSimRspFromUsimFcp((unsigned char**)&sr.simResponse, pData->fileid);
                }
