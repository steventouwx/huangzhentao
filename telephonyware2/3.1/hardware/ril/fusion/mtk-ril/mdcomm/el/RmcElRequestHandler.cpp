...
#define RFX_LOG_TAG "RmcElRequestHandler"
// register handler to channel
RFX_IMPLEMENT_HANDLER_CLASS(RmcElRequestHandler, RIL_CMD_PROXY_3);
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxIntsData, RfxEcholocateResponseData,
        RFX_MSG_REQUEST_GET_ECHOLOCATE_METRICS);
RmcElRequestHandler::RmcElRequestHandler(int slot_id, int channel_id) :
...
RmcElRequestHandler::~RmcElRequestHandler() {
}
void RmcElRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
    int id = msg->getId();
...
void RmcElRequestHandler::onHandleTimer() {
    // do something
}
void RmcElRequestHandler::onHandleEvent(const sp<RfxMclMessage>& msg) {
    int id = msg->getId();
    logD(RFX_LOG_TAG, "onHandleRequest: %s(%d)", idToString(id), id);
    switch (id) {
        default:
...
int RmcElRequestHandler::getEcholocateCmd(int index, char** cmdString) {
...
void RmcElRequestHandler::requestGetEcholocateMetrics(const sp<RfxMclMessage>& msg) {
