...
void RtcSuppServController::onInit() {
    ...
    registerToHandleRequest(atci_request_id_list,
            sizeof(atci_request_id_list)/sizeof(const int), HIGH);
...
bool RtcSuppServController::onHandleUrc(const sp<RfxMessage>& message) {
    ...
    switch(msg_id) {
        ...
        case RFX_MSG_UNSOL_ON_USSI: {
            ...
            if (ussdMode == 1) {
                mUssrWaitResponse = true;
                if (!getStatusManager()->getBoolValue(RFX_STATUS_KEY_USSD_SESSION_ONGOING)) {
                    logD(RFX_LOG_TAG, "USSD session ind, token = %d", token);
                    getStatusManager()->setBoolValue(RFX_STATUS_KEY_USSD_SESSION_ONGOING, true);
                }
...
void RtcSuppServController::handleSSResponse(const sp<RfxMessage>& message) {
    ...
    if (msg_id == RFX_MSG_REQUEST_QUERY_CALL_BARRING) {
        logD(RFX_LOG_TAG, "Direct to QUERY_CALL_BARRING to RtcCommSimController");
        msg_id = RFX_MSG_REQUEST_QUERY_FACILITY_LOCK;
        // Re-enqueue the message into main thread, RtcCommSimController will handle the response
        ...
        logD(RFX_LOG_TAG, "Direct to SET_CALL_BARRING to RtcCommSimController");
...
void RtcSuppServController::onSimIccidChanged(RfxStatusKeyEnum key, RfxVariant old_value,
    ...
    string sha256_result = RtcSuppServUtil::sha256(std::string(value.asString8()));

    if (!sha256_result.empty()) {
        // Hash current ICCID to avoid sensitive log/property issues
        int length = snprintf(hashedCurrentIccid, sizeof(hashedCurrentIccid), "%s", sha256_result.c_str());
        if (length < 0) {
            logD(RFX_LOG_TAG, "Hash ICC id error.");
        }
    } else {
        logD(RFX_LOG_TAG, "Sha256 result is empty.");
    }
