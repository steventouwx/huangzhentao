...
// The main test class for MtkRadioEx AIDL Modem.
class MtkRadioExModem : public RadioService {
public:
    MtkRadioExModem(int slot, RfxDispatchThread* dispatchThread);
public:
    /* mtkradioex modem service handle */
    std::shared_ptr<IMtkRadioExModem> mtkradioex_modem;
    /* mtkradioex modem response handle */
    std::shared_ptr<MtkRadioExModemResponse> mtkradioexRsp_modem;
    /* mtkradioex modem indication handle */
    std::shared_ptr<MtkRadioExModemIndication> mtkradioexInd_modem;
public:
  int32_t sendRequestRaw(int32_t in_serial, const std::vector<uint8_t>& in_data, int32_t in_clientId = CLIENT_TBOX) ;
  int32_t setModemPower(int32_t in_serial, bool in_isOn, int32_t in_clientId = CLIENT_TBOX) ;
};