...
RfxMclStatusManager* RfxMclStatusManager::getMclStatusManager(int slot_id) {
    RFX_ASSERT(slot_id >= RFX_SLOT_ID_UNKNOWN && slot_id < (int)(RfxRilUtils::rfxGetSimCount()));
    if (RFX_SLOT_ID_UNKNOWN == slot_id) {
        return s_self[RfxRilUtils::rfxGetSimCount()];
    } else {
        return s_self[(unsigned int)slot_id];
    }
}
...
const String8& RfxMclStatusManager::getString8Value(const RfxStatusKeyEnum key,
    ...
    if (m_status_list[key] == NULL ||
        ...
        return (m_status_list[key]->value).asString8();
    }
}
