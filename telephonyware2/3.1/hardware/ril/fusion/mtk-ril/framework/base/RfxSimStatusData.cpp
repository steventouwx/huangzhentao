...
RfxSimStatusData::RfxSimStatusData(void *_data, int _length) : RfxBaseData(_data, _length) {
    if (_data != NULL) {
        ...
        // Parameter add from android radio hidl v2.0
        /// New aidl structure value. @{
        pData->slotPort.physicalSlotId = pCard->slotPort.physicalSlotId;
        pData->slotPort.portId = pCard->slotPort.portId;
        /// @}
