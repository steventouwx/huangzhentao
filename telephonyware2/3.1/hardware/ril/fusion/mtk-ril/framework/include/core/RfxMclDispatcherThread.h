...
typedef struct MclMessageObj {
      sp<RfxMclMessage> msg;
      struct MclMessageObj *p_next;
} MclMessageObj;

static Mcl_dispatch_queue<MclMessageObj> mclPendingQueue;
...
public:
    ...
    static void addMclMessageToPendingQueue(const sp<RfxMclMessage>& message);

    static sp<RfxMclMessage> getAndRemoveMclMessageFromPendingQueue(int token);

    static sp<RfxMclMessage> getFrontMclMessageFromPendingQueue(void);
...
private:
    ...
    static sp<RfxMclDispatcherThread> s_self;
