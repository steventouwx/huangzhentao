...
#define RFX_LOG_TAG "RtmSbp"

RFX_IMPLEMENT_CLASS("RtmSbpController", RtmSbpController, RfxController);
...
RtmSbpController::RtmSbpController() {
}

RtmSbpController::~RtmSbpController() {
}

void RtmSbpController::onInit() {
    // Required: invoke super class implementation
    RfxController::onInit();
    int sbp_setter = getSetSbpPlace();
    if (sbp_setter == RTM_SBP_CXP_SET_SBP || sbp_setter == RTM_SBP_MODEM_SET_SBP) {
        return;
    }
    for (int i = 0; i < RfxRilUtils::rfxGetSimCount(); i++) {
        getStatusManager(i)->registerStatusChangedEx(RFX_STATUS_KEY_SBP_ID,
                RfxStatusChangeCallbackEx(this, &RtmSbpController::onSbpIdChanged));
        getStatusManager(i)->registerStatusChangedEx(RFX_STATUS_KEY_SBP_SUB_ID,
                RfxStatusChangeCallbackEx(this, &RtmSbpController::onSbpIdChanged));
    }
}

void RtmSbpController::onDeinit() {
    logD(RFX_LOG_TAG, "onDeinit");
    RfxController::onDeinit();
}

bool RtmSbpController::onHandleRequest(const sp<RfxMessage>& message) {
    RFX_UNUSED(message);
    return true;
}

bool RtmSbpController::onHandleUrc(const sp<RfxMessage>& message) {
    RFX_UNUSED(message);
    return true;
}

bool RtmSbpController::onHandleResponse(const sp<RfxMessage>& message) {
    RFX_UNUSED(message);
    return true;
}

bool RtmSbpController::responseToRilj(const sp<RfxMessage>& message) {
    RFX_UNUSED(message);
    return true;
}

void RtmSbpController::onSbpIdChanged(int slot_id, RfxStatusKeyEnum key __unused,
    ...
    if (sbp_setter == RTM_SBP_MODEM_PARTIALLY_SET_SBP && mdver == 1) {
        logI(RFX_LOG_TAG, "onSbpIdChanged[%d] not support, setter:%d, mdver:%d", slot_id,
                sbp_setter, mdver);
        return;
    }
    int switch_mode = getSwitchMode();
    if (switch_mode != RTM_SBP_AUTO_SWITCH && switch_mode != RTM_SBP_ONLY_CHECK_FIRST_SIM) {
        logI(RFX_LOG_TAG, "onSbpIdChanged[%d], don't set because switch_mode:%d",
                slot_id, switch_mode);
        return;
    } else if (RTM_SBP_ONLY_CHECK_FIRST_SIM == switch_mode && getFirstSimCheckedFlag() == 1) {
...
void RtmSbpController::switchSbp(int latest_sbp, int latest_sub_id) {
    int major_slot = getNonSlotScopeStatusManager()->getIntValue(
            RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT, 0);
...
/* Get SBP switch mode
 * Return:
 *  RTM_SBP_AUTO_SWITCH: Auto switch mode
 *  RTM_SBP_ONLY_CHECK_FIRST_SIM: Only check first valid SIM
 *  RTM_SBP_MANUAL_SWITCH: Manual switch mode
 *  RTM_SBP_ALWAYS_ASK: Always ask mode
 */
int RtmSbpController::getSwitchMode() {
 ...
 *  > 0: Operator SBP ID
 */
int RtmSbpController::getCurrentModemSbp() {
    char prop[RFX_PROPERTY_VALUE_MAX] = {0};
 ...
 *  > 0: Operator SBP ID
 */
int RtmSbpController::getSbpFromFile() {
    int fd = 0;
    char sbp_buf[16] = {0};
    int ret = 0;
    int sbp_id = -1;
...
void RtmSbpController::saveSbpIdToFile(int latest_sbp) {
 ...
 *  > 0: Operator SBP ID
 */
int RtmSbpController::getSbpFromProperty() {
    char prop[RFX_PROPERTY_VALUE_MAX] = {0};
 ...
 *  > 0: Operator SBP ID
 */
int RtmSbpController::getCustomSbp() {
    char prop[RFX_PROPERTY_VALUE_MAX] = {0};
 ...
 *  > 0: Operator SBP ID
 */
int RtmSbpController::getCurrentSubId() {
    char prop[RFX_PROPERTY_VALUE_MAX] = {0};
 ...
 */
int RtmSbpController::getSetSbpPlace() {
    char prop[RFX_PROPERTY_VALUE_MAX] = {0};
...
void RtmSbpController::sendRequestResetRadio() {
    int major_slot = getNonSlotScopeStatusManager()->getIntValue(
            RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT, 0);
...
void RtmSbpController::setFirstSimCheckedFlag() {
    rfx_property_set("persist.vendor.radio.sbp_flag", "1");
}

int RtmSbpController::getFirstSimCheckedFlag() {
    char prop[RFX_PROPERTY_VALUE_MAX] = {0};
 ...
 * > 0: SBP ID
 */
int RtmSbpController::getRefinedSbpId(int sbp_id) {
    char prop[RFX_PROPERTY_VALUE_MAX] = {0};
    int *sbp_table;
