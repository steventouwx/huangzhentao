...
RmmVtSharedMemory::RmmVtSharedMemory(void) {
    mLock = PTHREAD_MUTEX_INITIALIZER;
    mPLock = &mLock;
    mCond = PTHREAD_COND_INITIALIZER;
    mDataReadyCount = VT_RIL_SHARE_DATA_STATUS_RECV_NONE;
    mSharedMsg.slot_id = 0;
    mSharedMsg.size = 0;
    mSharedMsg.data = NULL;
}

RmmVtSharedMemory::~RmmVtSharedMemory() {
    if (mSharedMsg.data != NULL) {
        free(mSharedMsg.data);
    }
}

void RmmVtSharedMemory::setState(int state) {
    if (RmmVtReqHandler::isVTLogEnable()) {
...
int RmmVtSharedMemory::getState() {
    return mDataReadyCount;
}

bool RmmVtSharedMemory::checkState(int want_state) {
...
void RmmVtSharedMemory::setSlotId(int id) {
    mSharedMsg.slot_id = id;
}

void RmmVtSharedMemory::setSize(int size) {
    mSharedMsg.size = size;
}

void RmmVtSharedMemory::setData(char* data, int len) {
    mSharedMsg.data = (char*) calloc(len + 1, sizeof(char));
    if (mSharedMsg.data == NULL) {
        RFX_LOG_I(RFX_LOG_TAG, "fail to calloc mSharedMsg.data");
        return;
    }
    memcpy(mSharedMsg.data, data, len);
}

int RmmVtSharedMemory::getSlotId(void) {
    return mSharedMsg.slot_id;
}

int RmmVtSharedMemory::getSize(void) {
    return mSharedMsg.size;
}

void RmmVtSharedMemory::getData(char** data) {
    (*data) = mSharedMsg.data;
}

void RmmVtSharedMemory::clearData() {
    free(mSharedMsg.data);
}

void RmmVtSharedMemory::lock(const char* user) {
...
void RmmVtSharedMemory::unlock(const char* user) {
...
void RmmVtSharedMemory::wait(const char* user, int stay_state) {
...
void RmmVtSharedMemory::signal(const char* user) {
...
sp<RmmVtSharedMemory> RmmVtDataThreadController::sShareMemmory = new RmmVtSharedMemory();
pthread_t  RmmVtDataThreadController::sVtRilThd = 0;
pthread_t  RmmVtDataThreadController::sImcbRilThd = 0;
int RmmVtDataThreadController::sVtRilFd = 0;
int RmmVtDataThreadController::sVtsFd = 0;
int RmmVtDataThreadController::sIsVtConnected = 0;

RmmVtDataThreadController::RmmVtDataThreadController(void) {

    sShareMemmory->setState(VT_RIL_SHARE_DATA_STATUS_RECV_NONE);
}

RmmVtDataThreadController:: ~RmmVtDataThreadController() {
}

sp<RmmVtSharedMemory> RmmVtDataThreadController::getSharedMem(void) {
    return sShareMemmory;
}

void RmmVtDataThreadController::start(void) {
...
void * RmmVtDataThreadController::RIL_IMCB_THREAD(void *arg) {
...
void* RmmVtDataThreadController::VT_RIL_THREAD(void *arg) {
...
int RmmVtDataThreadController::ril_vt_looper() {
    ...
    while (1) {
        ...
        if (sVtsFd > 0 && FD_ISSET(sVtsFd, &rfds)) {
            ...
            if (msg_type == MSG_ID_WRAP_IMSVT_IMCB_GET_CAP_RSP) {
                if (recv_length != sizeof(VT_IMCB_CAP)) {
                    RFX_LOG_E(RFX_LOG_TAG, "[VT RIL THD] unexpected recv_length for MSG_ID_WRAP_IMSVT_IMCB_GET_CAP_RSP");
                    break;
                } else {
                    RFX_LOG_I(RFX_LOG_TAG, "[VT RIL THD] correct recv_length for MSG_ID_WRAP_IMSVT_IMCB_GET_CAP_RSP");
                }
            }
            ...
            RmmVtMsgParser* parser = RmmVtMsgParser::getInstance();
...
int RmmVtDataThreadController::ril_vt_recv(int fd, void* buffer, int size) {
...
bool RmmVtDataThreadController::isImsMessage(int msgId) {
    switch (msgId) {
...
void RmmVtDataThreadController::handleMessage(int msgId, int length, void* data, int slotId) {
    ...
    switch (msgId) {
        case MSG_ID_WRAP_IMSVT_MD_ANBR_REPORT_REQ: {
            ...
            sp<RfxMclMessage> msg = RfxMclMessage::obtainEvent(RFX_MSG_EVENT_REPORT_ANBR,
                    RfxIntsData(anbr_param, sizeof(anbr_param)/sizeof(anbr_param[0])), RIL_CMD_PROXY_2, slotId);
...
void RmmVtDataThreadController::handleImsMessage(int size, char* outBuffer, int slotId) {
