...
void RilOpProxy::initOpLibrary() {
    ...
    if (isMipcSupport() == true) {
        mtkLogI(LOG_TAG, "initOpLibrary, init mipc path");
        sDlOpHandler = dlopen(OP_RIL_MIPC_PATH, RTLD_NOW);
    } else {
        mtkLogI(LOG_TAG, "initOpLibrary, init non mipc path");
        sDlOpHandler = dlopen(OP_RIL_PATH, RTLD_NOW);
    }
    if (sDlOpHandler == NULL) {
        mtkLogI(LOG_TAG, "[%s] dlopen failed error: %s",
...
void RilOpProxy::registerOpService(
    ...
    if (func == NULL) {
        mtkLogI(LOG_TAG, "[%s] destroy not defined or exported error: %s",
