...
#define RFX_LOG_TAG "VT RIL RTM"

/*****************************************************************************
 * Class RtmDataAllowController
 * this is a none slot controller to manage DATA_ALLOW_REQUEST.
 *****************************************************************************/

RFX_IMPLEMENT_CLASS("RtmVtController", RtmVtController, RfxController);
...
static OperatorMap operatorMap[] = {
    ...
    {313450, 313450, 12},    //United States
    {314020, 314020, 446},    //United States
    ...
    {73003, 73003, 120},    //Claro Chile
    {73023, 73023, 120},    //Claro Chile
    ...
    {72406, 72406, 132},   // Vv-Brazil
    {72410, 72411, 132},   // Vv-Brazil
    {72423, 72423, 132},   // Vv-Brazil
    ...
    {73001, 73001, 136},   // Chile - Entel
    {73010, 73010, 136},   // Chile - Entel
    {73026, 73026, 136},   // Chile - Entel
    ...
    {21601, 21601, 156},   // Telenor - Hungark
    ...
    //OP175
    {23801, 23801, 175},   // TDC - Denmark
    {23810, 23810, 175},   // TDC - Denmark
    ...
    // OP220
    {73603, 73603, 220},   //Tigo - Bilivia

    {714020, 714020, 220},   //Tigo - Panama

    // OP242
    {60402, 60402, 242},   //INWI - Morocco

    // OP244
    {45608, 45608, 244},    //Metfone - Cambodia

    // OP277
    {29370, 29370, 277},    //Telemach - Slovenia

    // OP278
    {41001, 41001, 278},    //Jazz - Pakistan
    {41007, 41007, 278},    //Jazz - Pakistan

    // OP312
    {24602, 24602, 312},    //Bite - Lithuania
    {24705, 24705, 312},    //Bite - Latvia

    // OP326
    {62160, 62160, 326},    //9mobile - Nigeria

    // OP331
    {62124, 62124, 331},    //Spectranet - Nigeria

    // OP340
    {53704, 53704, 340},    //Vodafone - Paua New Guinea

    // OP344
    {23103, 23103, 344},    //4ka - Slovakia

    // OP353
    {42509, 42509, 353},    //we4G - Israel

    // OP362
    {40211, 40211, 362},   //B-Mobile Bhutan

    // OP365
    {311340, 311340, 365},   //Illinois Valley Cellular - USA

    // OP367
    {310920, 310920, 367},   //James Valley Wireless - USA

    // OP377
    {60503, 60503, 377},    //Ooredoo - Tunisia

    // OP381
    {41805, 41805, 381},   //Asia Cell - Iraq

    // OP385
    {21890, 21890, 385},   //BH Mobile

    // OP388
    {46015, 46015, 388},   //CBN China

    //OP396
    {311050,311050, 396},  // Thumb Cellular - US
    {311830,311830, 396},  // Thumb Cellular - US
    {310630,310630, 396},  // Thumb Cellular - US

    //OP407
    {72426,72426, 407},  // America net - Brazil

    // OP409
    {36439, 36439, 409},    //BTC Bahamas

    // OP410
    {21805, 21805, 410},    //Mtel - Bosnia and Herzegovina

    // OP411
    {310990, 310990, 411},  // Evolve Broadband - US

    // OP412
    {311530, 311530, 412},   //Standing Rock Telecom - US
    {313930, 313930, 412},   //Standing Rock Telecom - US

    // OP417
    {42410, 42410, 417},    //ATLAS TELECOM - UAE

    // OP419
    {64101, 64101, 419},    //Airtel - Uganda
    {64122, 64122, 419},    //Airtel - Uganda

    // OP423
    {27401, 27401, 423},    //Siminn - Lceland

    // OP430
    {62120, 62120, 430},    //Airtel - Nigeria

    // OP437
    {51089, 51089, 437},    //3 - Indonesia

    // OP439
    {64005, 64005, 439},    //Airtel - Tanzania
    ...
    // OP1004
    {46020, 46020, 1004},    //HW IODT

    // OP1005
    {26295, 26295, 1005},    //Ericsson MFI Lab
};


RtmVtController::RtmVtController() {
...
bool RtmVtController::isVTLogEnable(void) {
    char vt_log[100] = {0};
...
bool RtmVtController::isImsVideoCallon(void) {
...
void RtmVtController::onInit() {

    RfxController::onInit();  // Required: invoke super class implementation

    logD(RFX_LOG_TAG, "[RTM VT REQ HDLR] onInit");

    if (!isImsVideoCallon()) {
        logD(RFX_LOG_TAG, "[RTM VT REQ HDLR] ImsVideoCall trun off");
        return;
    }

    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_UICC_GSM_NUMERIC,
            RfxStatusChangeCallback(this, &RtmVtController::onUiccMccMncChanged));
    ...
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_CARD_TYPE,
            RfxStatusChangeCallback(this, &RtmVtController::onCardTypeChanged));

    //no need update opid to MCL because RMM gets operator ID from modem and saves it.
    ...
    if (RfxRilUtils::getRilRunMode() != RIL_RUN_MODE_MOCK) {
        ...
        logD(RFX_LOG_TAG, "[RTM VT REQ HDLR] send RFX_MSG_EVENT_VT_ENABLE_ANBR, enable");
    }
    /*
    getStatusManager()->registerStatusChanged(
        RFX_STATUS_KEY_NWS_MODE,
        RfxStatusChangeCallback(this, &RtmVtController::onCallStatusChanged));
    */
}

void RtmVtController::onDeinit() {

    logD(RFX_LOG_TAG, "[RTM VT REQ HDLR] onDeinit");
...
bool RtmVtController::onHandleRequest(const sp<RfxMessage>& message) {
...
bool RtmVtController::onHandleResponse(const sp<RfxMessage>& message) {
...
bool RtmVtController::onPreviewMessage(const sp<RfxMessage>& message) {
...
bool RtmVtController::onCheckIfResumeMessage(const sp<RfxMessage>& message) {
    // false mean FWK can send the message now
    return false;
}

void RtmVtController::handleGetInfoRequest(const sp<RfxMessage>& request) {
...
void RtmVtController::handleGetInfoResponse(const sp<RfxMessage>& response) {
...
void RtmVtController::handleUpdateOpidResponse(const sp<RfxMessage>& response) {
...
void RtmVtController::onCallStatusChanged(RfxStatusKeyEnum key, RfxVariant old_value, RfxVariant new_value) {
...
void RtmVtController::onUiccMccMncChanged(RfxStatusKeyEnum key, RfxVariant old_value, RfxVariant value) {
    ...
    char test_sim[100] = {0};
    rfx_property_get("vendor.gsm.sim.ril.testsim", test_sim, "0");

    if (atoi(test_sim) == 1 && key == RFX_STATUS_KEY_UICC_CDMA_NUMERIC &&
            strcmp(mccmncStr.string(), "11111") == 0) {
        logD(RFX_LOG_TAG, "ignore CDMA MCCMNC if test SIM and invaild CMDA MCCMNC");
        return;
    }
...
void RtmVtController::onCardTypeChanged(RfxStatusKeyEnum key, RfxVariant oldValue, RfxVariant newValue) {
    ...
    */
    ...
    if (cardType < 0) {
        ...
        //no need to set operator ID from RTM to RMM, because RMM gets operator ID from modem saves it.
        //updateOpId(mccmnc);
    }
}

void RtmVtController::updateOpId(int mccmnc) {
