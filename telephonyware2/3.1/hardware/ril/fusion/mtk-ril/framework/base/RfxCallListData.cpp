...
RfxCallListData::RfxCallListData(void *data, int length) : RfxBaseData(data, length)  {
    ...
    if (countOfCall > 0) {
        ...
        for (int i = 0; i < countOfCall; i++) {
            ...
            if (tmp[i]->forwardedNumber != NULL) {
                int len = strlen(tmp[i]->forwardedNumber);
                //Allocate and zero-initialize array
                pp_calls[i]->forwardedNumber =
                        (char *)calloc(len + 1, sizeof(char));
                RFX_ASSERT(pp_calls[i]->forwardedNumber != NULL);
                strncpy(pp_calls[i]->forwardedNumber,
                        tmp[i]->forwardedNumber, len);
            }
            if (tmp[i]->uusInfo != NULL) {
                pp_calls[i]->uusInfo = (RIL_UUS_Info *)calloc(1, sizeof(RIL_UUS_Info));
                memcpy(pp_calls[i]->uusInfo, tmp[i]->uusInfo, sizeof(RIL_UUS_Info));
                if (tmp[i]->uusInfo->uusLength <= 0) {
                    pp_calls[i]->uusInfo->uusData = NULL;
                } else {
                    pp_calls[i]->uusInfo->uusData = (char *)calloc(tmp[i]->uusInfo->uusLength + 1, sizeof(char));
                    if (tmp[i]->uusInfo->uusData != NULL) {
                        memcpy(pp_calls[i]->uusInfo->uusData, tmp[i]->uusInfo->uusData,
                            tmp[i]->uusInfo->uusLength);
                    }
                }
            }
        }
    ...
    if (pp_calls != NULL) {
        for (int i = 0; i < countOfCall; i++) {
            if (pp_calls[i] != NULL) {
                ...
                if (pp_calls[i]->forwardedNumber != NULL) {
                    free(pp_calls[i]->forwardedNumber);
                }
                if (pp_calls[i]->uusInfo != NULL) {
                    if (pp_calls[i]->uusInfo->uusData != NULL) {
                        free(pp_calls[i]->uusInfo->uusData);
                    }
                    free(pp_calls[i]->uusInfo);
                }
                free(pp_calls[i]);
            }
        }
