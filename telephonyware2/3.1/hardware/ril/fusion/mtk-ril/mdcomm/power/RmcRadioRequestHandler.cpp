...
RmcRadioRequestHandler::RmcRadioRequestHandler(int slotId, int channelId) :
    ...
    // Enable the SBP ETWS feature
    response = atSendCommand("AT+ESBP=5,\"SBP_ETWS_FOR_AOSP_DESIGN\",1");
    ...
    if (mainSlotId == slotId) {
        ...
        // check main slot radio state
        int retryCount = 0;
        RIL_RadioState state = (RIL_RadioState) getMclStatusManager(mainSlotId)->getIntValue(
                RFX_STATUS_KEY_RADIO_STATE, 1);

        while (state == RADIO_STATE_UNAVAILABLE) {
            logE(RFX_LOG_TAG, "main slot is not radio available, retryCount = %d", retryCount);
            usleep(100*1000);
            retryCount++;
            state = (RIL_RadioState) getMclStatusManager(mainSlotId)->getIntValue(
                    RFX_STATUS_KEY_RADIO_STATE, 1);
            if (retryCount > 100) {
                break;
            }
        ...
        // set non-main slot radio available after main slot radio available
        getMclStatusManager()->setIntValue(RFX_STATUS_KEY_RADIO_STATE, RADIO_STATE_OFF);
    }
}
...
void RmcRadioRequestHandler::handleEccIndication(int slotId, int onOff, int forEcc, int preEcc) {
    RFX_UNUSED(preEcc);
    sp<RfxAtResponse> response;
    bool isAtSent = false;
    ...
    if (!currEccPowerOnStaus && onOff == 1 && forEcc == 1) {
        if (RfxRilUtils::isEmciSupport()) {
            response = atSendCommand(String8().format("AT+EMCI=1"));
            isAtSent = true;
        } else if ((isSendEmcsBeforeTurnOnRadio()) && ((RfxRilUtils::getMajorSim() - 1) == slotId)){
            if (isOp12Support() || isOp07Support() || isOp236Support()) {
                if (!getNonSlotMclStatusManager()->getBoolValue(
                            RFX_STATUS_KEY_EMERGENCY_MODE_IN_FLIGHT_MODE, false)) {
                    response = atSendCommand(String8().format("AT+EMCS=1,1"));
                    getNonSlotMclStatusManager()->setBoolValue(
        ...
        setEccPowerOnStatus(true);
        logI(RFX_LOG_TAG, "update ecc power on status (true)");
    } else if (currEccPowerOnStaus && forEcc == 0) {
        if (RfxRilUtils::isEmciSupport()) {
            response = atSendCommand(String8().format("AT+EMCI=0"));
            isAtSent = true;
        }
        setEccPowerOnStatus(false);
        logI(RFX_LOG_TAG, "reset ecc power on status (false)");
    }

    if (isAtSent && response->getSuccess() == 0) {
        logE(RFX_LOG_TAG, "ecc indication failed (slot: %d, cause: %d)",
                slotId, response->atGetCmeError());
    }
}

/** M: CC: Ecc power on status update function */
void RmcRadioRequestHandler::setEccPowerOnStatus(bool isOnForEcc) {
    getNonSlotMclStatusManager()->setBoolValue(
            RFX_STATUS_KEY_RADIO_POWER_ON_FOR_ECC, isOnForEcc);
}

bool RmcRadioRequestHandler::getEccPowerOnStatus() {
...
void RmcRadioRequestHandler::updateEccPhone(int slotId, int preEcc) {
    if (slotId < 0 || slotId >= RfxRilUtils::rfxGetSimCount()) {
...
void RmcRadioRequestHandler::updateDsdaUrcEnable() {
    atSendCommand("AT+EDSDA=1");
}
    ...
    */
    ...
    do {
        ...
        if (MD_STA_EV_RESET_REQUEST == buffer->event_type
                ...
                || MD_STA_EV_EXCEPTION == buffer->event_type) {
            // LibDumpRildInfo::AddDumpMessage("radioPower",
            //        String8::format("MODEM State:%d", buffer->event_type));
...
int RmcRadioRequestHandler::isNormalBootUp() {
    ...
    logI(RFX_LOG_TAG, "isNormalBootUp = %d, boot_reason = %s", ret, boot_reason);
...
void RmcRadioRequestHandler::sendEGRAT() {
    int raf = 0;
    int rat;
