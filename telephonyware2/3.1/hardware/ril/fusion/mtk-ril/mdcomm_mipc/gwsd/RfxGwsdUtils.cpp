...
#define GWSD_RIL_V3_PATH "libgwsdv3-ril.so"
...
void RfxGwsdUtils::initHandler() {
    ...
    char gwsdver[RFX_PROPERTY_VALUE_MAX] = { 0 };
    rfx_property_get("ro.vendor.mtk_gwsd_capability", gwsdver, "0");

    //GWSD 2.0+MIPC
    if (atoi(gwsdver) == 2) {
        m_dlHandler = dlopen(GWSD_RIL_V3_PATH, RTLD_NOW);
    }

    if (m_dlHandler == NULL) {
        RFX_LOG_I(RFX_LOG_TAG, "[%s] %s", __FUNCTION__, dlerror());
        return;
    }
    ...
    if (createSetting == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "[%s] create not defined or exported in %s: %s",
                __FUNCTION__, GWSD_RIL_V3_PATH, dlsym_error);
        return;
    }
