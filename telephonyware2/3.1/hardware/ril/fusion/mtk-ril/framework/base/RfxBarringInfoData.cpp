...
RFX_IMPLEMENT_DATA_CLASS(RfxBarringInfoData);

RfxBarringInfoData::RfxBarringInfoData(void *data, int length) : RfxBaseData(data, length)  {
    if (data != NULL) {
        RIL_BarringInforResponse*tmp = (RIL_BarringInforResponse *) data;
        RIL_BarringInforResponse *pData =
                (RIL_BarringInforResponse *) calloc(1, length);
...
RfxBarringInfoData::~RfxBarringInfoData() {
    if (m_data) {
        RIL_BarringInforResponse *tmp = (RIL_BarringInforResponse *) m_data;
