...
#else
char g_NVRAM_BACKUP_DEVICE[128] = "/dev/disk/by-partlabel/nvram";
char g_LOG_DEVICE[128] = "/dev/disk/by-partlabel/nvram";
char g_MISC_DEVICE[128] = "/dev/disk/by-partlabel/para";
char g_NVRAM_PROINFO_DEVICE[128] = "/dev/disk/by-partlabel/proinfo";
#endif
...
//   int iBackup_Partition=get_partition_numb("nvram");
	...
	length = snprintf(cMtdDevName, sizeof(cMtdDevName),"%s", g_NVRAM_BACKUP_DEVICE);
	if (length < 0) {
		NVRAM_LOG("%s:snprintf failed \n", __func__);
	} else if (length >= sizeof(cMtdDevName)) {
		NVRAM_LOG("%s:snprintf truncated \n", __func__);
	}
...
int NVM_GetLIDByName(char* filename) {
	...
	if (gFileNameArray == NULL) {
		...
		for ( i = 0; i < g_i4CFG_File_Count; i++) {
			...
			length = snprintf(gFileNameArray[i].cFileName,
				sizeof(g_akCFG_File[i].cFileName),"%s",
				g_akCFG_File[i].cFileName);
			if (length < 0) {
				NVRAM_LOG("%s:snprintf failed \n", __func__);
			} else if (length >= sizeof(g_akCFG_File[i].cFileName)) {
				NVRAM_LOG("%s:snprintf truncated \n", __func__);
			}
		...
		for ( i = g_i4CFG_File_Count ;
			...
			length = snprintf(gFileNameArray[i].cFileName,
				sizeof(gFileNameArray[i].cFileName),
				"%s",g_akCFG_File_Custom[i-iCustomBeginLID].cFileName);
			if (length < 0) {
				NVRAM_LOG("%s:snprintf failed \n", __func__);
			} else if (length >= sizeof(gFileNameArray[i].cFileName)) {
				NVRAM_LOG("%s:snprintf truncated \n", __func__);
			}
			gFileNameArray[i].iLID = i;
	...
	for (iFilNO = iFileVerInfoLID; iFilNO < iCustomBeginLID; iFilNO++) {
		...
		for (i = iStrLen; i >= 0; i--) {
			if (g_akCFG_File[iFilNO].cFileName[i] == '/') {
				...
				length = snprintf(tempstr, sizeof(tempstr),"%s",
					g_akCFG_File[iFilNO].cFileName+i+1);
				if (length < 0) {
					NVRAM_LOG("%s:snprintf failed \n", __func__);
				} else if (length >= sizeof(tempstr)) {
					NVRAM_LOG("%s:snprintf truncated \n", __func__);
				}
				break;
			}
	...
	if (UpdateFlag == VerUpdate) {
		...
		for (iIndex = iStrLen; iIndex >= 0; iIndex--) {
			if ( pCfgFileTable[iRealFileLid].cFileName[iIndex] == '/') {
				...
				length = snprintf(tempstr1, sizeof(tempstr1),"%s",
					pCfgFileTable[iRealFileLid].cFileName + iIndex + 1);
				if (length < 0) {
					NVRAM_LOG("%s:snprintf failed \n", __func__);
				} else if (length >= sizeof(tempstr1)) {
					NVRAM_LOG("%s:snprintf truncated \n", __func__);
				}
				break;
			}
			...
			if (bIsNewVerInfo) {
				...
				for (i = iStrLen; i >= 0; i--) {
					if (g_akCFG_File[file_lid].cFileName[i] == '/') {
						...
						length = snprintf(tempstr, sizeof(tempstr),"%s", g_akCFG_File[file_lid].cFileName+i+1);
						if (length < 0) {
							NVRAM_LOG("%s:snprintf failed \n", __func__);
						} else if (length >= sizeof(tempstr)) {
							NVRAM_LOG("%s:snprintf truncated \n", __func__);
		...
		if (bAddedAtend) {
			...
			if (file_lid < iCustomBeginLID) {
				if (bIsNewVerInfo) {
					...
					for (i = iStrLen; i >= 0; i--) {
						if (g_akCFG_File[file_lid].cFileName[i] == '/') {
                                                        iResult = snprintf(tempstr, sizeof(tempstr), "%s",
                                                                           g_akCFG_File[file_lid].cFileName+i+1);
                                                        if (iResult < 0)
                                                                NVRAM_LOG("%s: snprintf failed \n", __func__);
                                                        else if (iResult >= sizeof(tempstr))
                                                                NVRAM_LOG("%s: snprintf truncated \n", __func__);
                                                        break;
						}
					}
...
restore:
	for (j = strlen(pCfgFielTable[iRealFileLid].cFileName) - 1; j >= 0; j--)
		if (pCfgFielTable[iRealFileLid].cFileName[j] == '/') {
			...
			if (length < 0) {
				NVRAM_LOG("%s:snprintf failed \n", __func__);
			} else if (length >= sizeof(filename)) {
				NVRAM_LOG("%s:snprintf truncated \n", __func__);
			}
...
F_ID NVM_GetFileDesc(int file_lid, int *pRecSize, int *pRecNum, bool IsRead) {
	...
	/*
	 * At 8678, we don't want upper layer to access WIFI/BT from yocto side because
	 * file definition of WIFI/BT is different at yocto and android.
	 */
        NVRAM_LOG("deny access to WIFI/BT\n");
        if (!strcmp(pCfgFielTable[iRealFileLid].cFileName, "/mnt/vendor/nvdata/APCFG/APRDEB/BT_Addr")
            || !strcmp(pCfgFielTable[iRealFileLid].cFileName, "/mnt/vendor/nvdata/APCFG/APRDEB/WIFI")) {
                NVRAM_LOG("try to access WIFI or BT_Addr, denied\n");
                return FileID;
        }
...
Solve_Special_Lid:
	...
	length = snprintf(cMtdDevName, sizeof(cMtdDevName),"%s", g_NVRAM_PROINFO_DEVICE);
	if (length < 0) {
		NVRAM_LOG("%s:snprintf failed \n", __func__);
	} else if (length >= sizeof(cMtdDevName)) {
		NVRAM_LOG("%s:snprintf truncated \n", __func__);
...
unsigned int NVM_GetSequenceNum(void) {
	...
	memset(cMtdDevName, 0, sizeof cMtdDevName);
	if (nvram_platform_layout_version == 0) {
		length = snprintf(cMtdDevName, sizeof(cMtdDevName),"%s", g_LOG_DEVICE);
		if (length < 0) {
			NVRAM_LOG("%s:snprintf failed \n", __func__);
		} else if (length >= sizeof(cMtdDevName)) {
			NVRAM_LOG("%s:snprintf truncated \n", __func__);
...
bool NVM_IncSequenceNum(unsigned int sequence) {
	...
	tm = time(&tm);
	if (tm == (time_t)-1) {
		NVRAM_LOG("NVM_IncSequenceNum: get the current time fail!!!\n");
		free(journal);
		free(buffer);
		return false;
	}
...
bool NVM_HistoryLog_add_file(const char *fix_dir, const char *pre_fix_file,nvram_journal_handle_t *journal)
{
     ...
     int size=0;
     int read_size=0;
     char acSrcPathName[MAX_NAMESIZE] = {0};
     int fd;
    ...
    while(1)
    {
		...
		if (sprintf(acSrcPathName, "%s/%s", fix_dir, entry->d_name) < 0) {
			NVRAM_LOG("%s:sprintf failed \n", __func__);
			closedir(dir);
			return false;
		}
...
{//add
    ...
    length = snprintf(journal->h_log, MAX_LOG_LENGTH, "%s:%s", time, log);
    if (length < 0) {
	    NVRAM_LOG("%s:snprintf failed \n", __func__);
    } else if (length >= MAX_LOG_LENGTH) {
	    NVRAM_LOG("%s:snprintf truncated \n", __func__);
    }
...
bool NVM_HistoryLog(unsigned int level, const char *func, unsigned int line,
	...
	memcpy(journal->h_func_name, func , strlen(func));
	length = snprintf(journal->h_log, MAX_LOG_LENGTH, "%s:%s", ctime(&tm), log);
	if (length < 0) {
		NVRAM_LOG("%s:snprintf failed \n", __func__);
	} else if (length >= MAX_LOG_LENGTH) {
		NVRAM_LOG("%s:snprintf truncated \n", __func__);
...
unsigned int NVM_MiscGetSeqNum(void) {
	...
	length = snprintf(cMtdDevName, sizeof(cMtdDevName),"%s", g_MISC_DEVICE);
	if (length < 0) {
		NVRAM_LOG("%s:snprintf failed \n", __func__);
	} else if (length >= sizeof(cMtdDevName)) {
		NVRAM_LOG("%s:snprintf truncated \n", __func__);
...
bool NVM_MiscIncSeqNum(unsigned int sequence) {
	...
	tm = time(&tm);
	if (tm == (time_t)-1) {
		NVRAM_LOG("NVM_MiscIncSeqNum: get the current time fail!!!\n");
		free(journal);
		free(buffer);
		return false;
	}
