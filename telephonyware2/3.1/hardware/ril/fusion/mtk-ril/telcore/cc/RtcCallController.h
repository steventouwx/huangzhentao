...
/*****************************************************************************
 * Class ForwardNumberInfo
 *****************************************************************************/
class ForwardNumberInfo {
public:
    ForwardNumberInfo(int callId, char * forwardedNumber)
            : mCallId(callId), mForwardedNumber(forwardedNumber) {
    }
...
public:
    const char* getForwardedNumber(int callId) {
        if (callId == mCallId) {
            return mForwardedNumber.string();
        } else {
            return NULL;
        }
    }
...
protected:
    ...
    virtual void createRedialController(RtcEccController *eccController);
...
private:
    ...
    void onWFCPDNStatusChanged(RfxStatusKeyEnum key, RfxVariant oldValue, RfxVariant newValue);

    void onRadioStateChanged(RfxStatusKeyEnum key, RfxVariant old_value, RfxVariant value);
    void handleGetSimultaneousCallingSupport(const sp<RfxMessage>& message);
    void onCardTypeChanged(RfxStatusKeyEnum key, RfxVariant oldValue, RfxVariant newValue);
    bool isCsCallExistAndNoEccExist();
    bool isCsActiveOrIncomingCallExistAndNoEccExist();
    bool isCdmaCallExist();
    ...
    void updateImsCallRat(int dataRegState, int dataRadioTech, int wfcRegState);
    ...
    CallType getConnectionCallType(int callId);
    ...
    void handleLastFailCauseResponse(const sp<RfxMessage>& message);
    ...
    Vector<RfxImsCallInfo*> getHoldCallOnOtherSub() ;
    ...
    void updateRQ(int RQ);

    void onAutoFakeAcceptTimer();

    void onSendDelayCallInfoTimer();
    uint8_t* obtainCallPrefRawBytes(int callId, int discCause);
    void stopAutoAnswerTimer(int callId);
    bool isExistDisallowHoldVideoOnOtherSub();
    void hangupAllCallsOnOtherSub();
    bool canResumeHandleAnswerRequest(const sp<RfxMessage>& message);
    void hangupHoldOnOtherSub(Vector<RfxImsCallInfo*> holdingCalls);
    ...
    int mAutoAnswerVideoMode;

    bool mWaitForSwitchCallResp;
    std::queue<int> mDisconnectedImsCallIdQueue;
    ...
    const char * const PAU_NUMBER_FIELD     = "<tel:";
    const char * const PAU_NAME_FIELD       = "<name:";
    const char * const PAU_SIP_NUMBER_FIELD = "<sip:";
    const char * const PAU_END_FLAG_FIELD    = ">";
    ...
    RtcEccController *mEccController;
    void createEccController();
    void onRadioPowerEccStatusChanged(RfxStatusKeyEnum key,
            RfxVariant oldValue, RfxVariant newValue);
    void onRadioOnForEccTimeout();
    void cancelRadioOnForEcc();
    void setEccPowerOnStatus(bool isOnForEcc);
    bool getEccPowerOnStatus();
    void setLocalEccIndication(bool onoff);
    Vector<ForwardNumberInfo> mForwardNumbers;
    void fillForwardedNumberInfo(const sp<RfxMessage>& message);
    void removeForwardedNumberInfo(int callId);
    int pendingOrResumeEcpi(const sp<RfxMessage>& message);
};
