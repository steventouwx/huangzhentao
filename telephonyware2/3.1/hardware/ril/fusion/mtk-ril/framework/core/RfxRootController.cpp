...
bool RfxRootController::processMessage(const sp<RfxMessage>& message) {
    ...
    if ((message->getType() == REQUEST || message->getType() == RESPONSE ||
        ...
        RFX_LOG_E(RFX_LOG_TAG, "no RfxBaseData, send error to RILJ  msg = [%s]", message->toString().string());
    ...
    if (message->getType() == REQUEST || message->getType() == SAP_REQUEST) {
        ...
        RFX_LOG_V(RFX_LOG_TAG, "Status Sync Message!");
    ...
    if (handler == NULL) {
        ...
        if ((REQUEST == message->getType()) &&
                handler->checkIfRejectMessage(message, isModemPowerOff, radioState)) {
...
#if defined(MTK_ONY_FOR_RILD_DEBUG)
    for (size_t i = 0; i < length; i++) {
        int id = urc_id_list[i];
        int ret = RfxDataCloneManager::checkDataIfRegister(id, URC);
        if (ret == 0) {
            RFX_LOG_E(RFX_LOG_TAG, "registerToHandleUrc fail :%d(%s)", id, RFX_ID_TO_STR(id));
        }
    }
#endif
...
RfxStatusManager *RfxRootController::getStatusManager(unsigned int slot_id) const {
    RFX_ASSERT(slot_id == RFX_SLOT_ID_UNKNOWN || (slot_id >= 0 && slot_id <= RFX_SLOT_COUNT));
    if (slot_id == RFX_SLOT_ID_UNKNOWN) {
        return m_status_managers[(unsigned int)RFX_SLOT_COUNT];
    } else {
        return m_status_managers[slot_id];
    }
}

RfxController *RfxRootController::getSlotRootController(unsigned int slot_id) const {
    RFX_ASSERT(slot_id == RFX_SLOT_ID_UNKNOWN || (slot_id >= 0 && slot_id <= RFX_SLOT_COUNT));
    if (slot_id == RFX_SLOT_ID_UNKNOWN) {
        return m_slot_root_controllers[(unsigned int)RFX_SLOT_COUNT];
    } else {
        return m_slot_root_controllers[slot_id];
    }
}

void RfxRootController::setSlotRootController(unsigned int slot_id, RfxController *slot_root) {
    RFX_ASSERT(slot_id == RFX_SLOT_ID_UNKNOWN || (slot_id >= 0 && slot_id <= RFX_SLOT_COUNT));
    if (slot_id == RFX_SLOT_ID_UNKNOWN) {
...
void RfxRootController::onInit() {
    ...
    RFX_OBJ_CREATE_EX(m_status_managers[(unsigned int)RFX_SLOT_COUNT], RfxStatusManager,
...
#if defined(MTK_ONY_FOR_RILD_DEBUG)
        RfxController *dupController = findMsgHandler(id_list[i], slot_id, list, priority, token);
        if (dupController != NULL) {
            RFX_LOG_E(RFX_LOG_TAG, "duplicated register controller = %s, dupController = %s, slot = %d id = %d(%s)",
                    controller->getClassInfo()->getClassName(),
                    dupController ? dupController->getClassInfo()->getClassName() : "",
                    slot_id, id_list[i], RFX_ID_TO_STR(id_list[i]));
        }
#endif
        ptr = controller;
