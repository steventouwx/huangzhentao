...
#define WP_LOG_TAG "RmmWp"
// register handler to channel
RFX_IMPLEMENT_HANDLER_CLASS(RmmWpRequestHandler, RIL_CMD_PROXY_1);

int RmmWpRequestHandler::bWorldModeSwitching = 0;
int RmmWpRequestHandler::ecsraUrcParams[5] = {0};
RIL_RadioState RmmWpRequestHandler::radioStateBackup[MAX_SIM_COUNT] ={RADIO_STATE_OFF};

RmmWpRequestHandler::RmmWpRequestHandler(int slot_id, int channel_id):RfxBaseHandler(slot_id, channel_id) {
    ...
    // Don't support TDS on Gen98
    // worldPhoneInitialize(slot_id);
}

RmmWpRequestHandler::~RmmWpRequestHandler() {
}

void RmmWpRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
...
void RmmWpRequestHandler::requestResumingWorldMode(const sp<RfxMclMessage>& msg) {
    // Don't support TDS from Gen98
    // Handle ECSRA in L5 if needed in future
    logD(WP_LOG_TAG, "Should not be here");
    /*
    for (int i = 0; i < 5; i++) {
        ecsraUrcParams[i] = RmmWpURCHandler::getEcsraUrcParam(i);
    }
    */
...
void RmmWpRequestHandler::handleWorldModeChanged(const sp<RfxMclMessage>& msg) {
    int err;
    int reterr[2] = {0};
...
int RmmWpRequestHandler::isWorldModeSwitching() {
...
void RmmWpRequestHandler::onWorldModeStateChanged(int state) {
    if (1 == state) {
...
void RmmWpRequestHandler::setActiveModemType() {
    int mdType = -1;
...
#endif
    len = sprintf(activeModem, "%d", mdType);
    if (len >= 0) {
        rfx_property_set("vendor.ril.active.md", activeModem);
        logD(WP_LOG_TAG, "[setActiveModemType] activeModem = %s", activeModem);
    } else {
        logD(WP_LOG_TAG, "[setActiveModemType] error: %d, %s, %d", len, activeModem, mdType);
    }

    return;
}

int RmmWpRequestHandler::getActiveModemType() {
    int activeModem = 0;
...
int RmmWpRequestHandler::queryActiveMode() {
    // Gen98 don't support TDS, always on TDD mode
...
WorldPhonePolicyType RmmWpRequestHandler::getWorldPhonePolicy() {
...
void RmmWpRequestHandler::setWorldPhonePolicy() {
...
int RmmWpRequestHandler::isSvlteLcgSupport() {
...
SVLTE_PROJ_TYPE RmmWpRequestHandler::getSvlteProjectType() {
