...
#ifndef __RTM_GSM_SMS_CONTROLLER_H__
#define __RTM_GSM_SMS_CONTROLLER_H__
...
class RtmGsmSmsController : public RfxController {
RFX_DECLARE_CLASS(RtmGsmSmsController);

public:
    RtmGsmSmsController();
    virtual ~RtmGsmSmsController();
...
private:
    ...
    void onSmsMdReadyTimer();

private:
    String8 mTag;
    bool mSmsMdReady;
    bool mSmsMdReadyState;
    TimerHandle mSmsTimerHandle;
    TimerHandle mSmsMdReadyTimerHandle;
    bool mSmsSending;
    bool mNeedStatusReport; // only for ims sms
    TimerHandle m_timer;
    const int MT_GSM_SMS_ACK_TIME = 10;
    const int SMS_MD_READY_TIME = 120;
#define GSM_SMS_MESSAGE_STRS_COUNT 2 // 0: smsc, 1: pdu
#define DELAY_SET_SMS_FWK_READY_TIMER 4000 //4s

};
#endif /* __RTM_GSM_SMS_CONTROLLER_H__ */
