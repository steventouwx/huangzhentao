...
bool RtcModemController::canHandleRequest(const sp<RfxMessage>& message) {
    ...
    if (msg_id == RFX_MSG_REQUEST_SHUTDOWN) {
        if (isDataConnected() == true) {
            logD(RFX_LOG_TAG, "canHandleRequest: false, data is connected");
            return false;
        }
    }
...
bool RtcModemController::isDataConnected() {
    bool ret = false;
    for (int i = RFX_SLOT_ID_0; i < RfxRilUtils::rfxGetSimCount(); i++) {
        int status = getStatusManager(i)->getIntValue(
                RFX_STATUS_KEY_DATA_CONNECTION_EXP_IMS, DATA_STATE_DISCONNECTED);
        if (status == DATA_STATE_CONNECTED) {
            ret = true;
            break;
        }
    }
