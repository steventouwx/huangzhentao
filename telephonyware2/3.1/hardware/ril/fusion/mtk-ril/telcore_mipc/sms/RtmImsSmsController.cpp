...
RFX_IMPLEMENT_CLASS("RtmImsSmsController", RtmImsSmsController, RfxController);
...
/*****************************************************************************
 * Class RtmImsSmsController
 *****************************************************************************/
RtmImsSmsController::RtmImsSmsController() {
        setTag(String8("RtmImsSmsCtrl"));
        mGsmCtrl = NULL;
        mCdmaCtrl = NULL;
        mConCatSmsRoot = NULL;
}
...
void RtmImsSmsController::onInit() {
    // Required: invoke super class implementation
    RfxController::onInit();

    RFX_OBJ_CREATE(mGsmCtrl, RtmGsmSmsController, this);
    RFX_OBJ_CREATE(mCdmaCtrl, RtmCdmaSmsController, this);
    RFX_OBJ_CREATE(mConCatSmsRoot, RtmConCatSmsRoot, this);
...
bool RtmImsSmsController::onCheckIfRejectMessage(
...
bool RtmImsSmsController::onHandleRequest(const sp<RfxMessage>& msg) {
        ...
        case RFX_MSG_REQUEST_IMS_SEND_SMS_EX:
            {
                ...
                if (pIms->tech == RADIO_TECH_3GPP) {
                    ...
                    if (isInCdmaCall()) {
                        sp<RfxMessage> rsp =
                                RfxMessage::obtainResponse(RIL_E_SMS_SEND_FAIL_RETRY, msg, false);
                        responseToRilj(rsp);
                        break;
                    }
...
bool RtmImsSmsController::onPreviewMessage(const sp<RfxMessage>& msg) {
    int msg_id = msg->getId();
    switch (msg_id) {
        case RFX_MSG_REQUEST_IMS_SEND_SMS:
        case RFX_MSG_REQUEST_IMS_SEND_SMS_EX:
...
bool RtmImsSmsController::onCheckIfResumeMessage(const sp<RfxMessage>& msg) {
    int msg_id = msg->getId();
    switch (msg_id) {
        case RFX_MSG_REQUEST_IMS_SEND_SMS:
        case RFX_MSG_REQUEST_IMS_SEND_SMS_EX:
...
bool RtmImsSmsController::removeReferenceIdCached(int ref) {
...
void RtmImsSmsController::addReferenceId(int ref) {
    mImsSmsRefIdVector.push_back(ref);
}

int RtmImsSmsController::getCacheSize() {
    return mImsSmsRefIdVector.size();
}

void RtmImsSmsController::sendCdmaSms(RtmCdmaSmsMessage *msg) {
    RFX_ASSERT(msg != NULL);
    mCdmaCtrl->sendCdmaSms(msg);
}

void RtmImsSmsController::sendGsmSms(RtmGsmSmsMessage *msg) {
    RFX_ASSERT(msg != NULL);
    mGsmCtrl->sendGsmSms(msg);
}

void RtmImsSmsController::sendCdmaSmsAck(const sp<RfxMessage>& message) {
    mCdmaCtrl->sendCdmaSmsAck(message);
}

void RtmImsSmsController::sendGsmSmsAck(int success, int cause, const sp<RfxMessage>& message) {
    mGsmCtrl->sendGsmSmsAck(success, cause, message);
}

bool RtmImsSmsController::isSupportSmsFormatConvert() {
