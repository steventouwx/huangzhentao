...
void RtcModeSwitchController::enterModeSwitch(int* card_type, int* card_state, bool* is_ct3g_dualmode,
    ...
    if (callingSlot >= RFX_SLOT_ID_0) {
        ...
        return;
    } else if (emcsSlot >= RFX_SLOT_ID_0 && ratSwitchController != NULL
            && ratSwitchController->isECBMRestrictedModeSupport()) {
...
void RtcModeSwitchController::onRatSwitchDone(int slotId, int error) {
    ...
    if (error == RIL_E_OPERATION_NOT_ALLOWED || error == RIL_E_REQUEST_NOT_SUPPORTED) {
        handleSendEratFailByCall(slotId);
        return;
    }
...
void RtcModeSwitchController::handleSendEratFailByCall(int slotId) {
    RFX_LOG_I(RTC_MODE_CONTROLLER_TAG, "[SMC][handleSendEratFailByCall] Slot %d", slotId);
//    bool switchSlotHasCall = (getStatusManager(slotId)->getIntValue(
//            RFX_STATUS_KEY_AP_VOICE_CALL_COUNT, 0) > 0);
    if (hasCall()) {
...
void RtcModeSwitchController::updateRadioCapability() {
    ...
    if (capabilityGetController != NULL) {
...
bool RtcModeSwitchController::resetCCapabilitySlot() {
    int cardTypes[MAX_RFX_SLOT_ID] = {0};
    int cardTypeStates[MAX_RFX_SLOT_ID] = {0};
    if (mCCapabilitySlot < 1) {
        RFX_LOG_I(RTC_MODE_CONTROLLER_TAG, "[SMC][resetCCapabilitySlot] No need, C:%d",
                mCCapabilitySlot);
        return false;
    }
    for (int i = 0; i < RFX_SLOT_COUNT; i++) {
        if (i == mCCapabilitySlot) {
            // CARD_TYPE_NONE makes no C2K RAT to be set
            // NOT_HOT_PLUG forces setting RAT
            cardTypes[i] = CARD_TYPE_NONE;
            cardTypeStates[i] = CARD_STATE_NOT_HOT_PLUG;
        } else {
            // Keep other slots no change
            cardTypes[i] = getStatusManager(i)->getIntValue(RFX_STATUS_KEY_CARD_TYPE,
                    CARD_TYPE_NONE);
            cardTypeStates[i] = CARD_STATE_NO_CHANGED;
        }
    }
    setCCapabilitySlotId(0);
    onCardTypeReady(cardTypes, cardTypeStates, RFX_SLOT_COUNT);
    return true;
}
