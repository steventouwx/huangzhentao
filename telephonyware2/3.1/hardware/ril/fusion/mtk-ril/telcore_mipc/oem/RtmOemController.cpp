...
RFX_REGISTER_DATA_TO_REQUEST_ID(RfxIntsData, RfxVoidData, RFX_MSG_REQUEST_SYNC_APP_EVENT_STATUS);
...
void RtmOemController::onHandleOemHookRaw(const sp<RfxMessage>& message) {
    ...
    if (sendOemHookRawToNwModule(message)) {
        return;
    } else if (isMainProtocolCommand(data)) {
...
bool RtmOemController::sendOemHookRawToNwModule(const sp<RfxMessage>& message) {
    RtmNwController* nwController = (RtmNwController *)findController
                        (RFX_OBJ_CLASS_INFO(RtmNwController));
    if (nwController) return nwController->onHandleOemHookRawRequest(message);
    return false;
}
