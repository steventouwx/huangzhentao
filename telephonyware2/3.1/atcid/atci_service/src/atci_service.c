...
static pthread_mutex_t atcid_socket_mutex;
...
void* waitForAtcidConnect(void *param){
    ...
    for (;;) {
        ...
        if (ret < 0) {
            ...
            pthread_mutex_lock(&atcid_socket_mutex);
            setMainExit();
            pthread_mutex_unlock(&atcid_socket_mutex);
        }
...
void process_generic_response(char* data, int dataLen){
    int sendLen = 0;
...
int main() {
    ...
    pthread_mutex_init(&atcid_socket_mutex, NULL);
    initSocket();
