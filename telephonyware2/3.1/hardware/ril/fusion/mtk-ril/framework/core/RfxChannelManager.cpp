...
sp<RfxSender> RfxChannelManager::getSender(int channelId) {
   RFX_ASSERT(channelId >= 0 && channelId < getSupportChannels());
   return sSelf->mChannels[(unsigned int)channelId]->getSender();
}

RfxChannel* RfxChannelManager::getChannel(int channelId) {
    return sSelf->mChannels[(unsigned int)channelId];
}
...
void RfxChannelManager::initMuxPath() {
    ...
    if (RFX_SLOT_COUNT >= 2) {
        ...
        // single sim case, PS1 should be always on SIM1
        rfx_property_set(PROPERTY_3G_SIM, "1");
    }
    for(int i = 0; i < getSupportChannels(); i++) {
...
void RfxChannelManager::initImsFd(int channel_id, int imsFd) {
    RFX_LOG_I(RFX_LOG_TAG, "init initImsFd, channel_id:%d, imsFd:%d", channel_id, imsFd);
    for (int i = 0; i < getSupportChannels(); i++) {
        if (sSelf->mChannels[i]->getChannelId() == channel_id) {
            sSelf->mChannels[i]->getReader()->setFd(imsFd);
            sSelf->mChannels[i]->getSender()->setFd(imsFd);
        }
    }
}
...
const char* RfxChannelManager:: getMuxPath(int channelId) {
    return sSelf->muxPath[channelId];
}
