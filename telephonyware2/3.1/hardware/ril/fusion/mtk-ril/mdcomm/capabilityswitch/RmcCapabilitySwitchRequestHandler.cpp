...
void RmcCapabilitySwitchRequestHandler::queryNrType() {
    // Modem don't support mmWave before Gen98, only support SUB6
...
void RmcCapabilitySwitchRequestHandler::switchChannelByRealId(int channel_id1, int channel_id2) {
    ...
    sp<RfxReader> reader1, reader2;
    sp<RfxSender> sender1, sender2;
    RfxChannelContext *tmp_context;
    int tmp_fd;
    if (channel_id1 == channel_id2) {
        return;
    }
    // logD(RFX_LOG_TAG, "switchChannel:%d <-> %d", channel_id1, channel_id2);
