...
static void _watch_dog(sigval_t sig) {
    ...
    RFX_LOG_I("RFX_WATCHDOG", "timeout message id: %d, %s", sTimeOutMsgId, RFX_ID_TO_STR(sTimeOutMsgId));
...
Mutex RfxMessageHandler::sMsgIgnoreMutex;
...
Mutex RfxMainThread::sWaitLooperMutex;
...
RfxMainThread::~RfxMainThread() {
    if (RfxRilUtils::isMipcSupport() == true) {
        if (m_looper != NULL) {
            RFX_LOG_D(RFX_LOG_TAG, "mipc deinit");
            mipc_deinit();
        }
        m_looper = NULL;
    }
...
void RfxMainThread::initControllers() {
    RFX_LOG_I(RFX_LOG_TAG, "initControllers start");
    // initialize controllers
    RfxControllerFactory::createControllers();

    RFX_LOG_I(RFX_LOG_TAG, "initControllers end");

    RFX_LOG_I(RFX_LOG_TAG, "initControllers process async queue start");
    ...
    RFX_LOG_I(RFX_LOG_TAG, "initControllers process async queue end");
#ifdef RFX_OBJ_DEBUG
    RfxDebugInfo::dumpIfNeed();
#endif
}
