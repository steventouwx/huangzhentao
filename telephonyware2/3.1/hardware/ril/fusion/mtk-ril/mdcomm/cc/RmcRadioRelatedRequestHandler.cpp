...
RmcRadioRelatedRequestHandler::RmcRadioRelatedRequestHandler(int slot_id, int channel_id) :
    ...
    const int requests[] = {
        ...
        RFX_MSG_REQUEST_LOCAL_SET_ECC_INDICATION, //AT+EMCI (since IRadio v1.5)
    };
...
void RmcRadioRelatedRequestHandler::requestSetEccIndication(const sp<RfxMclMessage>& msg) {
    ...
    if (!RfxRilUtils::isEmciSupport()) {
        logE(RFX_LOG_TAG, "requestSetEccIndication error (emci support false)");
        return;
    }
    /*
     * AT+EMCI=<emc_indication>
     * <emc_indication> :  1 : on, 0: off (to cancel if ECC not dial)
     */

    p_response = atSendCommand(String8::format("AT+EMCI=%d", (enable ? 1 : 0)));
