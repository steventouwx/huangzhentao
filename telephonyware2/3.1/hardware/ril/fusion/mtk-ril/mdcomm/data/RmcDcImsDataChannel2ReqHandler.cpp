        ...
        if (isUpdateState) {
            if (action == 1) {
                if(strncmp(notification->type, "ims", 3) == 0) {
                    ...
                    imsPdnState = RFX_ONGOING_IMS_PDN_ACT;
                    ...
                    eImsPdnState = RFX_ONGOING_EIMS_PDN_ACT;
                }
            } else if (action == 0) {
                if(strncmp(notification->type, "ims", 3) == 0) {
                    getMclStatusManager(m_slot_id)->setIntValue(RFX_STATUS_KEY_IMS_PDN_STATE,pdnState);
                    ...
                    imsPdnState = RFX_ONGOING_IMS_PDN_DEACT;
                } else if (strncmp(notification->type, "emergency", 9) == 0) {
                    getMclStatusManager(m_slot_id)->setIntValue(RFX_STATUS_KEY_EIMS_PDN_STATE,pdnState);
                    ...
                    eImsPdnState = RFX_ONGOING_EIMS_PDN_DEACT;
                }
            }
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] notification->type: %s, imsPdnState: %d, eImsPdnState: %d",
                     m_slot_id, __FUNCTION__, notification->type,imsPdnState,eImsPdnState);
        }
    }
...
void RmcDcImsDataChannel2ReqHandler::handleImsBearerConfirm(const sp<RfxMclMessage>& msg) {
    ...
    if (imsPdnState == RFX_ONGOING_IMS_PDN_ACT_DONE
            && RFX_NO_ONGOING_IMS_PDN_AID == getMclStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_PDN_DEACT_AID, -1)
            && aid == getMclStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_PDN_ACT_AID, -1)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] since ims activated: %d,no need to send confirm again",
                rid, __FUNCTION__,imsPdnState);
        return;
    }
    if (eImsPdnState == RFX_ONGOING_EIMS_PDN_ACT_DONE
           && RFX_NO_ONGOING_IMS_PDN_AID == getMclStatusManager()->getIntValue(RFX_STATUS_KEY_EIMS_PDN_DEACT_AID, -1)
           && aid == getMclStatusManager()->getIntValue(RFX_STATUS_KEY_EIMS_PDN_ACT_AID, -1)) {
        RFX_LOG_E(RFX_LOG_TAG, "[%d][%s] since eims activated: %d,no need to send confirm again",
                rid, __FUNCTION__,eImsPdnState);
        return;
    }
    ...
    if (action == 1) {
        ...
        if (aid == tempImsAid) {
            //getMclStatusManager()->setIntValue(RFX_STATUS_KEY_IMS_PDN_ACT_AID,RFX_NO_ONGOING_IMS_PDN_AID);
            if (RFX_NO_ONGOING_IMS_PDN_AID == getMclStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_PDN_DEACT_AID, -1)) {
                imsPdnState = RFX_ONGOING_IMS_PDN_ACT_DONE;
            } else {
                imsPdnState = RFX_ONGOING_IMS_PDN_DEACT;
            }
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] imsPdnState: %d", rid, __FUNCTION__, imsPdnState);
        } else if (aid == tempEimsAid) {
            //getMclStatusManager()->setIntValue(RFX_STATUS_KEY_EIMS_PDN_ACT_AID,RFX_NO_ONGOING_IMS_PDN_AID);
            if (RFX_NO_ONGOING_IMS_PDN_AID == getMclStatusManager()->getIntValue(RFX_STATUS_KEY_EIMS_PDN_DEACT_AID, -1)) {
                eImsPdnState = RFX_ONGOING_EIMS_PDN_ACT_DONE;
            } else {
                eImsPdnState = RFX_ONGOING_EIMS_PDN_DEACT;
            }
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] eImsPdnState: %d", rid, __FUNCTION__, eImsPdnState);
        } else {
...
void RmcDcImsDataChannel2ReqHandler::notifyUnsyncImsBearerRequest() {
    ...
    int imsState  = IMS_OR_EIMS_PDN_INIT;
    int aid = -1;

    std::unique_ptr<RIL_IMS_BearerNotification> notification(new RIL_IMS_BearerNotification());

    imsState = getMclStatusManager()->getIntValue(
        RFX_STATUS_KEY_IMS_PDN_STATE,IMS_OR_EIMS_PDN_INIT);

    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] ims State=%d", m_slot_id, __FUNCTION__, imsState);

    notification->phone = m_slot_id;
    notification->type = type;

    if (imsState == IMS_OR_EIMS_PDN_ACTIVATED) {
        aid = getMclStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_PDN_ACT_AID, -1);
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] aid=%d", m_slot_id, __FUNCTION__, aid);
        if (imsPdnState == RFX_ONGOING_IMS_PDN_ACT) {
            notification->aid = aid;
    ...
    } else if (imsState == IMS_OR_EIMS_PDN_DEACTIVATED) {
...
void RmcDcImsDataChannel2ReqHandler::handleResetImsConnection(const sp<RfxMclMessage>& msg) {
    static int ACTION_BEARER_ACTIVATION = 1;
    sp<RfxMclMessage> responseMsg;
    std::unique_ptr<RIL_IMS_BearerNotification> notification(new RIL_IMS_BearerNotification());
    notification->phone = m_slot_id;

    int imsConnState  = getMclStatusManager()->getIntValue(
        RFX_STATUS_KEY_IMS_PDN_STATE,IMS_OR_EIMS_PDN_INIT);
    int eimsConnState  = getMclStatusManager()->getIntValue(
        RFX_STATUS_KEY_EIMS_PDN_STATE,IMS_OR_EIMS_PDN_INIT);
    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] imsConnState: %d, eimsConnState: %d", m_slot_id, __FUNCTION__, imsConnState,eimsConnState);
    if (imsConnState == IMS_OR_EIMS_PDN_ACTIVATED) {
        int imsAid = getMclStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_PDN_ACT_AID, -1);
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] imsAid=%d, imsConnState=%d", m_slot_id, __FUNCTION__, imsAid,imsConnState);
        if (imsAid != RFX_NO_ONGOING_IMS_PDN_AID && imsPdnState == RFX_ONGOING_IMS_PDN_ACT_DONE) {
    ...
    if (eimsConnState == IMS_OR_EIMS_PDN_ACTIVATED) {
        int eimsAid = getMclStatusManager()->getIntValue(RFX_STATUS_KEY_EIMS_PDN_ACT_AID, -1);
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] eimsAid=%d, eimsConnState=%d", m_slot_id, __FUNCTION__, eimsAid,eimsConnState);
        if (eimsAid != RFX_NO_ONGOING_IMS_PDN_AID && eImsPdnState == RFX_ONGOING_EIMS_PDN_ACT_DONE) {
