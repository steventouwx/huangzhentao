...
class NetlinkEventHandler : public NetlinkListener {
    public:
        ...
        int getFlags(void *obj, int *flags);
        int getDeprecationTime(void *obj, long *deprecationTime);
        int getExpirationTime(void *obj, long *expirationTime);
        ...
        static long stringToBinaryBaseLong(char *str, int base, int *err);
    ...
    protected:
        ...
        void enqueueReqInfo(NA_CMD cmd, char *address, int ipv6PrefixLength, int interfaceId,
                ACTION action, int flags, long deprecationTime, long expirationTime);
