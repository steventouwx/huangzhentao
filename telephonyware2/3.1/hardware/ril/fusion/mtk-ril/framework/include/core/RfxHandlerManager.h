...
public:
    ...
    static void registerHandler(RfxCreateHandlerFuncptr func_ptr, unsigned int c_id, int slot_category,
            bool isOpReplaced, bool isOpHandler);

    static void registerToHandleRequest(RfxBaseHandler *handler, unsigned int channel_id, int slot_id,
            const int *request_id_list, int length);

    static void registerToHandleUrc(RfxBaseHandler *handler, unsigned int channel_id, int slot_id,
            const char **urc_prefix_list, int length, bool needAllMatch = false);

    static void registerToHandleMipcUrc(RfxBaseHandler *handler, unsigned int channel_id, int slot_id,
            const int *mipc_urc_id_list, int length);

    static void registerToHandleMipcCmd(RfxBaseHandler *handler, unsigned int channel_id, int slot_id,
            const int *mipc_cmd_id_list, int length);

    static void unregisterToHandleMipcUrc(RfxBaseHandler *handler, unsigned int channel_id, int slot_id,
            const int *mipc_urc_id_list, int length);

    static void registerToHandleEvent(RfxBaseHandler *handler, unsigned int channel_id, int slot_id,
...
private:
    ...
    void registerInternal(SortedVector<RfxHandlerRegisterEntry> &list, RfxBaseHandler *handler,
            unsigned int channel_id, int slot_id, const int *id_list, int length);

    void registerInternal(SortedVector<RfxHandlerRegisterEntry> &list, RfxBaseHandler *handler,
            unsigned int channel_id, int slot_id, int client_id, const int *id_list, int length);

    void registerInternal(SortedVector<RfxHandlerRegisterEntry> &list, RfxBaseHandler *handler,
            unsigned int channel_id, int slot_id, const char **urc_list, int length, bool needAllMatch);

    RfxBaseHandler* findMsgHandler(SortedVector<RfxHandlerRegisterEntry> &list, unsigned int channelId,
    ...
    RfxBaseHandler* findMsgHandlerInternal(SortedVector<RfxHandlerRegisterEntry> &list,
            unsigned int channelId, int slotId, int id, int client_id, const char* urc, bool needAllMatch);

    SortedVector<RfxHandlerRegisterEntry>* findListByType(int type);

    SortedVector<RfxHandlerRegisterEntry> findListByChannel(int type, unsigned int channel_id);
...
private:
    ...
    // Vector<RfxCreateHandlerFuncptr> m_mipc_slot_handler_list[RIL_PROXY_OFFSET];
    // Vector<RfxCreateHandlerFuncptr> m_mipc_non_slot_handler_list[RIL_PROXY_OFFSET];
    ...
    SortedVector<RfxHandlerRegisterEntry> m_mipc_urc_list[RIL_SUPPORT_CHANNELS];
    SortedVector<RfxHandlerRegisterEntry> m_mipc_cmd_list[RIL_SUPPORT_CHANNELS];
    mutable Mutex m_mutex[RIL_SUPPORT_CHANNELS];
    static Mutex sWaitChannelDoneMutex;
    static int sCreateChannelNumber;
};
