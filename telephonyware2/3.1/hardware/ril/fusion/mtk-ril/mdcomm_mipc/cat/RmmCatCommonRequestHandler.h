...
#ifndef __RMM_CAT_COMMON_REQUEST_HANDLER_H__
#define __RMM_CAT_COMMON_REQUEST_HANDLER_H__
...
#define RFX_LOG_TAG "RmmCatComReqHandler"
...
class RmmCatCommonRequestHandler : public RfxBaseHandler {
    RFX_DECLARE_HANDLER_CLASS(RmmCatCommonRequestHandler);

    public:
        RmmCatCommonRequestHandler(int slot_id, int channel_id);
        virtual ~RmmCatCommonRequestHandler();
    ...
    private:
        ...
        void initMdStatusQueryLoop();
        static void* mdStatusQueryLoop(void* params);
        ...
        int handleSendEnvelopeCnf(const sp<RfxMclMessage>& msg);
    private:
        bool isProaCmdQueued;
        bool isEventNotifyQueued;
        char* pProactiveCmd;
        char* pEventNotifyCmd;
        static int sMdStatusFd;
        pthread_t mMdStatusQueryThread;
};
#endif
