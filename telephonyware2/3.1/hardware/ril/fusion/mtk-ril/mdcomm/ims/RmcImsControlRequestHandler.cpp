...
void RmcImsControlRequestHandler::requestSetVodataCfg(const sp<RfxMclMessage>& msg) {
    int *params = (int *)msg->getData()->getData();
    sp<RfxAtResponse> p_response;
    RIL_Errno rilErrNo = RIL_E_SUCCESS;
    ...
    if (isCurrentSlotSupportIms()) {
        int vodata = params[0];
        // send AT command
        logD(RFX_LOG_TAG, "requestSetVodataCfg vodata:%d", vodata);
        p_response = atSendCommand(String8::format("AT+EVODATA=%d,0,0", vodata));
        // set result
        if (p_response->getError() != 0 || p_response->getSuccess() != 1) {
            rilErrNo = RIL_E_GENERIC_FAILURE;
        }
    } else {
        logD(RFX_LOG_TAG, "requestSetVodataCfg slotId %d not support IMS", m_slot_id);
        rilErrNo = RIL_E_GENERIC_FAILURE;
    }
...
void RmcImsControlRequestHandler::requestSetImsAddCap(const sp<RfxMclMessage>& msg) {
    int *params = (int *)msg->getData()->getData();
    int capbility_en = params[0];
    logD(RFX_LOG_TAG, "requestSetImsAddCap, error not supported, capbility_en:%d", capbility_en);

    // RIL_Errno rilErrNo = RIL_E_SUCCESS;
    // p<RfxAtResponse> p_response = atSendCommand(String8::format("AT+E?=%d", capbility_en));
    // if (p_response->getError() != 0 || p_response->getSuccess() != 1) {
    //    rilErrNo = RIL_E_GENERIC_FAILURE;
    // }

    sp<RfxMclMessage> response = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_REQUEST_NOT_SUPPORTED,
            RfxVoidData(), msg, false);
    responseToTelCore(response);
}
...
void RmcImsControlRequestHandler::requestMultiImsSupportCount() {
    ...
    if (sprintf(sMdMultiImsCount, "%d", multiImsCount) < 0) {
        logE(RFX_LOG_TAG, "requestMultiImsSupportCount, sprintf to multiImsCount fail");
    }
