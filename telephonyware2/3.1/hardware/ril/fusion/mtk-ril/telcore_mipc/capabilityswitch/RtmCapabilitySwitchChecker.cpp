 ...
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("RtmCapabilitySwitchChecker", RtmCapabilitySwitchChecker, RfxController);

RtmCapabilitySwitchChecker::RtmCapabilitySwitchChecker() :
...
bool RtmCapabilitySwitchChecker::isInCalling(bool log) {
    bool ret = false;
    for (int i = RFX_SLOT_ID_0; i < RfxRilUtils::rfxGetSimCount(); i++) {
...
bool RtmCapabilitySwitchChecker::isInPukLock(bool log) {
    bool ret = false;
    char feature[] = "SIM_SWITCH_DURING_SIMLOCK";
    int version = getFeatureVersion(feature, 0);
    if (version != 1) {
        for (int i = RFX_SLOT_ID_0; i < RfxRilUtils::rfxGetSimCount(); i++) {
...
bool RtmCapabilitySwitchChecker::isReadyForSwitch(bool log) {
    bool ret = !isCapabilitySwitching();
...
bool RtmCapabilitySwitchChecker::radioLockReady(bool log) {
    for (int i = RFX_SLOT_ID_0; i < RfxRilUtils::rfxGetSimCount(); i++) {
...
bool RtmCapabilitySwitchChecker::eccStateReady(bool log) {
    for (int i = RFX_SLOT_ID_0; i < RfxRilUtils::rfxGetSimCount(); i++) {
...
bool RtmCapabilitySwitchChecker::modemOffStateReady(bool log) {
...
bool RtmCapabilitySwitchChecker::simStateReady(bool log) {
    char feature[] = "SIM_SWITCH_DURING_SIMLOCK";
    int version = getFeatureVersion(feature, 0);
    if (log) {
        RFX_LOG_I(RFX_LOG_TAG, "simStateReady, SIM_SWITCH_DURING_SIMLOCK = %d", version);
    }
    ...
    for (int i = RFX_SLOT_ID_0; i < RfxRilUtils::rfxGetSimCount(); i++) {
        ...
        if (version == 1) {
            if (sim_state == RFX_SIM_STATE_UNKNOWN ||
                    (sim_state == RFX_SIM_STATE_NOT_READY && sim_onoff_state != SIM_OFF)) {
                if (log) {
                    RFX_LOG_I(RFX_LOG_TAG, "Not ready. sim%d_state:%d,%d",
                              i + 1, sim_state, sim_onoff_state);
                }
                return false;
            }
        } else {
            if (sim_state == RFX_SIM_STATE_PIN_REQUIRED ||
...
bool RtmCapabilitySwitchChecker::imsiReady(bool log) {
    for (int i = RFX_SLOT_ID_0; i < RfxRilUtils::rfxGetSimCount(); i++) {
        int sim_state = m_status_managers[i]->getIntValue(RFX_STATUS_KEY_SIM_STATE);
        int sim_onoff_state = getSimOnOffState(i, log);

        if (sim_state == RFX_SIM_STATE_ABSENT || sim_state == RFX_SIM_STATE_PERM_DISABLED ||
            sim_onoff_state == SIM_OFF) {
            continue;
        }
        const String8& imsi = getImsi(i);
        if (imsi.isEmpty()) {
            if (log == true) {
                RFX_LOG_I(RFX_LOG_TAG, "imsiReady, sim%d_state=%d,%d",
                        i + 1, sim_state, sim_onoff_state);
            }
            return false;
        }
...
bool RtmCapabilitySwitchChecker::waitingFlagReady(bool log) {
...
bool RtmCapabilitySwitchChecker::isCapabilitySwitching() {
...
void RtmCapabilitySwitchChecker::getWaiveEnhancementConfig() {
    ...
    if (strcmp(temp, "-1") == 0) {
        ...
        if (errno == ERANGE ||
            m_waive_enhance_config == LONG_MAX || m_waive_enhance_config == LONG_MIN) {
            RFX_LOG_D(RFX_LOG_TAG, "getWaiveEnhancementConfig error: %s", temp);
            if (version_1 == 1 && version_2 == 1) {
                m_waive_enhance_config = 0;
            } else {
                m_waive_enhance_config = (ENHANCEMENT_T_PLUS_C | ENHANCEMENT_W_PLUS_C);
...
const String8& RtmCapabilitySwitchChecker::getImsi(int slot_id) {
    if ((slot_id < 0) || (slot_id >= RfxRilUtils::rfxGetSimCount())) {
        RFX_LOG_D(RFX_LOG_TAG, "getImsi, slot id = %d is invalid", slot_id);
        slot_id = 0;
    }
    //check gsm first
...
int RtmCapabilitySwitchChecker::getSimOnOffState(int slot_id, bool log) {
    char temp[RFX_PROPERTY_VALUE_MAX] = { 0 };
    int sim_onoff_version;
    int state = -1;
    int uicc_state = UICC_APPLICATIONS_ENABLE_STATE_UNKNOWN;

    if ((slot_id < RFX_SLOT_ID_0) || (slot_id >= RfxRilUtils::rfxGetSimCount())) {
        RFX_LOG_E(RFX_LOG_TAG, "getSimOnOffState, slot_id(%d) is wrong", slot_id);
        return state;
    }

    rfx_property_get("ro.vendor.mtk_sim_card_onoff", temp, "-1");
    sim_onoff_version = atoi(temp);

    switch (sim_onoff_version) {
        case 1:
            // radio on/off
            state = SIM_ON;
            break;
        ...
        case 2:
            // mtk sim on/off
            state = m_status_managers[slot_id]->getIntValue(RFX_STATUS_KEY_SIM_ONOFF_STATE);
            break;

        case 3:
            // aosp sim on/off
            uicc_state = m_status_managers[slot_id]->getIntValue(
                    RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_STATE);
            if (uicc_state == UICC_APPLICATIONS_ENABLE_STATE_ENABLED) {
                state = SIM_ON;
            } else if (uicc_state == UICC_APPLICATIONS_ENABLE_STATE_DISABLED) {
                state = SIM_OFF;
            }
            break;
    ...
    if (log) {
        RFX_LOG_D(RFX_LOG_TAG, "getSimOnOffState, version = %d, sim%d_onoff = %d",
                  sim_onoff_version, slot_id, state);
    }

    return state;
}

bool RtmCapabilitySwitchChecker::isSupportSimSwitchEnhancement(int sim_type) {
    getWaiveEnhancementConfig();
    return !(RFX_FLAG_HAS(m_waive_enhance_config, sim_type));
}

int RtmCapabilitySwitchChecker::getHigherPrioritySlot() {
    ...
    if (isForceSimSwitch() == true) {
        RFX_LOG_D(RFX_LOG_TAG, "getHigherPrioritySlot, force to sim switch");
        return UNKNOWN_SLOT;
    }

    bool equal_c2k = false;
    char feature_version_1[] = "MODE_SWITCH_BY_DATA";
    int version_1 = getFeatureVersion(feature_version_1, 0);
    char feature_version_2[] = "EVDO DISABLE";
    int version_2 = getFeatureVersion(feature_version_2, 0);
    ...
    RFX_LOG_D(RFX_LOG_TAG, "getHigherPrioritySlot, sim_count=%d, mode=%d, cur=%d, equal_c2k=%d",
              sim_count, gemini_mode, current_major_slot, equal_c2k);
    ...
    if (sim_count > 3) {
        ...
        if ((gemini_mode != GEMINI_MODE_L_AND_L) || (RfxRilUtils::isMultipleImsSupport() == 0)) {
            ...
            return UNKNOWN_SLOT;
        } else if ((RatConfig_isC2kSupported() == 0 || equal_c2k == true) &&
                (RatConfig_isTdscdmaSupported() == 0)) {
            RFX_LOG_D(RFX_LOG_TAG, "not support C2K (or equal C2K) and TD-SCDMA, skip switch");
            return CURRENT_SLOT;
        }
    } else if (sim_count == 1) {
    ...
    if (isSupportSimSwitchEnhancement(ENHANCEMENT_T_PLUS_T) &&
        ...
        if (RatConfig_isC2kSupported() && equal_c2k == false) {
...
bool RtmCapabilitySwitchChecker::isSkipCapabilitySwitch(int new_major_slot) {
    int current_major_slot = m_status_managers[MAX_RFX_SLOT_ID]->getIntValue(
            RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT, 0);
    int sim_count = RfxRilUtils::rfxGetSimCount();
    bool ret = false;
...
bool RtmCapabilitySwitchChecker::isCdmaOnlySim(int slot_id) {
    bool ret = false;
    if ((slot_id < 0) || (slot_id >= RfxRilUtils::rfxGetSimCount())) {
        return false;
    }
...
bool RtmCapabilitySwitchChecker::isCdmaDualModeSim(int slot_id) {
    if ((slot_id < 0) || (slot_id >= RfxRilUtils::rfxGetSimCount())) {
        return false;
    }
    if (m_status_managers[slot_id]->getBoolValue(RFX_STATUS_KEY_CDMA3G_DUALMODE_CARD)) {
...
bool RtmCapabilitySwitchChecker::isCommonGsmSim(int slot_id) {
    if (RtmCapabilitySwitchUtil::isOp01Sim(getImsi(slot_id)) == false &&
...
bool RtmCapabilitySwitchChecker::isLteNetworkType(int nw_type) {
...
bool RtmCapabilitySwitchChecker::isForceSimSwitch() {
