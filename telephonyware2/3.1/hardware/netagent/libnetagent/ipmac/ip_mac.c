...
static int SendIpv6_DumpRequest(const char *ipStr, int ifIndex, struct sockaddr *WlanSocketAddr)
{
    ...
    memset(&ipv6_rth, 0, sizeof(struct rtnl_handle));
    ...
    if (rtnl_open(&ipv6_rth) < 0) {
        ret = -errno;
        goto dump_clean;
    }
    if (send(ipv6_rth.fd, &DumpReq, DumpReq.hdr.nlmsg_len, 0) < 0) {
        ALOGE("SendIpv6_DumpRequest exception: err = %s (%d)", strerror(errno), errno);
        ret = -errno;
        goto dump_clean;
    }

    ret = read_neighbor_nlmsg(ipv6_rth.fd, ipStr, WlanSocketAddr);
 ...
 * WlanSocketAddr: store MAC address
 */
static int SendNewNeighborMessage(int fd, const char *ipStr, int ifIndex,
	bool is_ipv6, struct sockaddr *WlanSocketAddr)
{
    ...
    if (send(fd, &NeighRequest, NeighRequest.hdr.nlmsg_len, 0) < 0) {
 ...
 */
static int inet_get_addr(const char *ip_str, bool is_ipv6, struct in_addr *ip, struct in6_addr *ip6)
{
    if (is_ipv6)
 ...
 */
static int neigh_mac_process(struct nlmsghdr *nh, const char *ipStr, struct sockaddr *WlanSocketAddr)
{
    struct ndmsg *ndmsg;
    int len = 0;
        ...
        if (tb[NDA_DST]) {
            ...
            if(!strncmp(ipStr, listen_ip, strlen(ipStr))) {
                if (ndmsg->ndm_state & NUD_VALID) {
                    if (tb[NDA_LLADDR]) {
                        memcpy((char *)WlanSocketAddr->sa_data, (char *)RTA_DATA(tb[NDA_LLADDR]), MACSIZE);
...
int Get_Mac_Addr(const char *ipStr, const char *ifname, struct sockaddr *WlanSocketAddr)
{
    ...
    if (!ret) {
        ...
        if ((ret = SendNewNeighborMessage(rth.fd, ipStr, wlan_index, is_ipv6, WlanSocketAddr)) < 0) {
