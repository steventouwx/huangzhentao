...
char* RfxAtLine::atTokNextSpecailstr(int *err) {
    *err = 0;
    if (m_pCur == NULL) {
        *err = -1;
        return NULL;
...
char* RfxAtLine::newNextTok() {
    ...
    if (m_pCur == NULL) {
        ...
        char *localPCur = m_pCur;
        char *local2PCur = m_pCur;
        localPCur++;
        localStr = strstr(localPCur, "\",");
        if (localStr == NULL) {
            local2PCur++;
            char *local = local2PCur;
            while (*local != '\0') {
                local++;
            }
            if (*local == '\0' && *(local - 1) == '"') {
