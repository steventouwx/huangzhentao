 ...
 * Class RfxController
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("RtmSmsNSlotController", RtmSmsNSlotController, RfxController);

RtmSmsNSlotController::RtmSmsNSlotController() :
        m_smsThread(NULL),
...
void RtmSmsNSlotController::onInit() {
    RfxController::onInit();
    m_smsThread = RtmSmsThread::create();
    m_smsThread->run("RtmSmsThread");
...
void RtmSmsNSlotController::onDeinit() {
...
void RtmSmsNSlotController::onHandleSmsMessage(const sp<RtmSmsParsingMessage> & message) {
    logD(SMS_TAG, "[RtmSmsNSlotController]onHandleSmsMessage %d",
        message->getId());
    switch (message->getId()) {
        ...
        default: {
            RFX_LOG_E(SMS_TAG, "[RtmSmsNSlotController]onHandleSmsMessage unknown!");
            break;
        }
    }
}

sp<RtmSmsThread> RtmSmsNSlotController::getSmsThread() {
    return m_smsThread;
}

void RtmSmsNSlotController::dispatchSms(const sp<RfxMessage>& msg) {
    if (mDispatcherList == NULL) {
        return;
    } else {
    ...
    p->writeInt32(slotId);
    p->writeCString(pdu);
    sp<RtmSmsParsingMessage> smsMsg = RtmSmsParsingMessage::obtainMessage(
              EVENT_SMS_DISPATCH_SUPL_SMS, p);
    sp<RtmSmsParsingThreadHandler> handler = new RtmSmsParsingThreadHandler(smsMsg);
    if (m_smsThread->getLooper() == NULL) {
        logD(SMS_TAG, "[RtmSmsNSlotController]sendSmsSmsMessage: ignore message");
        free(pdu);
        return;
    }
