...
bool RfxMessage::doingTrm = false;
...
RfxMessage::RfxMessage() :
    ...
    if (RfxDebugInfo::isRfxDebugInfoEnabled()) {
        m_debug_info = new (std::nothrow) RfxDebugInfo(static_cast<IRfxDebugLogger *>(this), (void *)this);
...
void RfxMessage::doTrm() {
    if (doingTrm == false) {
        doingTrm = true;
        RFX_LOG_E("RfxMessage", "Panic!! new RfxMessage failed, do trm to recover");
        rfx_property_set("vendor.ril.mux.report.case", "2");
        rfx_property_set("vendor.ril.muxreport", "1");
    }
    do {
        RFX_LOG_E("RfxMessage", "Panic!! waiting for do trm due to OOM");
        sleep(1000);
    } while(true);
}
...
sp<RfxMessage> RfxMessage::obtainRequest(int slotId, int id, RILD_RadioTechnology_Group dest) {
    ...
    msg = new (std::nothrow) RfxMessage();
    if (msg == nullptr) {
...
sp<RfxMessage> RfxMessage::obtainResponse(RIL_Errno e, const sp<RfxMessage>& msg,
        bool copyData) {
    sp<RfxMessage> new_msg = NULL;
    new_msg = new (std::nothrow) RfxMessage();
    if (new_msg == nullptr) {
        if (RfxRilUtils::isEngLoad()) {
            RFX_ASSERT(0);
        }
