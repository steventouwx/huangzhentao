...
void RtcImsConfigController::onInit() {
    ...
    if (RfxRilUtils::isCtVolteSupport() || RfxRilUtils::isCtMixVolteSupport()) {
        getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_SIM_ICCID,
                RfxStatusChangeCallback(this, &RtcImsConfigController::onSimIccidChanged));
    }
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_UICC_GSM_NUMERIC,
        RfxStatusChangeCallback(this, &RtcImsConfigController::onUiccMccMncChanged));
...
void RtcImsConfigController::onUiccMccMncChanged(RfxStatusKeyEnum key,
...
void RtcImsConfigController::triggerImsDisableCommand(int slot_id) {
...
void RtcImsConfigController::onSimIccidChanged(RfxStatusKeyEnum key, RfxVariant old_value,
          RfxVariant value) {
      RFX_UNUSED(key);
...
void RtcImsConfigController::setVoNREnable(const sp<RfxMessage> &message) {
    RIL_Errno error = RIL_E_SUCCESS;
    int *params = (int*) message->getData()->getData();
...
void RtcImsConfigController::isVoNREnabled(const sp<RfxMessage> &message) {
    RIL_Errno error = RIL_E_SUCCESS;
...
void RtcImsConfigController::triggerVodataCfgCommand(int slot_id) {
    if (!ImsConfigUtils::isPhoneIdSupportIms(slot_id)) {
...
void RtcImsConfigController::triggerImsAddCapabilityCommand(int value) {
    int params[1];
    params[0] = value;
