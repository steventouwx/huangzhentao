...
//MT2635: get service state @{
bool RfxNwServiceState::getVoiceRoaming() {
    if (mRilVoiceRegState == 5) {
        return true;
    } else {
        return false;
    }
...
bool RfxNwServiceState::getDataRoaming() {
    if (mRilDataRegState == 5) {
        return true;
    } else {
        return false;
    }
...
bool RfxNwServiceState::getRoaming() {
    return (getVoiceRoaming() || getDataRoaming());
}

bool RfxNwServiceState::isVoiceRadioTechnologyHigher(int nRadioTechnology) {
    if (mRilVoiceRadioTech == nRadioTechnology) {
        return false;
    } else if (mRilVoiceRadioTech == RADIO_TECH_LTE) {
        return true;
    } else if (nRadioTechnology == RADIO_TECH_LTE) {
        return false;
    } else if (mRilVoiceRadioTech == RADIO_TECH_GSM) {
        // ALPS02230032-START
        if (nRadioTechnology == RADIO_TECH_UNKNOWN) {
            return true;
        }
        // ALPS00230032-END
        return false;
    } else if (nRadioTechnology == RADIO_TECH_GSM) {
        // ALPS02230032-START
        if (mRilVoiceRadioTech == RADIO_TECH_UNKNOWN) {
            return false;
        }
        // ALPS00230032-END
        return true;
    } else if (mRilVoiceRadioTech > nRadioTechnology) {
        return true;
    } else {
        return false;
    }
...
bool RfxNwServiceState::isConcurrentVoiceAndDataAllowed() {
    if (mRilVoiceRadioTech == RADIO_TECH_UMTS
            || isVoiceRadioTechnologyHigher(RADIO_TECH_UMTS)) {
        return true;
    } else {
        return false;
    }
