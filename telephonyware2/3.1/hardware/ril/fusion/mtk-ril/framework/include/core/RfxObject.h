...
{
    _class_name *ptr = new (std::nothrow) _class_name();
    if (ptr == NULL) {
        RFX_ASSERT(0);
    }
    ptr->_init(_parent);
...
#define __RFX_OBJ_CREATE(_ptr, _class_name, _parent)                            \
    do                                                                          \
    {                                                                           \
        (_ptr) = new (std::nothrow) _class_name();                              \
        if ((_ptr) == NULL) {                                                   \
            RFX_ASSERT(0);                                                      \
        }                                                                       \
        (_ptr)->_init(_parent);                                                 \
...
#define RFX_OBJ_CREATE_EX(_ptr, _class_name, _parent, _param)                    \
    do                                                                           \
    {                                                                            \
        (_ptr) = new (std::nothrow) _class_name _param;                          \
        if ((_ptr) == NULL) {                                                    \
            RFX_ASSERT(0);                                                       \
        }                                                                        \
        (_ptr)->_init(_parent); \
...
#define RFX_OBJ_IMPLEMENT_SINGLETON_CLASS(_class_name) \
    ...
    { \
        if (s_instance == NULL) \
        {\
            s_instance = new (std::nothrow) _class_name(); \
            if (s_instance == NULL) { \
                RFX_ASSERT(0); \
            } \
