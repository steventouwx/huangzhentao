...
void RtcImsConferenceController::onInit() {
    ...
    mRtcImsVideoRingtoneHandler = new RtcImsVideoRingtoneHandler(getSlotId());
...
void RtcImsConferenceController::onDeinit() {
    ...
    delete mRtcImsVideoRingtoneHandler;
    mRtcImsVideoRingtoneHandler = NULL;
}
...
void RtcImsConferenceController::handleMergeConferenece(const sp<RfxMessage>& message) {
    ...
    logD(RFX_LOG_TAG, "handleMergeConferenece confCallId:%d, nonConfCallId count:%d",
            confCallId, (int)callIds.size());
...
void RtcImsConferenceController::handleImsEventPackageIndication(const sp<RfxMessage>& message) {
    ...
    if (msgType == 1) {
        ...
        mRtcImsVideoRingtoneHandler->handleImsVideoRingtoneMessage(message);
        return;
    }
}
...
void RtcImsConferenceController::handleSipCallProgressIndicator(const sp<RfxMessage>& message) {
    if (mRtcImsConferenceHandler != NULL) {
        mRtcImsConferenceHandler->handleOneKeyInviteNotifyResult(message);
    }
}
...
void RtcImsConferenceController::tryhandleCachedCEP() {
    ...
    if (mIsAddingMember || mIsRemovingMember) {
        ...
        while (!mCachedCEPMessageQueue.empty()) {
            if (mRtcImsConferenceHandler != NULL) {
                mRtcImsConferenceHandler->handleImsConfCallMessage(mCachedCEPMessageQueue.front());
            }
            mCachedCEPMessageQueue.pop();
        }
    }
