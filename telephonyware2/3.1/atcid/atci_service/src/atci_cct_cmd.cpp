...
bool ccap_init()
{
    ...
    if(!CCT_Req)
        MY_LOGE("CCT_Req is nullptr!");
    CCT_Cnf = malloc(sizeof_FT_CCT_CNF);
    if(!CCT_Cnf)
        MY_LOGE("CCT_Cnf is nullptr!");
...
int cct_cmd_handler(char* cmdline, ATOP_t at_op, char* response)
{
        ...
        case AT_TEST_OP:
        case AT_READ_OP:
            if(sprintf(response,"CCT_at_op(%d)\r\n", at_op) < 0)
                MY_LOGE("Fail to get response!");
            break;
        case AT_SET_OP:
            ...
            if (at_tok_nextint(&cmdline, &CCT_Operation) == -1) // retrieve operation from input string
            {
                ...
                if(sprintf(response,"AT_SET_OP, CCT_op(failed) Grab output size failed\r\n") < 0)
                    MY_LOGE("Fail to get response!");
                goto EXIT;
            }
            /*
            ...
            */
            ...
            if( ccap_init() == false )
            {
                if(sprintf(response,"ccap_init failed\r\n") < 0) 
                    MY_LOGE("Fail to get response!");
                goto EXIT;
            }
            ...
            if( ccapCtrl->intf_ccap_init() != 0 )
            {
                ...
                if(sprintf(response,"CCT_op(failed) META_CCAP_init failed\r\n") < 0)
                    MY_LOGE("Fail to get response!");
                goto EXIT;
            }
            ...
            else if(CCT_Operation == ACDK_CCT_IO_ATCI_NEXT)
            {
                ...
                if(Cnf_Cnt == -1)
                {
                    ...
                    {
                        MY_LOGE("Grab output size failed");
                        if(sprintf(response,"CCT_op(failed) Grab output size failed\r\n") < 0)
                            MY_LOGE("Fail to get response!");
                        goto EXIT;
                    }
            ...
            else if(CCT_Operation == ACDK_CCT_IO_ATCI_END)
            {
                ...
                // header + op + status
                if(sprintf(response, "CCT_op(%d),%d,%d,%d", CCT_Operation, Source[0], Source[1], Source[2]) < 0)
                    MY_LOGE("Fail to get response string!");
                Source += 3;
                for(int i=0; i<Cnf_Cnt; i++)
                {
                    char var[20] = { 0 };
                    if(sprintf(var, ",%d", Source[i]) < 0)
                        MY_LOGE("Fail to get var!");
            ...
            {
                if(sprintf(response,"CCT_op(failed) CCT_Operation(%d) is not supported\r\n", CCT_Operation) < 0)
                    MY_LOGE("Fail to get response!");
            }
            break;
        default:
            if(sprintf(response,"CCT_op(failed) at_op(%d) is not supported\r\n", at_op) < 0)
                MY_LOGE("Fail to get response!");
            break;
        }
