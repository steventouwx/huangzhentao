...
RFX_IMPLEMENT_DATA_CLASS(RfxRegistrationFailedData);

RfxRegistrationFailedData::RfxRegistrationFailedData(void *data, int length) : RfxBaseData(data, length)  {
    if (data != NULL) {
        RIL_RegistrationFailedResponse*tmp = (RIL_RegistrationFailedResponse *) data;
        RIL_RegistrationFailedResponse *pData =
                (RIL_RegistrationFailedResponse *) calloc(1, length);
...
RfxRegistrationFailedData::~RfxRegistrationFailedData() {
    if (m_data) {
        RIL_RegistrationFailedResponse *tmp = (RIL_RegistrationFailedResponse *) m_data;
