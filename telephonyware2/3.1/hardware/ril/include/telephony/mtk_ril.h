...
#define MAX_IPV4_ADDR_AND_MASK_LENGTH (MAX_IPV4_ADDRESS_LENGTH+3)
#define MAX_IPV6_ADDR_AND_MASK_LENGTH (MAX_IPV6_ADDRESS_LENGTH+3)

#define DOT_SEP_IPV4_ADDR_AND_SUBNETMASK_PART_NUM 8
#define DOT_SEP_IPV6_ADDR_AND_SUBNETMASK_PART_NUM 32
...
#define SETUP_DATA_CALL_TD_NONE 0
#define SETUP_DATA_CALL_TD_AOSP 1
#define SETUP_DATA_CALL_TD_EXT  2
#define SETUP_DATA_CALL_RESPONSE_AOSP 0
#define SETUP_DATA_CALL_RESPONSE_EXT  1

#define MAX_LCE_THRESHOLD_NUMBER 15

// TODO: replace with definition of correct module
...
#define SUGGEST_TIME_FOR_PDP_FAIL_REJECTED_BY_IWLAN 100
#ifdef TBOX_remove
#define MAX_SOCKET_NAME_LENGTH 20
#endif

#define APP_EVENT_STATUS_ARRAY_LEN 2
#define APP_EVENT_STATUS_KEY_INDEX 0
#define APP_EVENT_STATUS_VALUE_INDEX 1
#define APP_EVENT_VOIP_STATUS 1
#define APP_EVENT_VOIP_STATUS_ONGOING 1
...
typedef struct {
    ...
    char* reason_phrase;
    char* warn_text;
    char* cseq; // int to char*
    char* reason_cause; // int to char*
    char* reason_text;
    char* callId;
} RIL_SipRegInfo;
...
typedef struct {
    int rereg_flag;
    int rereg_remain_time;
    int resub_flag;
    int resub_remain_time;
} RIL_EregrtInfo;
...
typedef struct {
    ...
    int     mtu;        /* MTU received from network for IPv4
    ...
    int              qosSessionCount;
    RIL_QosSession * qosSessions;            /* vec<QosSession> qosSessions*/
    ...
    int              tdType;                 /* 0 for AOSP TD(OSAPPID,DNN),
                                                1 for full TD support (OS APP ID, DNN, IP 3 tuple, FQDN) */
...
/// M: request ca link type. @{
typedef enum {
    CA_LINK_TYPE_DL = 0,       // ca link type download
    CA_LINK_TYPE_UL,           // ca link type upload
    CA_LINK_TYPE_DL_AND_UL     // ca link type both download and upload
} MTK_RIL_CALinkType;
/// @}
...
/// Support serviceCategory with URN in AT+ESVC
typedef struct {
    int serviceCategory;
    const char *urn;
} RIL_ESVC_Data;
...
typedef struct {           // Default -1 for int and null for char*
    ...
    int32_t requestType;   // Reques type
...
enum MessageId {
    ...
    MSG_ID_UICC_TRAY_PLUG_OUT_WHEN_VSIM_ENABLED = 1011,
    MSG_ID_UICC_TRAY_PLUG_IN_WHEN_VSIM_ENABLED = 1012,
...
typedef struct {
  char           iccId[21];     //ICCID length is 20, and str '\0'
  char           unUsed[3];
  int8_t         logicalSlotId;
  uint8_t        portActive;
  uint8_t        emptyEuicc;
  char           padding[1];
} RIL_SimPortInfo;
...
typedef struct {
  ...
  char            *atr;
  char            *eid;
  /// M: Old value for HILD interface, Keep to avoid build error. @{
  ...
  int8_t          mepVersion;
  uint16_t        port_count;
  RIL_SimPortInfo info[MAX_RFX_SLOT_COUNT];
  RIL_MepMode     mepMode;
} RIL_SimSlotStatus;
...
typedef struct {
    ...
    int resultLen;
    char* result;
} RIL_EcholocateResponse;

//TMO Echolocate -Start
typedef enum {            // Request function ID
     GET_NETWORK_IDENTITY_METHOD = 1,
     GET_SIGNAL_CONDITION_METHOD = 2,
     GET_COMMON_RF_CONFIGURATION_METHOD = 3,
     GET_DOWNLINK_CARRIER_INFO_METHOD = 4,
     GET_UPLINK_CARRIER_INFO_METHOD = 5,
     GET_DOWNLINK_RF_CONFIGURATION_METHOD = 6,
     GET_UPLINK_RF_CONFIGURATION_METHOD = 7,
     GET_BEARER_CONFIGURATION_METHOD = 8,
     GET_DATA_SETTING_METHOD = 9,
     GET_ENDCLTELOG =10,
     GET_5GNRMMWCELLLOG = 11,
     GET_SSBBEANLOG = 12,
     GET_ENDCUPLINGLOG =13,
     GET_5GUILOG = 14,
     GET_API_VERSION = 15,
     GET_NETWORKTYPE_SEARCHING = 16
} RIL_EcholocateDataMetricsType;

#define INVALID_ID -1

#define PLMN_MVNO_DATA_PLMN_LEN     8
#define PLMN_MVNO_DATA_SPN_LEN      128
#define PLMN_MVNO_DATA_IMSI_LEN     32
#define PLMN_MVNO_DATA_GID_LEN      64
#define PLMN_MVNO_DATA_PNN_LEN      128
#define PLMN_MVNO_DATA_IMPI_LEN     160
typedef struct {
    char gsmPlmn[PLMN_MVNO_DATA_PLMN_LEN];
    char cdmaPlmn[PLMN_MVNO_DATA_PLMN_LEN];
    char gsmSpn[PLMN_MVNO_DATA_SPN_LEN];
    char cdmaSpn[PLMN_MVNO_DATA_SPN_LEN];
    char gsmImsi[PLMN_MVNO_DATA_IMSI_LEN];
    char cdmaImsi[PLMN_MVNO_DATA_IMSI_LEN];
    char gid1[PLMN_MVNO_DATA_GID_LEN];
    char pnn[PLMN_MVNO_DATA_PNN_LEN];
    char impi[PLMN_MVNO_DATA_IMPI_LEN];
} RIL_PlmnMvnoData;

#define ECHOLOCATE_API_5GSA_BASE 100
typedef enum {            // Request function ID
     GET_5GSA_DL_CARRIER = ECHOLOCATE_API_5GSA_BASE + 0,
     GET_5GSA_UL_CARRIER = ECHOLOCATE_API_5GSA_BASE + 1,
     GET_5GSA_RRC_LOG = ECHOLOCATE_API_5GSA_BASE + 2,
     GET_5GSA_NETWORK_LOG = ECHOLOCATE_API_5GSA_BASE + 3,
     GET_5GSA_API_VERSION = ECHOLOCATE_API_5GSA_BASE + 4
} RIL_EcholocateDataMetrics5gSaType;

#define ECHOLOCATE_VOICE_QOE 200
typedef enum {
    GET_VoiceAccessNetworkStateSignal = ECHOLOCATE_VOICE_QOE + 1
} RIL_EcholocateVoiceQoEType;

#define ECHOLOCATE_RF_METRICS 300
typedef enum {
    GET_RF_NTA_PRIMARY = ECHOLOCATE_RF_METRICS + 1,
    GET_RF_API_VERSION = ECHOLOCATE_RF_METRICS + 2,
    GET_RF_SRS_CONFIG  = ECHOLOCATE_RF_METRICS + 3
} RIL_EcholocateRFType;

typedef struct {
    int             status;      /* status 1 = active, 0 = not active */
...
typedef struct  {
    uint32_t dc_mss;
    uint8_t  client_ip[MAX_DC_IP_ADDR_LENGTH];  //MAX_DC_IP_ADDR_LENGTH, client IP address in binary, 16 bytes
    uint8_t  server_ip[MAX_DC_IP_ADDR_LENGTH]; //MAX_DC_IP_ADDR_LENGTH, server IP address in binary, 16 bytes
} RIL_DchDataPathConfig;

typedef struct RIL_dchDataPendingInd {
    int32_t slot;
    int32_t request;
    int32_t responseType;
    int32_t callId;
    char * data;
    int data_size;
    struct RIL_dchDataPendingInd *next;
} RIL_dchDataPendingInd;
...
static const int MAX_URI_LEN = 256;
...
static const int MAX_USER_AGENT_LEN = 256;
...
typedef struct {
    ...
    int32_t rcsMode;
    int64_t rcsFeatures;
    char localAddress[MAX_ADDRESS_LEN];
    int32_t localPort;
    char remoteAddress[MAX_ADDRESS_LEN];
    int32_t remotePort;
    char homeDomain[MAX_DOMAIN_LEN];
    int32_t transport;
    int32_t ipVersion;
    char IMPU[MAX_IMPU_LEN];
    char IMPI[MAX_IMPU_LEN];
    char userAgent[MAX_USER_AGENT_LEN];
    int32_t ipSecLocalPort;
    int32_t ipSecRemotePort;
    char svHeader[MAX_SECURIT_VERIFY_LEN];
    char pAssociatedUri[MAX_ASSOCIATED_URI];
    char instanceId[MAX_URI_LEN];
    char serviceRoute[MAX_SERVICE_ROUTE_LEN];
    char pAccessNetworkInfo[MAX_PANI_LEN];
    char regTimestamp[MAX_TIMESTAMP_LEN];
} RIL_RcsRegistrationInfo;
...
#define RIL_REQUEST_DEACTIVATE_NR_SCG_COMMUNICATION (RIL_REQUEST_VENDOR_BASE + 222)
#define RIL_REQUEST_GET_DEACTIVATE_NR_SCG_COMMUNICATION (RIL_REQUEST_VENDOR_BASE + 223)
...
/// M: SUBSIDYLOCK BLOB5.0
#define RIL_REQUEST_GET_SUBLOCK_VERSION (RIL_REQUEST_VENDOR_BASE + 231)
#define RIL_REQUEST_GET_SUBLOCK_UNLOCK_TIME (RIL_REQUEST_VENDOR_BASE + 232)

/// M: WIFI Relay @{
#define RIL_REQUEST_SEND_WIFI_ENABLED (RIL_REQUEST_VENDOR_BASE + 233)
#define RIL_REQUEST_SEND_WIFI_ASSOCIATED (RIL_REQUEST_VENDOR_BASE + 234)
#define RIL_REQUEST_SEND_WIFI_IP_ADDRESS (RIL_REQUEST_VENDOR_BASE + 235)
/// @}

// [Softbank requirement]: Enable or disable call sub address feature.
#define RIL_REQUEST_SET_CALL_SUB_ADDRESS (RIL_REQUEST_VENDOR_BASE + 236)
#define RIL_REQUEST_GET_CALL_SUB_ADDRESS (RIL_REQUEST_VENDOR_BASE + 237)

// [Video ringtone requirement]: Enable or disable call sub address feature.
#define RIL_REQUEST_VIDEO_RINGTONE_EVENT (RIL_REQUEST_VENDOR_BASE + 238)

#define RIL_REQUEST_SET_NR_OPTION (RIL_REQUEST_VENDOR_BASE + 239)

//M:NA new requirement
#define RIL_REQUEST_GET_TOE_INFO (RIL_REQUEST_VENDOR_BASE + 240)
#define RIL_REQUEST_DISABLE_ALL_CA_LINKS (RIL_REQUEST_VENDOR_BASE + 241)
#define RIL_REQUEST_GET_CA_LINK_ENABLE_STATUS (RIL_REQUEST_VENDOR_BASE + 242)
#define RIL_REQUEST_SET_CA_LINK_ENABLE_STATUS (RIL_REQUEST_VENDOR_BASE + 243)
#define RIL_REQUEST_GET_CA_LINK_CAPABILITY_LIST (RIL_REQUEST_VENDOR_BASE + 244)
#define RIL_REQUEST_GET_LTE_DATA (RIL_REQUEST_VENDOR_BASE + 245)
#define RIL_REQUEST_GET_LTE_RRC_STATE (RIL_REQUEST_VENDOR_BASE + 246)
#define RIL_REQUEST_SET_LTE_BAND_ENABLE_STATUS (RIL_REQUEST_VENDOR_BASE + 247)
#define RIL_REQUEST_GET_BAND_PRIORITY_LIST (RIL_REQUEST_VENDOR_BASE + 248)
#define RIL_REQUEST_SET_BAND_PRIORITY_LIST (RIL_REQUEST_VENDOR_BASE + 249)
#define RIL_REQUEST_SET_4X4MIMO_ENABLED (RIL_REQUEST_VENDOR_BASE + 250)
#define RIL_REQUEST_GET_4X4MIMO_ENABLED (RIL_REQUEST_VENDOR_BASE + 251)
#define RIL_REQUEST_GET_LTE_BSR_TIMER (RIL_REQUEST_VENDOR_BASE + 252)
#define RIL_REQUEST_SET_LTE_BSR_TIMER (RIL_REQUEST_VENDOR_BASE + 253)
#define RIL_REQUEST_GET_LTE_1XRTT_CELL_LIST (RIL_REQUEST_VENDOR_BASE + 254)
#define RIL_REQUEST_CLEAR_LTE_AVAILABLE_FILE (RIL_REQUEST_VENDOR_BASE + 255)
#define RIL_REQUEST_GET_BAND_MODE (RIL_REQUEST_VENDOR_BASE + 256)
#define RIL_REQUEST_GET_CA_BAND_MODE (RIL_REQUEST_VENDOR_BASE + 257)
#define RIL_REQUEST_GET_CAMPED_FEMTO_CELL_INFO (RIL_REQUEST_VENDOR_BASE + 258)
#define RIL_REQUEST_SET_QAM_ENABLED         (RIL_REQUEST_VENDOR_BASE + 259)
#define RIL_REQUEST_GET_QAM_ENABLED         (RIL_REQUEST_VENDOR_BASE + 260)
#define RIL_REQUEST_SET_TM9_ENABLED         (RIL_REQUEST_VENDOR_BASE + 261)
#define RIL_REQUEST_GET_TM9_ENABLED         (RIL_REQUEST_VENDOR_BASE + 262)
#define RIL_REQUEST_SET_LTE_SCAN_DURATION   (RIL_REQUEST_VENDOR_BASE + 263)
#define RIL_REQUEST_GET_LTE_SCAN_DURATION   (RIL_REQUEST_VENDOR_BASE + 264)
#define RIL_REQUEST_SET_DISABLE_2G (RIL_REQUEST_VENDOR_BASE + 265)
#define RIL_REQUEST_GET_DISABLE_2G (RIL_REQUEST_VENDOR_BASE + 266)

//Echolocate
#define RIL_REQUEST_GET_ECHOLOCATE_METRICS (RIL_REQUEST_VENDOR_BASE + 267)

#define RIL_REQUEST_IWLAN_REGISTRATION_STATE (RIL_REQUEST_VENDOR_BASE + 268)
#define RIL_REQUEST_GET_ALL_BAND_MODE (RIL_REQUEST_VENDOR_BASE + 269)
#define RIL_REQUEST_SET_NR_BAND_MODE (RIL_REQUEST_VENDOR_BASE + 270)

//WFC
#define RIL_REQUEST_GET_WFC_CONFIG (RIL_REQUEST_VENDOR_BASE + 271)

// SMS SCBM
#define RIL_REQUEST_EXIT_SMS_SCBM (RIL_REQUEST_VENDOR_BASE + 272)

//RCS
#define RIL_REQUEST_RCS_SWITCH_ROI_STATUS   (RIL_REQUEST_VENDOR_BASE + 273)
#define RIL_REQUEST_RCS_UPDATE_CAPABILITIES (RIL_REQUEST_VENDOR_BASE + 274)
#define RIL_REQUEST_RCS_UPDATE_SESSION_INFO (RIL_REQUEST_VENDOR_BASE + 275)
#define RIL_REQUEST_RCS_GET_USER_AGENT      (RIL_REQUEST_VENDOR_BASE + 276)

// SS barring sepcific call
#define RIL_REQUEST_GET_BARRING_SEPCIFIC_CALL (RIL_REQUEST_VENDOR_BASE + 277)
#define RIL_REQUEST_SET_BARRING_SEPCIFIC_CALL (RIL_REQUEST_VENDOR_BASE + 278)

#define RIL_REQUEST_SET_CARRIER_AGGREGATION_MODE (RIL_REQUEST_VENDOR_BASE + 279)

// Vendor data channel solution
#define RIL_REQUEST_DCH_BOOTSTRAP_CFG_QRY_CMD_RSP (RIL_REQUEST_VENDOR_BASE + 280)
#define RIL_REQUEST_DCH_CREATE_SIP_REQ      (RIL_REQUEST_VENDOR_BASE + 281)
#define RIL_REQUEST_DCH_MODIFY_SIP_REQ      (RIL_REQUEST_VENDOR_BASE + 282)
#define RIL_REQUEST_DCH_CLOSE_SIP_REQ       (RIL_REQUEST_VENDOR_BASE + 283)
#define RIL_REQUEST_DCH_CREATE_CMD_RSP      (RIL_REQUEST_VENDOR_BASE + 284)
#define RIL_REQUEST_DCH_ENABLE_REQ          (RIL_REQUEST_VENDOR_BASE + 285)

// MTK data channel solution
#define RIL_REQUEST_DCH_ACT_PACKET_ROUTE_REQ        (RIL_REQUEST_VENDOR_BASE + 286)
#define RIL_REQUEST_DCH_IFUP_PACKET_ROUTE_REQ       (RIL_REQUEST_VENDOR_BASE + 287)
#define RIL_REQUEST_DCH_DATA_PATH_CONFIG_SYNC_REQ   (RIL_REQUEST_VENDOR_BASE + 288)
#define RIL_REQUEST_DCH_CREATE_DATA_CHANNEL_REQ     (RIL_REQUEST_VENDOR_BASE + 289)
#define RIL_REQUEST_DCH_CLOSE_DATA_CHANNEL_REQ      (RIL_REQUEST_VENDOR_BASE + 290)

// extended setup data call with more TD
#define RIL_REQUEST_SETUP_DATA_CALL_SLICE (RIL_REQUEST_VENDOR_BASE + 291)

#define RIL_REQUEST_SYNC_APP_EVENT_STATUS (RIL_REQUEST_VENDOR_BASE + 292)

// RCS new requests definition
#define RIL_REQUEST_RCS_START_SERVICE       (RIL_REQUEST_VENDOR_BASE + 293)
#define RIL_REQUEST_RCS_STOP_SERVICE        (RIL_REQUEST_VENDOR_BASE + 294)
#define RIL_REQUEST_RCS_REGISTRATION_INFO   (RIL_REQUEST_VENDOR_BASE + 295)
#define RIL_REQUEST_RCS_ENABLE_DEREG_IND    (RIL_REQUEST_VENDOR_BASE + 296)
#define RIL_REQUEST_RCS_RESUME_DEREGISTRATION (RIL_REQUEST_VENDOR_BASE + 297)
#define RIL_REQUEST_RCS_TRIGGER_RESTORATION (RIL_REQUEST_VENDOR_BASE + 298)
#define RIL_REQUEST_RCS_SEND_SIP_MESSAGE    (RIL_REQUEST_VENDOR_BASE + 299)

//SIM
#define RIL_REQUEST_SWITCH_SIM_TYPE     (RIL_REQUEST_VENDOR_BASE + 300)
#define RIL_REQUEST_GET_SIM_TYPE_INFO   (RIL_REQUEST_VENDOR_BASE + 301)

//NITZ
#define RIL_REQUEST_GET_NITZ   (RIL_REQUEST_VENDOR_BASE + 302)
//GET IMS CONFIG
#define RIL_REQUEST_GET_IMSCFG   (RIL_REQUEST_VENDOR_BASE + 303)
/*********************************************************************************/
...
#define RIL_UNSOL_IMS_REG_FLAG_IND (RIL_UNSOL_VENDOR_BASE + 48)
...
#define RIL_UNSOL_SMART_RAT_SWITCH_INFO (RIL_UNSOL_VENDOR_BASE + 146)

/// M: SIM: Iccid changed indication
#define RIL_UNSOL_ICCID_CHANGED (RIL_UNSOL_VENDOR_BASE + 147)

#define RIL_UNSOL_PLMN_DATA (RIL_UNSOL_VENDOR_BASE + 148)

// Common RSU urc
#define RIL_UNSOL_SML_RSU_EVENT (RIL_UNSOL_VENDOR_BASE + 149)

#define RIL_UNSOL_RESPONSE_SMS_EINFO_EXTENSIONS (RIL_UNSOL_VENDOR_BASE + 150)

#define RIL_UNSOL_EREGRT_INFO_IND (RIL_UNSOL_VENDOR_BASE + 151)

/// M: IMS Video Ringtone Event indication
#define RIL_UNSOL_VIDEO_RINGTONE_EVENT_IND (RIL_UNSOL_VENDOR_BASE + 152)

/// M: NA requirement
#define RIL_UNSOL_TOE_INFO_IND (RIL_UNSOL_VENDOR_BASE + 153)
#define RIL_UNSOL_NETWORK_BAND_INFO_IND (RIL_UNSOL_VENDOR_BASE + 154)
#define RIL_UNSOL_SIB16_TIME_INFO_IND (RIL_UNSOL_VENDOR_BASE + 155)

/// M: VoDate usage
#define RIL_UNSOL_MD_INTERNET_USAGE_IND (RIL_UNSOL_VENDOR_BASE + 156)

/// M: Nw cfg info
#define RIL_UNSOL_NW_CFG_INFO (RIL_UNSOL_VENDOR_BASE + 157)
/// M: Rrc state
#define RIL_UNSOL_RRC_STATE (RIL_UNSOL_VENDOR_BASE + 158)
/// M: IWlan registration state
#define RIL_UNSOL_IWLAN_REGISTRATION_STATE (RIL_UNSOL_VENDOR_BASE + 159)
/// M: NA requirement
#define RIL_UNSOL_NR_CA_BAND_IND (RIL_UNSOL_VENDOR_BASE + 160)
///M: NW 5GUW
#define RIL_UNSOL_5GUW_INFO_IND (RIL_UNSOL_VENDOR_BASE + 161)
/// M: NR 5G system information
#define RIL_UNSOL_NR_SYS_INFO (RIL_UNSOL_VENDOR_BASE + 162)

// SCBM
#define RIL_UNSOL_ENTER_SMS_SCBM (RIL_UNSOL_VENDOR_BASE + 163)
#define RIL_UNSOL_EXIT_SMS_SCBM (RIL_UNSOL_VENDOR_BASE + 164)

//Vendor data channel solution
#define RIL_UNSOL_DCH_BOOTSTRAP_CFG_QRY_CMD  (RIL_UNSOL_VENDOR_BASE + 165)
#define RIL_UNSOL_DCH_CONFIG_INIT_IND        (RIL_UNSOL_VENDOR_BASE + 166)
#define RIL_UNSOL_DCH_CONFIG_UPDATE_IND      (RIL_UNSOL_VENDOR_BASE + 167)
#define RIL_UNSOL_DCH_CONFIG_DEINIT_IND      (RIL_UNSOL_VENDOR_BASE + 168)
#define RIL_UNSOL_DCH_CREATE_CMD             (RIL_UNSOL_VENDOR_BASE + 169)

// Add for URSP RAW data string
#define RIL_UNSOL_SLICING_CONFIG_CHANGED_EX (RIL_UNSOL_VENDOR_BASE + 170)

// MTK data channel solution
#define RIL_UNSOL_DCH_IFUP_PACKET_ROUTE_IND        (RIL_UNSOL_VENDOR_BASE + 171)
#define RIL_UNSOL_DCH_CONNECT_DATA_CHANNEL_IND     (RIL_UNSOL_VENDOR_BASE + 172)
#define RIL_UNSOL_DCH_CLOSE_DATA_CHANNEL_IND       (RIL_UNSOL_VENDOR_BASE + 173)
#define RIL_UNSOL_DCH_CREATE_DATA_CHANNEL_IND      (RIL_UNSOL_VENDOR_BASE + 174)

// RCS new requests
#define RIL_UNSOL_RCS_REGISTRATION_INFO_IND (RIL_UNSOL_VENDOR_BASE + 175)
#define RIL_UNSOL_RCS_DEREGISTRATION_IND    (RIL_UNSOL_VENDOR_BASE + 176)
#define RIL_UNSOL_RCS_SIP_MESSAGE_IND       (RIL_UNSOL_VENDOR_BASE + 177)


// IMS CC related
#define RIL_UNSOL_MT_SIP_INVITE_INFO (RIL_UNSOL_VENDOR_BASE + 178)

#define RIL_UNSOL_SIM_TYPE_INFO_IND (RIL_UNSOL_VENDOR_BASE + 179)
...
typedef enum {
    ...
    ACCESS_NW_TYPE_VODATA1 = 3,
    ACCESS_NW_TYPE_VODATA2 = 4,
    ACCESS_NW_TYPE_VODATA3 = 5,
    ACCESS_NW_TYPE_VODATA4 = 6
} ACCESS_NETWORK_TYPE;
