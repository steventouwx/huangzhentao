...
RfxSimSlotStatusData::RfxSimSlotStatusData(void *_data, int _length) : RfxBaseData(_data, _length) {
    ...
    if (countOfSlot > 0) {
        ...
        for (int i = 0; i < countOfSlot; i++) {
            ...
            /// New aidl structure value. @{
            pp_slot_status[i]->mepVersion = tmp[i]->mepVersion;
            pp_slot_status[i]->port_count = tmp[i]->port_count;
            RFX_ASSERT(pp_slot_status[i]->port_count <= MAX_RFX_SLOT_COUNT);
            for (int j = 0; j < pp_slot_status[i]->port_count ; j++) {
                int len = strlen(tmp[i]->info[j].iccId);
                strncpy(pp_slot_status[i]->info[j].iccId, tmp[i]->info[j].iccId, len);

                pp_slot_status[i]->info[j].logicalSlotId = tmp[i]->info[j].logicalSlotId;
                pp_slot_status[i]->info[j].portActive = tmp[i]->info[j].portActive;
            }

            pp_slot_status[i]->mepMode = tmp[i]->mepMode;
            /// @}

            /// Old hidl structure value. @{
