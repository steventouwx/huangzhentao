...
public:
    ...
    // Get service category and URN of emergency number
    //
    // RETURNS: service category value defined 3gpp 22.101, Section 10 - Emergency Calls
    int getServiceCategoryAndUrn(String8 number, String8 &urn);
...
private:
    ...
    void handleNetworkEcc(const sp<RfxMessage>& message);
    ...
    bool isCtCard();
    bool isCtPlmn(String8 plmn);
    bool isValidPlmn(String8 plmn);
    void dumpEccList();
    void testEcc();
    ...
    EmergencyServiceCategory getCategoryFromUrn(char* urn, char *strUrn);
    void updateNetworkEccList(bool notify);
    unsigned char hexChar2Number(char hex);
    unsigned char strHex2Byte(char*pHex);
    int gsm7bitToAscii(char *pDst, char *pSrc, int maxLen);
...
private:
    ...
    bool mNeedRefreshExtNwEcc;
    String8 mNwEccPlmn;
    int mExtNwEccValidity;
    Vector<EmergencyNumber> mNwEccList;
    Vector<EmergencyNumber> mExtNwEccList;
};
