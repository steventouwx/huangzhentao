...
RmcGsmSmsUrcHandler::~RmcGsmSmsUrcHandler() {
    if (mLastSms != NULL) {
        free(mLastSms);
        mLastSms = NULL;
    }
...
void RmcGsmSmsUrcHandler::onHandleUrc(const sp<RfxMclMessage>& msg) {
    ...
    } else if (strStartsWith(ss, "+EPSI:")) {
        handleSmsEInfoExtensions(msg);
    }
}

void RmcGsmSmsUrcHandler::onHandleEvent(const sp<RfxMclMessage>& msg) {
    int event = msg->getId();
    switch(event) {
...
void RmcGsmSmsUrcHandler::onNewSms(const sp<RfxMclMessage>& msg) {
    ...
    length = strlen(smsPdu->getLine());
    if (mLastSms != NULL && length == strlen(mLastSms) &&
            strcmp(smsPdu->getLine(), mLastSms) == 0) {
        sendEvent(RFX_MSG_EVENT_SMS_DUP_DELAY, RfxStringData(smsPdu->getLine(), length),
    ...
    mLastSms = (char*)malloc(length + 1);
    RFX_ASSERT(mLastSms != NULL);
    strncpy(mLastSms, smsPdu->getLine(), length);
    mLastSms[length] = '\0';

    unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_RESPONSE_NEW_SMS,
            m_slot_id, RfxStringData(smsPdu->getLine(), length));
    responseToTelCore(unsol);
...
void RmcGsmSmsUrcHandler::handleNewBroadcastSms(const sp<RfxMclMessage>& msg) {
    ...
    if (asprintf(&line, "00%s", pSmsPdu->getLine()) < 0) {
        goto error;
    }
    ...
    if (byteSmsPdu == NULL) {
        logE(mTag, "Failed to allocate memory for byteSmsPdu");
        return;
    }
...
void RmcGsmSmsUrcHandler::handleSmsEInfoExtensions(const sp<RfxMclMessage>& msg) {
    int err = 0;
    char *strPara = NULL;
    String8 info("");
    RfxAtLine *pUrc = msg->getRawUrc();
    ...
    unsol = RfxMclMessage::obtainUrc(RFX_MSG_URC_SMS_EINFO_EXTENSIONS,
...
error:
    logE(mTag, "There is something wrong in handleSmsEInfoExtensions");
}
