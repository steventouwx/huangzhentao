...
private:
    ...
    void requestSetSystemSelectionChannels(const sp<RfxMessage>& message);
    void responseSetSystemSelectionChannels(const sp<RfxMessage>& message);
    ...
    bool isNetworkTypeContainGsmCdma();
    ...
    void handleGmssRatForNrExtend(const sp<RfxMessage>& message);
...
private:
    ...
    bool mIsNeedTurnOffRadio;
    RIL_Errno mError;
    bool backupRadioPower[MAX_SIM_COUNT];
    int radioCount;

    int mDisable2GReqToken;
