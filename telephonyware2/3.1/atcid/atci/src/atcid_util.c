...
void get_substring(char *source_str, int start_index, int count, char *result_str) {
    int source_str_len = strlen(source_str);

    if (start_index < 0 || start_index >= source_str_len) {
        result_str[0] = '\0';
        return;
    }
    ...
    if (end_index >= source_str_len) {
        end_index = source_str_len - 1;
    }

    int result_str_len = end_index - start_index + 1;

    strncpy(result_str, source_str + start_index, result_str_len);
