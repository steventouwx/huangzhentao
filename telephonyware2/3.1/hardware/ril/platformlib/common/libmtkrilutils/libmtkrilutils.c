...
#include <ratconfig.h>
/// @}
#include "telephony/mtk_custom_feature_config.h"
...
static const char *sAtCmdForOemHook[] = {
    "AT+EGMC",
    "AT+EDMFAPP",
    "AT+EGCMD",
    "AT+EIMSTESTMODE",
...
static unsigned int sSimCount = -1;
unsigned int getSimCount() {
...
int getMdMaxSimCount() {
    char property_value[MTK_PROPERTY_VALUE_MAX] = { 0 };
    mtk_property_get("ro.vendor.num_md_protocol", property_value, "2");
    return atoi(property_value);
}
...
int isInternalLoad() {
    return 0;
    /* __PRODUCTION_RELEASE__  and internal load is not supported */
    /*
    #ifdef __PRODUCTION_RELEASE__
        return 0;
    #else
        static int isInternalload = -1;
        if (isInternalload == -1) {
            char property_value[MTK_PROPERTY_VALUE_MAX] = { 0 };
            mtk_property_get("vendor.ril.emulation.production", property_value, "0");
            isInternalload = (strcmp("1", property_value) != 0);
        }
        return isInternalload;
    #endif */ /* __PRODUCTION_RELEASE__ */

}

int isEngLoad() {
    static int isEngLoad = -1;
    if (isEngLoad == -1) {
        char property_value[MTK_PROPERTY_VALUE_MAX] = { 0 };
...
int isUserDebugLoad() {
    static int isUserDebugLoad = -1;
    if (isUserDebugLoad == -1) {
        char property_value[MTK_PROPERTY_VALUE_MAX] = { 0 };
        mtk_property_get("ro.build.type", property_value, "");
        property_value[MTK_PROPERTY_VALUE_MAX - 1] = '\0';
        isUserDebugLoad = (strcmp("userdebug", property_value) == 0);
    }
    return isUserDebugLoad;
}

int isUserLoad() {
    static int isUserLoad = -1;
    if (isUserLoad == -1) {
...
void setMSimProperty(int phoneId, char *pPropertyName, char *pUpdateValue) {
    ...
    if (pUpdateValue == NULL) {
        mtkLogD(LOG_TAG, "setMSimProperty ERROR: pUpdateValue is NULL");
        return;
    }
...
unsigned char* pii(char* input) {
    ...
    size_t outLength = 0;
    memset(sha1Result, 0, sizeof(sha1Result));
...
int isDisable2G() {
    ...
    if (strcmp("OP07", optr) == 0) {
        mtk_property_get("persist.vendor.radio.disable.2g", property_value, "1");
...
const char* mtkRequestToString(int request) {
    switch(request) {
        ...
        case RIL_REQUEST_SWITCH_SIM_TYPE: return "RIL_REQUEST_SWITCH_SIM_TYPE";
        case RIL_REQUEST_GET_SIM_TYPE_INFO: return "RIL_REQUEST_GET_SIM_TYPE_INFO";
        case RIL_UNSOL_SIM_TYPE_INFO_IND: return "RIL_UNSOL_SIM_TYPE_INFO";
        ...
        case RIL_UNSOL_RESPONSE_SMS_EINFO_EXTENSIONS: return "RIL_UNSOL_RESPONSE_SMS_EINFO_EXTENSIONS";
        // SMS-END
        ...
        case RIL_REQUEST_SET_CALL_SUB_ADDRESS: return "RIL_REQUEST_SET_CALL_SUB_ADDRESS";
        case RIL_REQUEST_GET_CALL_SUB_ADDRESS: return "RIL_REQUEST_GET_CALL_SUB_ADDRESS";
        ...
        case RIL_REQUEST_GET_BARRING_SEPCIFIC_CALL: return "RIL_REQUEST_GET_BARRING_SEPCIFIC_CALL";
        case RIL_REQUEST_SET_BARRING_SEPCIFIC_CALL: return "RIL_REQUEST_SET_BARRING_SEPCIFIC_CALL";
        // IMS SS : End
        // PHB START
        ...
        case RIL_UNSOL_ON_XUI: return "RIL_UNSOL_ON_XUI";
        case RIL_UNSOL_IMS_REG_FLAG_IND: return "RIL_UNSOL_IMS_REG_FLAG_IND";
        case RIL_UNSOL_EREGRT_INFO_IND: return "RIL_UNSOL_EREGRT_INFO_IND";
        case RIL_UNSOL_MD_INTERNET_USAGE_IND: return "RIL_UNSOL_MD_INTERNET_USAGE_IND";
        /// [IMS][framework] @}
        ...
        // M: URSP raw data
        case RIL_UNSOL_SLICING_CONFIG_CHANGED_EX: return "RIL_UNSOL_SLICING_CONFIG_CHANGED_EX";
        ...
        case RIL_UNSOL_PLMN_DATA: return "RIL_UNSOL_PLMN_DATA";
        ...
        /// @}
        case RIL_REQUEST_SET_DISABLE_2G: return "RIL_REQUEST_SET_DISABLE_2G";
        case RIL_REQUEST_GET_DISABLE_2G: return "RIL_REQUEST_GET_DISABLE_2G";
        ...
        case RIL_UNSOL_ICCID_CHANGED: return "RIL_UNSOL_ICCID_CHANGED";
        /// M: NW: nw cfg info
        case RIL_UNSOL_NW_CFG_INFO: return "RIL_UNSOL_NW_CFG_INFO";
        /// M: NW: rrc state change
        case RIL_UNSOL_RRC_STATE: return "RIL_UNSOL_RRC_STATE";
        /// M: NW: iwlan registration state
        case RIL_UNSOL_IWLAN_REGISTRATION_STATE: return "RIL_UNSOL_IWLAN_REGISTRATION_STATE";
        case RIL_UNSOL_NR_CA_BAND_IND: return "RIL_UNSOL_NR_CA_BAND_IND";
        case RIL_REQUEST_GET_ALL_BAND_MODE: return "RIL_REQUEST_GET_ALL_BAND_MODE";
        case RIL_REQUEST_SET_NR_BAND_MODE: return "RIL_REQUEST_SET_NR_BAND_MODE";
        case RIL_UNSOL_5GUW_INFO_IND: return "RIL_UNSOL_5GUW_INFO_IND";
        /// M: NW: NR 5G system information
        case RIL_UNSOL_NR_SYS_INFO: return "RIL_UNSOL_NR_SYS_INFO";
        // M: data channel @{
        case RIL_REQUEST_DCH_BOOTSTRAP_CFG_QRY_CMD_RSP:
            return "RIL_REQUEST_DCH_BOOTSTRAP_CFG_QRY_CMD_RSP";
        case RIL_REQUEST_DCH_CREATE_SIP_REQ:
            return "RIL_REQUEST_DCH_CREATE_SIP_REQ";
        case RIL_REQUEST_DCH_MODIFY_SIP_REQ:
            return "RIL_REQUEST_DCH_MODIFY_SIP_REQ";
        case RIL_REQUEST_DCH_CLOSE_SIP_REQ:
            return "RIL_REQUEST_DCH_CLOSE_SIP_REQ";
        case RIL_REQUEST_DCH_CREATE_CMD_RSP:
            return "RIL_REQUEST_DCH_CREATE_CMD_RSP";
        case RIL_REQUEST_DCH_ENABLE_REQ:
            return "RIL_REQUEST_DCH_ENABLE_REQ";
        case RIL_UNSOL_DCH_BOOTSTRAP_CFG_QRY_CMD:
            return "RIL_UNSOL_DCH_BOOTSTRAP_CFG_QRY_CMD";
        case RIL_UNSOL_DCH_CONFIG_INIT_IND:
            return "RIL_UNSOL_DCH_CONFIG_INIT_IND";
        case RIL_UNSOL_DCH_CONFIG_UPDATE_IND:
            return "RIL_UNSOL_DCH_CONFIG_UPDATE_IND";
        case RIL_UNSOL_DCH_CONFIG_DEINIT_IND:
            return "RIL_UNSOL_DCH_CONFIG_DEINIT_IND";
        case RIL_UNSOL_DCH_CREATE_CMD:
            return "RIL_UNSOL_DCH_CREATE_CMD";
        case RIL_REQUEST_DCH_ACT_PACKET_ROUTE_REQ:
            return "RIL_REQUEST_DCH_ACT_PACKET_ROUTE_REQ";
        case RIL_REQUEST_DCH_IFUP_PACKET_ROUTE_REQ:
            return "RIL_REQUEST_DCH_IFUP_PACKET_ROUTE_REQ";
        case RIL_REQUEST_DCH_DATA_PATH_CONFIG_SYNC_REQ:
            return "RIL_REQUEST_DCH_DATA_PATH_CONFIG_SYNC_REQ";
        case RIL_REQUEST_DCH_CREATE_DATA_CHANNEL_REQ:
            return "RIL_REQUEST_DCH_CREATE_DATA_CHANNEL_REQ";
        case RIL_REQUEST_DCH_CLOSE_DATA_CHANNEL_REQ:
            return "RIL_REQUEST_DCH_CLOSE_DATA_CHANNEL_REQ";
        case RIL_UNSOL_DCH_IFUP_PACKET_ROUTE_IND:
            return "RIL_UNSOL_DCH_IFUP_PACKET_ROUTE_IND";
        case RIL_UNSOL_DCH_CONNECT_DATA_CHANNEL_IND:
            return "RIL_UNSOL_DCH_CONNECT_DATA_CHANNEL_IND";
        case RIL_UNSOL_DCH_CLOSE_DATA_CHANNEL_IND:
            return "RIL_UNSOL_DCH_CLOSE_DATA_CHANNEL_IND";
        case RIL_UNSOL_DCH_CREATE_DATA_CHANNEL_IND:
            return "RIL_UNSOL_DCH_CREATE_DATA_CHANNEL_IND";
        // @}
        // extended setup data call with more TD
        case RIL_REQUEST_SETUP_DATA_CALL_SLICE:
            return "RIL_REQUEST_SETUP_DATA_CALL_SLICE";
        case RIL_REQUEST_SYNC_APP_EVENT_STATUS:
            return "RIL_REQUEST_SYNC_APP_EVENT_STATUS";
        // M: RCS requests @{
        case RIL_REQUEST_RCS_SWITCH_ROI_STATUS:
            return "RIL_REQUEST_RCS_SWITCH_ROI_STATUS";
        case RIL_REQUEST_RCS_UPDATE_CAPABILITIES:
            return "RIL_REQUEST_RCS_UPDATE_CAPABILITIES";
        case RIL_REQUEST_RCS_UPDATE_SESSION_INFO:
            return "RIL_REQUEST_RCS_UPDATE_SESSION_INFO";
        case RIL_REQUEST_RCS_GET_USER_AGENT:
            return "RIL_REQUEST_RCS_GET_USER_AGENT";
        case RIL_REQUEST_RCS_START_SERVICE:
            return "RIL_REQUEST_RCS_START_SERVICE";
        case RIL_REQUEST_RCS_STOP_SERVICE:
            return "RIL_REQUEST_RCS_STOP_SERVICE";
        case RIL_REQUEST_RCS_REGISTRATION_INFO:
            return "RIL_REQUEST_RCS_REGISTRATION_INFO";
        case RIL_REQUEST_RCS_ENABLE_DEREG_IND:
            return "RIL_REQUEST_RCS_ENABLE_DEREG_IND";
        case RIL_REQUEST_RCS_RESUME_DEREGISTRATION:
            return "RIL_REQUEST_RCS_RESUME_DEREGISTRATION";
        case RIL_REQUEST_RCS_TRIGGER_RESTORATION:
            return "RIL_REQUEST_RCS_TRIGGER_RESTORATION";
        case RIL_REQUEST_RCS_SEND_SIP_MESSAGE:
            return "RIL_REQUEST_RCS_SEND_SIP_MESSAGE";
        case RIL_UNSOL_RCS_REGISTRATION_INFO_IND:
            return "RIL_UNSOL_RCS_REGISTRATION_INFO_IND";
        case RIL_UNSOL_RCS_DEREGISTRATION_IND:
            return "RIL_UNSOL_RCS_DEREGISTRATION_IND";
        case RIL_UNSOL_RCS_SIP_MESSAGE_IND:
            return "RIL_UNSOL_RCS_SIP_MESSAGE_IND";
        // @}
...
int isOp236Support() {
    ...
    char optr_value[MTK_PROPERTY_VALUE_MAX] = {0};
    mtk_property_get("persist.vendor.operator.optr", optr_value, "0");
    if (strcmp(optr_value, OPERATOR_OP236) == 0) {
        ret = 1;
    }
    return ret;
}
...
int needToHidenLog(const char *target) {
    unsigned int num = NUM_ELEMS(sAtCmdWithoutContent);
...
/**
 * return >=0 for can send, 0 is not
*/
int canSendAtUsingOemHook(const char *target) {
    unsigned int num = NUM_ELEMS(sAtCmdForOemHook);
    for (unsigned int i = 0; i < num; i++) {
        if (strStartsWith(target, sAtCmdForOemHook[i])) {
            return i;
        }
    }
    return -1;
...
bool isMipcSupport() {
    ...
    isMipcSupport = atoi(rilMode);
    return (isMipcSupport == 1);
}

static int sRILDInitDoneState = 0;
bool getRildInitDoneState() {
    return (sRILDInitDoneState == 1);
}

void setRildInitDoneState(int state) {
    sRILDInitDoneState = state;
}

bool isAllowImsOffIfSimAbsentConfigured() {
    return MTK_ALLOW_IMS_OFF_IF_SIM_ABSENT_CONFIG == 1 ? true : false;
}

bool isImsEnabledOp15ProvisioningConfig() {
    return MTK_IMS_ENABLED_OP15_PROVISIONING_CONFIG == 1 ? true : false;
}

bool isImsEnabledSetUserAgent() {
    return MTK_IMS_ENABLED_SET_USER_AGENT == 1 ? true : false;
}

bool isWifiApnExplicitlyConfigured() {
    return MTK_WIFI_APN_BITMASK_EXPLICIT_CONFIG == 1 ? true : false;
}

int getDefault5GNsaDisplayRule() {
    return MTK_DEFAULT_5G_NSA_DISPLAY_RULE == 0 ? 0 : 1;
}

bool isUrcNrSysInfoConfigured() {
    return MTK_URC_CONFIG_NR_SYS_INFO == 1 ? true : false;
}

bool isUrcChInfoConfigured() {
    return MTK_URC_CONFIG_CH_INFO == 1 ? true : false;
}

bool isUrcNrCaBandConfigured() {
    return MTK_URC_CONFIG_NR_CA_BAND == 1 ? true : false;
}

int getUrcCaInfoConfigureValue() {
    if (MTK_URC_CONFIG_CA_INFO == 2) return 2;
    else if (MTK_URC_CONFIG_CA_INFO == 1)  return 1;
    else return 0;
}

bool isDisable2GWithRatConfigured() {
    return MTK_DISABLE_2G_WITH_ALLOWED_NETWORK_TYPE == 1 ? true : false;
}

bool isSpecificPsStateConfigured() {
    return MTK_SPECIFIC_PS_STATE_IN_CALL == 1 ? true : false;
}

bool isSpecificVoiceStateConfigured() {
    return MTK_SPECIFIC_VOICE_STATE == 1 ? true : false;
}

bool isUrcNwCfgInfoConfigured() {
    return MTK_URC_CONFIG_NWCFGINFO == 1 ? true : false;
}

bool isApShouldReportDataFailCause() {
    return MTK_AP_DATA_FAIL_CAUSE_REPORT == 1 ? true : false;
}

bool isAutoApproveIncomingCallIndication() {
    return MTK_AUTO_APPROVE_INCOMING_CALL_INDICATION == 1 ? true : false;
}

bool isApShouldReportCsCallDisconnectCause() {
    return (MTK_AP_CS_CALL_DISCONNECT_CAUSE_REPORT == 1) ? true : false;
}

bool isApShouldReportImsCallDisconnectCause() {
    return (MTK_AP_IMS_CALL_DISCONNECT_CAUSE_REPORT == 1) ? true : false;
}

bool isSendEmcsBeforeTurnOnRadio() {
    return (MTK_SEND_EMCS_BEFORE_TURN_ON_RADIO_IN_RILD == 1) ? true : false;
}

bool isUssdConvertToNotify() {
    return (MTK_USSD_CONVERT_TERMINATED_TO_NOTIFY == 1) ? true : false;
}

bool isAllowNotifyCustomizationSimRefreshTypeConfigured() {
    return MTK_ALLOW_NOTIFY_CUSTOM_SIMREFRESH_TYPE_CONFIG == 1 ? true : false;
}

bool isNddsRfOccupySupported() {
    return MTK_NON_DDS_RF_OCCUPY_SUPPORT == 1 ? true : false;
}
