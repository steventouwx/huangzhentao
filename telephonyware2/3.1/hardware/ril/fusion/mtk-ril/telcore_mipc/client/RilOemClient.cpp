...
void RilOemClient::requestComplete(RIL_Token token, RIL_Errno e, const void *response,
        size_t responselen) {
    RFX_UNUSED(responselen);
    RfxRequestInfo *info = (RfxRequestInfo *) token;
    if (RFX_MSG_REQUEST_RESET_RADIO == info->request) {
        if (commandFd >= 0) {
            close(commandFd);
        }
    ...
    } else if (RFX_MSG_REQUEST_CLIENT_RSU == info->request) {
        RFX_UNUSED(responselen);
        // op logic
        char* strResult = NULL;
        RFX_LOG_I(RFX_LOG_TAG, "[RSU-SIMLOCK] request for RSU returned");
        RIL_RsuResponse *rsuResponse = (RIL_RsuResponse*)response;
        if(RIL_E_SUCCESS == e){
            asprintf(&strResult, "%s",(char*)rsuResponse->data);
        } else {
        ...
        if (mRsuFd >= 0) {
            ...
            int fd = mRsuFd;
            if (strResult != NULL) {
                len = (int)strlen(strResult);
            }
            // to avoid client active after close() immediately just before mRsuFd = -1, the
            // client should ensure that the new request is sent only after received the response
            // of the previous one.
            mRsuFd = -1;
            ret = send(fd, &len, sizeof(int), MSG_NOSIGNAL);
            ret = send(fd, strResult, len, MSG_NOSIGNAL);
...
int RilOemClient::handleSpecialRequestWithArgs(int argCount, char** args) {
    ...
    if (1 == argCount) {
        ...
        line = new (std::nothrow) RfxAtLine(args[0], NULL);
        if (line == NULL) {
            RFX_ASSERT(0);
        }
        ...
        if (strcmp(cmd, (char *) "THERMAL") == 0) {
            ...
            delete line;
            return NO_ACTION;
         } else if (strcmp(cmd, "HSR_MODE") == 0) {
            executeHsrMode(orgArgs);
            delete line;
            return NO_ACTION;
            ...
            delete line;
            return NO_ACTION;
        } else if (strcmp(cmd, "SIMMELOCK_COMMON") == 0) {
            executeRsuOpt(orgArgs);
            delete line;
            return SUCCESS;
        } else {
            delete line;
...
void RilOemClient::executeThermal(char *arg) {
    RFX_LOG_E(RFX_LOG_TAG, "executeThermal(%s) should not be here for after gen93 chip!", arg);
}
...
void RilOemClient::executeShutDownByThermal(char* arg) {
    ...
    RfxAtLine *line = new (std::nothrow) RfxAtLine(arg, NULL);
    if (line == NULL) {
        RFX_ASSERT(0);
    }
    ...
    if (modemOn) {
        pRI->request = RFX_MSG_REQUEST_THERMAL_RADIO_ON;
    } else {
        pRI->request = RFX_MSG_REQUEST_THERMAL_RADIO_OFF;
    }
...
void RilOemClient::executeHsrMode(char* arg) {
    RFX_LOG_I(RFX_LOG_TAG, "executeHsrMode");
    char *cmd = NULL;
    int err = 0, hsrMode = 0;
    ...
    cmd = line->atTokNextstr(&err);
    hsrMode = line->atTokNextint(&err);
    int mainSlotId = RfxRilUtils::getMajorSim() - 1;
    RFX_LOG_I(RFX_LOG_TAG, "executeHsrMode line = %s, cmd: %s, hsrMode: %d", arg, cmd, hsrMode);
    ...
    pRI->token = 0xffffffff;
    pRI->clientId = (ClientId)CLIENT_ID_OEM;
    pRI->request = RFX_MSG_REQUEST_SET_HSR_MODE;

#if defined(ANDROID_MULTI_SIM)
    rfx_enqueue_request_message_client(RFX_MSG_REQUEST_SET_HSR_MODE,
            (void *)&hsrMode, sizeof(int), pRI, (RIL_SOCKET_ID)mainSlotId);
#else
    rfx_enqueue_request_message_client(RFX_MSG_REQUEST_SET_HSR_MODE,
            (void *)&hsrMode, sizeof(int), pRI);
#endif
    delete line;
}
...
void RilOemClient::executeLowLatencyMode(char *arg) {
    ...
    if (lowLatencyMode == 1) {
        ...
        RFX_LOG_D(RFX_LOG_TAG, "arg : %s", cmdString3aOn);
#if defined(ANDROID_MULTI_SIM)
        rfx_enqueue_request_message_client(RFX_MSG_REQUEST_OEM_HOOK_RAW,
    ...
    rfx_enqueue_request_message_client(RFX_MSG_REQUEST_OEM_HOOK_RAW,
        ...
        */
        ...
        RFX_LOG_D(RFX_LOG_TAG, "arg : %s", cmdString3aOff);
#if defined(ANDROID_MULTI_SIM)
        rfx_enqueue_request_message_client(RFX_MSG_REQUEST_OEM_HOOK_RAW,
...
#else
    rfx_enqueue_request_message_client(RFX_MSG_REQUEST_QUERY_CAPABILITY,
        (void *)msg, sizeof(RIL_QueryCap), pRI);
#endif
    delete line;
    free(msg->name);
    free(msg);
...
void RilOemClient::executeRsuOpt(char* arg) {
    char *cmd;
    RfxAtLine *line;
    int err = 0;
    int slotId = 0;
    ...
    RFX_LOG_D(RFX_LOG_TAG, "executeRsuOpt");
    line = new (std::nothrow) RfxAtLine(arg, NULL);
    if (line == NULL) {
        RFX_ASSERT(0);
    }
    cmd = line->atTokNextstr(&err);
    ...
    RFX_LOG_D(RFX_LOG_TAG, "executeRsuOpt cmd=%s", cmd);
    slotId = line->atTokNextint(&err);
    if (err < 0) {
        RFX_LOG_E(RFX_LOG_TAG, "invalid slotId");
        delete line;
        return;
    }
    RFX_LOG_D(RFX_LOG_TAG, "executeRsuOpt slotId=%d", slotId);
    if (line->atTokHasmore()) {
        RFX_LOG_I(RFX_LOG_TAG, "has data");
        if (err < 0) {
            RFX_LOG_E(RFX_LOG_TAG, "invalid data");
            delete line;
            return;
        }
        data = line->atTokNextstr(&err);
        RFX_LOG_D(RFX_LOG_TAG, "executeRsuOpt data=%s", data);
    }
    if (mRsuFd >= 0) {
        RFX_LOG_E(RFX_LOG_TAG, "executeRsuOpt close %d !!!", mRsuFd);
        close(mRsuFd);
    }

    mRsuFd = commandFd;
    commandFd = -1;
    RFX_LOG_I(RFX_LOG_TAG, "executeRsuOpt cmd: %s, %p", cmd, data);
    ...
    pRI->clientId = (ClientId) CLIENT_ID_OEM;
    //TODO: test
    pRI->request = RFX_MSG_REQUEST_CLIENT_RSU;
    //pRI->request = RFX_MSG_REQUEST_GET_SUBLOCK_MODEM_STATUS;

#if defined(ANDROID_MULTI_SIM)
    rfx_enqueue_request_message_client(pRI->request,
            data, (data == NULL) ? 0 : strlen(data), pRI, (RIL_SOCKET_ID) slotId);
#else
    rfx_enqueue_request_message_client(pRI->request,
            data, (data == NULL) ? 0 : strlen(data), pRI);
#endif
