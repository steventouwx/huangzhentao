...
#ifndef __RTM_DATA_CONTROLLER_H__
#define __RTM_DATA_CONTROLLER_H__
...
class RtmDataController : public RfxController {
    RFX_DECLARE_CLASS(RtmDataController);  // Required: declare this class

public:
    RtmDataController();

    virtual ~RtmDataController();
...
protected:
    ...
    void handleSyncDataSettingsToMD(const sp<RfxMessage>& response);
    void handleSetupDataRequest(const sp<RfxMessage>& message);
    void handleSetPreferredDataModem(const sp<RfxMessage>& message);
...
#endif /* __RTM_DATA_CONTROLLER_H__ */
