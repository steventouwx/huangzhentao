...
//        handleEmergencyBearerSupportInfo(msg);
    ...
    } else if (strStartsWith(urc, URC_MD_INTERNET_USAGE)) {
...
void RmcCallControlImsUrcHandler::handleVoiceDomainSelectResult(const sp<RfxMclMessage>& msg) {
    ...
    * 3: NR
    * 4: Reject
    * 5: VoData on protocol 1
    * 6: VoData on protocol 2
    * 7: VoData on protocol 3
    * 8: VoData on protocol 4
    */
    ...
    // Remap VoData as VoWifi
    if (callRat >= VDM_ADS_RESULT_IMS_VODATA_1 && callRat <= VDM_ADS_RESULT_IMS_VODATA_4) {
...
/**
 *  +MDINTERNET: <sim id>,<tx bytes>,<rx bytes>,<tx pkt>,<rx pkt>
 */
void RmcCallControlImsUrcHandler::handleMDInternetUsage(const sp<RfxMclMessage>& msg) {
    logD(RFX_LOG_TAG, "handleMDInternetUsage");
    const int maxLen = 5;
    int rfxMsg = RFX_MSG_URC_MD_INTERNET_USAGE_IND;
    notifyIntsDataToTcl(msg, rfxMsg, maxLen);
}
