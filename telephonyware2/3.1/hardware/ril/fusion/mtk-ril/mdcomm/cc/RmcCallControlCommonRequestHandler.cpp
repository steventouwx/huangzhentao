...
RFX_IMPLEMENT_HANDLER_CLASS(RmcCallControlCommonRequestHandler, RIL_CMD_PROXY_2);
...
void RmcCallControlCommonRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
    ...
    switch (request) {
        ...
        case RFX_MSG_REQUEST_SET_CALL_SUB_ADDRESS:
            requestSetCallSubAddress(msg);
            break;

        case RFX_MSG_REQUEST_GET_CALL_SUB_ADDRESS:
            requestGetCallSubAddress(msg);
            break;
...
void RmcCallControlCommonRequestHandler::requestGetCurrentCalls(const sp<RfxMclMessage>& msg) {
    ...
    if (hasImsCall(m_slot_id) && (p_response->getError() != 0 || p_response->getSuccess() != 1)) {
        logD(RFX_LOG_TAG, "cs poll call returns empty when ims call exists.");
        countCalls = 0;
    } else {
        // set result
...
int RmcCallControlCommonRequestHandler::callFromCLCCSLine(RfxAtLine *pLine, RIL_Call *p_call) {
    ...
    */
    ...
    if (pLine->atTokHasmore()) {
        pLine->atTokNextint(&ret);  // <numbertype>
        if (ret < 0) { goto error; }
...
void RmcCallControlCommonRequestHandler::requestLocalSetEccServiceCategory(
    ...
    RIL_ESVC_Data *pData = (RIL_ESVC_Data *)msg->getData()->getData();
    sp<RfxAtResponse> p_response;
    String8 cmd;
    if (pData->serviceCategory != EXTENDED_SUB_SERVICE) {
        cmd = String8::format("AT+ESVC=%d", pData->serviceCategory);
    } else {
        cmd = String8::format("AT+ESVC=%d,%zu,\"%s\"", pData->serviceCategory,
                strlen(pData->urn), pData->urn);
    }
...
void RmcCallControlCommonRequestHandler::handleCnapUpdate(const sp<RfxMclMessage>& msg) {
    ...
    if (cnap != NULL && strlen(cnap) > 0) {
        int err = snprintf(cachedCnap, MAX_CNAP_LENGTH, "%s", cnap);
        if (err < 0) {
            logD(RFX_LOG_TAG, "Cache CNAP error");
        }
    }
}
...
void RmcCallControlCommonRequestHandler::requestSetCallSubAddress(const sp<RfxMclMessage>& msg) {
    int *pInt = (int*)msg->getData()->getData();
    int enable = (pInt[0] == 0) ? 0 : 1;

    atSendCommand(String8::format("AT+ESBP=5,\"SBP_SUBADDRESS_SETTING\",%d", enable));
...
void RmcCallControlCommonRequestHandler::requestGetCallSubAddress(const sp<RfxMclMessage>& msg) {
    ...
    p_response = atSendCommandSingleline("AT+ESBP=7,\"SBP_SUBADDRESS_SETTING\"", "+ESBP:");
    do {
        if (p_response == NULL || p_response->getSuccess() == 0) {
            logE(RFX_LOG_TAG, "requestGetCallSubAddress AT+ESBP return ERROR");
            break;
        }
        line = p_response->getIntermediates();
        line->atTokStart(&err);
        if (err < 0) {
            logE(RFX_LOG_TAG, "requestGetCallSubAddress AT+ESBP atTokStart error:%d", err);
            break;
        }
        enabled = line->atTokNextint(&err);
        if (err < 0) {
            logE(RFX_LOG_TAG, "requestGetCallSubAddress AT+ESBP atTokNextint error:%d", err);
            break;
        }
    } while(0);
