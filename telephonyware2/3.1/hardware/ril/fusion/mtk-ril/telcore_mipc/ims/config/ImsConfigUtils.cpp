...
const char* ImsConfigUtils::PROPERTY_VONR_FORCE_ENABLE = "persist.vendor.mtk.vonr.force.enable";
const char* ImsConfigUtils::PROPERTY_5G_VOWIFI_FORCE_ENABLE = "persist.vendor.mtk.5gvowifi.force.enable";
const char* ImsConfigUtils::PROPERTY_VINR_ENALBE =  "persist.vendor.mtk.vinr.enable";
const char* ImsConfigUtils::PROPERTY_VODATA_ENALBE =  "persist.vendor.mtk.vodata.enable";
...
const char* ImsConfigUtils::PROPERTY_VONR_SETTING_SUPPORT = "persist.vendor.vonr_setting_support";
const char* ImsConfigUtils::PROPERTY_VENDOR_OPERATOR_OPTR = "persist.vendor.operator.optr";
...
int ImsConfigUtils::getSystemPropValueWithDefault(const char* propName, const char * defaultValue){
    char value[RFX_PROPERTY_VALUE_MAX] = { 0 };
    int result;

    rfx_property_get(propName, value, defaultValue);
    result = atoi(value);

    printLog(DEBUG, String8::format("getSystemPropValueWithDefault, propName:%s, value:%s",
                                    propName, value), 0);
...
bool ImsConfigUtils::isOp09SimCard(const char* icc_id) {
...
bool ImsConfigUtils::isOp01SimCard(const char* icc_id) {
    bool isOp01Card = false;
    if (strncmp(icc_id, "898600", 6) == 0||
        strncmp(icc_id, "898602", 6) == 0||
        strncmp(icc_id, "898604", 6) == 0||
        strncmp(icc_id, "898607", 6) == 0) {
        isOp01Card = true;
    }
    return isOp01Card;
}

bool ImsConfigUtils::hasForceSetVoNRValue() {
    return getSystemPropValueWithDefault(PROPERTY_VONR_FORCE_ENABLE, "-1") != -1;
}

int ImsConfigUtils::getforceSetVoNRValue() {
    return getSystemPropValueWithDefault(PROPERTY_VONR_FORCE_ENABLE, "-1");
}

int ImsConfigUtils::getCnopVoWifiSupportState() {
    char property_value[MTK_PROPERTY_VALUE_MAX] = { 0 };
    memset(property_value, 0, sizeof(property_value));
    rfx_property_get("persist.vendor.wfc.cnop_vowifi", property_value, "0");
    int cnopVoWifiSupport = atoi(property_value);
    return cnopVoWifiSupport;
}